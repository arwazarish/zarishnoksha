 ...
 *   1. Check .zarishnoksha/live/config.json (returns config_missing if first-ever run)
 ...
 *   - Open the project's live dev/preview URL in the browser (optional, if browser automation exists)—not `serverPort`; that port is the zarishnoksha helper for /live.js and /poll
...
import { readLiveServerInfo } from "./lib/zarishnoksha-paths.mjs";
...
Prepare everything for live variant mode in a single command:
  - Checks .zarishnoksha/live/config.json (required, created once per project)
      ...
      JSON.stringify(
        {
          ...
          ...targetSelection,
          hint: "Ask the user which app zarishnoksha should use, then rerun live from that child app cwd. Use --target <path> only as a fallback or explicit path diagnostic.",
        },
        null,
        2,
      ),
