/**
 * Shared library for the zarishnoksha design hook.
 *
...
export const ENVELOPE_PREFIX = "[zarishnoksha@1]";
...
const HOOK_IGNORE_MARKER_OPEN = "# zarishnoksha-hook-ignore-start";
const HOOK_IGNORE_MARKER_CLOSE = "# zarishnoksha-hook-ignore-end";
...
export function getConfigPath(cwd) {
  return path.join(cwd, ".zarishnoksha", "config.json");
}

export function getLocalConfigPath(cwd) {
  return path.join(cwd, ".zarishnoksha", "config.local.json");
}

export function getCachePath(cwd) {
  return path.join(cwd, ".zarishnoksha", "hook.cache.json");
}

export function getPendingPath(cwd) {
  return path.join(cwd, ".zarishnoksha", "hook.pending.json");
}
...
export function suppressionNotice(filePath) {
  return `${ENVELOPE_PREFIX} Suppressing further design hints on ${filePath}. More than ${EDIT_COUNT_THRESHOLD} edits in this session reached. Run /zarishnoksha audit to revisit.`;
}
...
export function renderTemplate(findings, filePath, config, opts = {}) {
  ...
  const more = remaining > 0 ? `... and ${remaining} more (see /zarishnoksha audit).` : null;
...
function renderGroupedTemplate(groups, config, opts = {}) {
  ...
  for (const group of realGroups) {
    ...
    if (hidden > 0) {
      lines.push(`- ... ${hidden} more in ${display} (see /zarishnoksha audit).`);
    }
  }
...
function clampGroupedToBudget(header, lines, footer, maxChars) {
  const assemble = (linesArr, omitted) =>
    [header, ...linesArr, ...(omitted ? ["... and more (see /zarishnoksha audit)."] : []), "", footer].join("\n");
...
function clampToBudget(header, lines, more, footer, maxChars) {
  ...
  while (assembled.length > maxChars && working.length > 1) {
    working.pop();
    moreText = "... and more (see /zarishnoksha audit).";
...
function formatFindingIgnoreCommand(finding) {
  ...
  return `/zarishnoksha hooks ignore-value ${rule} ${valueArg} --shared --reason ${reason}`;
}
...
export function resolveHarness(env = {}, event = null) {
  const explicit = env?.zarishnoksha_HOOK_HARNESS;
...
export function writeAuditLog(env, entry, cwd = process.cwd()) {
  ...
  let target = env?.zarishnoksha_HOOK_LOG;
...
// silence-on-clean can set `zarishnoksha_HOOK_QUIET=1` — runHook checks that
...
const STEER_LINE =
  "That does not mean the design is good: keep following the project design system and the zarishnoksha skill guidance.";
...
export function appendDesignSystemNote(text, scanOptions) {
  ...
  return `${text}\n\n${ENVELOPE_PREFIX} DESIGN.md is newer than .zarishnoksha/design.json. Run /zarishnoksha document to refresh the design-system sidecar.`;
}
...
function directiveFooter(display, opts = {}) {
  const ignoreFileCommand = `/zarishnoksha hooks ignore-file ${quoteCommandArg(display)}`;
  const fileIgnoreGuidance = opts.grouped
    ? "run `/zarishnoksha hooks ignore-file <path>` for the specific file"
  ...
  return [
    ...
    "",
    `Do not change intentional design just to satisfy the hook, and do not silence a real finding with an inline ignore comment to skip fixing it. Suppress a finding only after the user explicitly confirms it is intentional. Prefer a config ignore (one reviewable place, the commands below); reach for an inline \`zarishnoksha-disable <rule>\` comment only when the waiver must travel with a file that leaves the repo, such as an exported or standalone document. Prefer the narrowest persisted exception: run the exact \`/zarishnoksha hooks ignore-value ... --shared\` command shown next to a value-specific finding. For \`overused-font\`, use \`ignore-value\` for a specific font and use \`/zarishnoksha hooks ignore-rule overused-font --all-values\` only when the user asks to ignore overused fonts generally. For file-specific findings without an ignore-value command, ${fileIgnoreGuidance}; use \`/zarishnoksha hooks ignore-rule <id>\` only when the user asks to suppress the whole non-value-specific rule. Run /zarishnoksha audit for the full pass.`,
  ].join("\n");
}
...
export async function runHook({ stdinJson, env = {}, cwd = process.cwd(), now = Date.now, detector } = {}) {
  ...
  try {
    // Re-entrancy guard.
    if (depthIsSet(env.zarishnoksha_HOOK_DEPTH) || depthIsSet(env.CLAUDE_HOOK_DEPTH)) {
      return result({ reentrant: true, durationMs: 0 });
    }

    if (truthy(env.zarishnoksha_HOOK_DISABLED)) {
    ...
    if (truthy(env.zarishnoksha_HOOK_QUIET) || config.quiet === true) {
