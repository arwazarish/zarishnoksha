/**
 * zarishnoksha DevTools Extension - DevTools Page
 *
 * Creates the zarishnoksha panel and triggers an auto-scan when DevTools opens.
 * This page lives for the entire DevTools session -- its port disconnect
 * is the canonical signal that DevTools has closed.
 */

chrome.devtools.panels.create("zarishnoksha", "/icons/icon-32.png", "/devtools/panel.html");

// Sidebar pane in the Elements panel: shows findings for the currently selected element
chrome.devtools.panels.elements.createSidebarPane("zarishnoksha", (sidebar) => {
...
const portName = `zarishnoksha-devtools-${chrome.devtools.inspectedWindow.tabId}`;
...
function connectLifecycle() {
  ...
  // Default ('panel'): wait until the user opens the zarishnoksha panel or sidebar.
  ...
  if (firstConnect) {
    ...
    chrome.storage.sync.get({ autoScan: "panel" }, (settings) => {
      if (settings.autoScan === "devtools") {
        try {
          lifecyclePort?.postMessage({ action: "scan" });
        } catch {}
      }
    });
  }
...
setInterval(() => {
  try {
    lifecyclePort?.postMessage({ action: "ping" });
  } catch {}
}, 20000);
