/**
 * zarishnoksha DevTools Extension - Panel
 *
...
function getPort() {
  if (port) return port;
  port = chrome.runtime.connect({ name: `zarishnoksha-panel-${tabId}` });
...
// Maps each anti-pattern to the most relevant zarishnoksha skill(s) for fixing it.
...
async function formatFindingsForCopy(findings) {
  if (!findings.length) return "ZarishNoksha found no anti-patterns on this page.";
  const url = await getInspectedUrl();
  const lines = ["# ZarishNoksha findings"];
  ...
  if (skills.length) {
    lines.push(`Suggested ZarishNoksha skills to fix: ${skills.join(", ")}`);
    lines.push("");
  }

  lines.push("---");
  lines.push("Detected by [ZarishNoksha](https://zarishnoksha.pages.dev). Skills are suggestions, not required.");
  return lines.join("\n");
}
...
async function formatSingleFindingForCopy(item, finding) {
  ...
  const lines = [`# ZarishNoksha: ${finding.name}`];
  if (url) lines.push(`URL: ${url}`);
  ...
  lines.push("");
  lines.push(`Suggested ZarishNoksha skill(s) to fix: ${fixSkillFor(finding.type)}`);
  return lines.join("\n");
}
    ...
    for (const [type, group] of groups) {
      ...
      for (const item of group.items) {
        ...
        const tag = item.isPageLevel
          ...
          : item.isHidden
            ? '<span class="finding-tag tag-hidden" title="Element is currently hidden on the page">hidden</span>'
            : "";
        itemEl.innerHTML = `
        ${tag}
