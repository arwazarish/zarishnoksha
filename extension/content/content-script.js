/**
 * zarishnoksha DevTools Extension - Content Script
 *
 ...
 */
(() => {
  if (window.__zarishnoksha_CS_LOADED__) return;
  window.__zarishnoksha_CS_LOADED__ = true;
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg.action === "scan") {
      ...
      window.postMessage({ source: "zarishnoksha-command", action: "toggle-overlays" }, "*");
      sendResponse({ ok: true });
      ...
      window.postMessage({ source: "zarishnoksha-command", action: "highlight", selector: msg.selector }, "*");
      sendResponse({ ok: true });
    } else if (msg.action === "unhighlight") {
      window.postMessage({ source: "zarishnoksha-command", action: "unhighlight" }, "*");
      sendResponse({ ok: true });
    }
    return true;
  });
  ...
  window.addEventListener("message", (e) => {
    ...
    if (e.data.source === "zarishnoksha-overlays-toggled") {
      chrome.runtime
        .sendMessage({
  ...
  function sendScanCommand() {
    const msg = { source: "zarishnoksha-command", action: "scan" };
  ...
  function injectAndScan() {
    ...
    script.dataset.zarishnokshaExtension = "true";
    pendingScan = true;
