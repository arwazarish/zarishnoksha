/**
 * zarishnoksha DevTools Extension - Service Worker
 *
...
async function getSettings() {
  return chrome.storage.sync.get({
    ...
    autoScan: "panel", // 'panel' = scan when zarishnoksha UI opens, 'devtools' = scan when DevTools opens
  });
}
...
async function sendScanToTab(tabId) {
  ...
  if (!ok) {
    ...
    const message = url.startsWith("file:")
      ? "Can\u2019t scan local files. Enable \u201CAllow access to file URLs\u201D for zarishnoksha in chrome://extensions."
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.action === "findings" && tabId) {
    ...
    chrome.scripting
      ...
      .then(() => {
        // Detector will post zarishnoksha-ready, content script handles the rest
      })
      .catch((err) => {
        console.warn("[zarishnoksha] Fallback injection failed:", err);
      });
    sendResponse({ ok: true });
...
async function tearDownTab(tabId) {
  ...
  try {
    ...
    /* tab might be closed or content script gone */
  }
  const state = tabState.get(tabId);
  if (state) {
    state.findings = [];
...
chrome.runtime.onConnect.addListener((port) => {
  ...
  if (port.name.startsWith("zarishnoksha-devtools-")) {
    const tabId = parseInt(port.name.replace("zarishnoksha-devtools-", ""), 10);
  ...
  if (port.name.startsWith("zarishnoksha-panel-")) {
    const tabId = parseInt(port.name.replace("zarishnoksha-panel-", ""), 10);
  ...
  // Connecting the sidebar is a strong signal of "user engaged with zarishnoksha"
  // so we trigger a scan if no findings exist yet (matches the panel port behavior).
  if (port.name.startsWith("zarishnoksha-sidebar-")) {
    const tabId = parseInt(port.name.replace("zarishnoksha-sidebar-", ""), 10);
    if (!panelPorts.has(tabId)) panelPorts.set(tabId, new Set());
    panelPorts.get(tabId).add(port);
