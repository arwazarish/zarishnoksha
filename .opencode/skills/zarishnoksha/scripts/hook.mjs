#!/usr/bin/env node
/**
 * zarishnoksha design hook — PostToolUse entry point.
 *
...
async function main() {
  ...
  process.env.zarishnoksha_HOOK_DEPTH = process.env.zarishnoksha_HOOK_DEPTH || "1";
...
main().catch((err) => {
  ...
  if (process.env.zarishnoksha_HOOK_DEBUG) {
    process.stderr.write(`[zarishnoksha-hook] ${err}\n`);
  }
  process.exit(0);
});
