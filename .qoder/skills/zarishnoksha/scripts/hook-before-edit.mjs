#!/usr/bin/env node
/**
 * zarishnoksha design hook — Cursor preToolUse write gate.
 *
...
function cursorBlockMessage(findings, filePath, config, cwd) {
  ...
  const blocked = rendered.replace(
    "[zarishnoksha@1] Design hook findings requiring review",
    "[zarishnoksha@1] zarishnoksha design hook blocked this write before it landed. Design hook findings requiring review",
  );
...
async function main() {
  if (truthy(process.env.zarishnoksha_HOOK_DISABLED)) {
  ...
  if (denial.count > EDIT_COUNT_THRESHOLD) {
    const warning = `${message}\n\nThis is the ${denial.count}th repeated denial for the same file and finding signature, so zarishnoksha is allowing this write to avoid a loop. Reconsider the issue immediately after the tool runs.`;
    return allow(
      {
        ...audit,
        findings: (findings || []).length,
...
main().catch((err) => {
  if (process.env.zarishnoksha_HOOK_DEBUG) {
    process.stderr.write(`[zarishnoksha-hook-before-edit] ${err}\n`);
  }
