...
export async function wrapCli() {
  ...
  if (args.includes("--help") || args.includes("-h")) {
    console.log(`Usage: zarishnoksha wrap [options]
...
The agent should insert variant HTML at insertLine.`);
  ...
  const wrapperLines = isJsx
    ? [
        indent +
          '<div data-zarishnoksha-variants="' +
          id +
          '" data-zarishnoksha-variant-count="' +
          count +
          '" ' +
          styleContents +
          ">",
        indent + "  " + commentSyntax.open + " zarishnoksha-variants-start " + id + " " + commentSyntax.close,
        indent + "  " + commentSyntax.open + " Original " + commentSyntax.close,
        indent + '  <div data-zarishnoksha-variant="original">',
        reindentOriginal("    "),
        indent + "  </div>",
        indent + "  " + commentSyntax.open + " Variants: insert below this line " + commentSyntax.close,
        indent + "  " + commentSyntax.open + " zarishnoksha-variants-end " + id + " " + commentSyntax.close,
        indent + "</div>",
      ]
    : [
        indent + commentSyntax.open + " zarishnoksha-variants-start " + id + " " + commentSyntax.close,
...
function detectStyleMode(filePath) {
  ...
  if (ext === ".astro") {
    return {
      ...
      styleTag: '<style is:inline data-zarishnoksha-css="SESSION_ID">',
    };
  }
  return {
    mode: "scoped",
    styleTag: '<style data-zarishnoksha-css="SESSION_ID">',
  };
}

function buildCssSelectorPrefixExamples(styleMode, count) {
  if (styleMode !== "astro-global-prefixed") return [];
  return Array.from({ length: count }, (_, i) => `[data-zarishnoksha-variant="${i + 1}"]`);
}
...
function buildCssAuthoring(styleMode, count) {
  ...
  if (styleMode.mode === "astro-global-prefixed") {
    return {
      ...
      rulePattern: '[data-zarishnoksha-variant="N"] > .variant-class { ... }',
      selectorExamples: variantNumbers.map((n) => `[data-zarishnoksha-variant="${n}"] > .variant-class`),
      requirements: [
        ...
        'Prefix every preview selector with the matching [data-zarishnoksha-variant="N"] selector.',
  ...
  return {
    ...
    styleTag: styleMode.styleTag,
    strategy: "scope-rule",
    rulePattern: '@scope ([data-zarishnoksha-variant="N"]) { :scope > .variant-class { ... } }',
    selectorExamples: variantNumbers.map(
      (n) => `@scope ([data-zarishnoksha-variant="${n}"]) { :scope > .variant-class { ... } }`,
    ),
    requirements: [
      'Use @scope blocks keyed to each [data-zarishnoksha-variant="N"] wrapper.',
    ...
    forbidden: [
      'Do not use global [data-zarishnoksha-variant="N"] selector prefixes for this styleMode.',
...
function findElement(lines, query, tag = null) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lines[i].includes("data-zarishnoksha-variant")) continue;
