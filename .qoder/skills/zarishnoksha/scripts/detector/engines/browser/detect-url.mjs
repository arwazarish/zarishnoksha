      ...
      async () => {
        browserAnalyses = await page.evaluate(
          async ({ maxCandidates, scrollOffscreen }) => {
            if (typeof window.zarishnokshaAnalyzeVisualContrast !== "function") return [];
            return window.zarishnokshaAnalyzeVisualContrast({ maxCandidates, scrollOffscreen });
          },
  ...
  if (candidates.length === 0) {
    candidates = await profileStepAsync(
      ...
      () =>
        page.evaluate(
          ({ maxCandidates }) => {
            if (typeof window.zarishnokshaCollectVisualContrastCandidates !== "function") return [];
            return window.zarishnokshaCollectVisualContrastCandidates({ maxCandidates });
          },
          { maxCandidates },
        ),
    );
...
async function detectUrl(url, options = {}) {
  ...
  if (!externalBrowser) {
    try {
      ...
      throw new Error("puppeteer is required for URL scanning. Install: bun install puppeteer");
    }
  }
    ...
    await profileStepAsync(
      ...
      () =>
        page.evaluate((designSystem) => {
          window.__zarishnoksha_CONFIG__ = {
            ...(window.__zarishnoksha_CONFIG__ || {}),
            autoScan: false,
    ...
    results = await profileFindingsAsync(
      ...
      async () => {
        serializedGroups = await page.evaluate(() => {
          if (!window.zarishnokshaDetect) return [];
          return window.zarishnokshaDetect({ decorate: false, serialize: true });
        });
