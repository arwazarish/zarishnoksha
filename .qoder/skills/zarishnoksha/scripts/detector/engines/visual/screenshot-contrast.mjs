...
async function captureVisualContrastCandidate(page, candidate, viewport) {
  ...
  const token = `zarishnoksha-contrast-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const applied = await page.evaluate(
    ({ selector, token, backgroundClipText }) => {
      ...
      if (!el) return false;
      let style = document.getElementById("zarishnoksha-visual-contrast-hide-style");
      if (!style) {
        style = document.createElement("style");
        style.id = "zarishnoksha-visual-contrast-hide-style";
        style.textContent = [
          "[data-zarishnoksha-visual-contrast-target] {",
          "  color: transparent !important;",
          "  -webkit-text-fill-color: transparent !important;",
          "  text-shadow: none !important;",
          "}",
          '[data-zarishnoksha-visual-contrast-target][data-zarishnoksha-bgclip-text="true"] {',
          "  background-image: none !important;",
          "}",
        ].join("\n");
        document.head.appendChild(style);
      }
      el.setAttribute("data-zarishnoksha-visual-contrast-target", token);
      if (backgroundClipText) el.setAttribute("data-zarishnoksha-bgclip-text", "true");
      return true;
    },
    {
      ...
      .evaluate(
        ({ selector }) => {
          try {
            ...
            if (el) {
              el.removeAttribute("data-zarishnoksha-visual-contrast-target");
              el.removeAttribute("data-zarishnoksha-bgclip-text");
            }
          } catch {
