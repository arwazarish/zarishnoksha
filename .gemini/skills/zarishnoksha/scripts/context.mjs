 ...
 *   4. $zarishnoksha_CONTEXT_DIR (absolute or cwd-relative) — power-user
...
// can offer `npx zarishnoksha update`. Everything here is best-effort and
// silent on failure: a network problem, sandbox, or missing cache must never
// block context output or print an error.

const UPDATE_HOST = (process.env.zarishnoksha_UPDATE_HOST || "https://zarishnoksha.pages.dev").replace(/\/$/, "");
const UPDATE_CACHE_PATH =
  process.env.zarishnoksha_UPDATE_CACHE || path.join(os.homedir(), ".zarishnoksha", "update-check.json");
...
function resolveContext(cwd = process.cwd(), options = {}) {
  ...
  const rootContextDir =
    project.isMonorepo && project.repoRoot !== project.projectRoot ? resolveLocalContextDir(project.repoRoot) : null;
...
function resolveEnvContextDir(cwd) {
  const envDir = process.env.zarishnoksha_CONTEXT_DIR;
...
// outside both the project and repo roots (zarishnoksha_CONTEXT_DIR override).
...
function readWorkspacePatterns(repoRoot) {
  return [...readPackageWorkspaces(repoRoot), ...readPnpmWorkspaces(repoRoot), ...readLernaWorkspaces(repoRoot)].filter(
    Boolean,
  );
}
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    `UPDATE_AVAILABLE: A newer zarishnoksha skill is available ` +
    `(installed v${localVersion}, latest v${latestVersion}). ` +
    `Before continuing, ask the user once: "A newer zarishnoksha (v${latestVersion}) is available. ` +
    `Update now? It runs \`npx zarishnoksha update\`." ` +
    `If they agree, run \`npx zarishnoksha update\` (the update applies to the next session, not this one). ` +
 ...
 * the agent never nags. Opt out entirely with zarishnoksha_NO_UPDATE_CHECK=1.
 */
...
function updateCheckDisabledByConfig(cwd = process.cwd()) {
  ...
  for (const name of ["config.json", "config.local.json"]) {
    try {
      const raw = JSON.parse(fs.readFileSync(path.join(cwd, ".zarishnoksha", name), "utf-8"));
...
async function computeUpdateDirective(now = Date.now()) {
  try {
    if (process.env.zarishnoksha_NO_UPDATE_CHECK) return null;
...
function buildTargetSelectionDirective(selection) {
  return (
    ...
    "Ask the user which app zarishnoksha should use, then rerun zarishnoksha helper commands from that child app cwd using this same scripts directory. " +
