 ...
 * <div data-zarishnoksha-carbonize> so the slot keeps a single root node.
 */
function buildCarbonizeReplacement({
  ...
  const pushCarbonizeBody = (bodyIndent) => {
    ...
    lines.push(bodyIndent + commentSyntax.open + " zarishnoksha-carbonize-start " + id + " " + commentSyntax.close);
    lines.push(bodyIndent + '<style data-zarishnoksha-css="' + id + '">' + (isJsx ? "{`" : ""));
    ...
    lines.push(bodyIndent + commentSyntax.open + " zarishnoksha-carbonize-end " + id + " " + commentSyntax.close);
    lines.push(bodyIndent + '<div data-zarishnoksha-variant="' + variantNum + '" ' + variantStyleAttr + ">");
  ...
  if (isJsx) {
    ...
    lines.push(indent + '<div data-zarishnoksha-carbonize="' + id + '" ' + wrapperStyle + ">");
...
function handleAccept(id, variantNum, lines, targetFile, paramValues) {
  ...
  const hasHelperAttrs = variantText.includes("data-zarishnoksha-variant");
...
function readSourceShadowPreviewMeta(content, id) {
  ...
  const wrapperRe = new RegExp("<[^>]+data-zarishnoksha-variants=([\"'])" + escaped + "\\1[^>]*>");
  const match = String(content || "").match(wrapperRe);
  if (!match) return null;
  const tag = match[0];
  if (readHtmlAttr(tag, "data-zarishnoksha-preview") !== "source-shadow") return null;
  const sourceFile = readHtmlAttr(tag, "data-zarishnoksha-source-file");
  const sourceStartLine = Number(readHtmlAttr(tag, "data-zarishnoksha-source-start"));
  const sourceEndLine = Number(readHtmlAttr(tag, "data-zarishnoksha-source-end"));
...
function findMarkerBlock(id, lines) {
  let start = -1;
  let end = -1;
  const startPattern = "zarishnoksha-variants-start " + id;
  const endPattern = "zarishnoksha-variants-end " + id;
 ...
 * the `<div data-zarishnoksha-variants="ID">` outer wrapper so the picked
 ...
 * (with `data-zarishnoksha-variants="ID"`) and its matching `</div>`. We
...
function expandReplaceRange(block, lines, isJsx) {
  ...
  // Walk back for the wrapper `<div data-zarishnoksha-variants="..."` opener.
...
function isVariantEndMarkerLine(line, id) {
  return new RegExp("zarishnoksha-variants-end\\s+" + escapeRegExp(id) + "(?:\\s|--|\\*/|$)").test(line);
}

function hasVariantWrapperAttr(line, id) {
  const escaped = escapeRegExp(id);
  return new RegExp(`data-zarishnoksha-variants\\s*=\\s*(?:"${escaped}"|'${escaped}'|\\{["']${escaped}["']\\})`).test(
    line,
  );
}
 ...
 *   - CSS `@scope ([data-zarishnoksha-variant="N"])` strings that look like the
...
function extractOriginal(lines, block) {
  ...
  const inner = extractInnerByAttr(text, 'data-zarishnoksha-variant="original"');
...
function extractVariant(lines, block, variantNum) {
  const text = stripStyleAndJoin(lines, block);
  const inner = extractInnerByAttr(text, 'data-zarishnoksha-variant="' + variantNum + '"');
 ...
 *
 * Handles three shapes of `<style data-zarishnoksha-css="ID" ...>`:
...
function extractCss(lines, block, id) {
  const styleAttr = 'data-zarishnoksha-css="' + id + '"';
  let inStyle = false;
  const content = [];
...
function findSessionFile(id, cwd) {
  const marker = "zarishnoksha-variants-start " + id;
