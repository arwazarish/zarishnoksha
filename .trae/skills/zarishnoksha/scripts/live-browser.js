/**
 * zarishnoksha Live Variant Mode - Browser Script
 *
 * Injected into the user's page via <script src="http://localhost:PORT/live.js">.
 * The server prepends window.__zarishnoksha_TOKEN__ and window.__zarishnoksha_PORT__
 * before this code.
 *
...
(() => {
  ...
  if (window.__zarishnoksha_LIVE_INIT__) return;
  window.__zarishnoksha_LIVE_INIT__ = true;

  const TOKEN = window.__zarishnoksha_TOKEN__;
  const PORT = window.__zarishnoksha_PORT__;
  if (!TOKEN || !PORT) {
    window.__zarishnoksha_LIVE_INIT__ = false; // reset so the real load can init
    return;
  }
  ...
  const PREFIX = "zarishnoksha-live";
  const PICK_CURSOR_STYLE_ID = PREFIX + "-pick-cursor-style";
  const MANUAL_APPLY_STATE_TTL_MS = 15 * 60 * 1000;
  const sessionState = window.__zarishnoksha_LIVE_SESSION__?.createLiveBrowserSessionState({
    prefix: PREFIX,
    storage: localStorage,
    idFactory: () => crypto.randomUUID().replace(/-/g, "").slice(0, 8),
  });
  if (!sessionState) {
    console.error("[zarishnoksha] live-browser-session.js was not loaded. Live mode cannot start safely.");
    window.__zarishnoksha_LIVE_INIT__ = false;
    return;
  }
  ...
  // window.__zarishnoksha_VOCAB__ when it serves /live.js (same injection path as
  ...
  const VOCAB = Array.isArray(window.__zarishnoksha_VOCAB__) ? window.__zarishnoksha_VOCAB__ : [];
  const ICONS = {};
  ...
  let selectedAction = "zarishnoksha";
  let selectedCount = 3;
  ...
  const SCROLL_ANCHOR_LOCK_ID = "zarishnoksha-scroll-anchor-lock";
  ...
  const domHelpers = window.__zarishnoksha_LIVE_DOM__?.createLiveBrowserDomHelpers({
    prefix: PREFIX,
    skipTags: SKIP_TAGS,
    document,
  });
  if (!domHelpers) {
    console.error("[zarishnoksha] live-browser-dom.js was not loaded. Live mode cannot start safely.");
    window.__zarishnoksha_LIVE_INIT__ = false;
    return;
  }
  const {
  ...
  window.__zarishnoksha_LIVE_CHROME_CORE__ = {
    version: 1,
    adapter: window.__zarishnoksha_LIVE_ADAPTER__ || "dom",
  ...
  function showHighlight(el) {
    ...
    if (el.hasAttribute?.("data-zarishnoksha-insert-placeholder")) return;
  ...
  function onAnnotDown(e) {
    ...
    const resizeEdge = e.target.closest?.("[data-zarishnoksha-placeholder-resize]")?.dataset
      .zarishnokshaPlaceholderResize;
  ...
  function stripManualEditRuntimeState(root) {
    ...
    const nodes = [
      ...
      ...root.querySelectorAll(
        "[data-zarishnoksha-editable], [data-zarishnoksha-original-text], [data-zarishnoksha-text-wrap]",
      ),
    ];
    for (const node of nodes) {
      const runtimeEditable =
        node.hasAttribute("data-zarishnoksha-editable") || node.hasAttribute("data-zarishnoksha-original-text");
      node.removeAttribute("data-zarishnoksha-editable");
      node.removeAttribute("data-zarishnoksha-original-text");
      node.removeAttribute("data-zarishnoksha-text-wrap");
      if (runtimeEditable) {
  ...
  function selectionTagLabel(el) {
    if (!el) return "";
    if (el.hasAttribute?.("data-zarishnoksha-insert-placeholder")) return "slot";
  ...
  function applyPlaceholderSizingStyles(placeholder, sizing) {
    placeholder.dataset.zarishnokshaPlaceholderWidth = sizing.kind;
  ...
  function materializePlaceholderWidth(placeholder) {
    ...
    const kind = placeholder.dataset.zarishnokshaPlaceholderWidth;
    ...
    placeholder.dataset.zarishnokshaPlaceholderWidth = "explicit";
  }
  ...
  function resolveBarAnchor() {
    ...
    if (currentSessionId && (state === "GENERATING" || state === "CYCLING")) {
      const wrapper = document.querySelector('[data-zarishnoksha-variants="' + currentSessionId + '"]');
      if (wrapper) {
        const variantCount = wrapper.querySelectorAll(
          '[data-zarishnoksha-variant]:not([data-zarishnoksha-variant="original"])',
        ).length;
  ...
  function isInsertGeneratingSession() {
    ...
    const wrapper = document.querySelector('[data-zarishnoksha-variants="' + currentSessionId + '"]');
    return !!wrapper && wrapper.dataset.zarishnokshaMode === "insert";
  }
  ...
  function buildPlaceholderResizeHandles() {
    ...
    for (const spec of specs) {
      ...
      handle.dataset.zarishnokshaPlaceholderResize = spec.edge;
  ...
  function createInsertPlaceholder(anchor, position, layoutAxis) {
    ...
    placeholder.setAttribute("data-zarishnoksha-insert-placeholder", "true");
  ...
  function ensureConfigureInputStyle() {
    ...
    s.textContent =
      "@keyframes zarishnoksha-configure-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }" +
      "#" +
      PREFIX +
      "-input, #" +
      PREFIX +
      ...
      PREFIX +
      '-insert-voice[data-listening="true"] svg { animation: zarishnoksha-configure-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  function buildSavingRow() {
    ...
    const spinner = el("div", {
      ...
      animation: "zarishnoksha-spin 0.6s linear infinite",
      flexShrink: "0",
    });
  ...
  function recoverEmptyCycling(reason) {
    ...
    try {
      console.warn("[zarishnoksha] Refusing to render empty variant cycling state:", reason);
  ...
  // variant wrapper:
  //
  //   <div data-zarishnoksha-variant="1"
  //        data-zarishnoksha-params='[{"id":"density","kind":"steps",...}]'>
  //
  ...
  function getVisibleVariantEl() {
    ...
    return wrapper.querySelector('[data-zarishnoksha-variant="' + visibleVariant + '"]');
  }

  function parseVariantParams(variantEl) {
    // Svelte component variants can't carry a `data-zarishnoksha-params` attribute:
    ...
    if (!variantEl) return [];
    const raw = variantEl.getAttribute("data-zarishnoksha-params");
    if (!raw) return [];
    try {
      const parsed = JSON.parse(raw);
      return Array.isArray(parsed) ? parsed : [];
    } catch (err) {
      console.warn("[zarishnoksha] Invalid data-zarishnoksha-params JSON:", err.message);
      return [];
    }
  }
  ...
  function wrapMixedContentTextNodes(rootEl) {
    ...
    if (hasText && hasElement) {
      for (const node of children) {
        if (node.nodeType === 3 && /\S/.test(node.nodeValue || "")) {
          ...
          wrap.dataset.zarishnokshaTextWrap = "true";
    ...
    for (const child of Array.from(rootEl.children)) {
      if (!child.dataset || !child.dataset.zarishnokshaTextWrap) {
  ...
  function unwrapMixedContentTextNodes(rootEl) {
    if (!rootEl || rootEl.nodeType !== 1) return;
    const wraps = rootEl.querySelectorAll('[data-zarishnoksha-text-wrap="true"]');
  ...
  function enableInlineEdit(targetEl) {
    ...
    for (const row of rows) {
      ...
      row.el.dataset.zarishnokshaEditable = "true";
      row.el.dataset.zarishnokshaOriginalText = row.text;
  ...
  function disableInlineEdit(opts = {}) {
    for (const row of inlineEditRows) {
      ...
      delete row.el.dataset.zarishnokshaEditable;
      delete row.el.dataset.zarishnokshaOriginalText;
  ...
  function restoreInlineEditDrafts() {
    for (const row of inlineEditRows) {
      if (inlineEditDrafts.has(row.el)) {
        row.el.textContent = row.el.dataset.zarishnokshaOriginalText;
      }
    }
  }
  ...
  function documentRefClassSuffix(el) {
    ...
    for (const cls of el.classList) {
      if (!cls || cls.indexOf("zarishnoksha-") === 0) continue;
  ...
  function copyEditLeafContext(el, originalText, newText) {
    ...
    return {
      ...
      id: el.id || null,
      classes: el.classList ? [...el.classList].filter((cls) => cls.indexOf("zarishnoksha-") !== 0) : [],
      originalText,
      newText,
      textContent: (el.textContent || "").slice(0, 500),
  ...
  function nearbyEditableTextsForManualEdit(rows, activeEl, originalText, newText) {
    ...
    for (const row of rows || []) {
      ...
      out.push({
        ...
        classes: row.el?.classList ? [...row.el.classList].filter((cls) => cls.indexOf("zarishnoksha-") !== 0) : [],
        text,
      });
  ...
  async function applyEditing() {
    ...
    try {
      ...
      console.error("[zarishnoksha] manual edit stash failed:", err);
  ...
  function ensureSpinKeyframes() {
    ...
    style.textContent = "@keyframes zarishnoksha-spin { to { transform: rotate(360deg); } }";
    uiAppendStyle(style);
  }
  ...
  async function fetchPendingCount() {
    try {
      ...
      console.warn("[zarishnoksha] failed to fetch pending count:", err);
    }
  }
  ...
  async function onPendingPillClick() {
    ...
    try {
      ...
      if (result.failed && result.failed.length > 0) {
        console.warn("[zarishnoksha] some copy edits failed:", result.failed);
        showToast(
        ...
        if (n > 0) {
          ...
          console.warn("[zarishnoksha] apply returned no verified edits:", result);
          showToast("No edits applied - see console", 4000);
        }
      }
    } catch (err) {
      console.error("[zarishnoksha] commit failed:", err);
  ...
  async function onPendingTrashClick() {
    ...
    try {
      ...
      console.error("[zarishnoksha] discard failed:", err);
  ...
  async function onPendingKeepFixingClick() {
    ...
    try {
      ...
      console.error("[zarishnoksha] repair retry failed:", err);
  ...
  async function onPendingRollbackClick() {
    ...
    try {
      ...
      console.error("[zarishnoksha] manual Apply rollback failed:", err);
  ...
  function restoreDiscardedManualEdits(entries) {
    ...
    if (failures > 0) {
      console.warn(
        "[zarishnoksha] skipped unsafe copy edit DOM restore for",
        failures,
  ...
  function mixedTextWrapRestoreHint(el) {
    if (!el || !el.dataset || el.dataset.zarishnokshaTextWrap !== "true" || !el.parentElement) return null;
  ...
  function directMixedTextRestoreNodes(parent) {
    return Array.from(parent?.childNodes || []).filter((node) => {
      ...
      return (
        node.nodeType === 1 &&
        node.dataset &&
        node.dataset.zarishnokshaTextWrap === "true" &&
  ...
  function syncEditBadgeHitProxies() {
    ...
    for (const target of targets) {
      ...
      if (!proxy) {
        ...
        proxy.type = "button";
        proxy.tabIndex = -1;
        proxy.dataset.zarishnokshaEditBadgeProxy = "true";
  ...
  function initEditBadge() {
    ...
    if (!uiGetById(PREFIX + "-edit-badge-focus-style")) {
      ...
      s.textContent =
        ...
        '[data-zarishnoksha-editable="true"] { outline: none !important; box-shadow: none !important; }' +
        '[data-zarishnoksha-editable="true"]:focus { outline: none !important; box-shadow: none !important; }' +
        '[data-zarishnoksha-editable="true"]:focus-visible { outline: none !important; box-shadow: none !important; }';
      uiAppendStyle(s);
    }
  }
  ...
  async function showVariantInDOM(sessionId, num) {
    ...
    const wrapper = document.querySelector('[data-zarishnoksha-variants="' + sessionId + '"]');
    if (!wrapper) return false;
    for (const child of wrapper.children) {
      const v = child.dataset ? child.dataset.zarishnokshaVariant : null;
      if (!v) continue;
  ...
  function parseOriginalMarkupElement(originalMarkup) {
    ...
    const doc = parser.parseFromString('<div id="zarishnoksha-anchor">' + originalMarkup + "</div>", "text/html");
    return doc.getElementById("zarishnoksha-anchor")?.firstElementChild || null;
  }
  ...
  function isUsableInjectionAnchor(el) {
    return (
      ...
      !own(el) &&
      !el.closest?.("[data-zarishnoksha-variants]")
    );
  }
  ...
  function waitForVariantAnchorAndRetry({ filePath, sessionId, srcWrapper, checkpointReason }) {
    ...
    const origContent = srcWrapper?.querySelector('[data-zarishnoksha-variant="original"] > :first-child');
    ...
    pendingVariantAnchorRetryObserver = new MutationObserver(() => {
      ...
      const wrapperLanded = !!document.querySelector('[data-zarishnoksha-variants="' + sessionId + '"]');
      if (!wrapperLanded) {
  ...
  function loadSvelteRuntime(runtimeModule) {
    const modulePath = runtimeModule || "/src/lib/zarishnoksha/__runtime.js";
  ...
  // componentDir (keyed by variant number), because a `data-zarishnoksha-params`
  ...
  async function applySvelteComponentVariantStyle(variantNum) {
    ...
    if (!scopedCss) return;
    const style = document.createElement("style");
    style.dataset.zarishnokshaSvelteComponentStyle = sessionId;
    style.dataset.zarishnokshaVariant = String(variantNum);
  ...
  function scopeCssToSveltePreview(css, sessionId) {
    const prefix = '[data-zarishnoksha-variants="' + String(sessionId).replace(/"/g, '\\"') + '"] ';
  ...
  async function mountSvelteComponentVariant(variantNum) {
    ...
    try {
      ...
      console.error("[zarishnoksha] Failed to mount Svelte variant " + variantNum + " for " + sessionId + ":", err);
      return false;
    }
  }
  ...
  async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
    ...
    try {
      ...
      const existingWrapper = document.querySelector('[data-zarishnoksha-variants="' + sessionId + '"]');
      ...
      if (!liveEl?.parentElement) {
        console.warn("[zarishnoksha] Could not find original element in live DOM.");
        arrivedVariants = Number(manifest.count) || expectedVariants || 1;
        expectedVariants = arrivedVariants;
      ...
      wrapper.dataset.zarishnokshaVariants = sessionId;
      wrapper.dataset.zarishnokshaVariantCount = String(manifest.count || expectedVariants || 1);
      wrapper.dataset.zarishnokshaPreview = "svelte-component";
      wrapper.style.display = "contents";

      const mountTarget = document.createElement("div");
      mountTarget.dataset.zarishnokshaComponentMount = sessionId;
      ...
      positionBar();
      saveSession();
      console.log("[zarishnoksha] Mounted " + arrivedVariants + " Svelte component variants.");
    } catch (err) {
      console.error("[zarishnoksha] Failed to mount Svelte component variants:", err);
  ...
  function abortSvelteComponentInjection(sessionId, message) {
    try {
      if (svelteComponentSession?.sessionId === sessionId) {
        ...
        const orphan = document.querySelector('[data-zarishnoksha-variants="' + sessionId + '"]');
        if (orphan) orphan.remove();
      }
    } catch (err) {
      console.warn("[zarishnoksha] Svelte component abort cleanup failed:", err);
    }
    hideShaderOverlay();
    if (variantObserver) {
  ...
  function injectVariantsFromSource(filePath, sessionId) {
    ...
    fetch(url)
      ...
      .then((html) => {
        ...
        const startMark = "<!-- zarishnoksha-variants-start " + sessionId + " -->";
        const endMark = "<!-- zarishnoksha-variants-end " + sessionId + " -->";
        ...
        srcWrapper = doc.querySelector('[data-zarishnoksha-variants="' + sessionId + '"]');
        if (!srcWrapper) {
          console.warn("[zarishnoksha] Variant wrapper not found in source file.");
          return;
        }
        ...
        expectedVariants = parseInt(wrapper.dataset.zarishnokshaVariantCount || arrivedVariants);
        if (arrivedVariants <= 0) {
        ...
        positionBar();
        saveSession();
        console.log("[zarishnoksha] Injected " + arrivedVariants + " variants from source file.");
      })
      .catch((err) => {
        console.error("[zarishnoksha] Failed to fetch source:", err);
  ...
  function updateSelectedElement() {
    ...
    if (svelteComponentSession?.sessionId === currentSessionId) {
      const anchor = resolveSvelteComponentAnchor();
      if (anchor && !anchor.__zarishnokshaFrozenAnchor) selectedElement = anchor;
      return;
    }
    const wrapper = document.querySelector('[data-zarishnoksha-variants="' + currentSessionId + '"]');
    if (!wrapper) return;
  ...
  function readVisibleVariantFromDOM(sessionId) {
    ...
    for (const variant of variants) {
      ...
      const idx = parseInt(variant.dataset.zarishnokshaVariant || "0", 10);
      if (idx > 0) return idx;
    }
    return 0;
  }
  ...
  function pickVariantContent(wrapper, index) {
    if (!wrapper) return null;
    const variantDiv = wrapper.querySelector('[data-zarishnoksha-variant="' + index + '"]');
  ...
  function startScrollLock(sessionId, initialTargetY) {
    ...
    scrollLockObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.target?.closest?.('[data-zarishnoksha-variants="' + sessionId + '"]')) {
        ...
        for (const n of m.addedNodes) {
          if (
            n.nodeType === 1 &&
            (n.matches?.('[data-zarishnoksha-variants="' + sessionId + '"]') ||
              n.querySelector?.('[data-zarishnoksha-variants="' + sessionId + '"]'))
          ) {
  ...
  function startVariantObserver(sessionId) {
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      // Only react to mutations that add nodes with data-zarishnoksha-variant,
      // or mutations inside the variant wrapper. Ignore our own bar/UI changes.
      let dominated = false;
      for (const m of mutations) {
        if (m.target.closest?.("[data-zarishnoksha-variants]")) {
          dominated = true;
          break;
        }
        for (const n of m.addedNodes) {
          ...
          if (n.dataset?.zarishnokshaVariants || n.dataset?.zarishnokshaVariant) {
            dominated = true;
            break;
          }
          ...
          if (n.querySelector?.("[data-zarishnoksha-variants],[data-zarishnoksha-variant]")) {
            dominated = true;
            break;
          }
        }
      ...
      if (selectedElement && !document.body.contains(selectedElement)) {
        const isInsert = wrapper.dataset.zarishnokshaMode === "insert";
        if (isInsert) {
      ...
      const expected = parseInt(wrapper.dataset.zarishnokshaVariantCount || "0");
      ...
      if (arrivedVariants >= expectedVariants && expectedVariants > 0) {
        ...
        recoveryWaitingForAnchor = false;
        hideShaderOverlay();
        if (wrapper.dataset.zarishnokshaMode === "insert") finalizeInsertSession();
        updateSelectedElement();
        showOrUpdateCyclingBar();
        disableInlineEdit();
        refreshParamsPanel();
  ...
  function startScrollTracking() {
    function tick() {
      if (state === "CONFIGURING" || state === "GENERATING" || state === "CYCLING") {
        ...
        if (hiTarget && !hiTarget.hasAttribute?.("data-zarishnoksha-insert-placeholder")) {
  ...
  function connectSSE() {
      ...
      switch (msg.type) {
        case "connected":
          ...
          if (!hasProjectContext)
            showToast(
              "No PRODUCT.md found. Variants will be brand-agnostic. Run /zarishnoksha init to generate one.",
              7000,
            );
          console.log("[zarishnoksha] Live mode connected.");
        ...
        case "error":
          ...
          if (maybeCompleteSteer(msg)) break;
          console.error("[zarishnoksha] Error:", msg.message);
    ...
    evtSource.onerror = () => {
      ...
      if (sseRetries <= SSE_MAX_RETRIES) {
        console.log("[zarishnoksha] SSE connection lost. Retry " + sseRetries + "/" + SSE_MAX_RETRIES + "...");
        return; // EventSource auto-reconnects
      }
      // Server is gone. Clean up gracefully.
      console.log("[zarishnoksha] Live server unreachable. Cleaning up UI.");
      evtSource.close();
      evtSource = null;
      handleServerLost();
    };
  ...
  function sendEvent(msg, opts) {
    ...
    function handleFailure(err) {
      if (opts && opts.throwOnError) {
        console.error("[zarishnoksha] Failed to send event:", err);
        throw err;
      }
      console.debug("[zarishnoksha] Dropped optional live event:", err);
      return null;
    }
  ...
  function handleKeyDown(e) {
    ...
    if (e.target.isContentEditable && isInlineEditActive(e.target)) {
      ...
      e.preventDefault();
      e.stopPropagation();
      const original = e.target.dataset.zarishnokshaOriginalText;
  ...
  function shouldUseAncestorCropShaderProxy(el) {
    ...
    const adapter = String(window.__zarishnoksha_LIVE_ADAPTER__ || "").toLowerCase();
    if (adapter === "svelte" || adapter === "sveltekit") return true;
    if (currentPreviewMode === "svelte-component" || svelteComponentSession) return true;
    const wrapper = el?.closest?.("[data-zarishnoksha-variants]");
    return wrapper?.dataset?.zarishnokshaPreview === "svelte-component";
  }
    ...
    try {
      ...
      if (shouldUseAncestorCropShaderProxy(el)) {
        try {
          ...
          console.warn("[zarishnoksha] Svelte ancestor crop capture failed, falling back to element capture:", err);
        }
      }
  ...
  async function captureAndEmit(el, basePayload, snapshot, rect) {
    ...
    try {
      ...
      console.warn("[zarishnoksha] capture failed, proceeding without screenshot:", err);
    }
    ...
    if (blob && hasAnnotations) {
      try {
        ...
        if (uploadRes.ok) {
          ...
          console.warn("[zarishnoksha] annotation upload failed:", uploadRes.status);
        }
      } catch (err) {
        console.warn("[zarishnoksha] annotation upload failed:", err);
      }
    }
...
void main() {
  ...
  async function showShaderOverlay(el, blob, rect, paper) {
    ...
    try {
      ...
      console.warn("[zarishnoksha] shader setup failed:", err);
      canvas.remove();
      return;
    }
    ...
    try {
      ...
      console.warn("[zarishnoksha] shader bitmap decode failed:", err);
  ...
  async function handleAccept() {
    ...
    const acceptWrapper = document.querySelector('[data-zarishnoksha-variants="' + currentSessionId + '"]');
    ...
    const acceptedIsSvelteComponent =
      ...
      acceptWrapper?.dataset?.zarishnokshaPreview === "svelte-component";
  ...
  function snapshotAcceptedVariantDom(sessionId, variantId) {
    const wrapper = document.querySelector('[data-zarishnoksha-variants="' + sessionId + '"]');
    const accepted = wrapper?.querySelector?.('[data-zarishnoksha-variant="' + variantId + '"]');
  ...
  function acceptedDomAlreadyClean(pending) {
    ...
    return (
      matches.length > 0 &&
      matches.every(
        (el) => !el.closest("[data-zarishnoksha-variants],[data-zarishnoksha-variant],[data-zarishnoksha-carbonize]"),
      )
    );
  }
  ...
  function findAcceptedRuntimeWrappers(sessionId) {
    ...
    return [
      ...new Set([
        ...document.querySelectorAll('[data-zarishnoksha-variants="' + sessionId + '"]'),
        ...document.querySelectorAll('[data-zarishnoksha-carbonize="' + sessionId + '"]'),
      ]),
    ];
  }
  ...
  function reloadAfterMissingAcceptedDom(pending) {
    if (acceptedDomAlreadyClean(pending)) return;
    if (pending?.id && document.querySelector('[data-zarishnoksha-variants="' + pending.id + '"]')) return;
    location.reload();
  }
  ...
  function commitAcceptedVariantToDom(sessionId, variantId) {
    ...
    const style = wrapper.querySelector("style[data-zarishnoksha-css]");
    if (style && !document.querySelector('style[data-zarishnoksha-accepted-css="' + sessionId + '"]')) {
      const promotedStyle = style.cloneNode(true);
      promotedStyle.setAttribute("data-zarishnoksha-accepted-css", sessionId);
  ...
  function restoreFromActiveSessions(activeSessions, reason) {
    const wrapper = document.querySelector("[data-zarishnoksha-variants]");
    if (wrapper && wrapper.dataset.zarishnokshaPreview !== "svelte-component") return false;
  ...
  function cleanup() {
    ...
    if (svelteComponentSession?.sessionId === cleanupSessionId) {
      ...
      const wrapper = document.querySelector('[data-zarishnoksha-variants="' + cleanupSessionId + '"]');
      if (wrapper) wrapper.style.display = "none";
      setTimeout(() => {
        if (!cleanupSessionId) return;
        const lateWrapper = document.querySelector('[data-zarishnoksha-variants="' + cleanupSessionId + '"]');
        if (!lateWrapper) return;
        const orig = lateWrapper.querySelector('[data-zarishnoksha-variant="original"]');
        if (orig) {
  ...
  // If a [data-zarishnoksha-variants] wrapper exists in the DOM, the agent wrote
  ...
  function resumeSession() {
    ...
    const sessionId = wrapper.dataset.zarishnokshaVariants;
    ...
    // wrapper holds a single mount target, not [data-zarishnoksha-variant] nodes,
    ...
    if (wrapper.dataset.zarishnokshaPreview === "svelte-component" && svelteComponentSession?.sessionId !== sessionId) {
      wrapper.remove();
    ...
    if (wrapper.dataset.zarishnokshaPreview === "svelte-component") {
      if (!svelteComponentSession?.mountedVariant) {
        return true;
      }
      currentSessionId = sessionId;
      expectedVariants =
        Number(wrapper.dataset.zarishnokshaVariantCount) ||
    ...
    currentSessionId = sessionId;
    expectedVariants = parseInt(wrapper.dataset.zarishnokshaVariantCount || "0");
    const variants = wrapper.querySelectorAll(
      '[data-zarishnoksha-variant]:not([data-zarishnoksha-variant="original"])',
    );
    arrivedVariants = variants.length;
      ...
      if (shaderTarget) {
        (async () => {
          try {
            ...
            console.warn("[zarishnoksha] shader resume failed:", err);
          }
        })();
      }
    }
  ...
  const PICK_PREFS_KEY = "zarishnoksha-live-pick";
  const INTERACTION_PREFS_KEY = "zarishnoksha-live-interaction";
  ...
  function detectPageTheme() {
    try {
      // Dev override: set localStorage 'zarishnoksha-dev-theme' to 'light' or
      // 'dark' to preview the opposite palette without actually changing the
      // page bg. Used for screenshots and theme QA.
      const override = localStorage.getItem("zarishnoksha-dev-theme");
  ...
  function attachSteerFocusGuard() {
    if (window.__zarishnoksha_STEER_FOCUS_GUARD__) return;
    window.__zarishnoksha_STEER_FOCUS_GUARD__ = true;
  ...
  function steerFocusDebugEnabled() {
    try {
      return localStorage.getItem("zarishnoksha-steer-debug") === "1";
    } catch {
      return false;
    }
  }
  ...
  function steerFocusLog(reason, extra) {
    ...
    console.log("[zarishnoksha.steer]", reason, {
      state,
      pickActive,
  ...
  function attachSteerFocusDebug() {
    if (!steerFocusDebugEnabled()) return;
    if (window.__zarishnoksha_STEER_FOCUS_DEBUG__) return;
    window.__zarishnoksha_STEER_FOCUS_DEBUG__ = true;
    document.addEventListener(
      "focusin",
      (e) => {
    ...
    for (let i = 0; i < 3; i++) {
      wrap.appendChild(
        el("span", {
          ...
          animation: "zarishnoksha-steer-dot 1.05s ease-in-out " + i * 0.14 + "s infinite",
        }),
      );
    }
    return wrap;
  ...
  function startVoice(ctx) {
    ...
    rec.onerror = (event) => {
      ...
      console.warn("[zarishnoksha.voice] recognition error:", code);
    ...
    try {
      ...
      console.warn("[zarishnoksha.voice] start failed:", err);
      stopVoice({
        suppressSubmit: true,
  ...
  function initPageChat(parent, P) {
    ...
    if (!uiGetById(PREFIX + "-page-chat-style")) {
      ...
      s.textContent =
        "@keyframes zarishnoksha-steer-dot { 0%, 70%, 100% { opacity: 0.28; transform: scale(0.82); } 35% { opacity: 1; transform: scale(1); } }" +
        "@keyframes zarishnoksha-steer-processing { 0%, 100% { border-color: oklch(70% 0.12 188 / 0.28); box-shadow: 0 0 0 0 oklch(70% 0.12 188 / 0); } 50% { border-color: oklch(82% 0.07 188 / 0.55); box-shadow: 0 0 14px oklch(70% 0.12 188 / 0.18); } }" +
        "@keyframes zarishnoksha-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }" +
        "#" +
        PREFIX +
        '-page-chat[data-processing="true"] { animation: zarishnoksha-steer-processing 1.6s ease-in-out infinite; }' +
        "@media (prefers-reduced-motion: reduce) { #" +
        PREFIX +
        ...
        "#" +
        PREFIX +
        '-page-chat-voice[data-listening="true"] svg { animation: zarishnoksha-voice-pulse 1.1s ease-in-out infinite; }' +
        "@media (prefers-reduced-motion: reduce) { #" +
        PREFIX +
  ...
  // zarishnoksha mark - same paths as site/components/Header.astro + favicon.svg.
  ...
  function syncAgentPollingUi(connected) {
    ...
    globalBarBrandEl.setAttribute(
      "aria-label",
      connected ? "zarishnoksha live mode" : "zarishnoksha live mode - agent not polling",
    );
  ...
  function initGlobalBar() {
    ...
    if (!uiGetById(PREFIX + "-bar-focus-style")) {
      ...
      s.textContent =
        ...
        ";" +
        "}" +
        "@keyframes zarishnoksha-agent-dot { 0%, 100% { opacity: 0.45; transform: scale(0.9); } 50% { opacity: 1; transform: scale(1); } }" +
        "#" +
        PREFIX +
        '-global-bar-brand[data-agent-connected="false"] [data-agent-dot] { animation: zarishnoksha-agent-dot 1.4s ease-in-out infinite; }' +
        "@media (prefers-reduced-motion: reduce) { #" +
        PREFIX +
    ...
    // Brand mark - kinpaku zarishnoksha icon (site header / favicon paths).
    const brand = el("span", {
      display: "inline-flex",
      alignItems: "center",
      justifyContent: "center",
    ...
    brand.setAttribute("aria-label", "zarishnoksha live mode - agent not polling");
  ...
  function updateGlobalBarState() {
    ...
    document.querySelectorAll(".zarishnoksha-overlay").forEach((o) => {
      o.style.pointerEvents = pickActive || insertActive ? "none" : "";
    });
    syncPageInteractionCursor();
  }

  let detectReady = false; // true once detect script posts 'zarishnoksha-ready'
  ...
  function toggleDetect() {
    ...
    if (detectActive) {
      ...
      window.postMessage({ source: "zarishnoksha-command", action: "remove" }, "*");
  ...
  function loadDetectScript() {
    ...
    s.dataset.zarishnokshaExtension = "true";
  ...
  function onDetectMessage(e) {
    ...
    if (e.data.source === "zarishnoksha-ready") {
      detectReady = true;
    ...
    if (e.data.source === "zarishnoksha-results") {
  ...
  function teardown() {
    ...
    window.__zarishnoksha_LIVE_INIT__ = false;
    console.log("[zarishnoksha] Live mode exited.");
  }

  //
  // Design System Panel - visualizes the project's .zarishnoksha/design.json sidecar
  //

  const DESIGN_PREFS_KEY = "zarishnoksha-live-design-panel";
  ...
  const designState = {
    ...
    sidecar: null, // .zarishnoksha/design.json v2 payload (extensions + components + narrative)
    hasMd: false,
    hasSidecar: false,
  ...
  // Neutral panel palette - deliberately NOT zarishnoksha-branded. The panel is
  // a viewer of the project's design system, not an zarishnoksha surface.
  const DP = {
  ...
  function renderDesignBody() {
    ...
    if (designState.present === false) {
      ...
      empty.innerHTML = `<strong>No DESIGN.md yet</strong>Create one by running <code>/zarishnoksha document</code> in your terminal, then re-open this panel.`;
  ...
  function renderStaleHint() {
    ...
    box.innerHTML = `
      ...
      <span class="stale-text"><strong>DESIGN.md is newer than .zarishnoksha/design.json.</strong> Run <code>/zarishnoksha document</code> to refresh the sidecar.</span>
    `;
    return box;
  }
  ...
  function renderParsedMdCta() {
    ...
    box.innerHTML = `<strong>Basic view</strong>This panel reads the tokens in your <code>DESIGN.md</code> frontmatter. Running <code>/zarishnoksha document</code> also generates a <code>.zarishnoksha/design.json</code> sidecar with your project's actual component snippets (button, input, nav) and tonal ramps, rendered live below the tokens.`;
    return box;
  }
  ...
  function cssSafe(v) {
    ...
    // .zarishnoksha/design.json values rendered into inline style strings.
  ...
  function init() {
    ...
    if (!resumeSession()) {
      console.log("[zarishnoksha] Live variant mode ready. Hover over elements to pick one.");
      ...
      const scout = new MutationObserver(() => {
        ...
        scout.disconnect();
        if (resumeSession()) {
          console.log("[zarishnoksha] Resumed deferred session " + currentSessionId + " (post-hydration).");
        }
      });
      scout.observe(document.body, { childList: true, subtree: true });
    } else {
      console.log(
        "[zarishnoksha] Resumed active variant session " +
          currentSessionId +
          " (" +
          arrivedVariants +
          "/" +
