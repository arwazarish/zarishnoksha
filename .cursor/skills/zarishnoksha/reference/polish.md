...
4. **Pull in any prior critique** (optional signal): If `/zarishnoksha critique` has been run on the same target, its priority issues are a useful prior for what to address first. Resolve the target to a file path or URL, then:
   ```bash
   slug=$(node .cursor/skills/zarishnoksha/scripts/critique-storage.mjs slug "<resolved>")
   node .cursor/skills/zarishnoksha/scripts/critique-storage.mjs latest "$slug"
   ```
