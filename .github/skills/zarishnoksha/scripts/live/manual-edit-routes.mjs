...
// Cap JSON request bodies for the manual-edit POST routes. Staged edit metadata
// stays well under this; the cap guards against unbounded body accumulation.
const MAX_MANUAL_EDIT_BODY_BYTES = 10 * 1024 * 1024;

// Read the request body as parsed JSON, rejecting oversized payloads with 413
// (mirrors the /annotation upload cap in live-server.mjs).
function readBodyJson(req, res, onDone) {
  const chunks = [];
  let total = 0;
  let aborted = false;
  req.on("data", (c) => {
    if (aborted) return;
    total += c.length;
    if (total > MAX_MANUAL_EDIT_BODY_BYTES) {
      aborted = true;
    ...
    if (p === "/manual-edit-commit" && req.method === "POST") {
      ...
      (async () => {
        ...
        try {
          ...
          const requestedMode = (envValue.zarishnoksha_LIVE_COPY_AGENT || "auto").trim().toLowerCase();
          const useChatRoute = requestedMode === "chat" || (requestedMode === "auto" && chatAgentLikelyActive());
          if (useChatRoute) {
            routedProvider = "chat";
            const timeoutMs = Number(envValue.zarishnoksha_LIVE_COPY_AGENT_TIMEOUT_MS || 120000);
