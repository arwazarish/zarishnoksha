...
(() => {
  ...
  const REPEATED_KICKER_SKIP_SELECTOR = [
    ...
    '[class*="breadcrumb" i]',
    '[aria-hidden="true"]',
    "[data-zarishnoksha-allow-kickers]",
  ].join(",");
  ...
  function checkElementQualityDOM(el) {
    ...
    const rect = el.getBoundingClientRect();
    const lineMax = (typeof window !== "undefined" && window.__zarishnoksha_CONFIG__?.lineLengthMax) || 80;
  ...
  function checkTypography() {
    ...
    for (const el of document.querySelectorAll(
      ...
      // Skip zarishnoksha's own elements
      if (
        el.closest &&
        el.closest(".zarishnoksha-overlay, .zarishnoksha-label, .zarishnoksha-banner, .zarishnoksha-tooltip")
      )
        continue;
  ...
  if (IS_BROWSER) {
    ...
    const EXTENSION_MODE =
      (_myScript && _myScript.dataset.zarishnokshaExtension === "true") ||
    ...
    styleEl.textContent = `
    @keyframes zarishnoksha-reveal {
      from { opacity: 0; }
      to { opacity: 1; }
    }
    .zarishnoksha-overlay:not(.zarishnoksha-banner) {
      pointer-events: none;
      outline: 2px solid ${OUTLINE_COLOR};
      border-radius: 4px;
      transition: outline-color 0.15s ease;
      animation: zarishnoksha-reveal 0.4s cubic-bezier(0.16, 1, 0.3, 1) both;
      animation-play-state: paused;
      border-top-left-radius: 0;
    }
    .zarishnoksha-overlay.zarishnoksha-visible {
      animation-play-state: running;
    }
    .zarishnoksha-overlay.zarishnoksha-hover {
      outline-color: ${BRAND_COLOR_HOVER};
      z-index: 100001 !important;
    }
    .zarishnoksha-overlay.zarishnoksha-hover .zarishnoksha-label {
      background: ${BRAND_COLOR_HOVER};
    }
    .zarishnoksha-overlay.zarishnoksha-spotlight {
      z-index: 100002 !important;
    }
    .zarishnoksha-overlay.zarishnoksha-spotlight-dimmed {
      opacity: 0.15 !important;
      animation: none !important;
      filter: blur(3px);
    }
    .zarishnoksha-spotlight-backdrop {
      position: fixed;
    ...
    .zarishnoksha-spotlight-backdrop.zarishnoksha-visible {
      opacity: 1;
    }
    .zarishnoksha-hidden .zarishnoksha-overlay${EXTENSION_MODE ? "" : ":not(.zarishnoksha-banner)"} {
  ...
  `;
    ...
    function getSpotlightBackdrop() {
      if (!spotlightBackdrop) {
        ...
        spotlightBackdrop.className = "zarishnoksha-spotlight-backdrop";
    ...
    function showSpotlight(target) {
      ...
      if (window.__zarishnoksha_CONFIG__?.spotlightBlur === false) {
      ...
      bd.classList.add("zarishnoksha-visible");
    }

    function hideSpotlight() {
      spotlightTarget = null;
      if (spotlightBackdrop) spotlightBackdrop.classList.remove("zarishnoksha-visible");
    }
    ...
    function repositionOverlays() {
      for (const o of overlays) {
        if (!o._targetEl || o.classList.contains("zarishnoksha-banner")) continue;
    ...
    const visibilityObserver = new IntersectionObserver(
      (entries) => {
        for (const entry of entries) {
          const overlay = entry.target._zarishnokshaOverlay;
          if (!overlay) continue;
          if (entry.isIntersecting) {
            ...
            if (!overlay._revealed) {
              ...
              requestAnimationFrame(() => {
                overlay.classList.add("zarishnoksha-visible");
    ...
    function detachOverlay(overlay) {
      ...
      if (overlay._targetEl && overlay._targetEl._zarishnokshaOverlay === overlay) {
        visibilityObserver.unobserve(overlay._targetEl);
        delete overlay._targetEl._zarishnokshaOverlay;
      }
    ...
    document.addEventListener("transitionend", (e) => {
      ...
      for (const o of overlays) {
        if (!o._targetEl || o.classList.contains("zarishnoksha-banner") || o.style.display === "none") continue;
    ...
    const highlight = (el, findings) => {
      if (el._zarishnokshaOverlay) detachOverlay(el._zarishnokshaOverlay);
      const hasSlop = findings.some((f) => RULE_CATEGORY[f.type || f.id] === "slop");

      const fixed = isInFixedContext(el);
      const rect = el.getBoundingClientRect();
      const outline = document.createElement("div");
      outline.className = "zarishnoksha-overlay";
      ...
      label.className = "zarishnoksha-label";
      ...
      el._zarishnokshaOverlay = outline;
      ...
      const onMouseEnter = () => {
        isHovered = true;
        outline.classList.add("zarishnoksha-hover");
      ...
      const onMouseLeave = () => {
        isHovered = false;
        outline.classList.remove("zarishnoksha-hover");
    ...
    const showPageBanner = (findings) => {
      ...
      banner.className = "zarishnoksha-overlay zarishnoksha-banner";
      ...
      if (!EXTENSION_MODE) {
        ...
        toggle.addEventListener("click", () => {
          ...
          document.body.classList.toggle("zarishnoksha-hidden", !overlaysVisible);
    ...
    function buildSelectorSegment(el) {
      ...
      if (el.classList && el.classList.length > 0) {
        const classes = [...el.classList]
          .filter((c) => !c.startsWith("zarishnoksha-") && !isLikelyHashedClass(c))
          .slice(0, 2);
        if (classes.length > 0) {
    ...
    function collectVisualContrastCandidates(options = {}) {
      ...
      for (const el of document.querySelectorAll("*")) {
        ...
        if (el.closest(".zarishnoksha-overlay, .zarishnoksha-label, .zarishnoksha-banner, .zarishnoksha-tooltip"))
          continue;
        if (el.closest('[id^="zarishnoksha-live-"]')) continue;
    ...
    async function sampleVisualBackgroundAtPoint(el, point, textColor, depth = 0) {
      ...
      for (const node of nodes) {
        ...
        if (node.closest?.(".zarishnoksha-overlay, .zarishnoksha-label, .zarishnoksha-banner, .zarishnoksha-tooltip"))
          continue;
    ...
    const printSummary = (allFindings) => {
      if (allFindings.length === 0) {
        console.log("%c[zarishnoksha] No anti-patterns found.", "color: #22c55e; font-weight: bold");
        return;
      }
      console.group(
        `%c[zarishnoksha] ${allFindings.length} anti-pattern${allFindings.length === 1 ? "" : "s"} found`,
    ...
    function browserDesignSystemConfig() {
      const raw = window.__zarishnoksha_CONFIG__?.designSystem;
    ...
    function collectBrowserFindings() {
      ...
      const _disabled = EXTENSION_MODE ? window.__zarishnoksha_CONFIG__?.disabledRules || [] : [];
      ...
      for (const el of document.querySelectorAll("*")) {
        // Skip zarishnoksha's own elements and any descendants (overlays, labels, banner, nav buttons)
        if (el.closest(".zarishnoksha-overlay, .zarishnoksha-label, .zarishnoksha-banner, .zarishnoksha-tooltip"))
          continue;
        // Skip browser extension elements (Claude, etc.)
        const elId = el.id || "";
        if (elId.startsWith("claude-") || elId.startsWith("cic-")) continue;
        // Skip the zarishnoksha live-mode overlay (highlight, tooltip, bar, picker, toast).
      ...
      // Clone the document and strip zarishnoksha-live overlay nodes before the
      ...
      for (const node of docClone.querySelectorAll('[id^="zarishnoksha-live-"]')) {
        node.remove();
      }
    ...
    function shouldRunVisualContrast(options = {}) {
      return options.visualContrast === true || window.__zarishnoksha_CONFIG__?.visualContrast === true;
    }

    function visualContrastOptions(options = {}) {
      const config = window.__zarishnoksha_CONFIG__ || {};
    ...
    function reportVisualContrastError(err, detail = {}) {
      window.dispatchEvent(
        new CustomEvent("zarishnoksha-visual-contrast-error", {
          detail: {
            ...detail,
            message: err?.message || String(err),
          },
      ...
      if (EXTENSION_MODE) {
        ...
        console.warn("[zarishnoksha] visual contrast scan failed", err);
      }
    }
            ...
            waitForVisualPaint()
              ...
              .then((result) => {
                ...
                if (added) {
                  ...
                  window.dispatchEvent(
                    new CustomEvent("zarishnoksha-visual-contrast-resolved", {
                      detail: {
    ...
    if (EXTENSION_MODE) {
      ...
      window.addEventListener("message", (e) => {
        if (e.source !== window || !e.data || e.data.source !== "zarishnoksha-command") return;
        if (e.data.action === "scan") {
          if (e.data.config) window.__zarishnoksha_CONFIG__ = e.data.config;
          try {
            scan(e.data.config || {});
          } catch (err) {
            postExtensionError(err);
        ...
        if (e.data.action === "toggle-overlays") {
          const visible = !document.body.classList.contains("zarishnoksha-hidden");
          document.body.classList.toggle("zarishnoksha-hidden", visible);
          window.postMessage({ source: "zarishnoksha-overlays-toggled", visible: !visible }, "*");
        }
        if (e.data.action === "remove") {
          ...
          document.body.classList.remove("zarishnoksha-hidden");
        }
        if (e.data.action === "highlight") {
          try {
            ...
            if (target) {
              ...
              for (const o of overlays) {
                if (o.classList.contains("zarishnoksha-banner")) continue;
                const isMatch = o._targetEl === target;
                o.classList.toggle("zarishnoksha-spotlight", isMatch);
                o.classList.toggle("zarishnoksha-spotlight-dimmed", !isMatch);
                if (isMatch) {
                  // Force the matching overlay visible immediately, don't wait for IntersectionObserver
                  o.style.display = "";
                  o.style.animation = "none";
                  o.classList.add("zarishnoksha-visible");
                  o._revealed = true;
                  positionOverlay(o);
                }
              }
        ...
        if (e.data.action === "unhighlight") {
          hideSpotlight();
          for (const o of overlays) {
            o.classList.remove("zarishnoksha-spotlight");
            o.classList.remove("zarishnoksha-spotlight-dimmed");
          }
        }
      });
      window.postMessage({ source: "zarishnoksha-ready" }, "*");
    } else {
      if (window.__zarishnoksha_CONFIG__?.autoScan !== false) {
        const runAutoScan = () => {
          try {
            scan();
          } catch (err) {
            console.warn("[zarishnoksha] scan failed", err);
          }
        };
    ...
    window.zarishnokshaDetect = detect;
    window.zarishnokshaDetectAsync = detectAsync;
    window.zarishnokshaScan = scan;
    window.zarishnokshaScanAsync = scanAsync;
    window.zarishnokshaCollectVisualContrastCandidates = collectVisualContrastCandidates;
    window.zarishnokshaAnalyzeVisualContrast = analyzeVisualContrast;
    window.zarishnokshaGetLastVisualContrastAnalyses = () => lastVisualContrastAnalyses.slice();
  }
})();
