...
export const zarishnoksha_DIR = ".zarishnoksha";
export const LIVE_DIR = "live";
export const CRITIQUE_DIR = "critique";

export function getzarishnokshaDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), zarishnoksha_DIR);
}

export function getDesignSidecarPath(cwd = process.cwd(), options = {}) {
  return path.join(getzarishnokshaDir(cwd, options), "design.json");
}
...
export function getLiveDir(cwd = process.cwd(), options = {}) {
  return path.join(getzarishnokshaDir(cwd, options), LIVE_DIR);
}
...
export function resolveLiveConfigPath({ cwd = process.cwd(), scriptsDir, env = process.env, targetPath } = {}) {
  if (env.zarishnoksha_LIVE_CONFIG && env.zarishnoksha_LIVE_CONFIG.trim()) {
    const configured = env.zarishnoksha_LIVE_CONFIG.trim();
...
export function getLegacyLiveServerPath(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), ".zarishnoksha-live.json");
}
...
export function getLegacyLiveSessionsDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), ".zarishnoksha-live", "sessions");
}
...
export function getCritiqueDir(cwd = process.cwd(), options = {}) {
  return path.join(getzarishnokshaDir(cwd, options), CRITIQUE_DIR);
}

export function getLegacyLiveAnnotationsDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), ".zarishnoksha-live", "annotations");
}
