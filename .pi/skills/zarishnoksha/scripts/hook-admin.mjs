#!/usr/bin/env node
/**
 * `/zarishnoksha hooks <on|off|status|reset>` — manage the design hook runtime
 * via the `hook` key and shared detector ignores via the `detector` key in
 * .zarishnoksha/config.json / .zarishnoksha/config.local.json.
 *
 * Usage:
...
const HOOK_MANIFEST_TARGETS = [
  {
    provider: ".claude",
    skillRel: ".claude/skills/zarishnoksha",
    destRel: ".claude/settings.local.json",
    sharedDestRel: ".claude/settings.json",
    manifest: () => ({
      description:
        "zarishnoksha design detector: runs after Edit/Write/MultiEdit on UI files and surfaces findings as system reminders.",
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: "command",
                command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/zarishnoksha/scripts/hook.mjs"',
  ...
  {
    provider: ".agents",
    skillRel: ".agents/skills/zarishnoksha",
    destRel: ".codex/hooks.json",
    manifest: () => ({
      description:
        "zarishnoksha design detector: runs after Edit/Write/apply_patch on UI files and surfaces findings as system reminders.",
      hooks: {
        PostToolUse: [
          {
  ...
  {
    provider: ".cursor",
    skillRel: ".cursor/skills/zarishnoksha",
  ...
  },
  {
    ...
    provider: ".github",
    skillRel: ".github/skills/zarishnoksha",
    destRel: ".github/hooks/zarishnoksha.json",
    manifest: () => ({
      version: 1,
      hooks: {
        postToolUse: [
          {
            ...
            bash: 'node "$(git rev-parse --show-toplevel)/.github/skills/zarishnoksha/scripts/hook.mjs"',
...
function writeHookConfig(cwd, hookConfig, opts = {}) {
  ...
  // (consent, quiet, auditLog) survive a `/zarishnoksha hooks` edit.
...
function statusReport(cwd) {
  ...
  const envKill = process.env.zarishnoksha_HOOK_DISABLED;
  const envState = envKill ? `zarishnoksha_HOOK_DISABLED=${envKill}` : "unset";
  const cfgPath = path.relative(cwd, getConfigPath(cwd)) || ".zarishnoksha/config.json";
  const localPath = path.relative(cwd, getLocalConfigPath(cwd)) || ".zarishnoksha/config.local.json";
  const cachePath = path.relative(cwd, getCachePath(cwd)) || ".zarishnoksha/hook.cache.json";
...
function fileHaszarishnokshaHookMarker(filePath) {
...
function valueHaszarishnokshaHookMarker(value) {
  ...
  if (value && typeof value === "object") return Object.values(value).some(valueHaszarishnokshaHookMarker);
  return false;
}
...
function addIgnoreRule(cwd, args) {
  ...
  if (!rule) throw new Error("Pass a rule id, e.g. /zarishnoksha hooks ignore-rule side-tab");
  if (rule === "overused-font" && !parsed.allValues) {
    throw new Error(
      "overused-font is value-specific by default. Use /zarishnoksha hooks ignore-value overused-font <font> for a confirmed font, or /zarishnoksha hooks ignore-rule overused-font --all-values only when the user asked to ignore overused fonts generally.",
    );
  }
...
function addIgnoreFile(cwd, glob) {
  if (!glob) throw new Error('Pass a glob, e.g. /zarishnoksha hooks ignore-file "src/legacy/**"');
  const config = mergeDetectorConfig(readRawDetectorConfig(cwd));
...
function addIgnoreValue(cwd, args) {
  ...
  if (!parsed.rule || !parsed.value) {
    throw new Error("Pass a rule id and value, e.g. /zarishnoksha hooks ignore-value overused-font Inter");
  }
