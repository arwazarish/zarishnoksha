 ...
 * an ignore in `.zarishnoksha/config.json`.
 *
 ...
 * line, so the same marker works across every comment style zarishnoksha scans —
 * `//`, `/* *\/`, `<!-- -->`, `#`, `{/* *\/}`, `{# #}`. Trailing comment closers
 * are stripped before the rule list is parsed.
 *
 * Syntax (reason optional; eslint `--` or biome `:` separator):
 *
 *   zarishnoksha-disable <rule>[, <rule>...] [-- reason]   whole file
 *   zarishnoksha-disable-line <rule>...      [-- reason]   the same line
 *   zarishnoksha-disable-next-line <rule>... [-- reason]   the following line
 *   zarishnoksha-disable                                   bare / `*` = every rule
 *
 * Examples:
 *
 ...
 *   // zarishnoksha-disable-next-line bounce-easing: intentional playful affordance
 *
...
const DIRECTIVE_RE = /zarishnoksha-(disable-next-line|disable-line|disable)\b[ \t]*([^\n\r]*)/gi;
...
function parseInlineIgnores(content) {
  ...
  // Case-insensitive to match DIRECTIVE_RE's `i` flag (e.g. `zarishnoksha-Disable`).
  if (!/zarishnoksha-disable/i.test(text)) return result;
