 ...
 *
 * Variants are real .svelte components under node_modules/.zarishnoksha-live/<session-id>/.
...
export const SVELTE_COMPONENT_ROOT = "node_modules/.zarishnoksha-live";
export const SVELTE_RUNTIME_FILE = `${SVELTE_COMPONENT_ROOT}/__runtime.js`;
export const DEFERRED_ACCEPTS_FILE = ".zarishnoksha/live/deferred-svelte-component-accepts.json";

const MUSTACHE_RE = /\{([^{}]+)\}/g;

export function shouldUseSvelteComponentInjection(filePath) {
  if (/^(0|false|no)$/i.test(process.env.zarishnoksha_LIVE_SVELTE_COMPONENT || "")) return false;
...
function buildInsertVariantStub(variantNum) {
  return `${buildPropsScript([])}<div class="zarishnoksha-insert-preview">Insert variant ${variantNum}</div>\n\n<style>\n  .zarishnoksha-insert-preview { display: block; }\n</style>\n`;
}
...
function sanitizeAcceptedSvelteCss(cssLines, variantNum, paramValues = null, rootTag = "div") {
  ...
  if (!/data-zarishnoksha-variant|zarishnoksha-variant-ready/.test(css)) return cssLines;
...
function appendSanitizedCssRule(output, rule, variantNum, paramValues, rootTag) {
  ...
  if (!prelude || !body || /--zarishnoksha-variant-ready\s*:/.test(body)) return;

  if (/^@scope\b/i.test(prelude)) {
    if (/data-zarishnoksha-variant/.test(prelude) && !selectorHasVariant(prelude, variantNum)) return;
    ...
    for (const innerRule of inner) {
      ...
      if (!rewrittenPrelude || /--zarishnoksha-variant-ready\s*:/.test(innerRule.body)) continue;
...
function variantSelectorRegex(variantNum) {
  return new RegExp(`\\[data-zarishnoksha-variant=(["'])${escapeRegExp(String(variantNum))}\\1\\]`, "g");
}
...
function inlineSvelteComponentInsertAccept({
  ...
  if (/\bdata-zarishnoksha-[\w-]*\s*=/.test(markup)) {
    return {
      handled: false,
      error: "Accepted Svelte insert variant contains preview-only data-zarishnoksha attributes",
      ...resultBase,
    };
  }
...
export function deferredAcceptsPath(cwd = process.cwd()) {
  ...
  return path.join(os.tmpdir(), "zarishnoksha-live", key, "deferred-svelte-component-accepts.json");
}
...
export function buildSvelteComponentCssAuthoring(count) {
  ...
  return {
    ...
    requirements: [
      ...
      'Declare params in componentDir/params.json keyed by variant number (e.g. {"1": [...], "2": [...]}), NOT as a data-zarishnoksha-params attribute.',
      "Do not use @scope or data-zarishnoksha-variant selectors in component files.",
    ...
    forbidden: [
      ...
      "Do not add data-zarishnoksha-* attributes inside component files. Svelte parses { in attribute values as an expression, so data-zarishnoksha-params with JSON breaks the build; use componentDir/params.json instead.",
    ],
