...
export const SVELTE_LIVE_ROOT_COMPONENT = "src/lib/zarishnoksha/zarishnokshaLiveRoot.svelte";
export const SVELTE_LAYOUT_MARKER_OPEN = "<!-- zarishnoksha-live-svelte-start -->";
export const SVELTE_LAYOUT_MARKER_CLOSE = "<!-- zarishnoksha-live-svelte-end -->";
export const SVELTE_ROOT_IMPORT = "import zarishnokshaLiveRoot from '$lib/zarishnoksha/zarishnokshaLiveRoot.svelte';";
...
export function buildSvelteLiveRootComponent(port) {
  ...
  const HOST_ID = 'zarishnoksha-live-root';

  onMount(() => {
    let host = document.querySelector('zarishnoksha-live-root#' + HOST_ID) || document.getElementById(HOST_ID);
    if (!host) {
      host = document.createElement('zarishnoksha-live-root');
      host.id = HOST_ID;
      document.body.appendChild(host);
    }

    host.dataset.zarishnokshaLiveAdapter = 'sveltekit';
    ...
    if (!root.querySelector('style[data-zarishnoksha-live-reset]')) {
      const reset = document.createElement('style');
      reset.dataset.zarishnokshaLiveReset = 'true';
      reset.textContent = ':host, :host *, * { box-sizing: border-box; }';
      root.appendChild(reset);
    }

    window.__zarishnoksha_LIVE_ADAPTER__ = 'sveltekit';
    window.__zarishnoksha_LIVE_UI_ROOT__ = root;
    window.__zarishnoksha_LIVE_CHROME_MOUNT__ = {
      adapter: 'sveltekit',
      version: 1,
      host,
      root,
    ...
    script.src = LIVE_URL;
    script.async = true;
    script.dataset.zarishnokshaLiveScript = 'true';
    document.head.appendChild(script);

    return () => {
      script.remove();
      if (window.__zarishnoksha_LIVE_UI_ROOT__ === root) delete window.__zarishnoksha_LIVE_UI_ROOT__;
      if (window.__zarishnoksha_LIVE_CHROME_MOUNT__?.root === root) delete window.__zarishnoksha_LIVE_CHROME_MOUNT__;
      if (window.__zarishnoksha_LIVE_ADAPTER__ === 'sveltekit') delete window.__zarishnoksha_LIVE_ADAPTER__;
    };
  });
</script>
`;
