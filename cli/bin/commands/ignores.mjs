...
} from "../../lib/zarishnoksha-config.mjs";
...
function printUsage() {
  console.log(`Usage: zarishnoksha ignores <action> [options]

Manage detector ignores in .zarishnoksha config.
...
Scope:
  --shared                              Write .zarishnoksha/config.json (default)
  --local                               Write .zarishnoksha/config.local.json
...
Examples:
  zarishnoksha ignores add-file "src/legacy/**"
  zarishnoksha ignores add-value overused-font Inter --reason "Brand font"
  zarishnoksha ignores add-value design-system-color "*" --file "src/demo.css"
  zarishnoksha ignores remove-value overused-font Inter`);
}
...
function parseValueArgs(args, { allowUnscopedWildcard = false } = {}) {
  ...
  if (!rule || !value)
    throw new Error("Pass a rule id and value, e.g. zarishnoksha ignores add-value overused-font Inter");
...
function formatValues(values) {
  ...
  return values
    .map((entry) => {
      const fileSuffix = Array.isArray(entry.files) && entry.files.length ? ` [${entry.files.join(", ")}]` : "";
...
function addRule(cwd, args) {
  ...
  if (!rule) throw new Error("Pass a rule id, e.g. zarishnoksha ignores add-rule side-tab");
  if (rule === "overused-font" && !allValues) {
    throw new Error(
      "overused-font is value-specific by default. Use add-value overused-font <font>, or add-rule overused-font --all-values for broad suppression.",
    );
  }
...
function addFile(cwd, args) {
  ...
  if (!glob) throw new Error('Pass a glob, e.g. zarishnoksha ignores add-file "src/legacy/**"');
  const config = readScopeConfig(cwd, local);
...
function removeRule(cwd, args) {
  return removeFromScopes(cwd, args, (config, rest) => {
    const rule = String(rest[0] || "")
      .trim()
      .toLowerCase();
    if (!rule) throw new Error("Pass a rule id, e.g. zarishnoksha ignores remove-rule side-tab");
...
function removeFile(cwd, args) {
  return removeFromScopes(cwd, args, (config, rest) => {
    const glob = String(rest[0] || "").trim();
    if (!glob) throw new Error('Pass a glob, e.g. zarishnoksha ignores remove-file "src/legacy/**"');
...
function ignoreValueKey(entry) {
  ...
  return `${String(entry.rule || "")
    .trim()
    .toLowerCase()}\0${normalizeIgnoreValue(entry.value)}\0${files}`;
}
...
export async function run(args = [], opts = {}) {
  ...
  if (!action) {
    throw new Error(`Unknown ignores action: ${actionArg}. Run "zarishnoksha ignores --help".`);
  }
  ...
  switch (action) {
    case "list":
      ...
      break;
    case "add-rule":
      out = addRule(cwd, rest);
      break;
    case "add-file":
      out = addFile(cwd, rest);
      break;
    case "add-value":
      out = addValue(cwd, rest);
      break;
    case "remove-rule":
      out = removeRule(cwd, rest);
      break;
    case "remove-file":
      out = removeFile(cwd, rest);
      break;
    case "remove-value":
      out = removeValue(cwd, rest);
      break;
    case "clear":
      out = clear(cwd, rest);
      break;
