/**
 * `zarishnoksha skills` subcommand
 *
 * Usage:
 *   zarishnoksha help      Show all available skills and commands
 *   zarishnoksha install   Install compiled skills from the universal bundle
 *   zarishnoksha link      Symlink compiled skills from a local checkout
 *   zarishnoksha update    Update skills to latest version
 */
...
import { getHookConsent, setHookConsent } from "../../lib/zarishnoksha-config.mjs";

const __dirname = dirname(fileURLToPath(import.meta.url));
const API_BASE = "https://zarishnoksha.pages.dev";
...
const IGNORED_SKILL_DIR_NAMES = new Set(["codex-primary-runtime"]);
const zarishnoksha_HOOK_COMMAND_MARKERS = [
  "skills/zarishnoksha/scripts/hook-probe.mjs",
  "skills/zarishnoksha/scripts/hook.mjs",
  "skills/zarishnoksha/scripts/hook-before-edit.mjs",
  "skills/zarishnoksha/scripts/hook-after-edit.mjs",
  "skills/zarishnoksha/scripts/hook-stop.mjs",
];
const PROVIDER_HOOK_ARTIFACTS = {
  ...
  ".cursor": [{ sourceProvider: ".cursor", rel: "hooks.json", destProvider: ".cursor" }],
  ...
  ".agents": [{ sourceProvider: ".codex", rel: "hooks.json", destProvider: ".codex" }],
  ...
  ".github": [{ sourceProvider: ".github", rel: "hooks/zarishnoksha.json", destProvider: ".github" }],
};
...
class PromptAbortError extends Error {
  constructor() {
    ...
    this.code = "zarishnoksha_PROMPT_ABORT";
  }
}

function isPromptAbortError(error) {
  return error?.code === "zarishnoksha_PROMPT_ABORT";
}
...
async function showHelp() {
  ...
  try {
    ...
    console.error("Could not fetch command list from zarishnoksha.pages.dev. Check your network connection.");
    process.exit(1);
  }

  const pad = (s, n) => s + " ".repeat(Math.max(0, n - s.length));

  console.log("\n  zarishnoksha Skills & Commands\n");
  console.log("  Install:  npx zarishnoksha install");
  console.log("  Link:     npx zarishnoksha link --source=.zarishnoksha");
  console.log("  Update:   npx zarishnoksha update");
  console.log("  Docs:     https://zarishnoksha.pages.dev/cheatsheet\n");
  ...
  for (const cmd of commands.sort((a, b) => a.id.localeCompare(b.id))) {
    ...
    const desc = cmd.description.length > 72 ? cmd.description.substring(0, 69) + "..." : cmd.description;
...
/**
 * Read the skills version from the zarishnoksha SKILL.md frontmatter.
 */
function getSkillsVersion(root) {
  for (const d of PROVIDER_DIRS) {
    const skillMd = join(root, d, "skills", "zarishnoksha", "SKILL.md");
...
async function downloadAndExtractBundle() {
  const localBundle = process.env.zarishnoksha_BUNDLE_PATH;
  if (localBundle) return copyOrExtractLocalBundle(localBundle);

  const tmpZip = join(tmpdir(), `zarishnoksha-update-${Date.now()}.zip`);
  const tmpDir = join(tmpdir(), `zarishnoksha-update-${Date.now()}`);
...
async function copyOrExtractLocalBundle(sourceValue) {
  ...
  const tmpDir = join(tmpdir(), `zarishnoksha-local-bundle-${process.pid}-${Date.now()}`);
  mkdirSync(tmpDir, { recursive: true });
...
function normalizeForHash(content) {
  return content.replace(
    /\.(claude|cursor|agents|github|gemini|codex|kiro|opencode|pi|qoder|trae|trae-cn|rovodev)\/skills\//g,
    ".PROVIDER/skills/",
  );
}
 ...
 * that aren't part of zarishnoksha). Deduplicates providers that share the same
...
async function check() {
  ...
  if (!installed) {
    console.log("zarishnoksha is not installed in this project.");
    console.log("Run `npx zarishnoksha install` to install.");
    process.exit(0);
  }
  ...
  try {
    ...
    if (upToDate) {
      ...
      console.log("Run `npx zarishnoksha update` to update.");
    }
  } catch (e) {
...
// Check if zarishnoksha skills are already present in any provider folder
function isAlreadyInstalled(root) {
  for (const d of PROVIDER_DIRS) {
    ...
    try {
      ...
      // Look for 'zarishnoksha' skill (or prefixed variant, or legacy 'teach-zarishnoksha')
      if (
        entries.some(
          (e) =>
            e === "zarishnoksha" ||
            e.endsWith("-zarishnoksha") ||
            e === "teach-zarishnoksha" ||
            e.endsWith("-teach-zarishnoksha"),
        )
      ) {
        return d;
      }
...
function hasRealSkillEntries(skillsDir) {
  ...
  return entries.some(
    (name) => !name.startsWith(".") && !IGNORED_SKILL_DIR_NAMES.has(name) && isSkillDir(skillsDir, name),
  );
}
 ...
 * prefix (default `i-`), renaming the skill to e.g. `i-zarishnoksha`. The prefix
 * only earned its keep when every command was its own skill; with a single
 * `zarishnoksha` skill it does nothing, so it is no longer offered. Rename any
 * prefixed zarishnoksha skill back to the canonical `zarishnoksha` (the fresh
 * install/update content lands there next) so users aren't left with a stale,
 * orphaned `i-zarishnoksha` alongside the new one. Scoped to the zarishnoksha skill
 * by name -- never touches third-party skills that happen to start with `i-`.
 * Returns the number of skills migrated.
 */
function migrateUnprefixzarishnoksha(root) {
  let migrated = 0;
  for (const d of PROVIDER_DIRS) {
    const skillsDir = join(root, d, "skills");
    if (!existsSync(skillsDir)) continue;
    ...
    for (const name of entries) {
      // A prefixed zarishnoksha skill is `<prefix>zarishnoksha`, not the canonical
      // `zarishnoksha` and not an unrelated legacy skill name.
      if (name === "zarishnoksha" || name === "teach-zarishnoksha") continue;
      if (!name.endsWith("-zarishnoksha")) continue;
      if (!isRealSkillDir(skillsDir, name)) continue;

      const dest = join(skillsDir, "zarishnoksha");
      try {
...
function userSkillProbePaths(home, harnessDir, provider) {
  return uniquePaths([join(home, provider, "skills"), join(home, harnessDir, "skills")]);
}
...
function printInstallIntro() {
  ...
  console.log(`${ui.accent(ui.bold("zarishnoksha"))} ${ui.dim("install")}`);
  console.log("");
}
...
function formatInstallDetectionLines(projectRoot, detections, home = homedir(), { styled = false } = {}) {
  if (detections.length === 0) {
    ...
    return styled ? [`${ui.accent("◇")} ${ui.bold("Detected harnesses")}`, `  ${ui.dim(message)}`] : [message];
  }
...
function printInstallDetections(projectRoot, detections) {
  for (const line of formatInstallDetectionLines(projectRoot, detections, homedir(), { styled: isInteractivePrompt() }))
    console.log(line);
  console.log("");
}
...
async function promptForProviders(defaultProviders = []) {
  ...
  const suffix = defaultProviders.length > 0 ? ` [blank keeps ${formatProviderList(defaultProviders)}]` : "";
  while (true) {
...
async function promptDetectedInstallMode(detectedProviders) {
  ...
  while (true) {
    const answer = await ask(
      `Install target: [1] Detected only (${formatProviderList(detectedProviders)})  [2] Customize [1]: `,
    );
...
async function chooseInstallScope(projectRoot, targets, detections, { yes, scopeValue } = {}) {
  ...
  const answer = await ask(
    `Install location: project (${formatPathForDisplay(projectRoot)}) or global (${formatPathForDisplay(homedir())})? [${fallback === "user" ? "global" : fallback}] `,
  );
...
function hookScriptPathForProvider(skillRoot, provider) {
  ...
  // zarishnoksha.json`) is a committed, team-shared file that the Copilot cloud
  ...
  if (provider === ".cursor") {
    return join(skillRoot, provider, "skills", "zarishnoksha", "scripts", "hook-before-edit.mjs");
  }
  if (provider === ".claude" || provider === ".agents") {
    return join(skillRoot, provider, "skills", "zarishnoksha", "scripts", "hook.mjs");
  }
  return null;
}
...
function rewriteHookCommandsForSkillRoot(value, provider, skillRoot) {
  ...
  if (typeof value === "string") {
    if (valueHaszarishnokshaHookMarker(value)) return `node ${JSON.stringify(hookScript)}`;
    return value;
  }
...
// Whether a hook manifest file actually wires up the zarishnoksha hook. We parse
// the JSON and scan only the `hooks` subtree (via valueHaszarishnokshaHookMarker),
...
function fileHaszarishnokshaHookMarker(file) {
  ...
  return valueHaszarishnokshaHookMarker(parsed.hooks);
}
...
// zarishnoksha marker — not mere file existence — because the target files
...
function hookInstalledForProvider(root, provider) {
  ...
  return artifacts.every(({ destProvider, rel, destRel }) => {
    const writeRel = destRel || rel;
    if (fileHaszarishnokshaHookMarker(join(root, destProvider, writeRel))) return true;
    if (writeRel !== rel && fileHaszarishnokshaHookMarker(join(root, destProvider, rel))) return true;
    return false;
  });
}

function valueHaszarishnokshaHookMarker(value) {
  if (typeof value === "string") {
    return zarishnoksha_HOOK_COMMAND_MARKERS.some((marker) => value.includes(marker));
  }
  if (Array.isArray(value)) return value.some(valueHaszarishnokshaHookMarker);
  if (value && typeof value === "object") {
    return Object.values(value).some(valueHaszarishnokshaHookMarker);
  }
  return false;
}
...
function stripzarishnokshaHookEntry(entry) {
  ...
  if (
    valueHaszarishnokshaHookMarker(entry.command) ||
    valueHaszarishnokshaHookMarker(entry.args) ||
    valueHaszarishnokshaHookMarker(entry.bash) ||
    valueHaszarishnokshaHookMarker(entry.powershell)
  ) {
    return null;
  }
  if (!Array.isArray(entry.hooks)) return entry;

  const strippedHooks = entry.hooks.map(stripzarishnokshaHookEntry).filter(Boolean);

  if (strippedHooks.length === 0 && entry.hooks.some(valueHaszarishnokshaHookMarker)) {
    return null;
  }
...
function stripzarishnokshaHookEntries(entries) {
  if (!Array.isArray(entries)) return [];
  return entries.map(stripzarishnokshaHookEntry).filter(Boolean);
}
...
function prunezarishnokshaHookFromManifest(manifestPath) {
  if (!fileHaszarishnokshaHookMarker(manifestPath)) return false;
  let parsed;
  try {
  ...
  for (const [event, entries] of Object.entries(existingHooks)) {
    const kept = stripzarishnokshaHookEntries(entries);
...
function mergeHookManifests(existing, fresh) {
  ...
  for (const event of hookEvents) {
    const preserved = stripzarishnokshaHookEntries(existingHooks[event]);
...
function copyProviderHooks(bundleDir, root, providers, { force = false, skillRoot = root } = {}) {
  ...
  for (const provider of targets) {
    for (const { src, dest, sharedDest } of hookArtifactsForProvider(bundleDir, root, provider)) {
      ...
      // twice per edit.
      if (sharedDest && fileHaszarishnokshaHookMarker(sharedDest)) {
        prunezarishnokshaHookFromManifest(dest);
        continue;
      }

      const freshManifest = readJsonFile(src, "Bundled hook manifest");
      const fresh =
        skillRoot === root ? freshManifest : rewriteHookCommandsForSkillRoot(freshManifest, provider, skillRoot);
      let next = fresh;
...
const HOOK_EXPLAINER = [
  "",
  "zarishnoksha can install a design hook for this project. In Claude/Codex it",
  ...
  "/zarishnoksha hooks on|off.",
  "",
].join("\n");

// Decide whether to install the design hook. Prompts once (default yes) the
// first time, records the answer in .zarishnoksha/config.local.json, and never
...
function resolveLinkSource(sourceValue, root) {
  const sourcePath = sourceValue || ".zarishnoksha";
  ...
  throw new Error(
    `Could not find compiled skills in ${sourcePath}. Expected dist/universal/ or provider skill folders.`,
  );
}
...
async function link(flags) {
  ...
  if (!yes) {
    ...
    const ans = await ask(`Link zarishnoksha skills into ${targets.length} folder(s)? (Y/n) `);
    if (ans === "n" || ans === "no") {
  ...
  console.log(`Linked zarishnoksha into: ${targets.join(", ")} (${parts.join(", ")}).`);
  console.log(
...
async function install(flags) {
  ...
  if (existing && !force) {
    console.log(`zarishnoksha skills are already installed (found in ${existing}/).`);
    ...
    try {
      if (linkedTargets.length > 0) {
        ...
        console.log(
          "Update the source checkout with `git submodule update --remote`, then rerun `npx zarishnoksha link --source=.zarishnoksha` if new skills are added.",
        );
      ...
      if (!updateCheckSkipped && copyTargets.length > 0 && !isUpToDate(installRoot, copyTargets, bundleDir)) {
        migrateUnprefixzarishnoksha(installRoot);
      ...
      const writtenHookTargets =
        missingHookTargets.length > 0
  ...
  console.log("\nDownloading zarishnoksha skills...");
  let bundleDir;
  try {
    bundleDir = await downloadAndExtractBundle();
  } catch (e) {
  ...
  // canonical `zarishnoksha` dir instead of orphaning the prefixed one.
  migrateUnprefixzarishnoksha(installRoot);
  ...
  console.log(`Installed zarishnoksha into: ${targets.join(", ")} (${scope === "user" ? "global" : "project"})`);
  if (hookTargets.length > 0) console.log(`Installed hooks into: ${hookTargets.join(", ")}`);

  console.log("\nDone! Run /zarishnoksha init in your AI harness to set up design context.\n");
}
...
function findLinkedProviders(root, providers) {
  return providers.filter((provider) => {
    const skillDir = join(root, provider, "skills", "zarishnoksha");
    try {
...
async function update(flags = []) {
  ...
  // Download the latest skills directly from zarishnoksha.pages.dev.
  ...
  if (providers.length === 0) {
    console.log("No zarishnoksha skill folders found in this project.");
    console.log("Run `npx zarishnoksha install` to install first.");
    process.exit(1);
  }
  ...
  try {
    ...
    // canonical `zarishnoksha` dir rather than orphaning the prefixed copy.
    const migrated = migrateUnprefixzarishnoksha(root);
    if (migrated > 0)
      console.log("Migrated a prefixed install back to /zarishnoksha (the i- prefix is no longer used).");
...
export async function run(args) {
  ...
  if (!sub || sub === "help" || sub === "--help" || sub === "-h") {
    ...
    console.error(`Run 'zarishnoksha --help' for available commands.`);
    process.exit(1);
  }
}
