 ...
 *
 * Usage:
 *   npx zarishnoksha detect [file-or-dir-or-url...]
 *   npx zarishnoksha ignores <list|add-file|add-value|remove-...>
 *   npx zarishnoksha help|install|update
...
// Is this a detect target (the `npx zarishnoksha src/` shorthand) or a mistyped
...
async function main() {
  ...
  if (!command || command === "--help" || command === "-h") {
    console.log(`Usage: zarishnoksha <command> [options]

Commands:
  detect [file-or-dir-or-url...]   Scan for UI anti-patterns and design quality issues
  ignores                          Manage detector ignore rules, files, and values
  help                             List all available skills and commands
  install                          Install ZarishNoksha skills into your project or global harness
...
Compatibility:
  zarishnoksha skills <command>       Legacy namespace; still supported.`);
    process.exit(0);
  }
  ...
  if (command === "detect") {
    ...
    // Default: treat as detect arguments (allow `npx zarishnoksha src/` shorthand)
...
main().catch((error) => {
  if (error?.code === "zarishnoksha_PROMPT_ABORT") {
    console.log("\nAborted.");
    process.exit(130);
  }
