...
function theme(name, type, colors) {
  return {
    ...
    tokenColors: [
      ...
      {
        scope: ["keyword", "storage", "storage.type", "support.type.property-name"],
      ...
      {
        scope: ["constant.numeric", "constant.language", "constant.character", "variable.language"],
      ...
      {
        scope: ["entity.name.tag", "support.class.component", "entity.name.type", "entity.other.attribute-name"],
        settings: { foreground: colors.goldDeep || colors.gold || colors.patina },
      },
      {
        scope: ["variable", "meta.object-literal.key", "support.variable", "support.constant"],
...
export const zarishnokshaShikiThemes = {
  light: theme("zarishnoksha-kinpaku-light", "light", light),
  dark: theme("zarishnoksha-kinpaku-dark", "dark", dark),
};
