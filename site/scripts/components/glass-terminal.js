import { alphaCommands, commandCategories, commandProcessSteps, commandRelationships } from "../data.js";
import { initCommandDemo, renderCommandDemo } from "../demo-renderer.js";
...
const sourceCache = {}; // Cache fetched source content
...
const PALETTE_EXCLUDED = new Set(["zarishnoksha", "init", "extract", "document", "live"]);
...
function renderDesktopLayout(container, commands) {
  ...
  // too (when they were rendered as 'zarishnoksha craft' etc.) but are now
  ...
  const categoryCommandOrder = {
    create: ["zarishnoksha", "craft", "shape"],
  ...
  if (startIndex < 0) {
    startIndex = Math.max(
      0,
      filteredCommands.findIndex((c) => c.id === "clarify"),
    );
  }
  ...
  const fisheyeHTML = filteredCommands
    .map((cmd, i) => {
      const cat = commandCategories[cmd.id] || "other";
...
function renderSpread(cmd, index, isActive) {
  ...
  // The root skill is rendered as zarishnoksha; sub-commands are rendered as
  // /zarishnoksha on a smaller line above the command name, so the command name
  // stays the visual anchor at full display size.
  const isRoot = cmd.id === "zarishnoksha";
  const nameHTML = isRoot
    ? "zarishnoksha"
    : `<span class="spread-namespace"><span class="spread-slash">/</span>zarishnoksha</span>${cmd.id}`;
...
function setupMagazineIntersection(container) {
  ...
  magazineState.intersectionObserver = new IntersectionObserver(
    (entries) => {
...
function renderMobileLayout(container, commands) {
  ...
  // Carousel pills show bare command names for sub-commands, and /zarishnoksha
  // for the root entry.
  const carouselHTML = commands
    .map(
      (cmd, i) => `
        <button class="mobile-cmd-pill${i === 0 ? " active" : ""}" data-id="${cmd.id}">
            ${cmd.id === "zarishnoksha" ? "/zarishnoksha" : cmd.id}
        </button>
    `,
    )
    .join("");
  ...
  const infoCardsHTML = commands
    .map((cmd, i) => {
      const relationship = commandRelationships[cmd.id];
      let relationshipHTML = "";

      // Relationships show bare command names (e.g., "pairs with quieter")
      // because the invocation is /zarishnoksha <name>, not /<name>.
      if (relationship) {
        if (relationship.pairs) {
      ...
      const cardName =
        cmd.id === "zarishnoksha"
          ? "/zarishnoksha"
          : `<span class="mobile-cmd-namespace">/zarishnoksha</span> ${cmd.id}`;
