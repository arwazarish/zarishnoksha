...
export function initLiveDemo() {
  ...
  const reset = () => {
    ...
    if (palette)
      palette.querySelectorAll(".live-demo-ctx-palette-chip").forEach((r) => r.classList.remove("is-highlight"));
    if (cmdName) cmdName.textContent = defaultCmd;
    showVariant(0);
  };

  const clearAnnotations = () => annotations.classList.remove("is-visible", "is-pin-visible", "is-note-visible");
  ...
  const highlightCmd = (cmd) => {
    ...
    palette
      .querySelectorAll(".live-demo-ctx-palette-chip")
      .forEach((r) => r.classList.toggle("is-highlight", r === chip));
    return chip;
  };
