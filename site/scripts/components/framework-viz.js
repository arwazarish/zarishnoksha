...
import { alphaCommands, commandCategories, commandRelationships } from "../data.js";
...
// After the v3.0 consolidation, all commands except the root "zarishnoksha" are
// sub-commands of /zarishnoksha. The renderer handles the display label directly
// (bare name for sub-commands, "/zarishnoksha" for the root). This map is kept
...
export class PeriodicTable {
  ...
  showTooltip(el, cmd) {
    ...
    // because they're names, not invocations — the invocation is /zarishnoksha <name>.
    const relParts = [];
  ...
  createElement(cmd, category) {
    ...
    const invocation =
      cmd === "zarishnoksha" ? "/zarishnoksha" : cmd.startsWith("zarishnoksha ") ? `/${cmd}` : `/zarishnoksha ${cmd}`;
