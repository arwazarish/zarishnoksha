...
export function renderGallery(skills) {
  ...
  // Filter skills (hide zarishnoksha as per original)
  const filteredSkills = skills.filter((s) => s.id !== "zarishnoksha");
...
function formatName(id) {
  return id
    .split("-")
    .map((word) => (word === "ux" ? "UX" : word.charAt(0).toUpperCase() + word.slice(1)))
    .join(" ");
}
