...
`/zarishnoksha layout` is for pages where nothing is technically wrong but nothing is breathing either. Equal padding everywhere, monotonous card grids, content that runs edge to edge, hierarchy that relies on size alone. Reach for it when a layout "feels off" and you cannot articulate why.
...
```
/zarishnoksha layout the settings page
```
...
- **Confusing arrange with distill.** If the problem is too many things, run `/zarishnoksha distill` first. Layout is for arranging what is already the right set.
