...
<div class="docs-viz-hero">
  <div class="docs-viz-file">
    <div class="docs-viz-file-header">
      ...
      <span class="docs-viz-file-status">Output of /zarishnoksha shape</span>
    </div>
    ...
    <div class="docs-viz-file-footer">Hand it to <code>/zarishnoksha</code>, <code>/zarishnoksha craft</code>, or any implementation flow.</div>
  </div>
...
Most AI-generated UIs fail not because of bad code, but because of skipped thinking. The model jumps to "here is a card grid" without asking "what is the user trying to accomplish". `/zarishnoksha shape` inverts that order.
...
You answer naturally. The skill asks follow-ups, not a form. At the end it produces a design brief: a structured artifact you can hand to `/zarishnoksha` or any other implementation skill.

Note: if you want the full flow (discovery interview, then straight into building), use `/zarishnoksha craft` instead. It runs `/zarishnoksha shape` internally, then continues into implementation with visual iteration. `/zarishnoksha shape` standalone is for when you want just the brief, so you can take it to whatever implementation approach you prefer.

## Try it

```
/zarishnoksha shape a daily digest email preferences page
```
...
From there you can hand the brief to `/zarishnoksha`, `/zarishnoksha polish`, or any other skill. Or just use it as a reference while you build by hand.
