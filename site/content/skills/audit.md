...
<div class="docs-viz-hero">
  <div class="docs-viz-report">
    <div class="docs-viz-report-head">
      <div>
        <div class="docs-viz-report-title">/zarishnoksha audit the checkout flow</div>
  ...
  <p class="docs-viz-caption">Five dimensions scored 0 to 4, each finding tagged P0 (blocks release) to P3 (polish). Audit documents; it doesn't fix. Route the findings into <code>/zarishnoksha harden</code>, <code>/zarishnoksha polish</code>, or <code>/zarishnoksha optimize</code>.</p>
</div>

## When to use it

`/zarishnoksha audit` is the technical counterpart to `/zarishnoksha critique`. Where `/zarishnoksha critique` asks "does this feel right", `/zarishnoksha audit` asks "does this hold up". It runs accessibility, performance, theming, responsive design, and anti-pattern checks against the implementation, scores each dimension 0 to 4, and produces a plan with P0 to P3 severity ratings.
...
Audit does not fix anything. It documents. Route the findings to `/zarishnoksha polish`, `/zarishnoksha harden`, or `/zarishnoksha optimize` depending on the category.

## Try it

```
/zarishnoksha audit the checkout flow
```
...
Hand the P0s to `/zarishnoksha harden`, the theming and typography P1s to `/zarishnoksha typeset` and `/zarishnoksha polish`, the rest to `/zarishnoksha polish`.

## Pitfalls

- **Confusing it with `/zarishnoksha critique`.** Audit is implementation quality. Critique is design quality. Run both for a full picture.
