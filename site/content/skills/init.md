---
tagline: "Set up a project for ZarishNoksha, once. Context, live mode, and where to start."
---
...
Run `/zarishnoksha init` once at the start of a project. Without it, every other command has to guess: generic SaaS voice, safe-default fonts, the AI color palette. With it, every command reads your answers before it generates.

Reach for it when:

- **You just installed ZarishNoksha in a new project.** First thing to run. Other commands will nudge you toward it if you skip.
...
- **`DESIGN.md`** is the visual file. Colors, typography, elevation, components, do's and don'ts. Answers "how it looks". Written by the delegated `/zarishnoksha document` command, which init invokes at the end.
- **Live mode config.** Since the same crawl already knows your framework and entry files, init pre-configures `/zarishnoksha live` so it opens straight into variant mode with no first-time setup.
...
Along the way it offers to run `/zarishnoksha document` for you. Say yes unless you have a specific reason to hold off. A real DESIGN.md is what keeps variants, polishes, and audits on-brand.
