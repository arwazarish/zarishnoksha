...
`/zarishnoksha` is the main command. Use it in two ways:

- Run `/zarishnoksha` by itself when you want the skill to inspect the project and recommend what to do next.
- Add a plain-English request when you know the outcome but not the exact command.

Reach for `/zarishnoksha` directly when:
...
If this is a new project, start with `/zarishnoksha init`. That creates the setup files every other command reads.

## How it works

Most AI-generated UIs fail the same way: generic fonts, purple gradients, card grids on card grids, glassmorphism everywhere. `/zarishnoksha` gives the model stronger design instructions before it writes code.
...
On first use in a project, `/zarishnoksha` may route you into `init`: a short interview that writes `PRODUCT.md` and offers to write `DESIGN.md`. Future commands read those files without asking again.
...
```
/zarishnoksha build me a pricing page for a developer tool
```

Both prompts are vague on purpose. `/zarishnoksha` will choose the right command or run the work directly, using your setup files when they exist.
...
v3.0 consolidated 18 standalone skills into a single `/zarishnoksha` with 23 commands. If you miss the short form of a command, pin it back:

```
/zarishnoksha pin critique
```

From now on, `/critique` invokes `/zarishnoksha critique` directly. It writes a lightweight redirect skill that delegates to the parent, so updates to the skill flow through without re-pinning.

Useful pins to try:

- `/zarishnoksha pin polish` for final-pass work
- `/zarishnoksha pin audit` for deterministic a11y/perf checks
- `/zarishnoksha pin live` for the browser iteration flow
- `/zarishnoksha pin critique` for design review

To remove: `/zarishnoksha unpin critique`. Pins live as directories named after the command in your harness skills folder (`.claude/skills/critique/`, `.cursor/skills/critique/`, etc.), so you can also delete them manually.
...
- **Expecting it to fix existing code.** `/zarishnoksha` is for creation. For refinement, reach for `/zarishnoksha polish`, `/zarishnoksha distill`, or `/zarishnoksha critique` instead.
- **Running it before `init` has saved context.** On a fresh project it will interview you mid-flight, which is fine but slower. Running `/zarishnoksha init` first is smoother.
- **Picking the wrong brand/product lane.** Marketing pages and app screens need different defaults. If `PRODUCT.md` has no `## Register` field (legacy), run `/zarishnoksha init` to add it.
