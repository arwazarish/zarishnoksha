...
- **Polishing work that is not done.** If there are TODOs in the code, you are not ready. Run `/zarishnoksha polish` on finished features only.
- **Treating polish as redesign.** Polish refines what exists. If you find yourself rearchitecting a layout, you needed `/zarishnoksha critique` or `/zarishnoksha layout` instead.
- **Running `/zarishnoksha polish` without `/zarishnoksha audit` first.** Polish catches feel-based issues. Audit catches measurable ones. Use both.
