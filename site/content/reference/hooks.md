...
description: "Install, enable, disable, debug, and tune ZarishNoksha's provider-native design hook for automatic detector feedback on UI file edits."
section: automation
order: 2
---

The design hook runs ZarishNoksha's detector automatically when an AI coding tool edits UI files. It catches regressions while the agent still has the edit in context.
...
npx zarishnoksha update --no-hooks
```
...
```text
/zarishnoksha hooks ignore-value overused-font Inter --shared --reason "Brand font"
/zarishnoksha hooks ignore-file "src/legacy/Card.tsx"
/zarishnoksha hooks ignore-rule side-tab
```
...
The terminal equivalent is `npx zarishnoksha ignores ...`, which writes the same detector config. See [Config and ignores](/docs/config).
...
<details class="docs-prose-details">
  ...
  <div>
    <p><code>npx zarishnoksha install</code> and <code>npx zarishnoksha update</code> install provider-native hook manifests for Claude Code, GitHub Copilot, Codex, and Cursor.</p>
    <ul>
      <li>Claude Code: <code>.claude/settings.local.json</code> by default.</li>
      <li>GitHub Copilot: <code>.github/hooks/zarishnoksha.json</code>, a committed file shared by the Copilot CLI and the cloud agent.</li>
      <li>Codex: <code>.codex/hooks.json</code>.</li>
      <li>Cursor: <code>.cursor/hooks.json</code>.</li>
    </ul>
    <p>GitHub Copilot reads the committed <code>.github/hooks/zarishnoksha.json</code>. In the Copilot CLI the hook activates once that file is on the repository's default branch and you trust the folder; the cloud agent reads it straight from the repo.</p>
...
<details class="docs-prose-details">
  ...
  <div>
    <p>Hook lifecycle settings live under <code>hook</code> in <code>.zarishnoksha/config.json</code>:</p>
    <pre><code>{
  "hook": {
    "enabled": true,
    "quiet": false,
    ...
    <p>Per-developer choices, including install consent, live in <code>.zarishnoksha/config.local.json</code>.</p>
...
<details class="docs-prose-details">
  ...
  <div>
    ...
    <pre><code>/zarishnoksha hooks status</code></pre>
    <p>It shows the shared and local config paths, current ignores, hook state, and relevant environment overrides.</p>
    <p>For invocation logs, set <code>hook.auditLog</code> or use <code>zarishnoksha_HOOK_LOG</code>. The hook writes one NDJSON line per invocation. Leave audit logging off for normal work.</p>
    <p>If a manifest is malformed, install/update aborts by default. Re-run with <code>--force</code> only when you want ZarishNoksha to back up the malformed file as <code>.bak</code> and replace it.</p>
  </div>
</details>
