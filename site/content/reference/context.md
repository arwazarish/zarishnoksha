...
tagline: "Give ZarishNoksha enough project memory to make specific design decisions."
description: "Understand why ZarishNoksha needs design context, what to put in PRODUCT.md and DESIGN.md, and how to keep that context current."
section: concepts
order: 1
---

ZarishNoksha works best when it can read the same product and design decisions you would give a human designer. Without that context, it has to infer audience, tone, palette, type, and component rules from code alone. That usually produces safer, more generic answers.

<p class="docs-context-note">If ZarishNoksha gives you generic advice, the design context is usually missing, too vague, or stale.</p>
...
That creates `PRODUCT.md`, the strategy file. At the end, say yes when ZarishNoksha offers to run:

```text
/zarishnoksha document
```

That creates `DESIGN.md`, the visual-system file, plus a generated helper at `.zarishnoksha/design.json`. Review the two markdown files. Edit anything that does not match the real product.

<div class="docs-context-flow" aria-label="How ZarishNoksha uses design context">
  ...
  <div class="docs-context-flow-source docs-context-flow-source--generated">
    ...
    <strong>.zarishnoksha/design.json</strong>
...
| `.zarishnoksha/design.json` | What structured design data should automation use? | Do not edit it directly. Refresh it by running `/zarishnoksha document`. |
...
ZarishNoksha calls this choice the **register**. In daily use, just decide what kind of surface you are asking it to judge.
...
```text
/zarishnoksha polish the marketing homepage as a brand surface
/zarishnoksha audit the billing settings as a product surface
```

## How context changes the output

With context loaded, ZarishNoksha can:
...
|---|---|
| New audience, positioning, product purpose, brand voice, or register | `/zarishnoksha init` |
| New palette, type stack, component primitives, radius scale, or design rules | `/zarishnoksha document` |
| A hook says `DESIGN.md` is newer than `.zarishnoksha/design.json` | `/zarishnoksha document` |
| One-off intentional detector finding | Add a narrow ignore with `/zarishnoksha hooks ignore-value` or `npx zarishnoksha ignores`. |
...
<details class="docs-context-details">
  <summary>Where ZarishNoksha looks for context files</summary>
  <div>
    ...
    <p>The detector's design-system rules use the same root-first behavior for <code>DESIGN.md</code>. For generated design metadata, the primary path is <code>.zarishnoksha/design.json</code>. Legacy <code>DESIGN.json</code> files are still accepted as fallbacks, but new projects should use <code>.zarishnoksha/design.json</code>.</p>
  </div>
</details>
...
<details class="docs-context-details">
  ...
  <div>
    <p>When <code>DESIGN.md</code> exists, <code>npx zarishnoksha detect</code> and the design hook unlock design-system checks:</p>
    <ul>
