...
**Output**: A design brief that can be handed off to /zarishnoksha craft, or directly to /zarishnoksha for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
Once confirmed, the brief is complete. The user can now hand it to /zarishnoksha, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use /zarishnoksha craft instead, which runs this command internally.)
