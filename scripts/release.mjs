...
const COMPONENTS = {
  skill: {
    ...
    tweetHeader: (v) => `zarishnoksha v${v} is out.`,
    tweetCta: "Install / update: npx skills add arwazarish/zarishnoksha",
  },
  cli: {
    ...
    tweetHeader: (v) => `zarishnoksha CLI v${v} is out.`,
    tweetCta: "npm i -g zarishnoksha",
  },
  extension: {
    ...
    tweetHeader: (v) => `zarishnoksha browser extension v${v} is out.`,
    tweetCta: null,
  },
};

const REPO_URL = "https://github.com/arwazarish/zarishnoksha";
...
if (cfg.buildCmd) {
  ...
  if (dryRun) {
    ...
    if (postBuild) {
      fail(
        `Build produced uncommitted changes. Run \`${cfg.buildCmd}\`, commit the result, then re-run.\n${postBuild}`,
      );
    }
...
if (headerIdx === -1) {
  fail(
    `No changelog entry found for "${cfg.changelogLabel}${version}" in site/pages/changelog.astro. Add one before releasing.`,
  );
}
...
console.log(`\n--- Tweet (${tweet.length}/${TWEET_LIMIT} chars) for @zarishnoksha_ai ---`);
console.log(tweet);
