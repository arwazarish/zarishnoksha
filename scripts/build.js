...
import { buildClaudePluginHooksManifest, hooksJsonFor } from "./lib/transformers/hooks.js";
import { createTransformer, PROVIDERS } from "./lib/transformers/index.js";
import { readPatterns, readSourceFiles, restorePerProjectArtifacts, stashPerProjectArtifacts } from "./lib/utils.js";
...
function generateCounts(rootDir, skills, buildDir) {
  ...
  // of /zarishnoksha. Count them from the command router table in SKILL.md.
  const zarishnokshaSkill = skills.find((s) => s.name === "zarishnoksha");
  let commandCount;
  if (zarishnokshaSkill) {
    ...
    const routerMatches = zarishnokshaSkill.body.match(/^\| `[^`]+` \|/gm);
  ...
  for (const relPath of filesToCheck) {
    ...
    for (const match of strippedContent.matchAll(detectPattern)) {
      const num = parseInt(match[1]);
      if (num !== detectionCount && num > 10) {
        // ignore small numbers like "3 patterns"
...
function validatePluginVersions(rootDir) {
  ...
  for (const { relPath, found, expected } of mismatches) {
    console.error(`  ❌ ${relPath}: version "${found}" disagrees with .claude-plugin/plugin.json "${expected}"`);
  }
...
function validateSkillFrontmatter(skills) {
  ...
  for (const skill of skills) {
    if (skill.description && skill.description.length > 1024) {
      console.error(
        `❌ ${skill.filePath}: invalid description: exceeds maximum length of 1024 characters (${skill.description.length})`,
      );
      errors++;
    }
  }
...
function validateProse(rootDir) {
  const targets = ["site/components", "site/content", "site/layouts", "site/pages", "README.md", "README.npm.md"];
  ...
  const phraseRules = [
    {
      re: /\bload-bearing\b/i,
      rationale: "AI tell. Stolen-engineer diction; almost always vague. Name what the thing actually does.",
    },
    ...
    {
      re: /\breflex defaults?\b/i,
      rationale: 'Internal jargon leaking into user-facing copy. Say "instincts" or "first guesses".',
    },
    ...
    {
      re: /\bin summary\b|\bin conclusion\b/i,
      rationale: "Summarizing closer. End on the strongest sentence; trust the reader.",
    },
...
/**
 * Narrow prose check for the zarishnoksha skill source.
 *
...
function validateTheme(rootDir) {
  ...
  for (const name of ["color-paper", "color-cream", "color-bg"]) {
    ...
    if (light && Number(light[1]) >= 35) {
      console.error(
        `  ❌ tokens.css: --${name} is light (${val}). Kinpaku is the default; surfaces must be dark lacquer (var(--ks-lacquer*) or oklch < 35%).`,
      );
      errors++;
    }
  }
  ...
  if (accent && !accent[1].includes("var(--ks-")) {
    ...
    if (hue && Number(hue[1]) >= 300 && Number(hue[1]) <= 360) {
      console.error(
        `  ❌ tokens.css: --color-accent is magenta (${accent[1].trim()}). The accent is kinpaku gold — use var(--ks-kinpaku).`,
      );
      errors++;
    }
  }
...
function assembleUniversal(distDir) {
  ...
  fs.writeFileSync(
    ...
    `zarishnoksha. Design fluency for AI harnesses.
https://zarishnoksha.pages.dev
...
async function build() {
  ...
  console.log(
    `📖 Read ${skills.length} skills (${userInvocableCount} user-invocable) and ${patterns.patterns.length + patterns.antipatterns.length} pattern categories\n`,
  );
  ...
  if (BUILD_OPTIONS.syncRootOutputs) {
    ...
    const deprecatedLocalSkills = [
      ...
      "onboard",
      "extract",
      // v3.0 consolidation: standalone skills -> /zarishnoksha sub-commands
      "adapt",
      "animate",
      "audit",
      "bolder",
    ...
    // dirs above stay where they are because `npx skills add arwazarish/zarishnoksha`
    ...
    fs.writeFileSync(path.join(pluginManifestDir, "plugin.json"), JSON.stringify(pluginManifest, null, 2) + "\n");

    const claudeSkillsSrc = path.join(DIST_DIR, "claude-code", ".claude", "skills", "zarishnoksha");
    if (fs.existsSync(claudeSkillsSrc)) {
      fs.mkdirSync(pluginSkillsDir, { recursive: true });
      copyDirSync(claudeSkillsSrc, path.join(pluginSkillsDir, "zarishnoksha"));
    }
