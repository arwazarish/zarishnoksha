...
const output = buildBrowserDetectorBundle(
  ROOT,
  `/**
 * Anti-Pattern Browser Detector for zarishnoksha (Extension Variant)
...
function packZip(zipPath, cwd, excludes = []) {
  ...
  execSync(`zip -r ${JSON.stringify(zipPath)} .${exArgs ? " " + exArgs : ""}`, { cwd, stdio: "pipe" });
...
if (!serviceWorker) {
  throw new Error("extension/manifest.json: expected background.service_worker to derive the Firefox manifest");
}
...
fs.writeFileSync(path.join(ffStageDir, "manifest.json"), JSON.stringify(firefoxManifest, null, 2) + "\n");
