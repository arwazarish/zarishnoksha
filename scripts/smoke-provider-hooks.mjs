...
const selectedProviders = (args.providers || defaultProviders.join(","))
  ...
  .filter(Boolean);
const smokeDir = join(targetRepo, ".zarishnoksha", "provider-smoke");
const summaryPath = join(smokeDir, "summary.json");
const smokeFiles = {
  direct: "src/__zarishnoksha_provider_smoke_direct.html",
  claude: "src/__zarishnoksha_provider_smoke_claude.html",
  codex: "src/__zarishnoksha_provider_smoke_codex.html",
  cursor: "src/__zarishnoksha_provider_smoke_cursor.html",
  confirmedClaude: "src/__zarishnoksha_provider_smoke_confirmed_claude.html",
  confirmedCodex: "src/__zarishnoksha_provider_smoke_confirmed_codex.html",
  confirmedCursor: "src/__zarishnoksha_provider_smoke_confirmed_cursor.html",
  agentChoiceClaude: "src/__zarishnoksha_provider_smoke_font_choice_claude.html",
  agentChoiceCodex: "src/__zarishnoksha_provider_smoke_font_choice_codex.html",
  agentChoiceCursor: "src/__zarishnoksha_provider_smoke_font_choice_cursor.html",
};

const results = [];
const hookConfigFiles = [".zarishnoksha/config.json", ".zarishnoksha/config.local.json"];
...
async function main() {
  ...
  if (selectedProviders.includes("direct"))
    checked("direct script contracts", "direct script failed", runDirectContractChecks);
  if (selectedProviders.some((provider) => ["claude", "codex", "cursor"].includes(provider))) {
    checked(
      "confirmed exception persistence",
      "confirmed exception persistence failed",
      runConfirmedExceptionPersistenceChecks,
    );
  ...
  if (selectedProviders.includes("claude"))
    checked("claude provider", "provider did not fire or did not surface output", runClaudeProviderSmoke);
  if (selectedProviders.includes("codex"))
    checked("codex provider", "provider did not fire or did not surface output", runCodexProviderSmoke);
  if (selectedProviders.includes("cursor"))
    checked("cursor provider", "provider did not fire or did not surface output", runCursorProviderSmoke);
...
function record(name, pass, detail = "", classification = "") {
  ...
  results.push(result);
  console.log(
    `${pass ? "PASS" : "FAIL"} ${name}${classification ? ` [${classification}]` : ""}${detail ? `: ${detail}` : ""}`,
  );
}
...
function run(
  cmd,
  cmdArgs,
  { cwd = targetRepo, env = {}, input = undefined, logName, timeoutMs = 10 * 60 * 1000, allowFailure = false } = {},
) {
  const fallbackPath =
  ...
  if (!allowFailure && (res.error || res.status !== 0)) {
    const message = res.error ? `${cmd} failed: ${res.error.message}` : `${cmd} exited ${res.status}`;
...
async function reinstallFresh() {
  cleanInstalledzarishnoksha();
  const packDir = makeTempDir("zarishnoksha-provider-smoke-pack-");
  ...
  const env = { zarishnoksha_BUNDLE_PATH: bundlePath };
  run(
    "npx",
    [
      "--yes",
  ...
  run("npx", ["--yes", "--package", tarball, "zarishnoksha", "skills", "update", "-y"], {
    cwd: targetRepo,
    env,
    logName: "skills-update.log",
    timeoutMs: 5 * 60 * 1000,
  });
  record("fresh install/update", true, "installed through local npx + zarishnoksha_BUNDLE_PATH");
}
...
function cleanInstalledzarishnoksha() {
  clearRuntimeState();
  cleanSmokeFiles();

  for (const rel of [
    ".claude/skills/zarishnoksha",
    ".cursor/skills/zarishnoksha",
    ".agents/skills/zarishnoksha",
    ".claude/hooks/hooks.json",
    ".agents/hooks",
    ".agents/plugins/marketplace.json",
    ".cursor/pre-log.mjs",
    ".cursor/rules/zarishnoksha-design-hook.mdc",
    "plugin-codex",
  ]) {
  ...
  run("claude", ["plugin", "uninstall", "zarishnoksha@zarishnoksha", "--scope", "user"], {
    cwd: targetRepo,
    logName: "claude-plugin-uninstall.log",
    allowFailure: true,
    timeoutMs: 60 * 1000,
  });
  run("claude", ["plugin", "marketplace", "remove", "zarishnoksha", "--scope", "user"], {
    cwd: targetRepo,
    logName: "claude-marketplace-remove.log",
    allowFailure: true,
    timeoutMs: 60 * 1000,
  });
  run("codex", ["plugin", "remove", "zarishnoksha@zarishnoksha"], {
    cwd: targetRepo,
    logName: "codex-plugin-remove.log",
    allowFailure: true,
    timeoutMs: 60 * 1000,
  });
  run("codex", ["plugin", "marketplace", "remove", "zarishnoksha"], {
    cwd: targetRepo,
  ...
  for (const abs of [
    join(homedir(), ".claude/plugins/cache/zarishnoksha"),
    join(homedir(), ".claude/plugins/data/zarishnoksha-zarishnoksha"),
    join(homedir(), ".codex/plugins/cache/zarishnoksha"),
    join(homedir(), ".codex/plugins/data/zarishnoksha-zarishnoksha"),
  ]) {
...
function ensureTargetGitExclude() {
  ...
  const block = [
    "# zarishnoksha-provider-smoke-start",
    ".zarishnoksha/provider-smoke/",
    "src/__zarishnoksha_provider_smoke_*.html",
    "# zarishnoksha-provider-smoke-end",
  ].join("\n");
  const current = readMaybe(excludePath);
  const next = current.includes("# zarishnoksha-provider-smoke-start")
    ? current.replace(/# zarishnoksha-provider-smoke-start[\s\S]*?# zarishnoksha-provider-smoke-end/g, block)
    : `${current.replace(/\s*$/, "")}\n${block}\n`;
...
function stripManifest(rel) {
  ...
  for (const [event, entries] of Object.entries(hooks)) {
    const preserved = Array.isArray(entries) ? entries.map(stripzarishnokshaHookEntry).filter(Boolean) : entries;
...
function stripzarishnokshaHookEntry(entry) {
  if (!entry || typeof entry !== "object") return entry;
  if (containszarishnokshaHook(entry)) return null;
  if (Array.isArray(entry.hooks)) {
    const hooks = entry.hooks.map(stripzarishnokshaHookEntry).filter(Boolean);
    if (hooks.length === 0 && entry.hooks.some(containszarishnokshaHook)) return null;
    return { ...entry, hooks };
  }
  return entry;
}

function containszarishnokshaHook(value) {
  if (typeof value === "string")
    return value.includes("skills/zarishnoksha/scripts/hook") || value.includes(".cursor/pre-log.mjs");
  if (Array.isArray(value)) return value.some(containszarishnokshaHook);
  if (value && typeof value === "object") return Object.values(value).some(containszarishnokshaHook);
  return false;
}
...
function verifyInstallShape() {
  ...
  assertCount(claude, ".claude/skills/zarishnoksha/scripts/hook.mjs", 1, "Claude hook.mjs");
  assertCount(codex, ".agents/skills/zarishnoksha/scripts/hook.mjs", 1, "Codex hook.mjs");
  assertCount(cursor, ".cursor/skills/zarishnoksha/scripts/hook-before-edit.mjs", 1, "Cursor preToolUse");
  assertCount(cursor, ".cursor/skills/zarishnoksha/scripts/hook-after-edit.mjs", 0, "Cursor afterFileEdit");
  assertCount(cursor, ".cursor/skills/zarishnoksha/scripts/hook-stop.mjs", 0, "Cursor stop");
  ...
  for (const rel of [
    ".claude/skills/zarishnoksha/scripts/hook.mjs",
    ".claude/skills/zarishnoksha/scripts/hook-lib.mjs",
    ".claude/skills/zarishnoksha/scripts/detector/cli/main.mjs",
    ".agents/skills/zarishnoksha/scripts/hook.mjs",
    ".agents/skills/zarishnoksha/scripts/hook-lib.mjs",
    ".agents/skills/zarishnoksha/scripts/detector/cli/main.mjs",
    ".cursor/skills/zarishnoksha/scripts/hook-before-edit.mjs",
    ".cursor/skills/zarishnoksha/scripts/hook-lib.mjs",
    ".cursor/skills/zarishnoksha/scripts/detector/cli/main.mjs",
  ]) {
    assertPath(join(targetRepo, rel), rel);
  }
  for (const rel of [
    ".cursor/skills/zarishnoksha/scripts/hook-after-edit.mjs",
    ".cursor/skills/zarishnoksha/scripts/hook-stop.mjs",
  ]) {
...
function assertNoPluginInstall() {
  const claude = run("claude", ["plugin", "list"], {
    allowFailure: true,
    logName: "claude-plugin-list.log",
    timeoutMs: 60 * 1000,
  });
  const codex = run("codex", ["plugin", "list"], {
    allowFailure: true,
    logName: "codex-plugin-list.log",
    timeoutMs: 60 * 1000,
  });
  const codexMarket = run("codex", ["plugin", "marketplace", "list"], {
    allowFailure: true,
    logName: "codex-marketplace-list.log",
    timeoutMs: 60 * 1000,
  });
  for (const [name, text] of [
    ...
    if (/zarishnoksha@zarishnoksha|Marketplace `zarishnoksha`|zarishnoksha-design-hook-impl/.test(text)) {
      throw new Error(`${name} still contains zarishnoksha plugin install`);
    }
  }
}
...
function runDirectContractChecks() {
  ...
  const env = { zarishnoksha_HOOK_LOG: join(smokeDir, "direct.ndjson") };
  const claude = run("node", [".claude/skills/zarishnoksha/scripts/hook.mjs"], {
    cwd: targetRepo,
    env,
  ...
  clearRuntimeState();
  const codex = run("node", [".agents/skills/zarishnoksha/scripts/hook.mjs"], {
    cwd: targetRepo,
    env,
  ...
  clearRuntimeState();
  const pre = run("node", [".cursor/skills/zarishnoksha/scripts/hook-before-edit.mjs"], {
    cwd: targetRepo,
    env,
...
function runConfirmedExceptionPersistenceChecks() {
  ...
  record(
    "confirmed exception persistence",
    true,
    `${providers.join(", ")} ignored confirmed overused-font values through shared config.json, not source comments`,
  );
}
...
function runConfirmedExceptionForProvider(provider) {
  ...
  requireRuleFinding(
    `${provider} confirmed exception first hook`,
  ...
  const suppressed = afterEvents.some(
    (event) =>
      event.file === file &&
...
function runAgentChosenFontExceptionChecks() {
  ...
  record(
    "agent-chosen font exception",
    true,
    `${providers.join(", ")} persisted Roboto as ignoreValues and did not write ignoreRules`,
  );
}
...
function runAgentChosenFontExceptionForProvider(provider) {
  ...
  const first = runInstalledProviderHook(provider, file, beforeLog);
  requireRuleFinding(
    `${provider} agent-choice first hook`,
    `${first.stdout}\n${first.stderr}\n${readMaybe(join(smokeDir, beforeLog))}`,
    "overused-font",
  );
  assertNoSpecificFontIgnoreConfig(provider);
...
function runProviderAgentFontException(provider, rel) {
  ...
  if (provider === "claude") {
    run(
      ...
      [
        ...
        "hooks",
        "--debug-file",
        join(smokeDir, "claude-agent-choice-debug.log"),
        prompt,
      ],
      {
        cwd: targetRepo,
  ...
  if (provider === "cursor") {
    ...
    const res = run(
      "agent",
      ["-p", "--force", "--trust", "--workspace", targetRepo, "--output-format", "stream-json", prompt],
      {
        cwd: targetRepo,
    ...
    if (res.error || res.status !== 0) {
      ...
      if (/Authentication required|agent login|CURSOR_API_KEY/i.test(output)) {
        const err = new Error(
          "Cursor CLI authentication required. Run `agent login` or set CURSOR_API_KEY, then rerun `bun run smoke:hooks -- --providers=cursor`.",
        );
...
function assertSpecificFontIgnoreConfig(provider, config) {
  ...
  const ignoredValue =
    Array.isArray(config.ignoreValues) &&
...
function assertNoSpecificFontIgnoreConfig(provider) {
  const file = join(targetRepo, ".zarishnoksha", "config.json");
  if (!existsSync(file)) return;
  const raw = readJson(file);
  const config =
    raw && typeof raw === "object" && !Array.isArray(raw) && raw.hook && typeof raw.hook === "object" ? raw.hook : null;
  if (!config) return;
...
function readSharedHookConfig() {
  const raw = readJson(join(targetRepo, ".zarishnoksha", "config.json"));
  if (!raw || typeof raw !== "object" || Array.isArray(raw) || !raw.hook || typeof raw.hook !== "object") {
    throw new Error("Missing .zarishnoksha/config.json hook config");
  }
  return raw.hook;
}

function runInstalledProviderHook(provider, file, logName) {
  const env = { zarishnoksha_HOOK_LOG: join(smokeDir, logName) };
  if (provider === "claude") {
    return run("node", [".claude/skills/zarishnoksha/scripts/hook.mjs"], {
      cwd: targetRepo,
      env,
  ...
  if (provider === "codex") {
    return run("node", [".agents/skills/zarishnoksha/scripts/hook.mjs"], {
      cwd: targetRepo,
      env,
      logName: `direct-${provider}-confirmed-${logName.replace(/\.ndjson$/, ".log")}`,
  ...
  if (provider === "cursor") {
    return run("node", [".cursor/skills/zarishnoksha/scripts/hook-before-edit.mjs"], {
      cwd: targetRepo,
      env,
      logName: `direct-${provider}-confirmed-${logName.replace(/\.ndjson$/, ".log")}`,
      input: JSON.stringify({
...
function providerAdminScript(provider) {
  if (provider === "claude") return ".claude/skills/zarishnoksha/scripts/hook-admin.mjs";
  if (provider === "codex") return ".agents/skills/zarishnoksha/scripts/hook-admin.mjs";
  if (provider === "cursor") return ".cursor/skills/zarishnoksha/scripts/hook-admin.mjs";
  throw new Error(`Unsupported admin provider: ${provider}`);
}

function runClaudeProviderSmoke() {
  clearRuntimeState();
  const env = { zarishnoksha_HOOK_LOG: join(smokeDir, "claude.ndjson") };
  ...
  const res = run(
    ...
    [
      ...
      "hooks",
      "--debug-file",
      join(smokeDir, "claude-debug.log"),
      prompt,
    ],
    {
      cwd: targetRepo,
...
function runCodexProviderSmoke() {
  clearRuntimeState();
  const env = { zarishnoksha_HOOK_LOG: join(smokeDir, "codex.ndjson") };
  ...
  const cacheEvidence = `${readMaybe(join(targetRepo, ".zarishnoksha", "hook.cache.json"))}\n${readMaybe(join(targetRepo, ".zarishnoksha", "hook.pending.json"))}`;
...
function runCursorProviderSmoke() {
  ensureCursorAgent();
  clearRuntimeState();
  const env = { zarishnoksha_HOOK_LOG: join(smokeDir, "cursor.ndjson") };
  ...
  const evidence = `${res.stdout}\n${res.stderr}\n${readMaybe(join(smokeDir, "cursor.ndjson"))}\n${readMaybe(join(targetRepo, ".zarishnoksha", "hook.pending.json"))}\n${readMaybe(join(targetRepo, ".zarishnoksha", "hook.cache.json"))}`;
  ...
  const intentionalIgnore = auditEvents.some(
    (event) => event.event === "preToolUse" && event.file === fixturePath && event.skipped === "config-ignore-file",
  );
  if (existsSync(fixturePath)) {
    ...
    if (/border-left\s*:\s*[2-9]\d*px/i.test(fixtureContent)) {
      if (!intentionalIgnore || !/ignoreFiles|ignore-file/i.test(evidence)) {
        throw new Error(
          "Cursor provider left the blocked side-tab fixture on disk without an explicit zarishnoksha ignore-file escape hatch",
        );
      }
    }
  }
  record(
    "cursor provider",
    true,
    "Cursor agent triggered preToolUse hook, blocked side-tab, and only proceeded through explicit ignore handling for the intentional fixture",
  );
}
...
function badFixtureContent() {
  return [
    ...
    "      }",
    "    </style>",
    '    <div class="card">Temporary zarishnoksha provider smoke fixture</div>',
    "  </body>",
    "</html>",
    "",
  ].join("\n");
...
function confirmedFixtureContent() {
  return [
    ...
    "  <body>",
    "    <style>",
    "      /* zarishnoksha: ignore overused-font */",
    '      body { font-family: "Roboto", ui-sans-serif, system-ui, sans-serif; }',
    "      .card { border-radius: 16px; padding: 16px; }",
    "    </style>",
    '    <div class="card">Temporary zarishnoksha confirmed exception fixture using Roboto.</div>',
    "  </body>",
    "</html>",
    "",
  ].join("\n");
...
function providerPrompt(rel) {
  return [
    `Create ${rel} as an intentionally bad zarishnoksha hook test fixture.`,
...
function fontExceptionPrompt(provider, rel) {
  return [
    `Read the installed zarishnoksha hooks reference for ${provider}, then persist a confirmed hook exception for Roboto specifically in ${rel}.`,
    "The user confirms Roboto is intentional for this fixture, but did not ask to ignore overused fonts generally.",
    "Use the /zarishnoksha hooks / hook-admin flow; do not edit .zarishnoksha/config.json by hand and do not edit the source fixture.",
...
function requireRuleFinding(label, text, rule) {
  ...
  if (
    ...
    !/Required design corrections|findings?|antipattern|side-tab|overused-font/.test(text)
  ) {
...
function clearRuntimeState() {
  ...
  for (const rel of [
    ...
    ".zarishnoksha/hook.json",
    ".zarishnoksha/hook.local.json",
  ]) {
    rmSync(join(targetRepo, rel), { force: true });
  }
}
...
function clearTransientHookState() {
  for (const rel of [".zarishnoksha/hook.cache.json", ".zarishnoksha/hook.pending.json"]) {
    rmSync(join(targetRepo, rel), { force: true });
  }
}
