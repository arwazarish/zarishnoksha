...
export const SUITES = {
  core: {
    ...
    triggers: [
      ...
      /^skill\/(SKILL\.src\.md|agents\/|reference\/|scripts\/(cleanup-deprecated|context|context-signals|critique-storage|design-parser|hook|zarishnoksha-paths|is-generated))/,
      /^site\/(pages|content|components|layouts)\//,
      /^README(\.npm)?\.md$/,
      /^cli\/bin\//,
      /^tests\/(build|cleanup-deprecated|cli-ignores|context|context-signals|critique-storage|design-parser|docs-integrity|hook|hook-build|zarishnoksha-paths|shiki-theme|skills-cli|target-args|test-suites|windows-path-fix|zip)\.test\.(js|mjs)$/,
      /^tests\/lib\//,
    ],
    commands: [
      {
        ...
        files: [
          ...
          "tests/lib/zarishnoksha-config.test.js",
      ...
      },
      {
        ...
        files: [
          ...
          "tests/hook.test.mjs",
          "tests/zarishnoksha-paths.test.mjs",
  ...
  "cli-remote-e2e": {
    description: "Remote CLI install/update smoke tests against zarishnoksha.pages.dev.",
    optIn: true,
    triggers: [...COMMON_INFRA_PATTERNS, /^cli\/bin\/commands\/skills\.mjs$/, /^tests\/skills-cli\.test\.js$/],
    commands: [
      {
        runner: "bun",
        env: { zarishnoksha_CLI_REMOTE_E2E: "1" },
        files: ["tests/skills-cli.test.js"],
      },
    ],
  },
