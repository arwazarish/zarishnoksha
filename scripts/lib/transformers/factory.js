...
import { CATEGORY_ORDER, SKILL_CATEGORIES } from "../sub-pages-data.js";
import {
  cleanDir,
...
export function createTransformer(config) {
  ...
  return function transform(skills, distDir, options = {}) {
    ...
    for (const skill of skills) {
      ...
      if (frontmatterObj["argument-hint"]?.includes("{{command_hint}}")) {
        ...
        if (metaScript) {
          ...
          const grouped = CATEGORY_ORDER.map((cat) => commands.filter((c) => SKILL_CATEGORIES[c] === cat).join("|"))
            .filter(Boolean)
            .join(" · ");
          frontmatterObj["argument-hint"] = frontmatterObj["argument-hint"].replace("{{command_hint}}", grouped);
        }
      }
    ...
    const hooksInfo = hooksEmitted ? ` (${config.hooksManifestRel || path.join("hooks", "hooks.json")})` : "";
