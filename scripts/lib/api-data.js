...
export function generateApiData(outDir, skills, patterns, rootDir) {
  ...
  // /zarishnoksha. Load them from command-metadata.json and include the root
  // zarishnoksha skill itself so UI surfaces like the cheatsheet can list them.
  ...
  if (!fs.existsSync(metadataPath)) {
    throw new Error(
      `command-metadata.json is missing at ${metadataPath}. This file is required to generate the commands API.`,
    );
  }
  const zarishnoksha = skills.find((s) => s.name === "zarishnoksha");
  if (!zarishnoksha) {
    throw new Error(
      "zarishnoksha skill not found at skill/SKILL.src.md. The build system expects exactly one skill at that path.",
    );
  }
  ...
  const commandsData = [
    {
      id: "zarishnoksha",
      name: "zarishnoksha",
      description: zarishnoksha.description,
      tagline: readTagline("zarishnoksha"),
      userInvocable: true,
    },
  ...
  // (skill/scripts/context.mjs) to nudge users toward `npx zarishnoksha skills
