...
//   New installs write project config at .zarishnoksha/live/config.json instead.
...
// bundled engine. `cli/engine/cli/main.mjs` imports `../../lib/zarishnoksha-config.mjs`,
// which in the source CLI resolves to `cli/lib/zarishnoksha-config.mjs`. The detector
...
// `scripts/lib/zarishnoksha-config.mjs`. Copy the dependency there or the bundled
// detector fails at import time with "Cannot find module .../lib/zarishnoksha-config.mjs".
const DETECTOR_EXTERNAL_DEPS = [{ src: "cli/lib/zarishnoksha-config.mjs", dest: "lib/zarishnoksha-config.mjs" }];
  ...
  for (const line of lines) {
    ...
    if (leadingSpaces === 0) {
      ...
      if (colonIndex > 0) {
        ...
        const shouldCoerceBoolean = key === "user-invocable" || key === "user-invokable" || !isQuoted;
...
/**
 * Read and parse the zarishnoksha skill source.
 ...
 * through to a compiled harness dir (`.agents/skills/zarishnoksha`) instead.
 */
export function readSourceFiles(rootDir) {
  ...
  skills.push({
    name: frontmatter.name || "zarishnoksha",
 ...
 *
 * Defaults to the main zarishnoksha SKILL.md but accepts any relative path so
...
const EXCLUDED_FROM_SUGGESTIONS = new Set([
  "zarishnoksha", // foundational skill, not a steering command
  "teach-zarishnoksha", // deprecated shim
  "frontend-design", // deprecated shim
]);

// Sub-commands of /zarishnoksha that should appear in {{available_commands}}.
// These are the commands that audit/critique/etc. reference when suggesting next steps.
const zarishnoksha_SUB_COMMANDS = [
  "adapt",
  "animate",
  "audit",
  "bolder",
...
export function replacePlaceholders(content, provider, commandNames = [], allSkillNames = []) {
  ...
  // After the v3.0 consolidation, commands are sub-commands of /zarishnoksha.
  // If there's only one user-invocable skill (zarishnoksha), generate sub-command references.
  ...
  if (nonExcluded.length === 0) {
    // Single-skill architecture: list sub-commands as /zarishnoksha <sub>
    commandList = zarishnoksha_SUB_COMMANDS.map((n) => `${cmdPrefix}zarishnoksha ${n}`).join(", ");
  } else {
  ...
  if (cmdPrefix !== "/" && allSkillNames.length > 0) {
    ...
    for (const name of sorted) {
      result = result.replace(new RegExp(`\\/(?=${escapeRegex(name)}(?:[^a-zA-Z0-9_-]|$))`, "g"), cmdPrefix);
    }
  }
