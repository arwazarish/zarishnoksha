...
import { parseFrontmatter, readSourceFiles, replacePlaceholders } from "./utils.js";
...
const EXCLUDED_SKILLS = new Set([
  "frontend-design", // deprecated, renamed to zarishnoksha
  "teach-zarishnoksha", // deprecated, folded into /zarishnoksha init
...
export async function loadCommandDemos(rootDir) {
  ...
  const demos = {};
  const files = fs.readdirSync(demosDir).filter((f) => f.endsWith(".js") && f !== "index.js");
...
export async function buildSubPageData(rootDir) {
  ...
  // After the v3.0 consolidation there's only one source skill (zarishnoksha).
  ...
  const zarishnokshaSkill = rawSkills.find((s) => s.name === "zarishnoksha");
  ...
  const allCommandNames = ["zarishnoksha", ...Object.keys(commandMetadata)];
  const resolvePlaceholders = (content) => replacePlaceholders(content, "claude-code", [], allCommandNames);

  const skills = [];

  // 1. The root zarishnoksha skill itself.
  if (zarishnokshaSkill && !EXCLUDED_SKILLS.has(zarishnokshaSkill.name)) {
    const editorial = readEditorialWrapper(contentDir, "skills", "zarishnoksha");
    const demo = commandDemos["zarishnoksha"] || null;
    skills.push({
      id: "zarishnoksha",
      name: "zarishnoksha",
      description: zarishnokshaSkill.description,
      argumentHint: zarishnokshaSkill.argumentHint,
      category: SKILL_CATEGORIES["zarishnoksha"],
      body: resolvePlaceholders(zarishnokshaSkill.body),
      references: (zarishnokshaSkill.references || []).map((r) => ({
        ...r,
  ...
  if (missing.length > 0) {
    throw new Error(`SKILL_CATEGORIES in scripts/lib/sub-pages-data.js is missing entries for: ${missing.join(", ")}`);
  }
