...
const BROWSER_FIXTURES = ["cramped-padding.html", "quality.html", "body-text-viewport-edge.html"];
...
async function measureCase({ name, engine, mode, target, run }) {
  ...
  try {
    ...
    const findings = Array.isArray(result) ? result.length : Number.isFinite(result?.findings) ? result.findings : 0;
    return {
      name,
      engine,
      mode,
...
function selectQuickFiles(files, predicate, preferredNames) {
  const preferred = preferredNames.map((name) => files.find((file) => path.basename(file) === name)).filter(Boolean);
...
async function runFileBenchmarks(args) {
  ...
  const selectedText = args.quick ? textFiles.slice(0, 2) : textFiles;
  const selectedHtml = args.quick ? selectQuickFiles(htmlFiles, isHtml, ["color.html", "quality.html"]) : htmlFiles;
  const directoryFiles = args.quick ? [...selectedHtml, ...selectedText].sort() : files;
...
async function runBrowserBenchmarks(args) {
  ...
  try {
    for (const fileName of browserFiles) {
      ...
      if (
        fresh.status === "failed" &&
        /Could not find Chrome|Failed to launch|executable|spawn|puppeteer/i.test(fresh.error || "")
      ) {
        cases.push(
          skippedCase({
            name: `detectUrl:fresh-load:${fileName}`,
        ...
        run: async (profile) => {
          ...
          try {
            ...
            const browserScript = fs.readFileSync(
              path.join(ROOT, "cli", "engine", "detect-antipatterns-browser.js"),
              "utf-8",
            );
            ...
            findings = await page.evaluate(() => {
              const serialized = window.zarishnokshaDetect({ decorate: false, serialize: true });
            ...
            const overlayGroupCount = await page.evaluate(() => window.zarishnokshaScan().length);
            addEvent(profile, {
              engine: "browser",
              phase: "scan",
              ruleId: "overlay-scan",
  ...
  return cases.map((testCase) => {
    if (
      testCase.engine === "browser" &&
      testCase.status === "failed" &&
      /Chromium unavailable|Failed to launch|Could not find Chrome|executable|spawn|puppeteer/i.test(
        testCase.error || "",
      )
    ) {
      return skippedCase({
...
async function main() {
  ...
  const cases = [...(await runFileBenchmarks(args))];
  if (args.browser) {
