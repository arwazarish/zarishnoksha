...
import { readBuffer, writeBuffer } from "../skill/scripts/live/manual-edits-buffer.mjs";
import { commitManualEdits } from "../skill/scripts/live-commit-manual-edits.mjs";
import { buildCopyEditBatchPrompt, runCopyEditPostApplyChecks } from "../skill/scripts/live-copy-edit-agent.mjs";
...
describe("live-commit-manual-edits.mjs batched AI apply", () => {
  ...
  it("keeps failed entries staged when the AI reports partial success", () => {
    ...
    assert.deepEqual(
      result.failed.map((item) => [item.id, item.reason]),
      [
  ...
  it("rolls back when a failed multi-op entry leaves one op in source", () => {
    ...
    const result = runCommit([], {
      zarishnoksha_LIVE_COPY_AGENT_MOCK_WRITES: JSON.stringify({
        "src/page.html": [
    ...
    assert.equal(
      result.failed.some(
        (item) => item.id === "title" && item.reason === "rolled_back_due_to_failed_entry_source_changed",
      ),
      true,
    );
  ...
  it("rolls back when an omitted entry is changed in a reported file", () => {
    const file = path.join(tmpDir, "src", "page.html");
    const before = ['<h1 class="hero">Old title</h1>', '<span class="secondary-action">Learn more</span>', ""].join(
      "\n",
    );
    fs.writeFileSync(file, before);
    writeBuffer(tmpDir, {
  ...
  it("treats entries reported as both applied and failed as a malformed result", () => {
    fs.writeFileSync(path.join(tmpDir, "src", "a.html"), "<h1>A new</h1>\n");
    writeBuffer(tmpDir, {
      entries: [entry({ id: "a", ops: [{ ref: "a", tag: "h1", originalText: "A original", newText: "A new" }] })],
    });
  ...
  it("treats done without explicit appliedEntryIds as failed and keeps staged entries", () => {
    ...
    writeBuffer(tmpDir, {
      entries: [entry({ id: "e1", ops: [{ ref: "a", tag: "h1", originalText: "Welcome", newText: "Hello" }] })],
    });
  ...
  it("fails source verification when applied IDs are reported but newText is absent", () => {
    ...
    writeBuffer(tmpDir, {
      entries: [
        entry({
          id: "e1",
          ops: [{ ref: "a", tag: "h1", classes: ["hero"], originalText: "Welcome", newText: "Hello" }],
        }),
      ],
    });
  ...
  it("deduplicates failed entries across repeated repair attempts", async () => {
    ...
    const result = await commitManualEdits({
      cwd: tmpDir,
      provider: "chat",
      env: { zarishnoksha_LIVE_MANUAL_EDIT_REPAIR_ATTEMPTS: "2" },
  ...
  it("does not delete pre-existing reported files that were outside the rollback snapshot", () => {
    ...
    assert.equal(
      result.rollbackFailures.some((item) => item.file === "src/extra.html" && item.reason === "no_snapshot"),
      true,
    );
    assert.equal(readBuffer(tmpDir).entries.length, 1);
  });
  ...
  it("fails source verification for legacy empty newText entries instead of treating them as applied", () => {
    ...
    assert.equal(result.failed[0].reason, "source_verification_failed");
    assert.equal(
      result.failed[0].checks[0].failures[0].detail,
  ...
  it("verifies current hero edits against Astro source hints before clearing", () => {
    ...
    const originalHook =
      "Great design prompts require design vocabulary. Most people don't have it. zarishnoksha teaches your AI deep design knowledge and gives you 23 commands to steer the result.";
    ...
    writeAstro({ title: "zarishnoksha", hook: originalHook });
    writeBuffer(tmpDir, {
      entries: [
        entry({
          id: "hero-title",
          ops: [
            {
              ...
              originalText: "zarishnoksha",
              newText: "zarishnoksha Wow",
    ...
    assert.equal(
      failed.failed.every((item) => item.reason === "source_verification_failed"),
      true,
    );
    assert.equal(readBuffer(tmpDir).entries.length, 2);

    writeAstro({ title: "zarishnoksha Wow", hook: "YESSSSS" });
  ...
  it("keeps all entries staged when the AI runner fails", () => {
    writeBuffer(tmpDir, {
      entries: [entry({ id: "e1", ops: [{ ref: "a", tag: "h1", originalText: "A", newText: "B" }] })],
    });

    const result = runCommit([], {
      zarishnoksha_LIVE_COPY_AGENT: "off",
    });
  ...
  it("rolls back source writes when the AI runner throws after writing", () => {
    ...
    fs.writeFileSync(file, before);
    writeBuffer(tmpDir, {
      entries: [entry({ id: "e1", ops: [{ ref: "a", tag: "h1", originalText: "Old", newText: "New" }] })],
    });
    ...
    const result = runCommit([], {
      ...
      zarishnoksha_LIVE_COPY_AGENT_MOCK_RESULT: "{not json",
    });

    assert.equal(result.cleared, 0);
    assert.equal(result.applied.length, 0);
    assert.match(result.failed[0].reason, /Invalid zarishnoksha_LIVE_COPY_AGENT_MOCK_RESULT JSON/);
    assert.deepEqual(result.rolledBackFiles, ["src/page.html"]);
  ...
  it("reports a corrupt pending buffer instead of treating it as empty", () => {
    const bufferPath = path.join(tmpDir, ".zarishnoksha", "live", "pending-manual-edits.json");
  ...
  it("passes repeated card and dynamic data evidence to the batch prompt path", () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, "site/scripts/data.js"),
    ...
    fs.writeFileSync(
      path.join(tmpDir, "site/scripts/components/foundation-animations.js"),
    ...
    assert.equal(
      candidates.some((item) => item.file === "site/scripts/data.js"),
      true,
    );
    assert.equal(
      candidates.some((item) => item.file === "site/scripts/components/foundation-animations.js"),
      true,
    );
    assert.equal(readBuffer(tmpDir).entries.length, 1);
  });
  ...
  it("allows non-numeric count display copy when integer source data stays typed", () => {
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          scripts: {
            "zarishnoksha:manual-edit-validate": "node src/validate.mjs",
          },
        },
        null,
        2,
    ...
    fs.writeFileSync(path.join(tmpDir, "src", "stats.mjs"), "export const stats = { count: 7 };\n");
    fs.writeFileSync(
    ...
    assert.equal(result.failed.length, 0);
    assert.equal(
      fs.readFileSync(path.join(tmpDir, "src", "stats.mjs"), "utf-8"),
      "export const stats = { count: 7, countLabel: '7 seats' };\n",
    );
    assert.equal(readBuffer(tmpDir).entries.length, 0);
  });
  ...
  it("keeps source for repair when Apply coerces an integer source model into display text that crashes validation", () => {
    ...
    assert.equal(
      result.failed[0].checks.some((check) => check.reason === "manual_edit_validation_failed"),
      true,
    );
    assert.match(
      result.failed[0].checks.find((check) => check.reason === "manual_edit_validation_failed").message,
      /stats\.count must stay integer/,
    );
    assert.equal(result.rolledBackFiles, undefined);
  ...
  it("fails validation and keeps staged entries when touched JS is invalid or markers remain", () => {
    fs.writeFileSync(
      path.join(tmpDir, "src", "broken.js"),
      "const label = 'XX29';\nconst answer = ;\n// zarishnoksha-carbonize-start\n",
    );
    writeBuffer(tmpDir, {
      entries: [entry({ id: "bad", ops: [{ ref: "a", tag: "span", originalText: "29", newText: "XX29" }] })],
    });
  ...
  it("keeps invalid touched JSON for repair before clearing staged edits", () => {
          ...
          ops: [
            {
              ...
              originalText: "Old",
              newText: "New",
              sourceHint: { file: "src/data.json", line: 1 },
            },
          ],
        }),
      ],
    ...
    assert.equal(result.failed[0].reason, "post_apply_validation_failed");
    assert.equal(
      result.failed[0].checks.some((check) => check.reason === "invalid_json"),
      true,
    );
    assert.equal(result.rolledBackFiles, undefined);
  ...
  it("keeps invalid touched CJS for repair before clearing staged edits", () => {
          ...
          ops: [
            {
              ...
              originalText: "Old",
              newText: "New",
              sourceHint: { file: "src/config.cjs", line: 1 },
            },
          ],
        }),
      ],
    ...
    assert.equal(result.failed[0].reason, "post_apply_validation_failed");
    assert.equal(
      result.failed[0].checks.some((check) => check.reason === "invalid_js"),
      true,
    );
    assert.equal(result.rolledBackFiles, undefined);
  ...
  it("strips live edit runtime attributes from the copy-edit prompt context", () => {
    const decoratedHtml =
      '<section><h1 class="hero" contenteditable="true" data-zarishnoksha-editable="true" data-zarishnoksha-original-text="Original" style="user-select: text; cursor: text; outline: none; -webkit-user-modify: read-write-plaintext-only;">Edited</h1></section>';
    ...
    assert.match(
      prompt,
      /do not reformat surrounding markup, indentation, attributes, blank lines, or unrelated whitespace/,
    );
    ...
    assert.doesNotMatch(
      serializedBatch,
      /data-zarishnoksha-original-text|data-zarishnoksha-editable|contenteditable|-webkit-user-modify/,
    );
  ...
  it("fails post-apply validation when live edit runtime attributes leak into source", () => {
    fs.writeFileSync(
      path.join(tmpDir, "src", "page.astro"),
      '<h1 class="hero" data-zarishnoksha-original-text="Original">Edited</h1>\n',
    );

    const checks = runCopyEditPostApplyChecks({ cwd: tmpDir, files: ["src/page.astro"] });

    assert.equal(checks.ok, false);
    assert.equal(checks.failures[0].reason, "leftover_zarishnoksha_marker");
    assert.match(checks.failures[0].marker, /data-zarishnoksha-original-text/);
  });
  ...
  it("keeps touched source files when post-apply validation needs repair", () => {
    ...
    fs.writeFileSync(file, before);
    writeBuffer(tmpDir, {
      entries: [entry({ id: "bad", ops: [{ ref: "a", tag: "span", originalText: "Old", newText: "New" }] })],
    });
  ...
  it("keeps source for repair when Apply leaks edit-mode contenteditable attributes into source", () => {
    ...
    const result = runCommit([], {
      zarishnoksha_LIVE_COPY_AGENT_MOCK_WRITES: JSON.stringify({
        "src/App.jsx":
          'export default function App() { return <h1 className="hero" contenteditable="true" data-zarishnoksha-editable="true" data-zarishnoksha-original-text="Old">Old New</h1>; }\n',
      }),
      zarishnoksha_LIVE_COPY_AGENT_MOCK_RESULT: JSON.stringify({
        status: "done",
    ...
    assert.equal(result.failed[0].reason, "post_apply_validation_failed");
    assert.equal(
      result.failed[0].checks.some((check) => /data-zarishnoksha-(editable|original-text)/.test(check.marker)),
      true,
    );
    assert.equal(result.rolledBackFiles, undefined);
  ...
  it("keeps source for repair when Apply leaves carbonize or variant scaffolding in source", () => {
    ...
    const result = runCommit([], {
      zarishnoksha_LIVE_COPY_AGENT_MOCK_WRITES: JSON.stringify({
        "src/page.html": [
          "<!-- zarishnoksha-carbonize-start deadbeef -->",
          '<div data-zarishnoksha-variants="deadbeef" data-zarishnoksha-variant-count="3">',
          '  <div data-zarishnoksha-variant="original"><h1 class="hero">New</h1></div>',
          "</div>",
          "<!-- zarishnoksha-carbonize-end deadbeef -->",
          "",
        ].join("\n"),
      }),
      zarishnoksha_LIVE_COPY_AGENT_MOCK_RESULT: JSON.stringify({
    ...
    assert.equal(result.failed[0].reason, "post_apply_validation_failed");
    assert.equal(
      result.failed[0].checks.some((check) =>
        /zarishnoksha-carbonize-start|data-zarishnoksha-variant/.test(check.marker),
      ),
      true,
    );
    assert.equal(result.rolledBackFiles, undefined);
    assert.match(fs.readFileSync(file, "utf-8"), /zarishnoksha-carbonize-start/);
    assert.equal(
      readBuffer(tmpDir)
        .entries.map((item) => item.id)
        .join(","),
  ...
  it("keeps the whole batch for repair when one applied edit leaves UI scaffolding behind", () => {
    ...
    writeBuffer(tmpDir, {
      entries: [
        ...
        entry({
          ...
          ops: [
            {
              ref: "body>article.card",
              tag: "article",
              classes: ["card"],
              originalText: "Old card",
              newText: "New card",
            },
          ],
        }),
    ...
    const result = runCommit([], {
      zarishnoksha_LIVE_COPY_AGENT_MOCK_WRITES: JSON.stringify({
        ...
        "src/card.html": '<article class="card"><span data-zarishnoksha-text-wrap="true">New card</span></article>\n',
      }),
      zarishnoksha_LIVE_COPY_AGENT_MOCK_RESULT: JSON.stringify({
        status: "done",
    ...
    assert.equal(result.reason, "manual_edit_repair_needs_decision");
    assert.deepEqual(
      result.failed.map((item) => item.reason),
      ["post_apply_validation_failed", "post_apply_validation_failed"],
    );
    assert.equal(
      result.failed[0].checks.some((check) => /data-zarishnoksha-text-wrap/.test(check.marker)),
      true,
    );
    assert.equal(result.rolledBackFiles, undefined);
    assert.equal(fs.readFileSync(pageFile, "utf-8"), '<h1 class="hero">New title</h1>\n');
    assert.match(fs.readFileSync(cardFile, "utf-8"), /data-zarishnoksha-text-wrap/);
    assert.deepEqual(
      readBuffer(tmpDir).entries.map((item) => item.id),
      ["title", "card"],
    );
  });
