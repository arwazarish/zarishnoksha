 ...
 * 0-byte universal.zip and every `npx zarishnoksha install` failed with
 ...
 * made `npx zarishnoksha install` exit 0 after writing only a fraction of the
 * bundle.
 */
...
import { createAllZips, createProviderZip } from "../scripts/lib/zip.js";

function makeUniversalTree(distDir) {
  const skillDir = join(distDir, "universal", "skills", "zarishnoksha");
 ...
 */
function makeLargeUniversalTree(
  distDir,
  {
    providers = [".claude", ".cursor", ".agents", ".gemini", ".github", ".kiro"],
    scriptCount = 8,
    extraSkills = ["audit", "polish"],
  } = {},
) {
  let files = 0;
  for (const provider of providers) {
    // The zarishnoksha skill ships a scripts/ dir with many files, like the real
    // bundle. This is where most entries live.
    const scriptsDir = join(distDir, "universal", provider, "skills", "zarishnoksha", "scripts");
    mkdirSync(scriptsDir, { recursive: true });
    writeFileSync(
      join(distDir, "universal", provider, "skills", "zarishnoksha", "SKILL.md"),
      `---\nname: zarishnoksha\n---\nprovider ${provider}\n`,
    );
    files += 1;
...
describe("release bundle zip writer", () => {
  it("createAllZips produces a non-empty universal.zip that unpacks to the skill tree", async () => {
    ...
    const skillMd = join(out, "skills", "zarishnoksha", "SKILL.md");
    assert.ok(existsSync(skillMd), "unpacked bundle is missing skills/zarishnoksha/SKILL.md");
    assert.match(readFileSync(skillMd, "utf8"), /name: zarishnoksha/);
  ...
  it("REGRESSION: extracts every entry of a many-file bundle (no silent partial extraction on Node v24.16.0+)", async () => {
    ...
    // because nothing else keeps the event loop alive -- `npx zarishnoksha install`
  ...
  it("createProviderZip throws when the source has no files (no silent 0-byte artifact)", async () => {
    ...
    await assert.rejects(() => createProviderZip(join(dist, "universal"), dist, "universal"), /no entries|0 bytes/i);
  ...
  it("createProviderZip throws when the source directory is missing", async () => {
    ...
    await assert.rejects(() => createProviderZip(join(dist, "does-not-exist"), dist, "universal"), /not found/i);
