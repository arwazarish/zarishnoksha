...
import { detectProvider, getModel, hasKey, PROVIDERS, resolveModelList } from "./providers.mjs";

const CRAFT_PROMPT = "/zarishnoksha craft a landing page for the project in this workspace";
const PRIMER_PROMPT =
  "Take a quick look at the project. What register is this? Run the zarishnoksha context loader once if you need to.";

const VERBOSE = process.env.zarishnoksha_SKILL_BEHAVIOR_VERBOSE === "1";
  ...
  describe(`skill behavior :: ${modelId}`, () => {
    ...
    it("scenario 1: no PRODUCT.md / DESIGN.md", async () => {
      ...
      try {
        ...
        const initLoaded =
          readsMatching(trace, "init.md").length > 0 || bashCommandsMatching(trace, "init.md").length > 0;
        assert.ok(
          initLoaded,
    ...
    it("scenario 4: context already loaded in prior turn", async () => {
      ...
      try {
        ...
        const turn2 = await runTurn({
          workspace,
          model,
          userPrompt: "Now, /zarishnoksha craft a landing page based on what you saw.",
        ...
        const brandLoadedAcrossTurns = fileLoaded(turn1.trace, "brand.md") || fileLoaded(turn2.trace, "brand.md");
        assert.ok(
    ...
    it("scenario 6: sub-command routing (`/zarishnoksha polish` loads polish.md)", async () => {
      const workspace = prepareWorkspace({
        files: {
      ...
      try {
        const { trace, text } = await runTurn({
          workspace,
          model,
          userPrompt: "/zarishnoksha polish index.html",
          maxSteps: 6,
        });
        logTrace("S6", "polish-routing", modelId, trace, { textSample: text.slice(0, 300) });
        assert.ok(
          fileLoaded(trace, "polish.md"),
          `agent should load polish.md when /zarishnoksha polish is invoked.\n` +
            `Trace: ${JSON.stringify(summarizeTrace(trace), null, 2)}`,
        );
      } finally {
        cleanupWorkspace(workspace);
    ...
    it("scenario 7: sub-command routing (`/zarishnoksha audit` loads audit.md)", async () => {
      const workspace = prepareWorkspace({
        files: {
          "PRODUCT.md": PRODUCT_MD_SAMPLE,
          "DESIGN.md": DESIGN_MD_SAMPLE,
      ...
      try {
        const { trace, text } = await runTurn({
          workspace,
          model,
          userPrompt: "/zarishnoksha audit index.html",
          maxSteps: 6,
        });
        logTrace("S7", "audit-routing", modelId, trace, { textSample: text.slice(0, 300) });
        assert.ok(
          fileLoaded(trace, "audit.md"),
          `agent should load audit.md when /zarishnoksha audit is invoked.\n` +
            `Trace: ${JSON.stringify(summarizeTrace(trace), null, 2)}`,
        );
      } finally {
        cleanupWorkspace(workspace);
    ...
    it("scenario 8: existing SvelteKit project (agent explores design system)", async () => {
      ...
      try {
        const { trace, text } = await runTurn({
          workspace,
          model,
          userPrompt: "/zarishnoksha polish src/routes/+page.svelte",
          maxSteps: 8,
        });
        ...
        // / reference files.
        const projectReads = trace.readPaths.filter(
          (p) => /\.(css|svelte|tsx?|jsx?|astro)$/i.test(p) && !p.includes(".claude/skills/"),
        );
        assert.ok(
    ...
    it("scenario 9: update-available directive is surfaced, never auto-run", async () => {
      ...
      // surface it and keep working, but must NOT run `npx zarishnoksha update`
      ...
      const workspace = prepareWorkspace({
        files: {
          "PRODUCT.md": PRODUCT_MD_SAMPLE,
          "index.html": MINIMAL_LANDING_HTML,
          ".zarishnoksha-update.json": JSON.stringify({ lastCheck: Date.now(), latestVersion: "99.0.0" }),
        },
        skillVersion: "3.5.0",
      });
      try {
        const { trace, text } = await runTurn({
          ...
          userPrompt: "/zarishnoksha polish index.html",
          maxSteps: 6,
          env: { zarishnoksha_UPDATE_CACHE: path.join(workspace, ".zarishnoksha-update.json") },
        });
        ...
        const ranUpdate = [
          ...bashCommandsMatching(trace, "zarishnoksha update"),
