 ...
 *   2. Symlinks the real .claude/skills/zarishnoksha into the workspace so
...
function loadSkillBody() {
  ...
  // and the scripts the skill references live at .claude/skills/zarishnoksha/
  ...
  md = md
    ...
    .replaceAll("{{scripts_path}}", ".claude/skills/zarishnoksha/scripts")
 ...
 *
 * - `.claude/skills/zarishnoksha` is symlinked at the SOURCE skill dir (not
 *   the built `.claude/skills/zarishnoksha/`) so the test exercises whatever
...
export function prepareWorkspace({ files = {}, skillVersion = null } = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zarishnoksha-skill-test-"));
  const skillDest = path.join(dir, ".claude", "skills", "zarishnoksha");
...
export function makeTools(workspace, extraEnv = {}) {
  ...
  const tools = {
    bash: tool({
      description:
        "Run a bash command in the workspace root. Use this to invoke skill scripts (e.g. `node .claude/skills/zarishnoksha/scripts/load-context.mjs`).",
