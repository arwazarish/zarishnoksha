...
describe("compileProviderBlocks", () => {
  test("keeps matching provider block bodies and removes tags", () => {
    const content = ["Before", "<codex>", "Codex-only guidance.", "</codex>", "After"].join("\n");

    expect(compileProviderBlocks(content, ["codex"])).toBe(["Before", "Codex-only guidance.", "After"].join("\n"));
  });

  test("removes non-matching provider blocks", () => {
    const content = ["Before", "<codex>", "Codex-only guidance.", "</codex>", "After"].join("\n");

    expect(compileProviderBlocks(content, ["claude-code"])).toBe(["Before", "", "After"].join("\n"));
  });
  ...
  test("preserves unknown standalone tags", () => {
    const content = ["Before", "<aside>", "Normal markdown HTML.", "</aside>", "After"].join("\n");

    expect(compileProviderBlocks(content, ["codex"])).toBe(content);
  });

  test("keeps codex blocks for targets that opt into the codex tag", () => {
    const content = ["<codex>", "Codex repo skill guidance.", "</codex>"].join("\n");
