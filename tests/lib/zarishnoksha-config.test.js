...
} from "../../cli/lib/zarishnoksha-config.mjs";

describe("cli/lib/zarishnoksha-config", () => {
  let root;
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "imp-cfg-"));
  });
  ...
  test("setHookConsent preserves unrelated keys in config.local.json", () => {
    mkdirSync(join(root, ".zarishnoksha"), { recursive: true });
  ...
  test("writing consent gitignores config.local.json via .git/info/exclude", () => {
    ...
    expect(exclude).toContain(".zarishnoksha/config.local.json");
    ...
    expect((again.match(/zarishnoksha-config-ignore-start/g) || []).length).toBe(1);
  ...
  test("writeDetectionConfig local ignores do not create an implicit design-system override", () => {
    ...
    expect(readFileSync(join(root, ".git", "info", "exclude"), "utf-8")).toContain(".zarishnoksha/config.local.json");
  });
    ...
    const findings = [
      {
        ...
        file: join(root, "src", "main.css"),
        line: 1,
        snippet: "Primary font: Avenir Next",
      },
  ...
  test("extractFindingIgnoreValue handles fonts, Google font URLs, and motion snippets", () => {
    expect(
      extractFindingIgnoreValue({ antipattern: "overused-font", snippet: "Primary font: Avenir Next (80% of text)" }),
    ).toBe("avenir next");
    expect(
      extractFindingIgnoreValue({
        antipattern: "overused-font",
        snippet: "https://fonts.googleapis.com/css2?family=Alumni+Sans:wght@700",
      }),
    ).toBe("alumni sans");
    expect(
      extractFindingIgnoreValue({ antipattern: "bounce-easing", snippet: "animation: bounce-ball 1s infinite" }),
    ).toBe("bounce-ball");
  });
});
