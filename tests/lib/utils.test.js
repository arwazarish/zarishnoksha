...
Audit the code.`;
  ...
  test("should read skill with reference files", () => {
    const skillContent = `---
name: zarishnoksha
description: zarishnoksha design skill
---

zarishnoksha design instructions.`;
  ...
  test('should fall back to "zarishnoksha" when frontmatter has no name', () => {
    const skillDir = path.join(testRootDir, "skill");
    ensureDir(skillDir);
    ...
    expect(skills).toHaveLength(1);
    expect(skills[0].name).toBe("zarishnoksha");
  });
...
describe("readPatterns", () => {
  ...
  test("ignores its arguments (curated, not extracted from any SKILL.md)", () => {
    ...
    expect(fromBogusRoot.patterns.map((p) => p.name)).toEqual(fromRealRoot.patterns.map((p) => p.name));
...
describe("replacePlaceholders", () => {
  ...
  test("should exclude zarishnoksha from {{available_commands}}", () => {
    const result = replacePlaceholders("Commands: {{available_commands}}", "claude-code", [
      "audit",
      "zarishnoksha",
      "polish",
  ...
  test("should exclude legacy teach-zarishnoksha from {{available_commands}}", () => {
    const result = replacePlaceholders("Commands: {{available_commands}}", "claude-code", [
      "audit",
      "teach-zarishnoksha",
      "polish",
  ...
  test("lists /zarishnoksha sub-commands for {{available_commands}} when no command names are passed", () => {
    // v3.0 single-skill architecture: with no command names, the list falls back
    // to the zarishnoksha_SUB_COMMANDS sub-commands rendered as `/zarishnoksha <sub>`.
    const result = replacePlaceholders("Commands: {{available_commands}}", "claude-code", []);
    expect(result.startsWith("Commands: /zarishnoksha ")).toBe(true);
    expect(result).toContain("/zarishnoksha audit");
    expect(result).toContain("/zarishnoksha polish");
  });
