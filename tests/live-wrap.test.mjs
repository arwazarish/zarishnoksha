...
import { buildSearchQueries, detectCommentSyntax, findClosingLine, findElement } from "../skill/scripts/live-wrap.mjs";
...
describe("findElement", () => {
  it("finds an element by class name", () => {
    const lines = ["<html>", "<body>", '  <div class="hero">', "    <h1>Hello</h1>", "  </div>", "</body>", "</html>"];
  ...
  it("finds an element by ID", () => {
    const lines = ['<section id="features">', "  <p>Content</p>", "</section>"];
  ...
  it("skips comments containing the query", () => {
    const lines = ["<!-- hero section -->", '<div class="hero">', "  <p>Content</p>", "</div>"];
    const result = findElement(lines, "hero");
    assert.ok(result);
    assert.equal(result.startLine, 1); // skips the comment on line 0
  });

  it("skips lines that contain data-zarishnoksha-variant", () => {
    const lines = ['<div class="hero" data-zarishnoksha-variant="original">Old</div>', '<div class="hero">Real</div>'];
    const result = findElement(lines, "hero");
    assert.ok(result);
...
describe("findClosingLine", () => {
  ...
  it("handles nested tags of the same type", () => {
    const lines = ['<div class="outer">', '  <div class="inner">', "    <p>Content</p>", "  </div>", "</div>"];
  ...
  it("handles self-closing tags", () => {
    const lines = ["<div>", '  <img src="test.png" />', "  <br />", "</div>"];
...
describe("wrapCli integration", () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-wrap-test-"));
...
<body>
  <div class="hero-section">
    ...
    assert.ok(modified.includes('data-zarishnoksha-variants="test123"'));
    assert.ok(modified.includes('data-zarishnoksha-variant-count="3"'));
    assert.ok(modified.includes('data-zarishnoksha-variant="original"'));
    assert.ok(modified.includes("display: contents"));
    assert.ok(modified.includes("zarishnoksha-variants-start test123"));
    assert.ok(modified.includes("zarishnoksha-variants-end test123"));
    // Original should NOT be hidden (stays visible until variants arrive)
    assert.ok(!modified.includes('data-zarishnoksha-variant="original" style="display: none"'));
  });
  ...
  return (
    ...
    assert.ok(modified.includes("{/* zarishnoksha-variants-start jsx123"));
    assert.ok(modified.includes('data-zarishnoksha-variant-count="2"'));
  });
...
<div id="pricing">
  ...
  <p>Plans start at $10/mo.</p>
    ...
    assert.ok(modified.includes('data-zarishnoksha-variants="id123"'));
...
<div class="after">After</div>`;
    ...
    assert.ok(modified.includes('data-zarishnoksha-variants="pres123"'));
  });
  ...
  <h1>Plain title</h1>
    ...
    assert.equal(result.cssAuthoring.styleTag, '<style data-zarishnoksha-css="SESSION_ID">');
...
<section class="hero-shell">
  <h1>{title}</h1>
    ...
    assert.equal(
      ...
      "event=live_wrap.astro_css_mode actor=agent operation=wrap_astro_file risk=astro_scopes_preview_css_away expected=styleMode astro-global-prefixed actual=" +
        result.styleMode +
        " suggestion=inspect live-wrap output metadata for .astro files",
    );
    assert.deepEqual(result.cssSelectorPrefixExamples, [
      '[data-zarishnoksha-variant="1"]',
      '[data-zarishnoksha-variant="2"]',
      '[data-zarishnoksha-variant="3"]',
    ]);
    assert.equal(result.cssAuthoring.mode, "astro-global-prefixed");
    assert.equal(result.cssAuthoring.strategy, "global-prefixed");
    assert.equal(result.cssAuthoring.styleTag, '<style is:inline data-zarishnoksha-css="SESSION_ID">');
    assert.match(result.cssAuthoring.rulePattern, /^\[data-zarishnoksha-variant="N"\]/);
    assert.ok(
...
// .zarishnoksha/live/pending-manual-edits.json from local dev tripped the
// fail-loud check in live-wrap. Clear the buffer around each test.
function clearManualEditsBuffer() {
  try {
    const p = join(process.cwd(), ".zarishnoksha/live/pending-manual-edits.json");
    rmSync(p, { force: true });
  } catch {}
}
...
describe("live-wrap — JSX / TSX correctness", () => {
  let tmp;
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-wrap-jsx-"));
    clearManualEditsBuffer();
  });
  afterEach(() => {
    rmSync(tmp, { recursive: true, force: true });
  ...
  return (
    ...
    assert.ok(modified.includes("zarishnoksha-variants-start wrapA"), "wrapper was created");
    ...
    const originalMatch = modified.match(/data-zarishnoksha-variant="original"[^>]*>([\s\S]*?)\s*<\/div>/);
  ...
  return (
    ...
    assert.ok(!/style\s*=\s*"display:\s*contents"/.test(modified), "no HTML-style style attribute on outer wrapper");
...
export default function App() {
  ...
  return (
    ...
    // The outer wrapper <div data-zarishnoksha-variants="..."> appears BEFORE
    // both marker comments — markers are tucked inside.
    const wrapperIdx = modified.indexOf('data-zarishnoksha-variants="frag1"');
    const startMarkerIdx = modified.indexOf("zarishnoksha-variants-start frag1");
    const endMarkerIdx = modified.indexOf("zarishnoksha-variants-end frag1");
  ...
  it("HTML wrapper keeps marker comments OUTSIDE the wrapper <div> (existing layout)", () => {
    ...
    const modified = readFileSync(join(tmp, "page.html"), "utf-8");
    const wrapperIdx = modified.indexOf('data-zarishnoksha-variants="htmlFrag"');
    const startMarkerIdx = modified.indexOf("zarishnoksha-variants-start htmlFrag");
  ...
  return (
    ...
    const modified = readFileSync(join(tmp, "Page.tsx"), "utf-8");
    const originalMatch = modified.match(/data-zarishnoksha-variant="original"[\s\S]*?<\/div>/);
  ...
  return (
    ...
    assert.ok(
      modified.includes('data-zarishnoksha-variants="short1"'),
  ...
  <section class="multiline-target">
    ...
    // line (the zarishnoksha-variants-end marker for HTML).
    assert.match(
      lines[result.endLine - 1],
      /zarishnoksha-variants-end ml1/,
  ...
  return (
    ...
    assert.ok(modified.includes('data-zarishnoksha-variants="dyn1"'), "wrapped (first-match fallback)");
  });
  ...
  return (
    ...
    assert.ok(
      Array.isArray(errPayload.candidates) && errPayload.candidates.length === 2,
...
describe("findClosingLine — edge cases", () => {
  it("recognises an opener line where the tag sits at end-of-line (multi-line JSX)", () => {
    const lines = ["<section", '  className="hero"', ">", "  <h1>Hi</h1>", "</section>"];
