...
// to iterate on. Set zarishnoksha_E2E_ONLY=<name>[,<name>...] to scope the run.
const onlyNames = parseFixtureFilter(process.env.zarishnoksha_E2E_ONLY);
const fixtures = onlyNames.size > 0 ? allFixtures.filter((f) => onlyNames.has(f.name)) : allFixtures;
const missingOnlyNames = [...onlyNames].filter((name) => !allFixtures.some((f) => f.name === name));
if (missingOnlyNames.length > 0) {
  throw new Error(`Unknown zarishnoksha_E2E_ONLY fixture(s): ${missingOnlyNames.join(", ")}`);
}

const manualOnly =
  process.env.zarishnoksha_E2E_MANUAL_ONLY === "1" || process.env.zarishnoksha_E2E_MANUAL_ONLY === "true";
const reloadVariants =
  process.env.zarishnoksha_E2E_RELOAD_VARIANTS === "1" || process.env.zarishnoksha_E2E_RELOAD_VARIANTS === "true";
const scenarioNames = parseFixtureFilter(process.env.zarishnoksha_E2E_SCENARIOS);
const liveE2eTestTimeoutMs = readPositiveIntEnv("zarishnoksha_E2E_TEST_TIMEOUT_MS");
...
before(async () => {
  ...
  try {
    ...
    throw new Error(`Playwright is required for live-e2e tests (${err.message}). Run: npx playwright install chromium`);
  }
  try {
...
for (const { name, fixture } of fixtures) {
  describe(`live-e2e · ${name} (${fixture.runtime.styling || "unknown-styling"})`, () => {
    it("drives the full click → Go → cycle → accept cycle", liveE2eTestOptions, async (t) => {
      ...
      if (manualOnly || process.env.zarishnoksha_E2E_MANUAL_SCENARIO) {
      ...
      // Pick the agent. `zarishnoksha_E2E_AGENT=llm` opts into Claude first,
      ...
      const agentMode = process.env.zarishnoksha_E2E_AGENT || "fake";
      let agent;
      if (agentMode === "llm") {
        const llmConfig = resolveLlmAgentConfig({
          model: process.env.zarishnoksha_E2E_LLM_MODEL,
        });
        ...
        if (!agent) {
          t.skip(`zarishnoksha_E2E_AGENT=llm with provider=${llmConfig.provider} requires ${llmConfig.requiredEnv}`);
          return;
        }
      ...
      const insertDomSelector =
        agentMode === "llm" && insertCfg.expectSelectorLlm
          ? insertCfg.expectSelectorLlm
          : insertCfg.expectSelector || ".inserted-strip";
      const domSelector = isInsert ? insertDomSelector : pickSelector;
      ...
      const variantContentSelector = isInsert
        ? usesSvelteComponentPreview
          ? ".inserted-copy"
          : '[data-zarishnoksha-variant="2"] .inserted-copy'
        : usesSvelteComponentPreview
          ? pickSelector
          : '[data-zarishnoksha-variant="2"] > :first-child';
      ...
      try {
        ...
        if (isInsert) {
          ...
          if (process.env.zarishnoksha_E2E_DEBUG) {
            const barText = await page.evaluate(() => {
              const bar = document.querySelector("#zarishnoksha-live-bar");
              return bar
                ? { display: bar.style.display, text: bar.textContent || "", html: bar.innerHTML.slice(0, 500) }
                : null;
            });
        ...
        if (svelteComponentSession) {
          ...
          if (isInsert) {
            ...
            if (agentMode === "fake") {
              ...
              assert.match(
                variantBody,
                new RegExp(insertCfg.expectSourcePattern, "i"),
                "Svelte insert variant component contains prompt-matching content",
              );
            } else {
              assert.match(
                variantBody,
                /<([a-z][\w:-]*)\b[\s\S]*<\/\1>|<[a-z][\w:-]*\b[^>]*\/>/i,
                "Svelte insert variant component contains a root element",
              );
            }
          } else {
            assert.match(
              variantBody,
              new RegExp(`<${svelteComponentSession.expectedTag}\\b`),
              "Svelte variant component contains target element",
            );
          }
          assert.doesNotMatch(
            routeBody,
            /data-zarishnoksha-variants="/,
            "Svelte route source is not edited during generation",
          );
        } else {
          assert.match(after, /data-zarishnoksha-variants="/, "wrapper inserted");
        }
        if (isInsert) {
          if (svelteComponentSession) {
            assert.equal(svelteComponentSession.manifest.mode, "insert", "Svelte insert uses component preview mode");
          } else {
            assert.match(after, /data-zarishnoksha-mode="insert"/, "insert mode wrapper");
            assert.doesNotMatch(after, /data-zarishnoksha-variant="original"/, "insert has no original variant");
          }
        ...
        if (svelteComponentSession) {
          assert.match(
            readFileSync(join(tmp, svelteComponentSession.manifest.componentDir, "v2.svelte"), "utf-8"),
            /<style>/,
            "Svelte component variant has scoped style block",
          );
        } else if (sourceFile.endsWith(".astro")) {
          assert.match(
            after,
            /<style is:inline data-zarishnoksha-css="/,
            "Astro live CSS uses an inline compiler-bypassing style block",
          );
          assert.match(
            after,
            /\[data-zarishnoksha-variant="1"\]\s*>\s*(?:h1|\.[\w-]+)/,
          ...
          assert.doesNotMatch(
            ...
            "Astro live CSS does not use raw @scope",
          );
        } else {
          assert.match(after, /<style data-zarishnoksha-css="/, "colocated <style> block present");
          assert.match(after, /@scope \(\[data-zarishnoksha-variant="1"\]\)/, "scoped CSS for variant 1");
          assert.match(after, /@scope \(\[data-zarishnoksha-variant="2"\]\)/, "scoped CSS for variant 2");
          assert.match(after, /@scope \(\[data-zarishnoksha-variant="3"\]\)/, "scoped CSS for variant 3");
        }
        ...
        if (agentMode === "fake") {
          ...
          assert.match(
            paramsSource,
            svelteComponentSession ? /"1"\s*:/ : /data-zarishnoksha-params=/,
            "params manifest emitted",
          );
        ...
        const cycleSequence =
          Array.isArray(fixture.runtime.variantSequence) && fixture.runtime.variantSequence.length > 0
        ...
        for (const targetVariant of cycleSequence) {
          ...
          if (agentMode === "fake" && targetVariant === 2 && !checkedVariantTwoStyle) {
            await page
              .waitForFunction(
                (sel) => {
                  const query = window.__zarishnokshaLiveQuery || ((s) => document.querySelector(s));
            ...
            if (variantWeight !== "900") {
              const styleSnapshot = await evaluatePageWithTimeout(
                ...
                (sel) => {
                  const query = window.__zarishnokshaLiveQuery || ((s) => document.querySelector(s));
                  const el = query(sel) || document.querySelector(sel);
                  const styleEl = document.querySelector("style[data-zarishnoksha-css]");
                  const rules = [];
            ...
            assert.equal(
              variantWeight,
              "900",
              "event=live_e2e.variant_css_applied actor=browser operation=render_visible_variant risk=unstyled_live_preview expected=font-weight 900 actual=" +
                variantWeight +
                " suggestion=inspect live CSS style mode and selector shape",
            );
        ...
        if (reloadVariants && usesSvelteComponentPreview) {
          ...
          assert.equal(
            savedBeforeReload.visible,
            visibleBeforeReload,
            "local session stores visible variant before reload",
          );
          assert.equal(
            savedBeforeReload.previewMode,
            "svelte-component",
            "local session stores Svelte preview mode before reload",
          );
          ...
          assert.equal(
            savedAfterReload.previewFile,
            savedBeforeReload.previewFile,
            "preview manifest survives refresh",
          );
          ...
          assert.equal(
            await getVisibleVariant(page),
            visibleBeforeReload,
            "same visible variant is restored after refresh",
          );
          ...
          if (visibleBeforeReload < expectedCount) {
            ...
            assert.equal(
              await getVisibleVariant(page),
              visibleBeforeReload + 1,
              "next arrow still works after refresh restore",
            );
            ...
            assert.equal(
              await getVisibleVariant(page),
              visibleBeforeReload,
              "prev arrow still works after refresh restore",
            );
          } else {
            await clickPrev(page);
            await waitForVariantCounter(page, visibleBeforeReload - 1, expectedCount, {
        ...
        if (svelteComponentTarget) {
          assert.equal(
            existsSync(svelteComponentTarget.manifestPath),
            false,
            "Svelte temp preview session removed after accept",
          );
          const snapshotPath = join(
            tmp,
            ".zarishnoksha/live/sessions",
            `${svelteComponentTarget.manifest.id}.snapshot.json`,
          );
          const snapshot = JSON.parse(readFileSync(snapshotPath, "utf-8"));
          assert.equal(snapshot.phase, "completed");
          assert.equal(snapshot.sourceFile, svelteComponentTarget.manifest.sourceFile);
          assert.doesNotMatch(snapshot.sourceFile, /node_modules\/\.zarishnoksha-live/);
        }
        assert.doesNotMatch(final, /data-zarishnoksha-variants="/, "variants wrapper removed");
        assert.doesNotMatch(final, /zarishnoksha-variants-start/, "variants-start marker removed");
        assert.doesNotMatch(final, /zarishnoksha-carbonize-start/, "carbonize-start marker removed");
        assert.doesNotMatch(final, /zarishnoksha-carbonize-end/, "carbonize-end marker removed");
        assert.doesNotMatch(final, /data-zarishnoksha-carbonize=/, "carbonize wrapper removed");
        assert.doesNotMatch(final, /data-zarishnoksha-variant="/, "no leftover variant scaffolding");
        if (isInsert) {
          if (agentMode === "fake") {
          ...
          const acceptedSourcePattern =
            fixture.runtime.acceptedSourcePattern || '<h1[^>]*(class|className)="[^"]*\\bhero-title\\b[^"]*"';
          assert.match(final, new RegExp(acceptedSourcePattern), "accepted source element survives");
        }
        ...
        if (fixture.runtime.probe?.expectConsoleClean) {
          const realErrors = consoleErrors.filter(
            (e) =>
              !/(Download the React DevTools|StrictMode|Failed to load resource: the server responded with a status of 404)/i.test(
                e,
              ),
          );
          ...
          assert.equal(realErrors.length, 0, `expected clean console, got:\n${realErrors.join("\n")}`);
        }
      } catch (err) {
    ...
    if (
      shouldRunScenario("manual") &&
      Array.isArray(fixture.runtime.manualEditScenarios) &&
      fixture.runtime.manualEditScenarios.length > 0
    ) {
      const manualScenarioFilter = process.env.zarishnoksha_E2E_MANUAL_SCENARIO || "";
        ...
        it(`Edit copy → Save → Apply/commit: ${scenario.name}`, liveE2eTestOptions, async (t) => {
          ...
          try {
            ...
            if (scenario.probeMalformedAckBeforeApply) {
              ...
              assert.equal(
                probeState?.applyCalls,
                1,
                "manual_edit_apply event should not be redelivered after the correct ack",
              );
            }
          } finally {
            await teardownAndResetBrowser(teardown);
    ...
    if (shouldRunScenario("annotations") && fixture.runtime.liveChrome?.annotations) {
      it("uploads annotations with generate and still accepts the variant", liveE2eTestOptions, async (t) => {
        ...
        try {
          ...
          assert.match(
            generateEvent.screenshotPath,
            /\.zarishnoksha\/live\/annotations\//,
            "annotation screenshot is stored under live annotations",
          );
...
async function captureLiveE2eFailure({ name, fixture, session, sourceFile, error, log = () => {} }) {
  const root = process.env.zarishnoksha_E2E_ARTIFACT_DIR;
  ...
  try {
    ...
    copyDirIfExists(join(tmp, ".zarishnoksha", "live"), join(dir, "zarishnoksha-live"));
    copyDirIfExists(join(tmp, "node_modules", ".zarishnoksha-live"), join(dir, "zarishnoksha-live-preview"));

    if (session.page) {
      const html = await withCaptureTimeout(session.page.content(), 5_000, "page content").catch(
        (err) => `capture failed: ${err.message}`,
      );
...
async function createManualScenarioAgent(t, scenario = {}) {
  const requested = (process.env.zarishnoksha_E2E_MANUAL_AGENT || process.env.zarishnoksha_E2E_AGENT || "auto")
    .trim()
    .toLowerCase();
  ...
  return {
    ...
    async applyManualEdits(event, context = {}) {
      ...
      const buffer = JSON.parse(
        readFileSync(join(context.tmp, ".zarishnoksha/live/pending-manual-edits.json"), "utf-8"),
      );
...
function wrapTargetFromPickedElement(event) {
  ...
  const tag = typeof element.tagName === "string" ? element.tagName.trim().toLowerCase() : "";
...
async function runManualEditStage(page, stage, { t, fixture, session, agentMode, defaultSelector }) {
  ...
  await pickElement(page, stage.element?.selector || defaultSelector, {
    position: stage.element?.position,
    resetPickMode: true,
  });
...
async function waitForAcceptedSelectionReady(page, selector, { timeout }) {
  await page.waitForFunction(
    (sel) => {
      ...
      for (const el of all) {
        if (el.closest("[data-zarishnoksha-variants],[data-zarishnoksha-variant]")) return false;
      }
      return true;
    },
    selector,
...
async function readLiveSessionStorage(page) {
  return page.evaluate(() => {
    const raw = localStorage.getItem("zarishnoksha-live-session");
...
async function waitForVariantCounter(page, variant, count, { timeout = 15_000 } = {}) {
  try {
    await page.waitForFunction(
      ({ variant, count }) => {
        const query = window.__zarishnokshaLiveQuery || ((sel) => document.querySelector(sel));
        const bar = query("#zarishnoksha-live-bar");
    ...
    const snapshot = await page
      .evaluate(() => {
        ...
        return {
          ...
          debugState: window.__zarishnoksha_LIVE_CHROME_CORE__?.debugState?.() || null,
          storage: localStorage.getItem("zarishnoksha-live-session"),
          wrapper: wrapper
            ? {
                preview: wrapper.dataset.zarishnokshaPreview,
                count: wrapper.dataset.zarishnokshaVariantCount,
                html: wrapper.outerHTML.slice(0, 500),
              }
            : null,
        };
      })
...
async function waitForAcceptedDom(page, selector, { allowVariantRoot = false, timeout = 20_000 } = {}) {
  await page.waitForFunction(
    ({ sel, allowVariantRoot }) => {
      ...
      if (all.length < 1) return false;
      for (const el of all) {
        if (el.closest("[data-zarishnoksha-variants]")) return false;
        if (el.closest("[data-zarishnoksha-carbonize]")) return false;
        if (!allowVariantRoot && el.closest("[data-zarishnoksha-variant]")) return false;
      }
      return true;
    },
...
function assertSourceMissing(tmp, file, text) {
  ...
  assert.equal(body.includes(text), false, `source ${file} should not contain discarded text ${JSON.stringify(text)}`);
}

function assertSourceContains(tmp, file, text) {
  const full = join(tmp, file);
  const body = readFileSync(full, "utf-8");
  assert.equal(body.includes(text), true, `source ${file} should still contain ${JSON.stringify(text)}`);
}
...
function assertSourceMatches(tmp, file, pattern) {
  ...
  assert.equal(re.test(body), true, `source ${file} should match ${pattern}`);
}
...
async function assertVisibleText(page, selector, text, { timeout = 20_000 } = {}) {
  try {
    ...
    const actual = await page
      .evaluate((sel) => document.querySelector(sel)?.textContent || null, selector)
      .catch(() => null);
    throw new Error(
      `visible text ${selector} did not include ${JSON.stringify(text)}; actual=${JSON.stringify(actual)}; ${err.message}`,
    );
  }
}
...
async function waitForServerManualEditStashCount(live, expectedCount, { pageUrl = "/", timeout = 20_000 } = {}) {
    ...
    try {
      ...
      if (expectedCount === 0 && Date.now() - lastStatusCheck > 1_000) {
        ...
        if (lastActivity?.type === "manual_edit_repair_needs_decision") {
          throw new Error(
            `manual edit Apply needs repair decision before stash cleared; last=${last}; lastActivity=${JSON.stringify(lastActivity)}`,
          );
        }
      }
    } catch (err) {
  ...
  throw new Error(
    `manual edit stash count did not reach ${expectedCount}; last=${last}; lastError=${lastError?.message || "none"}; lastActivity=${JSON.stringify(lastActivity)}`,
  );
}
...
async function waitForSourceClean(filePath, timeoutMs, { svelteComponentTarget: knownSvelteTarget = null } = {}) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    const dirty =
      ...
      last.includes("data-zarishnoksha-variants=") ||
      last.includes("zarishnoksha-variants-start") ||
      last.includes("zarishnoksha-carbonize-start") ||
      last.includes("data-zarishnoksha-carbonize=") ||
      last.includes("data-zarishnoksha-variant=");
...
function sourceShadowTargetFor(filePath) {
  ...
  if (!body.includes('data-zarishnoksha-preview="source-shadow"')) return null;
  const match = body.match(/\bdata-zarishnoksha-source-file=(["'])(.*?)\1/);
  if (!match) return null;
  const root = filePath.includes("/.zarishnoksha/")
    ? filePath.slice(0, filePath.indexOf("/.zarishnoksha/"))
    : dirname(filePath);
...
function svelteComponentTargetFor(filePath) {
  ...
  const markers = [`${sep}node_modules${sep}.zarishnoksha-live${sep}`, `${sep}src${sep}lib${sep}zarishnoksha${sep}`];
...
function fixtureUsesSvelteKitAdapter(fixture) {
  return (
    Array.isArray(fixture?.config?.files) &&
...
async function assertStateProbe(page, probe, label, { baseline = null } = {}) {
  ...
  if (probe.textSelector) {
    ...
    assert.equal(snapshot.text, normalizeText(probe.expectedText), `stateProbe text ${label}`);
  }
  if (probe.windowProperty) {
    ...
    if (Object.hasOwn(probe, "expectedWindowValue")) {
      assert.equal(actual, probe.expectedWindowValue, `stateProbe ${probe.windowProperty} ${label}`);
    }
    if (probe.expectWindowUnchanged && baseline) {
      assert.equal(actual, baseline.windowValue, `stateProbe ${probe.windowProperty} unchanged ${label}`);
    }
  }
  return snapshot;
}
...
async function locateSessionFile(tmp) {
  ...
  for (const f of candidates) {
    ...
    if (
      body.includes("data-zarishnoksha-variants=") ||
      body.includes("zarishnoksha-carbonize-start") ||
      body.includes("zarishnoksha-variants-start")
    ) {
      return f;
    }
  }
...
function walkSvelteComponentManifests(root) {
  const results = [];
  const stack = [join(root, "node_modules/.zarishnoksha-live"), join(root, "src/lib/zarishnoksha")];
  while (stack.length) {
    const dir = stack.pop();
    let entries;
    try {
