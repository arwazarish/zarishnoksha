...
import { collectPluginVersions, readSkillFrontmatterVersion } from "../scripts/lib/validate-plugin-versions.js";
...
function writeFixture(root, { plugin, marketplace, subtreePlugin, skill } = {}) {
  ...
  if (plugin !== undefined) {
    write(".claude-plugin/plugin.json", JSON.stringify({ name: "zarishnoksha", version: plugin }, null, 2));
  }
  if (marketplace !== undefined) {
    write(
      ".claude-plugin/marketplace.json",
      JSON.stringify({ plugins: [{ name: "zarishnoksha", version: marketplace }] }, null, 2),
    );
  }
  if (subtreePlugin !== undefined) {
    write(
      "plugin/.claude-plugin/plugin.json",
      JSON.stringify({ name: "zarishnoksha", version: subtreePlugin, skills: "./skills/" }, null, 2),
    );
  }
  if (skill !== undefined) {
    write("plugin/skills/zarishnoksha/SKILL.md", skillMd(skill));
  }
}
...
describe("collectPluginVersions", () => {
  let root;
  beforeEach(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), "zarishnoksha-ver-"));
  });
  afterEach(() => {
  ...
  test("flags a lagging marketplace.json (the half the sync workflow cannot repair)", () => {
    ...
    expect(mismatches).toEqual([{ relPath: ".claude-plugin/marketplace.json", found: "3.1.1", expected: "3.7.1" }]);
  });
  ...
  test("flags a stale ./plugin subtree manifest", () => {
    ...
    const { mismatches } = collectPluginVersions(root);
    expect(mismatches).toEqual([{ relPath: "plugin/.claude-plugin/plugin.json", found: "3.6.0", expected: "3.7.1" }]);
  });
  ...
  test("flags a stale bundled SKILL.md frontmatter version", () => {
    ...
    const { mismatches } = collectPluginVersions(root);
    expect(mismatches).toEqual([{ relPath: "plugin/skills/zarishnoksha/SKILL.md", found: "3.1.1", expected: "3.7.1" }]);
  });
  ...
  test("reports every drifted file at once", () => {
    ...
    expect(mismatches.map((m) => m.relPath)).toEqual([
      ".claude-plugin/marketplace.json",
      "plugin/.claude-plugin/plugin.json",
      "plugin/skills/zarishnoksha/SKILL.md",
    ]);
  });
  ...
  test("flags a root plugin.json that exists but has no version field", () => {
    fs.mkdirSync(path.join(root, ".claude-plugin"), { recursive: true });
    fs.writeFileSync(path.join(root, ".claude-plugin/plugin.json"), JSON.stringify({ name: "zarishnoksha" }));
    const { source, errors } = collectPluginVersions(root);
...
describe("collectPluginVersions with CRLF line endings", () => {
  let root;
  beforeEach(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), "zarishnoksha-ver-crlf-"));
  });
  afterEach(() => {
    fs.rmSync(root, { recursive: true, force: true });
  });
  ...
  test("a CRLF-saved SKILL.md at the right version is not a false mismatch", () => {
    ...
    const skillPath = path.join(root, "plugin/skills/zarishnoksha/SKILL.md");
