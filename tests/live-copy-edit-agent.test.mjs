...
describe("live-copy-edit-agent", () => {
  it("builds a batch prompt with duplicate-card context and candidate evidence", () => {
    ...
    assert.match(prompt, /zarishnoksha:manual-edit-validate/);
  ...
  it("parses partial batch results", () => {
    assert.deepEqual(
      parseCopyEditBatchResult(
        '{"status":"partial","appliedEntryIds":["a"],"failed":[{"entryId":"b","reason":"ambiguous"}],"files":["src/page.js"]}',
      ),
      {
        status: "partial",
        message: null,
  ...
  it("preserves structured warnings in batch results", () => {
    assert.deepEqual(
      parseCopyEditBatchResult(
        '{"status":"done","appliedEntryIds":["a"],"files":["src/page.js"],"warnings":[{"reason":"repair_followup","file":"src/page.js"},"plain warning"]}',
      ),
      {
        status: "done",
        message: null,
        ...
        files: ["src/page.js"],
        notes: [],
        warnings: [{ reason: "repair_followup", file: "src/page.js" }, { message: "plain warning" }],
      },
    );
  });
  ...
  it("flags invalid JS and actual leftover carbonize markers in post-apply checks", () => {
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(tmp, "src", "page.html"),
        "<!-- zarishnoksha-carbonize-end abcdef12 -->\n<h1>Hi</h1>\n",
      );
      const checks = runCopyEditPostApplyChecks({ cwd: tmp, files: ["src/bad.js", "src/page.html"] });
      assert.equal(checks.ok, false);
      assert.equal(
        checks.failures.some((item) => item.reason === "leftover_zarishnoksha_marker"),
        true,
      );
      assert.equal(
        checks.failures.some((item) => item.reason === "invalid_js"),
        true,
      );
    } finally {
  ...
  it("flags invalid JSX syntax in post-apply checks", () => {
    ...
    try {
      ...
      assert.equal(checks.ok, false);
      assert.equal(
        checks.failures.some((item) => item.reason === "invalid_source_syntax"),
        true,
      );
    } finally {
      fs.rmSync(tmp, { recursive: true, force: true });
  ...
  it("flags invalid JSON in post-apply checks", () => {
    ...
    try {
      ...
      assert.equal(checks.ok, false);
      assert.equal(
        checks.failures.some((item) => item.reason === "invalid_json"),
        true,
      );
    } finally {
      fs.rmSync(tmp, { recursive: true, force: true });
  ...
  it("does not flag live-mode marker words when they only appear as source literals", () => {
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          'const words = "zarishnoksha-carbonize- data-zarishnoksha-variant zarishnoksha_VARIANT zarishnoksha-live-variant";',
          'const selector = "[data-zarishnoksha-variant=\\"1\\"]";',
          'const example = "<div data-zarishnoksha-variant=\\"1\\">demo</div>";',
  ...
  it("respects off mode before trying local AI commands", () => {
    assert.equal(chooseCopyEditAgent({ env: { zarishnoksha_LIVE_COPY_AGENT: "off" } }), null);
    assert.equal(chooseCopyEditAgent({ env: { zarishnoksha_LIVE_COPY_AGENT: "false" } }), null);
    assert.equal(chooseCopyEditAgent({ env: { zarishnoksha_LIVE_COPY_AGENT: "mock" } }), "mock");
  });

  it("surfaces Claude CLI auth errors from is_error JSON output", () => {
    const output =
      '{"type":"result","subtype":"success","is_error":true,"duration_ms":36,' +
  ...
  it("describeNoProviderError pinpoints which CLI needs which action", () => {
    ...
    assert.match(nothingInstalled, /zarishnoksha_LIVE_COPY_AGENT=mock/);
  ...
  it("auto mode picks the first authenticated provider via injected authCheck", () => {
    ...
    assert.equal(
      chooseCopyEditAgent({ env: { zarishnoksha_LIVE_COPY_AGENT: "auto" }, authCheck: claudeUnauthedCodexAuthed }),
      "codex",
    );

    const onlyClaudeAuthed = (cmd) => cmd === "claude";
    assert.equal(
      chooseCopyEditAgent({ env: { zarishnoksha_LIVE_COPY_AGENT: "auto" }, authCheck: onlyClaudeAuthed }),
      "claude",
    );

    const noneAuthed = () => false;
    assert.equal(chooseCopyEditAgent({ env: {}, authCheck: noneAuthed }), null);
  });
  ...
  it("auto mode falls back to chat when no CLI is authenticated and chat is available", () => {
    ...
    assert.equal(
      chooseCopyEditAgent({
        env: { zarishnoksha_LIVE_COPY_AGENT: "auto" },
  ...
  it("describeNoProviderError mentions starting zarishnoksha live when chat is the missing piece", () => {
    ...
    assert.match(noChatPolling, /Chat: no zarishnoksha live session is currently polling/);
    assert.match(noChatPolling, /Start zarishnoksha live/);
  });
});
