...
import { filterDetectionFindings } from "../cli/lib/zarishnoksha-config.mjs";
...
describe("detectUrl — browser-only fixtures", () => {
  ...
  it("design-system: URL scans apply injected design context", async () => {
    ...
    assert.ok(
      designFindings.some((r) => r.antipattern === "design-system-font"),
      "expected unsupported font",
    );
    assert.ok(
      designFindings.some((r) => r.antipattern === "design-system-color"),
      "expected undocumented colors",
    );
    assert.ok(
      designFindings.some((r) => r.antipattern === "design-system-radius"),
      "expected undocumented radius",
    );
  ...
  it("typography side-by-side: element-level flag cases get regular overlays", async () => {
    ...
    try {
      ...
      await page.evaluate(() => {
        window.__zarishnoksha_CONFIG__ = { autoScan: false };
      });
      await page.evaluate(browserScript);
      const result = await page.evaluate(() => {
        const groups = window.zarishnokshaScan();
        ...
        return {
          ...
          hasBanner: Boolean(document.querySelector(".zarishnoksha-banner")),
          overlays: document.querySelectorAll(".zarishnoksha-overlay:not(.zarishnoksha-banner)").length,
        };
      });
      for (const id of ["tight-leading", "tiny-text", "all-caps-body", "wide-tracking", "justified-text"]) {
        assert.ok(
          result.types.includes(id),
          `expected browser typography scan to include ${id}: ${JSON.stringify(result)}`,
        );
      }
      assert.ok(
        result.pageTypes.includes("overused-font"),
        `expected browser typography scan to include page-level overused-font: ${JSON.stringify(result)}`,
      );
  ...
  it("body-text-viewport-edge: 3 flag paragraphs/list-items, 0 pass cases", async () => {
    ...
    assert.equal(
      edges.length,
      3,
      `expected 3 body-text-viewport-edge findings, got ${edges.length}: ${JSON.stringify(edges.map((e) => e.snippet))}`,
    );
  });
  ...
  it("text-overflow: flags content wider than its box, skips real scroll regions", async () => {
    ...
    assert.equal(
      hits.length,
      2,
      `expected exactly 2 text-overflow findings, got ${hits.length}: ${JSON.stringify(hits.map((h) => h.snippet))}`,
    );
  });
  ...
  it("visual contrast: browser fallback catches low contrast on image backgrounds", async () => {
    ...
    const visualFindings = f.filter(
      (r) => r.antipattern === "low-contrast" && /(?:browser|pixel) contrast/i.test(r.snippet || ""),
    );
    assert.equal(
    ...
    assert.ok(
      f.some(
        (r) => r.antipattern === "low-contrast" && /Translucent white text on a pale pattern/i.test(r.snippet || ""),
      ),
    ...
    assert.equal(
      f.some(
        (r) => r.antipattern === "low-contrast" && /Should (?:flag|pass) after pixel sampling/i.test(r.snippet || ""),
      ),
      false,
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        const before = document.querySelectorAll(
          ".zarishnoksha-overlay, .zarishnoksha-label, .zarishnoksha-banner",
        ).length;
        const analyses = await window.zarishnokshaAnalyzeVisualContrast({ maxCandidates: 20, scrollOffscreen: true });
  ...
  it("browser API: visual contrast scan decorates visible findings without scrolling by default", async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        const syncScanResult = window.zarishnokshaScan({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        const syncDetectResult = window.zarishnokshaDetect({
          visualContrast: true,
          serialize: true,
        });
        const groups = await window.zarishnokshaScanAsync({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        ...
        return {
          ...
          overlays: document.querySelectorAll(".zarishnoksha-overlay:not(.zarishnoksha-banner)").length,
          labels: document.querySelectorAll(".zarishnoksha-label").length,
          analyses: window.zarishnokshaGetLastVisualContrastAnalyses().filter((item) => item.status === "fail").length,
          scrollEvents,
          maxScrollY,
          ...
          hasAsyncApi:
            typeof window.zarishnokshaScanAsync === "function" && typeof window.zarishnokshaDetectAsync === "function",
        };
      });
      const visualGroups = result.groups.filter(
        (group) =>
          group.types.includes("low-contrast") &&
          /(?:White text on light image|Dark text on dark image|Translucent white text|Muted gray text)/i.test(
            group.text,
          ),
      );
      ...
      assert.ok(
        result.overlays >= 3,
        `expected regular overlays for visible visual findings, got: ${JSON.stringify(result)}`,
      );
      assert.ok(
        result.labels >= 3,
        `expected regular labels for visible visual findings, got: ${JSON.stringify(result)}`,
      );
      assert.equal(
        result.maxScrollY,
        0,
        `visual scan should not scroll the page by default: ${JSON.stringify(result)}`,
      );
      assert.equal(result.finalScrollY, 0, `visual scan should preserve scroll by default: ${JSON.stringify(result)}`);
      assert.equal(
        result.syncScanIsArray,
        true,
        `zarishnokshaScan should keep a synchronous Array return: ${JSON.stringify(result)}`,
      );
      assert.equal(
        result.syncDetectIsArray,
        true,
        `zarishnokshaDetect should keep a synchronous Array return: ${JSON.stringify(result)}`,
      );
      assert.equal(
        result.hasAsyncApi,
        true,
        `visual contrast should expose explicit async APIs: ${JSON.stringify(result)}`,
      );

      const refreshedOverlayResult = await page.evaluate(async () => {
        window.scrollTo(0, 0);
        const target = [...document.querySelectorAll("p")].find((node) =>
          /White text on light image should be sampled/i.test(node.textContent || ""),
        );
        target.style.fontSize = "10px";
        const initialGroups = window.zarishnokshaScan({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        const initialTargetGroup = initialGroups.find((group) => group.el === target);
        const deadline = Date.now() + 1000;
        while (Date.now() < deadline && !/low contrast/i.test(target?._zarishnokshaOverlay?.textContent || "")) {
          const nextButton = target?._zarishnokshaOverlay?.querySelector("button:last-of-type");
          if (nextButton) nextButton.click();
          await new Promise((resolve) => setTimeout(resolve, 25));
        }
        const labelVariants = [];
        const overlay = target?._zarishnokshaOverlay;
        for (let i = 0; i < 3; i++) {
        ...
        return {
          ...
          labelText: target?._zarishnokshaOverlay?.textContent || "",
          labelVariants,
          overlayConnected: Boolean(target?._zarishnokshaOverlay?.isConnected),
        };
      });
      assert.ok(
        refreshedOverlayResult.initialTypes.includes("tiny-text"),
        `test setup should create an initial sync overlay on the target: ${JSON.stringify(refreshedOverlayResult)}`,
      );
      assert.ok(
        refreshedOverlayResult.labelVariants.some((text) => /tiny body text/i.test(text)),
        `expected refreshed overlay to keep the sync finding label: ${JSON.stringify(refreshedOverlayResult)}`,
      );
      assert.ok(
        refreshedOverlayResult.labelVariants.some((text) => /low contrast/i.test(text)),
        `expected visual contrast to refresh the existing overlay label: ${JSON.stringify(refreshedOverlayResult)}`,
      );
      assert.equal(
        refreshedOverlayResult.overlayConnected,
        true,
        `expected refreshed overlay to stay connected: ${JSON.stringify(refreshedOverlayResult)}`,
      );

      const lazyResult = await page.evaluate(async () => {
        const target = [...document.querySelectorAll("p")].find((node) =>
          /Muted gray text on a misty image/i.test(node.textContent || ""),
        );
        ...
        return {
          ...
          labels: document.querySelectorAll(".zarishnoksha-label").length,
          analyses: window.zarishnokshaGetLastVisualContrastAnalyses().filter((item) => item.status === "fail").length,
          targetHasOverlay: Boolean(target?._zarishnokshaOverlay),
      ...
      assert.equal(
        lazyResult.analyses,
        4,
        `expected lazy visual resolution after scrolling into view, got: ${JSON.stringify(lazyResult)}`,
      );
      assert.ok(
        lazyResult.overlays >= 4,
        `expected lazy visual overlay after scrolling into view, got: ${JSON.stringify(lazyResult)}`,
      );
      assert.ok(
        lazyResult.labels >= 4,
        `expected lazy visual label after scrolling into view, got: ${JSON.stringify(lazyResult)}`,
      );
      assert.equal(
        lazyResult.targetHasOverlay,
        true,
        `expected lazy visual target to get a regular overlay, got: ${JSON.stringify(lazyResult)}`,
      );
      assert.ok(
        lazyResult.scrollY > 0,
        `test should have naturally scrolled to the offscreen case: ${JSON.stringify(lazyResult)}`,
      );
      ...
      const staleOverlayResult = await page.evaluate(async () => {
        ...
        window.scrollTo(0, 0);
        await new Promise((resolve) => setTimeout(resolve, 50));
        await window.zarishnokshaScanAsync({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        const staleCleared = !target?._zarishnokshaOverlay;
        target?.scrollIntoView({ block: "center", inline: "nearest", behavior: "instant" });
        await new Promise((resolve) => setTimeout(resolve, 250));
        return {
          staleCleared,
          targetHasOverlay: Boolean(target?._zarishnokshaOverlay),
          targetOverlayConnected: Boolean(target?._zarishnokshaOverlay?.isConnected),
          overlays: document.querySelectorAll(".zarishnoksha-overlay:not(.zarishnoksha-banner)").length,
          analyses: window.zarishnokshaGetLastVisualContrastAnalyses().filter((item) => item.status === "fail").length,
        };
      });
      assert.equal(
        staleOverlayResult.staleCleared,
        true,
        `expected clearOverlays to remove stale target overlay refs, got: ${JSON.stringify(staleOverlayResult)}`,
      );
      assert.equal(
        staleOverlayResult.targetHasOverlay,
        true,
        `expected lazy visual target to be highlightable after a rescan, got: ${JSON.stringify(staleOverlayResult)}`,
      );
      assert.equal(
        staleOverlayResult.targetOverlayConnected,
        true,
        `expected lazy visual overlay after rescan to be connected, got: ${JSON.stringify(staleOverlayResult)}`,
      );
      ...
      const offscreenVisualGroups = offscreenResult.groups.filter(
        (group) =>
          group.types.includes("low-contrast") &&
          /(?:White text on light image|Dark text on dark image|Translucent white text|Muted gray text)/i.test(
            group.text,
      ...
      assert.equal(
        offscreenResult.analyses,
        4,
        `expected 4 opt-in visual failures, got: ${JSON.stringify(offscreenResult)}`,
      );
      assert.equal(
        offscreenVisualGroups.length,
        4,
        `expected 4 opt-in visual groups, got: ${JSON.stringify(offscreenResult)}`,
      );
      assert.ok(
        offscreenResult.maxScrollY > 0,
        `offscreen opt-in should be allowed to scroll: ${JSON.stringify(offscreenResult)}`,
      );
      assert.equal(
        offscreenResult.finalScrollY,
        0,
        `offscreen opt-in should restore scroll: ${JSON.stringify(offscreenResult)}`,
      );
    } finally {
      await browser.close().catch(() => {});
    }
  ...
  it("extension mode remove cancels pending lazy visual contrast work", async () => {
    ...
    try {
      ...
      const browserScript = fs.readFileSync(path.join(ROOT, "cli/engine/detect-antipatterns-browser.js"), "utf-8");
      await page.evaluate(() => {
        document.documentElement.dataset.zarishnokshaExtension = "true";
        window.__zarishnokshaMessages = [];
        window.addEventListener("message", (event) => {
          if (event.source !== window || !event.data?.source?.startsWith("zarishnoksha-")) return;
          window.__zarishnokshaMessages.push(event.data);
        });
      });
      await page.evaluate(browserScript);
      const result = await page.evaluate(async () => {
        ...
        while (
          ...
          !window
            .zarishnokshaGetLastVisualContrastAnalyses()
            .some((item) => item.status === "unresolved" && item.reason === "text outside viewport")
        ) {
          await new Promise((resolve) => setTimeout(resolve, 25));
        }
        const unresolvedBeforeRemove = window
          .zarishnokshaGetLastVisualContrastAnalyses()
        ...
        const resultsAfterRemove = window.__zarishnokshaMessages.filter(
          (message) => message.source === "zarishnoksha-results",
        ).length;
        return {
          unresolvedBeforeRemove,
          overlayCount: document.querySelectorAll(".zarishnoksha-overlay").length,
          targetHasOverlay: Boolean(target?._zarishnokshaOverlay),
          resultsAfterRemove,
        };
      });
      assert.ok(
        result.unresolvedBeforeRemove > 0,
        `test setup should leave lazy visual candidates pending: ${JSON.stringify(result)}`,
      );
      assert.equal(
        result.overlayCount,
        0,
        `remove should not allow lazy visual overlays to reappear: ${JSON.stringify(result)}`,
      );
      assert.equal(
        result.targetHasOverlay,
        false,
        `remove should clear stale target overlay refs: ${JSON.stringify(result)}`,
      );
      await page.close();
    } finally {
      await browser.close().catch(() => {});
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        try {
          ...
          while (
            Date.now() < deadline &&
            !window.__zarishnokshaMessages.some((message) => message.source === "zarishnoksha-error")
          ) {
            await new Promise((resolve) => setTimeout(resolve, 25));
          }
          return {
            ready: window.__zarishnokshaMessages.some((message) => message.source === "zarishnoksha-ready"),
            results: window.__zarishnokshaMessages.some((message) => message.source === "zarishnoksha-results"),
            errors: window.__zarishnokshaMessages
              .filter((message) => message.source === "zarishnoksha-error")
      ...
      assert.equal(
        result.results,
        true,
        `expected initial sync results before async visual error, got: ${JSON.stringify(result)}`,
      );
      assert.ok(
  ...
  it("extension mode echoes scan ids on result messages", async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        while (
          Date.now() < deadline &&
          !window.__zarishnokshaMessages.some(
            (message) => message.source === "zarishnoksha-results" && message.scanId === "scan-2",
          )
        ) {
          await new Promise((resolve) => setTimeout(resolve, 25));
        }
        const resultMessage = window.__zarishnokshaMessages.find(
          (message) => message.source === "zarishnoksha-results" && message.scanId === "scan-2",
        );
        return {
          ready: window.__zarishnokshaMessages.some((message) => message.source === "zarishnoksha-ready"),
  ...
  it("browser API: zarishnokshaDetect is pure, zarishnokshaScan decorates", async () => {
    const puppeteer = await import("puppeteer");
    const browser = await puppeteer.default.launch({
      headless: true,
      args: process.env.CI ? ["--no-sandbox", "--disable-setuid-sandbox"] : [],
    ...
    try {
      ...
      const pure = await page.evaluate(() => {
        ...
        const findings = window.zarishnokshaDetect({ decorate: false, serialize: true });
        const after = document.querySelectorAll(
          ".zarishnoksha-overlay, .zarishnoksha-label, .zarishnoksha-banner",
        ).length;
  ...
  it("browser API: async scan and detect reject instead of throwing synchronously", async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        try {
          const scan = await window.zarishnokshaScanAsync().then(
            () => ({ state: "resolved" }),
            (error) => ({ state: "rejected", message: error?.message || String(error) }),
          );
          const detect = await window.zarishnokshaDetectAsync().then(
            () => ({ state: "resolved" }),
            (error) => ({ state: "rejected", message: error?.message || String(error) }),
          );
          return { scan, detect };
