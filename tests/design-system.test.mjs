...
function mkTmp() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zarishnoksha-design-system-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("loadDesignSystemForCwd()", () => {
  it("loads DESIGN.md plus .zarishnoksha/design.json and marks stale sidecars", () => {
    const cwd = mkTmp();
    fs.mkdirSync(path.join(cwd, ".zarishnoksha"), { recursive: true });
    const designMd = path.join(cwd, "DESIGN.md");
    const sidecarJson = path.join(cwd, ".zarishnoksha", "design.json");
