...
import { isAllowedColorRaw, loadDesignSystemForCwd } from "../cli/engine/design-system.mjs";
...
describe("zarishnoksha Shiki themes", () => {
  it("only use colors from DESIGN.md and the sidecar ramps", () => {
    ...
    for (const [mode, theme] of Object.entries(zarishnokshaShikiThemes)) {
  ...
  it("keeps every token foreground at AA contrast against its code block background", () => {
    for (const [mode, theme] of Object.entries(zarishnokshaShikiThemes)) {
      ...
      for (const foregroundValue of themeForegrounds(theme)) {
        ...
        assert.ok(ratio >= 4.5, `${mode} foreground ${foregroundValue} only has ${ratio.toFixed(2)}:1 contrast`);
      }
    }
  });
});
