...
describe("live-poll reply payloads", () => {
  it("preserves structured data for durable carbonize recovery acknowledgements", () => {
    ...
    assert.deepEqual(
      payload.data,
      { carbonize: true },
      'event=live_poll.reply_data actor=agent operation=completion_ack risk=carbonize_flag_dropped_before_server_journal expected={"carbonize":true} actual=' +
...
describe("live-poll --reply arg parsing", () => {
  ...
  it("reaches the POST body unchanged through buildPollReplyPayload", () => {
    const reply = parseReplyArgs([
      "--reply",
      "abc12345",
      "done",
      "--data",
      '{"status":"partial","appliedEntryIds":["abc12345"]}',
    ]);
  ...
  it("requires an explicit reply status after the event id", () => {
    assert.throws(
      ...
      (err) => err.code === "INVALID_REPLY_ARGS" && /Missing reply status/.test(err.message),
    );
  });
