...
describe("live-e2e LLM agent provider config", () => {
  ...
  it("explicitly selects DeepSeek over Anthropic", () => {
    const config = resolveLlmAgentConfig(
      {},
      {
        zarishnoksha_E2E_LLM_PROVIDER: "deepseek",
        ANTHROPIC_API_KEY: "claude-key",
        DEEPSEEK_API_KEY: "deepseek-key",
      },
    );
  ...
  it("allows explicit model and base URL overrides", () => {
    const config = resolveLlmAgentConfig(
      ...
      {
        zarishnoksha_E2E_LLM_PROVIDER: "deepseek",
        zarishnoksha_E2E_LLM_MODEL: "ignored-model",
        DEEPSEEK_API_KEY: "test-key",
      },
    );
  ...
  it("rejects unsupported providers", () => {
    assert.throws(
      () => resolveLlmAgentConfig({}, { zarishnoksha_E2E_LLM_PROVIDER: "other" }),
      /Unsupported zarishnoksha_E2E_LLM_PROVIDER: other/,
    );
  });
});
...
describe("live-e2e LLM agent provider replay", () => {
  it("applies a generic label/count/source-key batch through the selected provider", async (t) => {
    if (process.env.zarishnoksha_LLM_REPLAY !== "1") {
      t.skip("set zarishnoksha_LLM_REPLAY=1 to run provider-backed manual edit replay");
      return;
    }

    const config = resolveLlmAgentConfig({
      provider: process.env.zarishnoksha_E2E_LLM_PROVIDER || "deepseek",
...
function createSourceKeyReplayProject(mode) {
  const root = mkdtempSync(join(tmpdir(), "zarishnoksha-llm-replay-"));
  ...
  if (mode === "restore") {
    ...
    writeFileSync(
      join(root, "src/visuals.js"),
      ["export const visualsByLabel = {", "  'New Label': '<svg></svg>',", "};", ""].join("\n"),
    );
    return root;
  }
  writeFileSync(
  ...
  writeFileSync(
    join(root, "src/visuals.js"),
    ["export const visualsByLabel = {", "  'Old Label': '<svg></svg>',", "};", ""].join("\n"),
  );
  return root;
}
...
describe("live-e2e LLM agent manual edit prompt", () => {
  ...
  it("tells the model not to return source edits for failed entries", () => {
    assert.match(
      MANUAL_EDIT_SYSTEM_INSTRUCTIONS,
      /Never return sourceEdits for failed, omitted, or unreported entries/,
    );
  });
...
describe("live-e2e LLM agent manual edit coverage validation", () => {
  const batch = {
    entries: [
      {
        id: "entry-a",
        ops: [{ newText: "Five-leaf stress title applied" }, { originalText: "7", newText: "7 workshop seats remain" }],
      },
    ],
  };
    ...
    const error = validateManualEditCoverage(
      ...
      },
      {
        entries: [
          {
            id: "entry-a",
            ops: [{ originalText: "7", newText: "7 workshop seats remain" }],
          },
        ],
      },
    );
        ...
        candidates: [
          {
            entryId: "entry-a",
            ref: "label",
            objectKeyMatches: [{ file: "src/visuals.js", line: 5, needle: "Old label" }],
          },
        ],
      },
    );
    ...
    const error = validateManualEditCoverage(
      ...
      },
      {
        entries: [
          {
            id: "entry-a",
            ops: [{ ref: "label", originalText: "Old label", newText: "New label" }],
          },
        ],
        candidates: [
          {
        ...
        candidates: [
          {
            entryId: "entry-a",
            ref: "label",
            objectKeyMatches: [{ file: "src/data.js", line: 5, needle: "Old label" }],
          },
        ],
      },
    );
...
describe("live-e2e LLM agent variant copy validation", () => {
  it("allows variants that preserve the picked element text", () => {
    const result = validateVariantVisibleCopy(
      {
        variants: [{ innerHtml: '<h1 class="hero-title"><span>Manual Title Applied</span></h1>' }],
      },
  ...
  it("rejects variants that rewrite the picked element text", () => {
    const result = validateVariantVisibleCopy(
      {
        variants: [{ innerHtml: '<h1 class="hero-title">Generated Fresh Title</h1>' }],
      },
      { textContent: "Manual Title Applied" },
    );
  ...
  it("uses outerHTML as a fallback when textContent is absent", () => {
    const result = validateVariantVisibleCopy(
      {
        variants: [{ innerHtml: '<section class="hero-copy"><h1>Batch Title</h1><p>Batch Body</p></section>' }],
      },
  ...
  it("rejects variants that are source-identical to the picked element", () => {
    const result = validateVariantMaterialChange(
      {
        variants: [{ innerHtml: '<h1 class="hero-title">Manual Title Applied</h1>' }],
      },
  ...
  it("rejects bare text variants that split the copy across sibling text nodes", () => {
    const result = validateVariantMaterialChange(
      {
        variants: [{ innerHtml: '<h1 class="title">Manual <span>Title</span></h1>' }],
      },
  ...
  it("allows bare text variants that wrap the full copy in one child", () => {
    const result = validateVariantMaterialChange(
      {
        variants: [{ innerHtml: '<h1 class="title"><span>Manual Title</span></h1>' }],
      },
      { outerHTML: '<h1 class="title">Manual Title</h1>' },
    );
...
describe("live-e2e LLM agent parseVariantResponse", () => {
  const validParsed = {
    scopedCss: '@scope ([data-zarishnoksha-variant="1"]) {}',
  ...
  it("strips a single surrounding ```json fence", () => {
    const parsed = parseVariantResponse("```json\n" + JSON.stringify(validParsed) + "\n```");
    assert.deepEqual(parsed, validParsed);
  });
  ...
  it("rejects scopedCss that includes an outer style tag", () => {
    const body = JSON.stringify({
      scopedCss: '<style data-zarishnoksha-css="SESSION_ID">@scope ([data-zarishnoksha-variant="1"]) {}</style>',
      variants: [{ innerHtml: "<h1>x</h1>" }],
    });
    assert.throws(() => parseVariantResponse(body), /scopedCss must contain CSS rules only/);
  });

  it("rejects scopedCss that would break JSX template literals", () => {
    const body = JSON.stringify({
      scopedCss: '@scope ([data-zarishnoksha-variant="1"]) { .title::before { content: `bad`; } }',
      variants: [{ innerHtml: "<h1>x</h1>" }],
    });
    assert.throws(() => parseVariantResponse(body), /scopedCss must not contain backticks/);
  });

  it("rejects scopedCss with template interpolation", () => {
    const body = JSON.stringify({
      scopedCss: '@scope ([data-zarishnoksha-variant="1"]) { .title { color: ${bad}; } }',
      variants: [{ innerHtml: "<h1>x</h1>" }],
    });
    assert.throws(() => parseVariantResponse(body), /scopedCss must not contain template interpolation/);
  });

  it("rejects malformed scopedCss before it reaches framework compilers", () => {
    const body = JSON.stringify({
      scopedCss: '@scope ([data-zarishnoksha-variant="1"]) { .title { color: red; }',
      variants: [{ innerHtml: "<h1>x</h1>" }],
    });
    assert.throws(() => parseVariantResponse(body), /scopedCss has unbalanced CSS braces/);
  });
  ...
  it("rejects variant HTML that includes its own style tag", () => {
    ...
    assert.throws(() => parseVariantResponse(body), /innerHtml must not include a <style> tag/);
  });
  ...
  it("rejects framework-shaped variant HTML", () => {
    ...
    assert.throws(() => parseVariantResponse(body), /must use HTML class= attributes/);
  });

  it("rejects variant HTML that tries to emit wrapper scaffolding", () => {
    const body = JSON.stringify({
      scopedCss: '@scope ([data-zarishnoksha-variant="1"]) {}',
      variants: [{ innerHtml: '<div data-zarishnoksha-variant="1"><h1>x</h1></div>' }],
    });
    assert.throws(() => parseVariantResponse(body), /must not include zarishnoksha wrapper attributes/);
  });
  ...
  it("echoes the parsed payload on empty variants array", () => {
    ...
    assert.throws(
      () => parseVariantResponse(body),
      (err) =>
        /variants must be a non-empty array/.test(err.message) && /Parsed \(first 500 chars\):/.test(err.message),
    );
  });
  ...
  it("echoes the parsed payload on non-array params", () => {
    ...
    assert.throws(
      () => parseVariantResponse(body),
      (err) =>
        /variants\[0\]\.params must be an array/.test(err.message) && /Parsed \(first 500 chars\):/.test(err.message),
    );
  });
});
