  ...
  it("covers live chrome, replace, edit, annotations, insert, refresh recovery, accept/discard, and exit", async (t) => {
    ...
    try {
      ...
      const afterExit = await page.evaluate(() => ({
        ...
        hasGlobalBar: Boolean(window.__zarishnokshaLiveQuery?.("#zarishnoksha-live-global-bar")),
      }));
...
function loadConsolidatedFixture() {
  ...
  fixture.runtime = {
    ...
    stateProbe: {
      ...
      windowProperty: "__zarishnokshaStatefulMounts",
    ...
    steer: {
      ...
      expectSelector: 'h1.hero-title[data-zarishnoksha-steer="e2e"]',
      expectSourceContains: 'data-zarishnoksha-steer="e2e"',
    },
    probe: {
      expectLiveInit: true,
...
async function runReplaceDiscardRecoveryFlow({ page, tmp, evidence }) {
  ...
  assert.match(session.previewFile, /^node_modules\/\.zarishnoksha-live\/[^/]+\/manifest\.json$/);
  ...
  assert.equal(
    readFileSync(join(tmp, ROUTE_FILE), "utf-8"),
    sourceBefore,
    "real Svelte source is not mutated while cycling",
  );
...
async function runEditCopyFlow({ page, tmp, live, evidence }) {
  ...
  await page.waitForFunction(() => !window.__zarishnokshaLiveQuery('[data-zarishnoksha-editable="true"]'), null, {
    timeout: 5_000,
  });
  ...
  assert.doesNotMatch(
    sourceAfter,
    /contenteditable|data-zarishnoksha-editable|data-zarishnoksha-original-text|zarishnoksha-variants/,
    "manual Apply does not leak live scaffolding",
  );
}
...
async function runAcceptReplaceFlow({ page, tmp, evidence }) {
  ...
  assert.doesNotMatch(
    source,
    /data-zarishnoksha-variants|zarishnoksha-variants-start|data-zarishnoksha-variant=/,
    "accepted replace source is clean",
  );
  assert.equal(
    existsSync(join(tmp, dirname(session.previewFile))),
    false,
    "temp Svelte preview folder deleted after accept",
  );
  return session;
}
...
async function runInsertFlowWithRecovery({ page, tmp, evidence }) {
  ...
  const insertEvent = await waitForLatestJournalEvent(
    tmp,
    (event) => event.type === "generate" && event.mode === "insert",
  );
  ...
  await assertVariantCounter(page, 3, 3);
  await page.waitForFunction(() => Boolean(document.querySelector(".empty-card + *")), null, { timeout: 20_000 });
  ...
  assert.match(
    source,
    /automatisch|synchron|sync|abgeglichen|shared expenses/i,
    "accepted insert content lands in real source",
  );
  assert.doesNotMatch(
    source,
    /data-zarishnoksha-variants|zarishnoksha-variants-start|data-zarishnoksha-variant=/,
    "accepted insert source is clean",
  );
  assert.equal(
    existsSync(join(tmp, dirname(session.previewFile))),
    false,
    "insert temp Svelte preview folder deleted after accept",
  );
  return session;
}
...
async function assertSvelteAdapterInjection(tmp) {
  ...
  assert.doesNotMatch(appHtml, /live\.js|zarishnoksha-live-start/, "Svelte adapter leaves src/app.html untouched");
  assert.match(
    layout,
    /zarishnokshaLiveRoot|zarishnoksha-live-svelte-start/,
    "Svelte adapter patches layout with live root",
  );
}

async function assertShadowChrome(page) {
  const result = await page.evaluate(() => {
    const host = document.getElementById("zarishnoksha-live-root");
    const root = host?.shadowRoot;
    return {
      hasHost: Boolean(host),
      hasShadowRoot: Boolean(root),
      hasGlobalBar: Boolean(root?.getElementById("zarishnoksha-live-global-bar")),
      documentHasGlobalBar: Boolean(document.querySelector("#zarishnoksha-live-global-bar")),
    };
  });
...
async function assertHostileCssIsolation(page) {
  ...
  const hostileStyle = await page.addStyleTag({
    content: `
      button, div, input, svg, #zarishnoksha-live-root, zarishnoksha-live-root * {
...
async function waitForGlobalBarStable(page) {
  ...
  await page.waitForFunction(
    () => {
      const bar = window.__zarishnokshaLiveQuery?.("#zarishnoksha-live-global-bar");
      if (!bar) return false;
...
async function assertSelectedElementChrome(page) {
  ...
  const result = await page.evaluate(() => {
    ...
    const bar = q("#zarishnoksha-live-bar");
    const picker = q("#zarishnoksha-live-picker");
    const badge = q("#zarishnoksha-live-edit-badge");
    const highlight = q("#zarishnoksha-live-highlight");
    const input = q("#zarishnoksha-live-input");
    return {
      hasBar: Boolean(bar),
...
async function selectAction(page, label) {
  ...
  const opened = await page.evaluate(() => {
    const bar = window.__zarishnokshaLiveQuery("#zarishnoksha-live-bar");
  ...
  await page.waitForFunction(
    (expected) => {
      const picker = window.__zarishnokshaLiveQuery("#zarishnoksha-live-picker");
      return (
  ...
  const clicked = await page.evaluate((expected) => {
    const picker = window.__zarishnokshaLiveQuery("#zarishnoksha-live-picker");
    const btn = [...(picker?.querySelectorAll("button") || [])].find((candidate) =>
...
async function clickEditBadgeAction(page, label) {
  const clicked = await page.evaluate((expected) => {
    const badge = window.__zarishnokshaLiveQuery("#zarishnoksha-live-edit-badge");
...
async function clickPendingTrash(page) {
  ...
  const clicked = await page.evaluate(() => {
    const dock = window.__zarishnokshaLiveQuery("#zarishnoksha-live-pending-dock");
    const btn = [...(dock?.querySelectorAll("button") || [])].find((candidate) =>
      /Discard copy edits/i.test(candidate.getAttribute("aria-label") || candidate.title || ""),
    );
    if (!btn) return false;
    btn.click();
    return true;
...
async function assertVariantCounter(page, variant, count) {
  await page.waitForFunction(
    ({ variant, count }) => {
      ...
      const style = getComputedStyle(bar);
      return style.display !== "none" && style.visibility !== "hidden" && Number.parseFloat(style.opacity || "0") > 0.5;
    },
    { variant, count },
    { timeout: 10_000 },
  );
...
async function waitForSvelteAcceptComplete(tmp, session, { timeout }) {
  ...
  while (Date.now() < deadline) {
    ...
    if (
      ...
      !/data-zarishnoksha-variants|zarishnoksha-variants-start|data-zarishnoksha-variant=/.test(source)
    ) {
      return;
    }
  ...
  throw new Error(
    `Svelte accept did not complete for ${session.id}; snapshot=${JSON.stringify(readSessionSnapshot(tmp, session.id))}`,
  );
}
...
function assertCompletedRealSource(tmp, id) {
  ...
  assert.match(snapshot.previewFile || "", /node_modules\/\.zarishnoksha-live\/[^/]+\/manifest\.json/);
}
...
function readSessionSnapshot(tmp, id) {
  const file = join(tmp, ".zarishnoksha/live/sessions", `${id}.snapshot.json`);
...
async function getPendingDockCount(live) {
  const res = await fetch(
    `http://127.0.0.1:${live.port}/manual-edit-stash?token=${encodeURIComponent(live.token)}&pageUrl=%2F`,
  );
...
async function assertNoCyclingZero(page) {
  const text = await page.evaluate(() => window.__zarishnokshaLiveQuery("#zarishnoksha-live-bar")?.textContent || "");
...
async function readStateProbe(page) {
  return page.evaluate(() => ({
    ...
    mounts: window.__zarishnokshaStatefulMounts,
  }));
}
...
function assertFinalSourceClean(tmp) {
  ...
  assert.doesNotMatch(
    source,
    /node_modules\/\.zarishnoksha-live|data-zarishnoksha-variants|zarishnoksha-variants-start|data-zarishnoksha-variant=/,
  );
}
...
function readJournalEvents(tmp) {
  const dir = join(tmp, ".zarishnoksha/live/sessions");
...
function createEvidenceWriter({ page, tmp, live, consoleErrors }) {
  ...
  return {
    async capture(name) {
      ...
      try {
        const dom = await page.evaluate(() => {
          ...
          return {
            ...
            liveState: window.__zarishnoksha_LIVE_CHROME_CORE__?.debugState?.() || null,
            localSession: localStorage.getItem("zarishnoksha-live-session"),
          };
        });
      ...
      copyDirIfExists(join(tmp, ".zarishnoksha/live/sessions"), join(dir, "sessions"));
      ...
      try {
        const status = await fetch(`http://127.0.0.1:${live.port}/status?token=${encodeURIComponent(live.token)}`).then(
          (res) => res.json(),
        );
...
function listPreviewFiles(tmp) {
  const root = join(tmp, "node_modules/.zarishnoksha-live");
...
function createArtifactRoot() {
  const explicit = process.env.zarishnoksha_SVELTE_DEEPSEEK_ARTIFACT_DIR;
...
function assertNoConsoleErrors(consoleErrors) {
  const ignored = consoleErrors.filter((line) => !/favicon|ResizeObserver loop limit exceeded/i.test(line));
