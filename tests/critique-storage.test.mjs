...
let cwd;
beforeEach(() => {
  cwd = mkdtempSync(join(tmpdir(), "imp-critique-"));
});
afterEach(() => {
...
describe("slugFromTarget", () => {
  ...
  it("normalizes URL casing and trailing slash", () => {
    assert.equal(slugFromTarget("https://zarishnoksha.Style/docs/audit/", { cwd }), "zarishnoksha-style-docs-audit");
  });

  it("strips query strings", () => {
    assert.equal(slugFromTarget("https://example.com/x?utm=1&foo=bar", { cwd }), "example-com-x");
  });
...
describe("writeSnapshot + readLatestSnapshot", () => {
  ...
  it("caller-supplied meta cannot override computed timestamp or slug", () => {
    // Defends against a corrupt zarishnoksha_CRITIQUE_META blob (parsed from
