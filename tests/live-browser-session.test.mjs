...
function loadFactory() {
  ...
  return context.__zarishnoksha_LIVE_SESSION__.createLiveBrowserSessionState;
}
...
describe("live-browser-session state helper", () => {
  it("persists session, handled state, and scroll independently", () => {
    ...
    const state = createState({ prefix: "zarishnoksha-live", storage, idFactory: () => "owner-a" });
  ...
  it("carries checkpoint revision across reload-equivalent helper instances", () => {
    const createState = loadFactory();
    const storage = createMemoryStorage();
    const first = createState({ prefix: "zarishnoksha-live", storage, idFactory: () => "owner-a" });
    ...
    const second = createState({ prefix: "zarishnoksha-live", storage, idFactory: () => "owner-b" });
    const restored = second.loadSession();
    assert.equal(restored.checkpointRevision, 2);
    assert.equal(
      second.nextCheckpointRevision(),
      3,
      "event=live_browser_session.revision_resume actor=browser operation=reload_checkpoint risk=durable_store_ignores_stale_checkpoint expected=3 actual=" +
        second.currentCheckpointRevision(),
    );
  });
});
