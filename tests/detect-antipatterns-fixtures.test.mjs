...
import { detectHtml, detectText, normalizeDesignSystem } from "../cli/engine/detect-antipatterns.mjs";
...
describe("detectHtml — static HTML/CSS fixtures", () => {
  ...
  it("color: flag column triggers all color rules, pass column adds none", async () => {
    ...
    assert.ok(
      f.some((r) => r.antipattern === "gray-on-color"),
      "expected gray-on-color",
    );
    assert.ok(
  ...
  it("color: white text on background-image url() ancestor is not flagged as low-contrast", async () => {
    ...
    const falsePositive = f.filter(
      (r) => r.antipattern === "low-contrast" && /#ffffff on #fafafa/i.test(r.snippet || ""),
    );
    assert.equal(
  ...
  it("color: styled <a> and <button> with their own background get contrast checks", async () => {
    ...
    const f = await detectHtml(path.join(FIXTURES, "color.html"));
    const pillBtnFlag = f.some(
      (r) => r.antipattern === "low-contrast" && /#5b4f44/i.test(r.snippet || "") && /#1f1a15/i.test(r.snippet || ""),
    );
    assert.ok(pillBtnFlag, "expected low-contrast finding for styled <a> pill button");
    const styledButtonFlag = f.some(
      (r) => r.antipattern === "low-contrast" && /#6c7280/i.test(r.snippet || "") && /#374151/i.test(r.snippet || ""),
    );
  ...
  it("color: inline <a> without own background remains skipped (no regression)", async () => {
    ...
    // page on the web.
    const f = await detectHtml(path.join(FIXTURES, "color.html"));
    const inlineLinkFalsePositive = f.some((r) => r.antipattern === "low-contrast" && /#aaaaaa/i.test(r.snippet || ""));
  ...
  it("color: styled <a> with good contrast does not flag", async () => {
    ...
    const f = await detectHtml(path.join(FIXTURES, "color.html"));
    const goodPillFalsePositive = f.some(
      (r) => r.antipattern === "low-contrast" && /#f5f0e8/i.test(r.snippet || "") && /#141419/i.test(r.snippet || ""),
    );
  ...
  it("typography: side-by-side page has visible element-level flag cases", async () => {
    ...
    assert.ok(
      ids.has("overused-font"),
      "expected typography side-by-side fixture to include a page-level overused-font finding",
    );
  });
  ...
  it("design-system: flags only values outside the provided DESIGN.md tokens", async () => {
    ...
    assert.ok(
      designFindings.some(
        (r) => r.antipattern === "design-system-font" && /Google Fonts: Poppins/.test(r.snippet || ""),
      ),
      "expected source-level Google Fonts usage in HTML to be flagged",
    );
    assert.doesNotMatch(
      snippets,
      /Undocumented color #ff00aa/,
      "source and computed color findings should not duplicate",
    );
    assert.doesNotMatch(snippets, /font-family: Poppins/, "source and computed font findings should not duplicate");
    assert.doesNotMatch(
      snippets,
      /border-radius: 18px is outside/,
      "source and computed radius findings should not duplicate",
    );
    assert.doesNotMatch(
      snippets,
      /on style "\.design-system-fixture/,
      "static DOM design pass should skip <style> content",
    );
    assert.equal(
...
describe("detectHtml — icon-tile-stack", () => {
  ...
  const SHOULD_FLAG = ["Lightning Fast", "Secure Storage", "Easy Setup", "Powerful Analytics", "Emoji Inline Icon"];
  const SHOULD_PASS = [
    "Sarah Chen",
    "Article Headline",
    "Inline Side By Side",
...
describe("detectHtml — repeated-section-kickers", () => {
  const SHOULD_FLAG = ["The Future Is Admitted", "A Private Rehearsal", "Reviewed, Not Sold", "Touch the Future"];
  const SHOULD_PASS = [
...
describe("detectHtml — cramped-padding (wrapper variant)", () => {
  ...
  it("cramped-padding (wrapper): flags only the should-flag column", async () => {
    ...
    for (const cls of SHOULD_PASS_CLASSES) {
      assert.ok(!flagged.has(cls), `".${cls}" should NOT be flagged as cramped-padding`);
    }
  });
});
...
describe("detectHtml — cream-palette", () => {
  it("cream-palette: flags a warm cream/beige page background", async () => {
    ...
    assert.equal(
      f.filter((r) => r.antipattern === "cream-palette").length,
      1,
      `expected one cream-palette finding, got: ${f
        .filter((r) => r.antipattern === "cream-palette")
        .map((r) => r.snippet)
        .join("; ")}`,
    );
  });
  ...
  it("cream-palette: does not fire on a neutral (non-cream) page", async () => {
    const f = await detectHtml(path.join(FIXTURES, "typography-should-pass.html"));
    assert.equal(
      f.some((r) => r.antipattern === "cream-palette"),
      false,
      "neutral page must not flag cream-palette",
    );
  });
...
describe("detectHtml — gated provider tells (--gpt / --gemini)", () => {
  ...
  it("gpt-tells: with providers:[gpt], flag column triggers all three, pass column adds none", async () => {
    ...
    for (const id of GPT_IDS) {
      assert.equal(
        f.filter((r) => r.antipattern === id).length,
        1,
  ...
  it("gemini-tells: gated OFF by default, ON under providers:[gemini]", async () => {
    ...
    assert.equal(
      off.some((r) => r.antipattern === "image-hover-transform"),
      false,
    ...
    assert.equal(
      on.filter((r) => r.antipattern === "image-hover-transform").length,
      2,
