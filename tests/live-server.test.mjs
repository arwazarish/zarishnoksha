...
function startServer(port = 8499, { cwd = REPO_ROOT, env = {} } = {}) {
  return new Promise((resolve, reject) => {
    const proc = spawn("node", [SERVER_SCRIPT, "--port=" + port], {
      ...
      env: { ...process.env, zarishnoksha_LIVE_COPY_AGENT: "off", ...env },
    });
    let output = "";
...
async function waitForManualActivity(server, type, { timeoutMs = 1000 } = {}) {
  ...
  assert.fail(
    "timed out waiting for manual edit activity " +
      type +
      "; last=" +
      JSON.stringify(last?.manualEdits?.lastActivity || null),
  );
}
...
it("gitignores local zarishnoksha runtime artifacts", () => {
  const ignored = execFileSync(
    "git",
    [
      "check-ignore",
      ".zarishnoksha/live/manual-edit-apply-transaction.json",
      ".zarishnoksha/live/manual-edit-evidence/example.json",
      ".zarishnoksha/hook.cache.json",
      ".zarishnoksha/hook.pending.json",
      ".zarishnoksha/config.local.json",
      ".zarishnoksha/live/deferred-svelte-component-accepts.json",
    ],
    { cwd: REPO_ROOT, encoding: "utf-8" },
  );
  assert.match(ignored, /\.zarishnoksha\/live\/manual-edit-apply-transaction\.json/);
  assert.match(ignored, /\.zarishnoksha\/live\/manual-edit-evidence\/example\.json/);
  assert.match(ignored, /\.zarishnoksha\/hook\.cache\.json/);
  assert.match(ignored, /\.zarishnoksha\/hook\.pending\.json/);
  assert.match(ignored, /\.zarishnoksha\/config\.local\.json/);
  assert.match(ignored, /\.zarishnoksha\/live\/deferred-svelte-component-accepts\.json/);
});
...
describe("live-server integration", () => {
  ...
  before(async () => {
    ...
    // never land in the real repo's `.zarishnoksha/live/sessions/`. Those would
    // otherwise be replayed into the poll queue on the next real `live` run.
    serverCwd = mkdtempSync(join(tmpdir(), "zarishnoksha-live-server-"));
    ...
    writeFileSync(join(serverCwd, "package.json"), JSON.stringify({ name: "zarishnoksha" }));
  ...
  it("/live.js injects the canonical command vocabulary", async () => {
    // live-browser.js builds its action picker from window.__zarishnoksha_VOCAB__
    ...
    assert.match(body, /window\.__zarishnoksha_VOCAB__\s*=/);
    const injected = JSON.parse(body.match(/window\.__zarishnoksha_VOCAB__\s*=\s*(\[.*?\]);/s)[1]);
  ...
  it("/status returns durable recovery state", async () => {
    ...
    assert.equal(data.status, "ok");
    assert.equal(
      data.activeSessions.some((s) => s.id === "a1b2c3d5"),
      true,
    );
    assert.equal(
      data.pendingEvents.some((e) => e.id === "a1b2c3d5" && e.type === "generate"),
      true,
    );
  ...
  it("/live.js serves script with token injected", async () => {
    ...
    assert.ok(text.includes("__zarishnoksha_TOKEN__"));
    assert.ok(text.includes(server.token));
    assert.ok(text.includes("__zarishnoksha_PORT__"));
    const preludeIndex = text.indexOf("window.__zarishnoksha_VOCAB__");
    const sessionPartIndex = text.indexOf("zarishnoksha live script part: session-state (live-browser-session.js)");
    const domPartIndex = text.indexOf("zarishnoksha live script part: dom-helpers (live-browser-dom.js)");
    const browserPartIndex = text.indexOf("zarishnoksha live script part: browser-ui (live-browser.js)");
    const sessionHelperIndex = text.indexOf("__zarishnoksha_LIVE_SESSION__");
    const domHelperIndex = text.indexOf("__zarishnoksha_LIVE_DOM__");
    const browserInitIndex = text.indexOf("__zarishnoksha_LIVE_INIT__");
    ...
    assert.ok(
      ...
      "event=live_server.browser_script_order actor=browser operation=load_live_js risk=prelude_after_script_part expected=prelude before parts actual=" +
        preludeIndex +
        ":" +
        sessionPartIndex,
    );
    assert.ok(
      ...
      "event=live_server.browser_script_order actor=browser operation=load_live_js risk=dom_part_before_session_helper expected=session part before dom part actual=" +
        sessionPartIndex +
        ":" +
        domPartIndex,
    );
    assert.ok(
      ...
      "event=live_server.browser_script_order actor=browser operation=load_live_js risk=browser_part_before_dom_helpers expected=dom part before browser part actual=" +
        domPartIndex +
        ":" +
        browserPartIndex,
    );
    assert.ok(
      ...
      "event=live_server.browser_helper_order actor=browser operation=load_live_js risk=session_helper_missing_before_browser_init expected=session helper before live init actual=" +
        sessionHelperIndex +
        ":" +
        browserInitIndex,
    );
    assert.ok(
      ...
      "event=live_server.browser_helper_order actor=browser operation=load_live_js risk=dom_helper_missing_before_browser_init expected=dom helper before live init actual=" +
        domHelperIndex +
        ":" +
        browserInitIndex,
    );
  ...
  it("/design-system.json reads DESIGN.md plus .zarishnoksha/design.json", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-design-system-"));
    let designServer;
    try {
      writeFileSync(
        join(tmp, "DESIGN.md"),
...
`,
  ...
  it("/design-system.json falls back to legacy root DESIGN.json", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-design-system-legacy-"));
    let designServer;
    try {
      writeFileSync(
        join(tmp, "DESIGN.md"),
...
`,
  ...
  it("/manual-edit-commit runs the batched AI apply path and clears successful entries", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-commit-server-"));
    let commitServer;
    try {
      ...
      commitServer = await startServer(8522, {
        cwd: tmp,
        env: {
          zarishnoksha_LIVE_COPY_AGENT: "mock",
          zarishnoksha_LIVE_COPY_AGENT_MOCK_DELAY_MS: "400",
          zarishnoksha_LIVE_COPY_AGENT_MOCK_RESULT: JSON.stringify({
            status: "done",
      ...
      const commitPromise = fetch(
        `http://localhost:${commitServer.port}/manual-edit-commit?token=${commitServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      );
  ...
  it("/manual-edit-commit async mode returns immediately and reports completion through status/SSE activity", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-commit-server-async-"));
    let asyncServer;
    try {
      mkdirSync(join(tmp, "src"), { recursive: true });
      const sourcePath = join(tmp, "src", "page.html");
      ...
      asyncServer = await startServer(8546, {
        ...
        env: {
          zarishnoksha_LIVE_COPY_AGENT: "mock",
          zarishnoksha_LIVE_COPY_AGENT_MOCK_DELAY_MS: "300",
          zarishnoksha_LIVE_COPY_AGENT_MOCK_RESULT: JSON.stringify({
            status: "done",
      ...
      const commit = await fetch(
        `http://localhost:${asyncServer.port}/manual-edit-commit?token=${asyncServer.token}&pageUrl=%2F&async=1`,
        {
          method: "POST",
        },
      );
      ...
      const stashAfter = await fetch(
        `http://localhost:${asyncServer.port}/manual-edit-stash?token=${asyncServer.token}&pageUrl=%2F`,
      );
  ...
  it("/manual-edit-commit routes through the chat agent poll loop when configured", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-commit-chat-"));
    let chatServer;
    try {
      mkdirSync(join(tmp, "src"), { recursive: true });
      const sourcePath = join(tmp, "src", "page.html");
      ...
      chatServer = await startServer(8524, {
        cwd: tmp,
        env: { zarishnoksha_LIVE_COPY_AGENT: "chat" },
      });
      ...
      const agentLoop = (async () => {
        ...
        const pollRes = await fetch(
          `http://localhost:${chatServer.port}/poll?token=${chatServer.token}&timeout=10000&leaseMs=30000`,
        );
      ...
      // Trigger Apply.
      const commitPromise = fetch(
        `http://localhost:${chatServer.port}/manual-edit-commit?token=${chatServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      );
  ...
  it("/manual-edit-commit includes compact source candidates in chat Apply events", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-commit-chat-candidates-"));
    let candidateServer;
    try {
      mkdirSync(join(tmp, "site/scripts/components"), { recursive: true });
      writeFileSync(
        join(tmp, "site/scripts/data.js"),
        [
          "export const skillFocusAreas = { zarishnoksha: [",
      ...
      writeFileSync(
        join(tmp, "site/scripts/components/foundation-animations.js"),
        "export const foundationAnimations = { 'Typography': '<svg>type</svg>' };\n",
      );
      writeFileSync(
        join(tmp, "site/scripts/components/foundation-grid.js"),
        [
      ...
      await stashManualEdit(candidateServer, {
        ...
        element: {
          ...
          textContent: "Typography 33 Scale, rhythm, hierarchy, expression",
        },
        ops: [
          {
      ...
      const commitPromise = fetch(
        `http://localhost:${candidateServer.port}/manual-edit-commit?token=${candidateServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      );
      ...
      const event = await fetch(
        `http://localhost:${candidateServer.port}/poll?token=${candidateServer.token}&timeout=10000&leaseMs=30000`,
      ).then((res) => res.json());
      assert.equal(event.type, "manual_edit_apply");
      ...
      assert.ok(
        ...
        "chat Apply poll payload should stay compact with filtered candidates",
      );
  ...
  it("/manual-edit-commit rejects malformed chat Apply results without rolling back before retry", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-commit-chat-invalid-result-"));
    let chatServer;
    try {
      mkdirSync(join(tmp, "src"), { recursive: true });
      const sourcePath = join(tmp, "src", "page.html");
      ...
      const agentLoop = (async () => {
        ...
        await rejectReply(
          { status: "done", appliedEntryIds: [], failed: [], files: [], notes: [] },
          "done_result_missing_applied_entry_ids",
        );
        await rejectReply(
          { status: "done", appliedEntryIds: ["not-this-event"], failed: [], files: [], notes: [] },
          "applied_entry_id_not_in_event",
        );
        await rejectReply(
          { status: "partial", appliedEntryIds: ["badc0de1"], failed: "nope", files: [], notes: [] },
          "failed_must_be_array",
        );
  ...
  it("/manual-edit-commit chunks chat Apply events by op count and aggregates replies", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-commit-chat-chunks-"));
    let chunkServer;
    try {
      mkdirSync(join(tmp, "src"), { recursive: true });
      const sourcePath = join(tmp, "src", "page.html");
      writeFileSync(
        sourcePath,
        Array.from({ length: 7 }, (_, index) => `<p>Item ${String(index + 1).padStart(2, "0")}</p>`).join("\n") + "\n",
      );

      chunkServer = await startServer(8528, {
        cwd: tmp,
        env: {
          zarishnoksha_LIVE_COPY_AGENT: "chat",
          zarishnoksha_LIVE_MANUAL_EDIT_CHUNK_SIZE: "3",
        },
      });
      ...
      const agentLoop = (async () => {
        ...
        for (const [index, expectedSize] of expectedChunkSizes.entries()) {
          const pollRes = await fetch(
            `http://localhost:${chunkServer.port}/poll?token=${chunkServer.token}&timeout=10000&leaseMs=30000`,
          );
          const event = await pollRes.json();
          assert.equal(event.type, "manual_edit_apply");
          ...
          assert.equal(Array.isArray(event.batch.candidates), true);
          assert.ok(
            JSON.stringify(event).length < 9000,
            "chat Apply poll payload should stay compact; full evidence lives at evidencePath",
          );
          ...
          assert.equal(
            event.batch.entries.reduce((sum, entry) => sum + entry.ops.length, 0),
            expectedSize,
          );
      ...
      const commitPromise = fetch(
        `http://localhost:${chunkServer.port}/manual-edit-commit?token=${chunkServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      );
      ...
      assert.deepEqual(
        evidencePaths.map((file) => existsSync(file)),
        [false, false, false],
      );
    } finally {
      if (chunkServer) {
  ...
  it("/manual-edit-commit keeps fitting multi-op entries together across chat chunks", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-commit-chat-entry-chunks-"));
    let chunkServer;
    try {
      mkdirSync(join(tmp, "src"), { recursive: true });
      const sourcePath = join(tmp, "src", "page.html");
      writeFileSync(
        sourcePath,
        ["<h1>Alpha</h1>", "<p>Bravo</p>", "<h2>Charlie</h2>", "<p>Delta</p>", "<button>Echo</button>"].join("\n") +
          "\n",
      );
      ...
      await stashManualEdit(chunkServer, {
        ...
        ops: [
          {
            ref: "body>h2:nth-of-type(1)",
            tag: "h2",
            originalText: "Charlie",
            newText: "Charlie edited",
            sourceHint: { file: "src/page.html", line: 3 },
          },
          {
            ref: "body>p:nth-of-type(2)",
            tag: "p",
            originalText: "Delta",
            newText: "Delta edited",
            sourceHint: { file: "src/page.html", line: 4 },
          },
        ],
      });
      await stashManualEdit(chunkServer, {
        ...
        ops: [
          {
            ref: "body>button:nth-of-type(1)",
            tag: "button",
            originalText: "Echo",
            newText: "Echo edited",
            sourceHint: { file: "src/page.html", line: 5 },
          },
        ],
      });
      ...
      const agentLoop = (async () => {
        ...
        for (const [index, expectation] of expected.entries()) {
          ...
          assert.deepEqual(
            event.batch.entries.map((entry) => entry.id),
            expectation.ids,
          );
          assert.deepEqual(
            event.batch.entries.map((entry) => entry.ops.length),
            expectation.ids.map((id) => (id === "cc333333" ? 1 : 2)),
          );
          assert.deepEqual(event.chunk, {
            index: index + 1,
            total: 2,
  ...
  it("/manual-edit-commit splits one multi-op entry and clears it only after every chunk applies", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-commit-chat-split-entry-"));
    let splitServer;
    try {
      mkdirSync(join(tmp, "src"), { recursive: true });
      const sourcePath = join(tmp, "src", "page.html");
      writeFileSync(
        sourcePath,
        ["Alpha", "Bravo", "Charlie", "Delta", "Echo"].map((text) => `<span>${text}</span>`).join("\n") + "\n",
      );
      ...
      await stashManualEdit(splitServer, {
        ...
        element: {
          tagName: "section",
          outerHTML: "<section>Alpha Bravo Charlie Delta Echo</section>",
          textContent: "Alpha Bravo Charlie Delta Echo",
        },
      ...
      const agentLoop = (async () => {
        for (const [index, expectedSize] of [3, 2].entries()) {
          const event = await fetch(
            `http://localhost:${splitServer.port}/poll?token=${splitServer.token}&timeout=10000&leaseMs=30000`,
          ).then((res) => res.json());
          assert.equal(event.type, "manual_edit_apply");
          assert.equal(event.agentAction.required, "apply_source_edits_then_reply");
          assert.equal(event.agentAction.replyCommand, `live-poll.mjs --reply ${event.id} done --data '<json>'`);
      ...
      const commitPromise = fetch(
        `http://localhost:${splitServer.port}/manual-edit-commit?token=${splitServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      );
  ...
  it("/manual-edit-commit rolls back a split entry when a later chat chunk fails", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-commit-chat-chunk-fail-"));
    let failServer;
    try {
      mkdirSync(join(tmp, "src"), { recursive: true });
      const sourcePath = join(tmp, "src", "page.html");
      ...
      const agentLoop = (async () => {
        const firstEvent = await fetch(
          `http://localhost:${failServer.port}/poll?token=${failServer.token}&timeout=10000&leaseMs=30000`,
        ).then((res) => res.json());
      ...
      const commitPromise = fetch(
        `http://localhost:${failServer.port}/manual-edit-commit?token=${failServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      );
      ...
      const nextEvent = await fetch(
        `http://localhost:${failServer.port}/poll?token=${failServer.token}&timeout=100&leaseMs=1`,
      ).then((res) => res.json());
  ...
  it("/manual-edit-commit keeps entries staged when the chat agent does not ack", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-commit-timeout-"));
    let timeoutServer;
    try {
      mkdirSync(join(tmp, "src"), { recursive: true });
      const sourcePath = join(tmp, "src", "page.html");
      ...
      timeoutServer = await startServer(8525, {
        ...
        env: {
          zarishnoksha_LIVE_COPY_AGENT: "chat",
          zarishnoksha_LIVE_APPLY_EVENT_HARD_TIMEOUT_MS: "300",
          zarishnoksha_LIVE_APPLY_EVENT_SOFT_DEADLINE_MS: "250",
        },
      });
      ...
      const pollPromise = fetch(
        `http://localhost:${timeoutServer.port}/poll?token=${timeoutServer.token}&timeout=10000&leaseMs=30000`,
      ).then((res) => res.json());
      const commitPromise = fetch(
        `http://localhost:${timeoutServer.port}/manual-edit-commit?token=${timeoutServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      );
  ...
  it("/manual-edit-commit repairs post-apply validation failures instead of rolling back", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-repair-success-"));
    let repairServer;
    try {
      mkdirSync(join(tmp, "src"), { recursive: true });
      ...
      const agentLoop = (async () => {
        const event = await fetch(
          `http://localhost:${repairServer.port}/poll?token=${repairServer.token}&timeout=10000&leaseMs=30000`,
        ).then((res) => res.json());
        assert.equal(event.type, "manual_edit_apply");
      ...
      const commit = await fetch(
        `http://localhost:${repairServer.port}/manual-edit-commit?token=${repairServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      );
  ...
  it("/manual-edit-commit asks for a decision after repeated repair failures", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-repair-decision-"));
    let decisionServer;
    try {
      mkdirSync(join(tmp, "src"), { recursive: true });
      writeFileSync(join(tmp, "package.json"), '{"type":"module"}\n');
      ...
      const agentLoop = (async () => {
        for (let index = 0; index < 4; index += 1) {
          const event = await fetch(
            `http://localhost:${decisionServer.port}/poll?token=${decisionServer.token}&timeout=10000&leaseMs=30000`,
          ).then((res) => res.json());
          assert.equal(event.type, "manual_edit_apply");
          if (index === 0) {
            assert.equal(event.repair, undefined);
      ...
      const commit = await fetch(
        `http://localhost:${decisionServer.port}/manual-edit-commit?token=${decisionServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      );
      ...
      const decisionStatus = await fetch(
        `http://localhost:${decisionServer.port}/status?token=${decisionServer.token}`,
      );
      ...
      const bufferBeforeRollback = JSON.parse(
        readFileSync(join(getLiveDir(tmp), "pending-manual-edits.json"), "utf-8"),
      );
      assert.equal(bufferBeforeRollback.entries.length, 1);

      const rollback = await fetch(
        `http://localhost:${decisionServer.port}/manual-edit-repair-decision?token=${decisionServer.token}&pageUrl=%2F`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      const repairWithoutTransaction = await fetch(
        `http://localhost:${decisionServer.port}/manual-edit-commit?token=${decisionServer.token}&pageUrl=%2F&repair=1`,
        {
          method: "POST",
        },
      );
  ...
  it("/manual-edit-discard cancels leased chat Apply events instead of redelivering them", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-discard-apply-"));
    ...
    try {
      ...
      const pollPromise = fetch(
        `http://localhost:${discardApplyServer.port}/poll?token=${discardApplyServer.token}&timeout=10000&leaseMs=30000`,
      ).then((res) => res.json());
      const commitPromise = fetch(
        `http://localhost:${discardApplyServer.port}/manual-edit-commit?token=${discardApplyServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      );
      ...
      const discard = await fetch(
        `http://localhost:${discardApplyServer.port}/manual-edit-discard?token=${discardApplyServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      );
      ...
      assert.deepEqual(
        discardBody.canceledApplyEvents.map((item) => item.id),
        [event.id],
      );
      ...
      const nextPoll = await fetch(
        `http://localhost:${discardApplyServer.port}/poll?token=${discardApplyServer.token}&timeout=100&leaseMs=1`,
      );
  ...
  it("/manual-edit-commit rolls back abandoned chunk transactions after server restart", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-abandoned-transaction-"));
    let abandonedServer;
    let restarted;
    try {
      mkdirSync(join(tmp, "src"), { recursive: true });
      ...
      const commitPromise = fetch(
        `http://localhost:${abandonedServer.port}/manual-edit-commit?token=${abandonedServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      ).catch((err) => err);
      ...
      const firstEvent = await fetch(
        `http://localhost:${abandonedServer.port}/poll?token=${abandonedServer.token}&timeout=10000&leaseMs=30000`,
      ).then((res) => res.json());
      assert.equal(firstEvent.type, "manual_edit_apply");
      assert.equal(firstEvent.chunk.index, 1);
      ...
      assert.equal(
        statusBody.manualEdits.lastActivity.reason,
        "manual_edit_server_start_recovered_abandoned_transaction",
      );
    } finally {
      if (abandonedServer) {
        try {
          await stopServer(abandonedServer.port, abandonedServer.token);
        } catch {}
  ...
  it("/manual-edit-discard only cancels in-flight Apply events for the discarded page", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-discard-page-scope-"));
    let pageScopeServer;
    try {
      mkdirSync(join(tmp, "src"), { recursive: true });
      ...
      const homePollPromise = fetch(
        `http://localhost:${pageScopeServer.port}/poll?token=${pageScopeServer.token}&timeout=10000&leaseMs=30000`,
      ).then((res) => res.json());
      const homeCommitPromise = fetch(
        `http://localhost:${pageScopeServer.port}/manual-edit-commit?token=${pageScopeServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      );
      ...
      const docsPollPromise = fetch(
        `http://localhost:${pageScopeServer.port}/poll?token=${pageScopeServer.token}&timeout=10000&leaseMs=30000`,
      ).then((res) => res.json());
      const docsCommitPromise = fetch(
        `http://localhost:${pageScopeServer.port}/manual-edit-commit?token=${pageScopeServer.token}&pageUrl=%2Fdocs`,
        {
          method: "POST",
        },
      );
      ...
      const discardHome = await fetch(
        `http://localhost:${pageScopeServer.port}/manual-edit-discard?token=${pageScopeServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      );
      ...
      assert.deepEqual(
        discardHomeBody.canceledApplyEvents.map((item) => item.id),
        [homeEvent.id],
      );
      ...
      assert.equal(
        existsSync(docsEvent.evidencePath),
        false,
        "successful unrelated Apply should then clean its evidence",
      );
    } finally {
      if (pageScopeServer) {
  ...
  it("/manual-edit-discard returns discarded entries so the browser can restore visible text", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-discard-server-"));
    let discardServer;
    try {
      ...
      const discard = await fetch(
        `http://localhost:${discardServer.port}/manual-edit-discard?token=${discardServer.token}&pageUrl=%2F`,
        {
          method: "POST",
        },
      );
  ...
  it("/manual-edit-stash rejects a corrupt pending buffer instead of overwriting it", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-manual-stash-corrupt-"));
    let stashServer;
    try {
  ...
  it("events flow from browser POST to agent poll", async () => {
    ...
    const pollPromise = fetch(`http://localhost:${server.port}/poll?token=${server.token}&timeout=5000`).then((r) =>
      r.json(),
    );
  ...
  it("accepts checkpoint events without exposing them as agent poll work", async () => {
    ...
    const polled = await fetch(`http://localhost:${server.port}/poll?token=${server.token}&timeout=50`).then((r) =>
      r.json(),
    );
    assert.equal(
      polled.type,
      "timeout",
      "event=live_server.checkpoint_not_polled actor=browser operation=checkpoint risk=checkpoint_starves_agent_queue expected=timeout actual=" +
        polled.type +
        " suggestion=journal checkpoint without enqueueing agent work",
    );
  ...
  it("redelivers an unacknowledged browser event after helper server restart", async () => {
    const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-server-restart-"));
    let firstServer;
    let restarted;
    try {
      ...
      const replayed = await fetch(
        `http://localhost:${restarted.port}/poll?token=${restarted.token}&timeout=250&leaseMs=50`,
      ).then((r) => r.json());

      assert.equal(
        replayed.id,
        "a1b2c3d8",
        "event=live_server.restart_replay actor=agent operation=poll_after_helper_restart risk=server_restart_loses_unpolled_event expected=a1b2c3d8 actual=" +
          replayed.id +
          " suggestion=rebuild pending poll queue from live-session-store active snapshots on startup",
      );
  ...
  it("manual live-complete acknowledges the running helper queue before writing fallback journal state", async () => {
    ...
    const polled = await fetch(`http://localhost:${server.port}/poll?token=${server.token}&timeout=50&leaseMs=50`).then(
      (r) => r.json(),
    );
    assert.equal(polled.id, "a1b2c3dc");

    const completed = JSON.parse(
      execFileSync(process.execPath, [COMPLETE_SCRIPT, "--id", "a1b2c3dc"], { cwd: server.cwd, encoding: "utf-8" }),
    );
    assert.equal(completed.phase, "completed");

    await new Promise((r) => setTimeout(r, 75));
    const stale = await fetch(`http://localhost:${server.port}/poll?token=${server.token}&timeout=50&leaseMs=50`).then(
      (r) => r.json(),
    );
    assert.equal(
      stale.type,
      "timeout",
      "event=live_complete.running_server_ack actor=agent operation=manual_complete risk=completed_session_redelivered_from_memory expected=timeout actual=" +
        stale.id,
    );
  });
  ...
  it("does not drop polled events until the agent acknowledges them", async () => {
    ...
    const first = await fetch(`http://localhost:${server.port}/poll?token=${server.token}&timeout=100&leaseMs=50`).then(
      (r) => r.json(),
    );
    assert.equal(first.id, "a1b2c3da");

    const leased = await fetch(`http://localhost:${server.port}/poll?token=${server.token}&timeout=25&leaseMs=50`).then(
      (r) => r.json(),
    );
    assert.equal(leased.type, "timeout", "leased event should not be redelivered before lease expiry");

    await new Promise((r) => setTimeout(r, 75));
    const redelivered = await fetch(
      `http://localhost:${server.port}/poll?token=${server.token}&timeout=100&leaseMs=50`,
    ).then((r) => r.json());
    assert.equal(
      redelivered.id,
      "a1b2c3da",
      "event=live_poll.lease_redelivery actor=agent operation=poll_after_missed_ack risk=agent_missed_event_loses_live_state expected=same event redelivered after lease expiry actual=" +
        redelivered.id +
        " suggestion=inspect pending event lease bookkeeping",
    );
    ...
    const acked = await fetch(`http://localhost:${server.port}/poll?token=${server.token}&timeout=50&leaseMs=50`).then(
      (r) => r.json(),
    );
  ...
  it("wakes a parked poll as soon as a missed-ack lease expires", async () => {
    ...
    const first = await fetch(`http://localhost:${server.port}/poll?token=${server.token}&timeout=100&leaseMs=60`).then(
      (r) => r.json(),
    );
    ...
    const startedAt = Date.now();
    const redelivered = await fetch(
      `http://localhost:${server.port}/poll?token=${server.token}&timeout=500&leaseMs=60`,
    ).then((r) => r.json());
    ...
    assert.equal(
      redelivered.id,
      "a1b2c3db",
      "event=live_poll.lease_expiry_wakeup actor=agent operation=poll_before_lease_expiry risk=parked_poll_waits_full_timeout expected=a1b2c3db actual=" +
        redelivered.id,
    );
    assert.ok(
      elapsed < 250,
      "event=live_poll.lease_expiry_latency actor=agent operation=poll_before_lease_expiry risk=redelivery_waits_full_timeout expected=<250 actual=" +
        elapsed,
    );
  ...
  it("agent reply is forwarded via SSE to browser", async () => {
    ...
    const controller = new AbortController();
    const sseRes = await fetch(`http://localhost:${server.port}/events?token=${server.token}`, {
      signal: controller.signal,
    });
    ...
    const queueRes = await fetch(`http://localhost:${server.port}/events`, {
      ...
      body: JSON.stringify({
        ...
        element: {
          ...
          outerHTML: '<h1 class="hero-title">Hello</h1>',
          textContent: "Hello",
        },
      }),
    });
  ...
  it("/source reads project files with valid token", async () => {
    ...
    assert.equal(res.status, 200);
    const text = await res.text();
    assert.ok(text.includes('"zarishnoksha"'));
  });
  ...
  it("POST /events accepts insert-mode generate with prompt only", async () => {
    ...
    assert.equal(postRes.status, 200);
    const polled = await fetch(`http://localhost:${server.port}/poll?token=${server.token}&timeout=2000`).then((r) =>
      r.json(),
    );
