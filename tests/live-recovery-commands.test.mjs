...
function withTempProject(fn) {
  const cwd = mkdtempSync(join(tmpdir(), "zarishnoksha-live-recovery-"));
  try {
    return fn(cwd);
  } finally {
...
describe("live recovery CLI commands", () => {
  it("prints active durable session status without a running helper server", () =>
    withTempProject((cwd) => {
  ...
  it("resumes the pending event and reports the next safe agent action", () =>
    withTempProject((cwd) => {
      const store = createLiveSessionStore({ cwd });
      store.appendEvent({
        type: "generate",
        ...
        count: 2,
        pageUrl: "/",
        element: { outerHTML: "<section>Hero</section>" },
      });
      ...
      assert.match(
        resume.nextAction,
        /live-poll\.mjs/,
        "event=live_resume.next_action actor=agent operation=recover_session risk=agent_has_state_but_no_next_step expected=live-poll.mjs actual=" +
          resume.nextAction,
      );
    }));

  it("resumes manual Apply with the structured reply action, not a plain ack", () =>
    withTempProject((cwd) => {
      const store = createLiveSessionStore({ cwd });
      store.appendEvent({
  ...
  it("resumes carbonize-required sessions with a cleanup-specific next action", () =>
    withTempProject((cwd) => {
      const store = createLiveSessionStore({ cwd });
      ...
      assert.match(
        ...
        "event=live_resume.carbonize_next_action actor=agent operation=recover_carbonize risk=carbonize_cleanup_hidden_after_accept expected=cleanup-specific action actual=" +
          resume.nextAction,
      );
    }));

  it("marks a session completed through the canonical completion command", () =>
    withTempProject((cwd) => {
      const store = createLiveSessionStore({ cwd });
      store.appendEvent({
        type: "generate",
