/**
 * End-to-end tests for `zarishnoksha skills` subcommands.
 *
 ...
 * `bun run test:cli-remote-e2e` (zarishnoksha_CLI_REMOTE_E2E=1), skipping
 * gracefully when zarishnoksha.pages.dev is unreachable.
 */
...
function createFakeSkills(root, skills = ["audit", "polish", "zarishnoksha"], providers = [".claude"]) {
    ...
    for (const skill of skills) {
      ...
      writeFileSync(
        ...
        [
          ...
          "Use the zarishnoksha skill for setup.",
        ].join("\n"),
      );
    }
  }
...
function createFakeLinkSource(root, providers = [".claude"]) {
  for (const provider of providers) {
    writeSkill(join(root, ".zarishnoksha", "dist", "universal"), provider, "zarishnoksha");
  }
}
...
function createFakeUniversalBundle(root, providers = [".claude", ".agents", ".cursor"]) {
  ...
  for (const provider of providers) {
    const skillDir = join(bundleRoot, provider, "skills", "zarishnoksha");
      ...
      JSON.stringify(
        {
          ...
          hooks: {
            PostToolUse: [
              {
                matcher: "Edit",
                hooks: [{ type: "command", command: 'node ".claude/skills/zarishnoksha/scripts/hook.mjs"' }],
              },
            ],
          },
        },
  ...
  if (providers.includes(".cursor")) {
    ...
    writeFileSync(
      join(bundleRoot, ".cursor", "hooks.json"),
      JSON.stringify(
        {
          version: 1,
          hooks: { preToolUse: [{ command: 'node ".cursor/skills/zarishnoksha/scripts/hook-before-edit.mjs"' }] },
        },
        null,
        2,
      ),
  ...
  if (providers.includes(".agents")) {
    ...
    writeFileSync(
      join(bundleRoot, ".codex", "hooks.json"),
      JSON.stringify(
        {
            ...
            PostToolUse: [
              {
                ...
                hooks: [
                  {
                    type: "command",
                    command: 'node "$(git rev-parse --show-toplevel)/.agents/skills/zarishnoksha/scripts/hook.mjs"',
                  },
                ],
              },
            ],
 ...
 * skill lives at `<prefix>zarishnoksha`. Optionally drop in a third-party skill
...
function createPrefixedInstall(root, { prefix = "i-", providers = [".claude"], foreign = null } = {}) {
  for (const provider of providers) {
    writeSkill(root, provider, `${prefix}zarishnoksha`);
...
// Remote e2e blocks (real bundle downloads from zarishnoksha.pages.dev) run only
// under `bun run test:cli-remote-e2e` (zarishnoksha_CLI_REMOTE_E2E=1). The default
...
const WANT_CLI_REMOTE_E2E = process.env.zarishnoksha_CLI_REMOTE_E2E === "1";
let bundleReachable = false;
if (WANT_CLI_REMOTE_E2E) {
  try {
    execSync("curl -sfIL --max-time 10 https://zarishnoksha.pages.dev/api/download/bundle/universal -o /dev/null", {
      stdio: "pipe",
    });
...
describe("skills install: already-installed detection", () => {
  test("detects zarishnoksha sentinel and bails", () => {
    ...
    const output = run("skills install -y", {
      cwd: tmp,
      env: { ...process.env, zarishnoksha_BUNDLE_PATH: bundleRoot },
    });
  ...
  test("already-installed projects keep working when the update check is offline", () => {
    ...
    execSync("git init", { cwd: tmp });
    createFakeSkills(tmp, ["zarishnoksha"], [".claude"]);
    writeFileSync(
      join(tmp, ".claude", "settings.local.json"),
      JSON.stringify({
        hooks: {
    ...
    const output = run("skills install -y --providers=claude", {
      cwd: tmp,
      env: { ...process.env, zarishnoksha_BUNDLE_PATH: join(tmp, "missing-bundle") },
    });

    expect(output).toContain("already installed");
    expect(output).toContain("Could not check for skill updates");
    expect(output).toContain("Existing skills were left unchanged.");
    expect(existsSync(join(tmp, ".claude", "skills", "zarishnoksha", "SKILL.md"))).toBe(true);

    rmSync(tmp, { recursive: true, force: true });
  }, 15000);

  test("detects prefixed i-zarishnoksha", () => {
    const tmp = mkdtempSync(join(tmpdir(), "imp-test-"));
    execSync("git init", { cwd: tmp });

    const skillDir = join(tmp, ".cursor", "skills", "i-zarishnoksha");
    mkdirSync(skillDir, { recursive: true });
  ...
  test("repairs the hook when settings.local.json exists without the zarishnoksha marker", () => {
    ...
    expect(JSON.stringify(merged)).toContain("skills/zarishnoksha/scripts/hook.mjs");
  ...
  test("a permissions entry mentioning the hook path is not mistaken for an installed hook", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          permissions: { allow: ["Bash(node .claude/skills/zarishnoksha/scripts/hook.mjs:*)"] },
        },
        null,
        2,
      ),
    ...
    expect(merged.permissions.allow[0]).toContain("skills/zarishnoksha/scripts/hook.mjs");
...
describe("skills link: submodule installs", () => {
  test("creates relative skill symlinks from dist/universal", () => {
    ...
    const output = run("skills link --source=.zarishnoksha --providers=claude,cursor -y", { cwd: tmp });
    expect(output).toContain("Linked zarishnoksha into: .claude, .cursor");

    for (const provider of [".claude", ".cursor"]) {
      const dest = join(tmp, provider, "skills", "zarishnoksha");
      const src = join(tmp, ".zarishnoksha", "dist", "universal", provider, "skills", "zarishnoksha");
  ...
  test("is idempotent when links already point at the same source", () => {
    ...
    run("skills link --source=.zarishnoksha --providers=claude -y", { cwd: tmp });
    const before = readlinkSync(join(tmp, ".claude", "skills", "zarishnoksha"));
    const output = run("skills link --source=.zarishnoksha --providers=claude -y", { cwd: tmp });

    expect(output).toContain("already linked");
    expect(readlinkSync(join(tmp, ".claude", "skills", "zarishnoksha"))).toBe(before);
  ...
  test("does not overwrite an existing real skill unless forced", () => {
    ...
    execSync("git init", { cwd: tmp });
    createFakeLinkSource(tmp);
    writeSkill(tmp, ".claude", "zarishnoksha");

    expect(() => run("skills link --source=.zarishnoksha --providers=claude -y", { cwd: tmp })).toThrow();
    const dest = join(tmp, ".claude", "skills", "zarishnoksha");
    expect(lstatSync(dest).isSymbolicLink()).toBe(false);

    const output = run("skills link --source=.zarishnoksha --providers=claude -y --force", { cwd: tmp });
  ...
  test("maps codex and rovo-dev provider aliases to their install folders", () => {
    ...
    run("skills link --source=.zarishnoksha --providers=codex,rovo-dev -y", { cwd: tmp });

    expect(lstatSync(join(tmp, ".agents", "skills", "zarishnoksha")).isSymbolicLink()).toBe(true);
    expect(lstatSync(join(tmp, ".rovodev", "skills", "zarishnoksha")).isSymbolicLink()).toBe(true);
  ...
  test("plain install leaves linked installs on the submodule path", () => {
    ...
    const linkedDest = join(tmp, ".claude", "skills", "zarishnoksha");
    const before = readlinkSync(linkedDest);
    const copiedDest = join(tmp, ".cursor", "skills", "zarishnoksha");
    ...
    expect(readFileSync(join(copiedDest, "scripts", "context.mjs"), "utf8")).toBe(
      'console.log("local bundle context");\n',
    );
  ...
  test("deduplicates providers that share one skills directory", () => {
    ...
    run("skills link --source=.zarishnoksha --providers=claude,codex -y", { cwd: tmp });

    const dest = join(tmp, ".agents", "skills", "zarishnoksha");
    const src = join(tmp, ".zarishnoksha", "dist", "universal", ".claude", "skills", "zarishnoksha");
    expect(lstatSync(dest).isSymbolicLink()).toBe(true);
    expect(realpathSync(dest)).toBe(realpathSync(src));
...
// when each command was its own skill). migrateUnprefixzarishnoksha retires any
// old `<prefix>zarishnoksha` install back to the canonical `zarishnoksha`, so an
...
describe("skills: unprefix migration", () => {
  test("renames i-zarishnoksha back to zarishnoksha across every provider", () => {
    const tmp = mkdtempSync(join(tmpdir(), "imp-test-mig-"));
    createPrefixedInstall(tmp, { prefix: "i-", providers: [".claude", ".cursor"] });

    const migrated = migrateUnprefixzarishnoksha(tmp);
    expect(migrated).toBe(2); // one skill x two providers

    for (const provider of [".claude", ".cursor"]) {
      const skills = readdirSync(join(tmp, provider, "skills"));
      expect(skills).toContain("zarishnoksha");
      expect(skills).not.toContain("i-zarishnoksha");
    }

    rmSync(tmp, { recursive: true, force: true });
  });

  test("migrates a custom prefix too (x-zarishnoksha)", () => {
    const tmp = mkdtempSync(join(tmpdir(), "imp-test-mig-x-"));
    createPrefixedInstall(tmp, { prefix: "x-" });

    expect(migrateUnprefixzarishnoksha(tmp)).toBe(1);
    expect(readdirSync(join(tmp, ".claude", "skills"))).toContain("zarishnoksha");
  ...
  test("REGRESSION: never touches third-party skills, even ones starting with i-", () => {
    ...
    // A foreign skill that shares the i- prefix but is NOT zarishnoksha.
    createPrefixedInstall(tmp, { prefix: "i-", foreign: "i-cool-skill" });

    const migrated = migrateUnprefixzarishnoksha(tmp);
    expect(migrated).toBe(1); // only i-zarishnoksha
  ...
  test("leaves a clean zarishnoksha install alone (no-op)", () => {
    const tmp = mkdtempSync(join(tmpdir(), "imp-test-mig-clean-"));
    createFakeSkills(tmp, ["zarishnoksha"], [".claude"]);

    expect(migrateUnprefixzarishnoksha(tmp)).toBe(0);
    expect(readdirSync(join(tmp, ".claude", "skills"))).toContain("zarishnoksha");
  ...
  test("leaves unrelated legacy skill names alone", () => {
    ...
    createFakeSkills(tmp, ["teach-zarishnoksha"], [".claude"]);

    expect(migrateUnprefixzarishnoksha(tmp)).toBe(0);
    expect(readdirSync(join(tmp, ".claude", "skills"))).toContain("teach-zarishnoksha");
...
describe("skills install/update: local universal bundle e2e", () => {
  test("root help advertises top-level skills commands", () => {
    ...
    expect(output).toContain("install                          Install ZarishNoksha skills");
    expect(output).toContain("update                           Update skills to the latest version");
    expect(output).toContain("zarishnoksha skills <command>       Legacy namespace; still supported.");
    expect(output).not.toContain("skills install                   Install ZarishNoksha skills");
  });
  ...
  test("top-level install aliases the legacy skills install command", () => {
    ...
    const output = run("install -y --providers=claude --no-hooks", {
      cwd: tmp,
      env: { ...process.env, HOME: home, zarishnoksha_BUNDLE_PATH: bundleRoot },
    });

    expect(output).toContain("Installed zarishnoksha into: .claude (project)");
    expect(existsSync(join(tmp, ".claude", "skills", "zarishnoksha", "SKILL.md"))).toBe(true);
  ...
  test("formats detected harnesses as concise source-to-target rows", () => {
    ...
    expect(lines).toEqual(["Detected harnesses:", "  Claude Code  ~/.claude", "  Codex CLI    ~/.codex"]);
  ...
  test("installs provider-specific skills into a fresh project", () => {
    ...
    for (const provider of [".claude", ".agents", ".cursor"]) {
      const skillDir = join(tmp, provider, "skills", "zarishnoksha");
  ...
  test("interactive install explains home detections and can target the project root", () => {
    ...
    for (const provider of [".claude", ".agents", ".cursor", ".gemini"]) {
      expect(existsSync(join(tmp, provider, "skills", "zarishnoksha", "SKILL.md"))).toBe(true);
      expect(existsSync(join(home, provider, "skills", "zarishnoksha", "SKILL.md"))).toBe(false);
    }
  ...
  test("interactive install can add providers beyond detected harnesses", () => {
    ...
    expect(output).toContain("Installed zarishnoksha into: .claude, .agents (project)");
    expect(existsSync(join(tmp, ".claude", "skills", "zarishnoksha", "SKILL.md"))).toBe(true);
    expect(existsSync(join(tmp, ".agents", "skills", "zarishnoksha", "SKILL.md"))).toBe(true);
  ...
  test("interactive install defaults config-only home detections to project scope", () => {
    ...
    expect(output).toContain("Installed zarishnoksha into: .claude, .agents, .cursor, .gemini (project)");
    for (const provider of [".claude", ".agents", ".cursor", ".gemini"]) {
      expect(existsSync(join(tmp, provider, "skills", "zarishnoksha", "SKILL.md"))).toBe(true);
      expect(existsSync(join(home, provider, "skills", "zarishnoksha", "SKILL.md"))).toBe(false);
    }
  ...
  test("interactive install defaults home detections with real skills to user scope", () => {
    ...
    expect(output).toContain("Installed zarishnoksha into: .claude, .agents, .cursor, .gemini (global)");
    for (const provider of [".claude", ".agents", ".cursor", ".gemini"]) {
      expect(existsSync(join(home, provider, "skills", "zarishnoksha", "SKILL.md"))).toBe(true);
      expect(existsSync(join(tmp, provider, "skills", "zarishnoksha", "SKILL.md"))).toBe(false);
    }
  ...
  test("Codex system/runtime-only skills do not count as real user skills", () => {
    ...
    expect(output).toContain("Installed zarishnoksha into: .agents (project)");
    expect(existsSync(join(tmp, ".agents", "skills", "zarishnoksha", "SKILL.md"))).toBe(true);
    expect(existsSync(join(home, ".agents", "skills", "zarishnoksha", "SKILL.md"))).toBe(false);
  ...
  test("--scope=global installs skills globally and project hooks point there", () => {
    ...
    expect(output).toContain("Installed zarishnoksha into: .claude, .agents, .cursor (global)");
    for (const provider of [".claude", ".agents", ".cursor"]) {
      expect(existsSync(join(home, provider, "skills", "zarishnoksha", "SKILL.md"))).toBe(true);
      expect(existsSync(join(tmp, provider, "skills", "zarishnoksha", "SKILL.md"))).toBe(false);
    }
    expect(readFileSync(join(tmp, ".claude", "settings.local.json"), "utf8")).toContain(
      join(home, ".claude", "skills", "zarishnoksha", "scripts", "hook.mjs"),
    );
    expect(readFileSync(join(tmp, ".codex", "hooks.json"), "utf8")).toContain(
      join(home, ".agents", "skills", "zarishnoksha", "scripts", "hook.mjs"),
    );
    expect(readFileSync(join(tmp, ".cursor", "hooks.json"), "utf8")).toContain(
      join(home, ".cursor", "skills", "zarishnoksha", "scripts", "hook-before-edit.mjs"),
    );
  ...
  test('recorded consent "declined" skips the hook (no prompt, no --no-hooks needed)', () => {
    ...
    writeFileSync(join(tmp, ".zarishnoksha", "config.local.json"), JSON.stringify({ hook: { consent: "declined" } }));

    const output = run("skills install -y --providers=claude", {
      cwd: tmp,
      env: { ...process.env, zarishnoksha_BUNDLE_PATH: bundleRoot },
    });
    expect(output).toContain("Installed zarishnoksha into: .claude");
    expect(output).not.toContain("Installed hooks into");
    expect(existsSync(join(tmp, ".claude", "settings.local.json"))).toBe(false);
  ...
  test('recorded consent "accepted" installs the hook even non-interactively', () => {
    ...
    const bundleRoot = createFakeUniversalBundle(tmp, [".claude"]);
    mkdirSync(join(tmp, ".zarishnoksha"), { recursive: true });
    writeFileSync(join(tmp, ".zarishnoksha", "config.local.json"), JSON.stringify({ hook: { consent: "accepted" } }));
  ...
  test("--no-hooks records no consent decision (one-off skip)", () => {
    ...
    expect(existsSync(join(tmp, ".zarishnoksha", "config.local.json"))).toBe(false);
  ...
  test("updates stale copied skills from the local bundle", () => {
    ...
    const skillDir = join(tmp, ".claude", "skills", "zarishnoksha");
    mkdirSync(skillDir, { recursive: true });
    writeFileSync(join(skillDir, "SKILL.md"), "---\nname: zarishnoksha\nstale: true\n---\nOld content.\n");
  ...
  test("skills update refreshes script-only bundle changes when SKILL.md is unchanged", () => {
    ...
    const scriptPath = join(tmp, ".claude", "skills", "zarishnoksha", "scripts", "context.mjs");
  ...
  test("plain install refreshes an already-installed stale skill", () => {
    ...
    writeFileSync(
      join(skillDir, "SKILL.md"),
      readFileSync(join(bundleRoot, ".claude", "skills", "zarishnoksha", "SKILL.md"), "utf8"),
    );
    ...
    expect(output).toContain("already installed");
    expect(output).toContain("Updated");
    expect(readFileSync(join(skillDir, "scripts", "context.mjs"), "utf8")).toBe(
      'console.log("local bundle context");\n',
    );
  ...
  test("plain install only refreshes selected copied providers", () => {
    ...
    expect(output).toContain("already installed");
    expect(output).toContain("Updated");
    expect(readFileSync(join(tmp, ".claude", "skills", "zarishnoksha", "SKILL.md"), "utf8")).toContain(
      "version: 9.9.9-local",
    );
    expect(readFileSync(join(tmp, ".cursor", "skills", "zarishnoksha", "SKILL.md"), "utf8")).toContain(
      "stale: .cursor",
    );
    expect(readFileSync(join(tmp, ".cursor", "skills", "zarishnoksha", "scripts", "context.mjs"), "utf8")).toBe(
      'console.log("old .cursor script");\n',
    );
  ...
  test("--force reinstall over an old prefixed install lands on canonical zarishnoksha", () => {
    const tmp = mkdtempSync(join(tmpdir(), "imp-test-local-force-"));
    execSync("git init", { cwd: tmp });
    const bundleRoot = createFakeUniversalBundle(tmp, [".claude"]);
    const prefixed = join(tmp, ".claude", "skills", "i-zarishnoksha");
...
describe("hook manifest merge helpers", () => {
  test("mergeHookManifests refreshes fresh description and version while preserving third-party hooks", () => {
    const merged = mergeHookManifests(
      {
        ...
        hooks: {
          preToolUse: [
            ...
            { command: "node .cursor/skills/zarishnoksha/scripts/hook-before-edit.mjs" },
          ],
        },
      },
      {
        ...
        hooks: {
          preToolUse: [{ command: "node .cursor/skills/zarishnoksha/scripts/hook-before-edit.mjs" }],
        },
      },
    );
    ...
    expect(merged.hooks.preToolUse.map((entry) => entry.command)).toEqual([
      ...
      "node .cursor/skills/zarishnoksha/scripts/hook-before-edit.mjs",
    ]);
  });
});
...
describeRemote("skills update: refreshes from the production universal bundle", () => {
  ...
  beforeAll(() => {
    ...
    // Stale zarishnoksha skill that the update should overwrite with fresh,
    // compiled content. v3.0 ships a single `zarishnoksha` skill (with
  ...
  test("downloads the bundle and refreshes the zarishnoksha skill", () => {
    const output = run("skills update -y", { cwd: tmp });
    expect(output).toContain("Updated");

    // The skill now carries fresh, compiled content (no 'stale: true').
    const content = readFileSync(join(tmp, ".claude", "skills", "zarishnoksha", "SKILL.md"), "utf8");
    expect(content).not.toContain("stale: true");
  ...
  test("refreshed skill ships its compiled scripts directory", () => {
    ...
    expect(existsSync(join(tmp, ".claude", "skills", "zarishnoksha", "scripts"))).toBe(true);
  });
});
...
describeRemote("skills install: production universal bundle download", () => {
  ...
  test("--force reinstall over an old prefixed install lands on canonical zarishnoksha", () => {
    ...
    // retire i-zarishnoksha so we are left with the canonical name only.
    const prefixed = join(tmp, ".claude", "skills", "i-zarishnoksha");
    mkdirSync(prefixed, { recursive: true });
    writeFileSync(join(prefixed, "SKILL.md"), "---\nname: i-zarishnoksha\n---\n");
