 ...
 *    `<style data-zarishnoksha-css="ID">` block with `@scope ([data-zarishnoksha-variant="N"])`
 *    rules, a `data-zarishnoksha-params` JSON manifest covering range + steps + toggle
...
export const STEER_MARKER_ATTR = "data-zarishnoksha-steer";
 ...
 * @property {string}        scopedCss   Contents of the <style data-zarishnoksha-css>
 ...
 *   - <style data-zarishnoksha-css="ID"> with @scope rules per variant
 *   - data-zarishnoksha-params manifest with range + steps + toggle kinds
...
export function createFakeAgent() {
  return {
    ...
    async generateVariants(event, context = {}) {
      ...
      const cls =
        (event.element?.classes || ["hero-title"]).filter((name) => !/^svelte-[\w-]+$/.test(name)).join(" ") ||
        "hero-title";
      ...
      const scopedCss = useAstroGlobalCss
        ? [
            `[data-zarishnoksha-variant="1"] > ${tag} {`,
            "  color: oklch(var(--p-lightness, 0.5) 0.25 25);",
            "}",
            `[data-zarishnoksha-variant="2"] > ${tag} { font-weight: 900; }`,
            `[data-zarishnoksha-variant="2"][data-p-face="serif"] > ${tag} { font-family: ui-serif, serif; }`,
            `[data-zarishnoksha-variant="2"][data-p-face="mono"]  > ${tag} { font-family: ui-monospace, monospace; }`,
            `[data-zarishnoksha-variant="3"] > ${tag} { text-transform: uppercase; letter-spacing: 0.04em; }`,
            `[data-zarishnoksha-variant="3"][data-p-italic] > ${tag} { font-style: italic; }`,
          ].join("\n")
        : [
            ...
            "}",
            '@scope ([data-zarishnoksha-variant="3"]) {',
...
function generateInsertFakeVariants(context = {}) {
  ...
  const scopedCss = useAstroGlobalCss
    ? [
        '[data-zarishnoksha-variant="1"] .inserted-copy {',
        "  color: oklch(var(--p-lightness, 0.5) 0.25 25);",
        "}",
        '[data-zarishnoksha-variant="2"] .inserted-copy { font-weight: 900; }',
        '[data-zarishnoksha-variant="2"][data-p-face="serif"] .inserted-copy { font-family: ui-serif, serif; }',
        '[data-zarishnoksha-variant="2"][data-p-face="mono"]  .inserted-copy { font-family: ui-monospace, monospace; }',
        '[data-zarishnoksha-variant="3"] .inserted-copy { text-transform: uppercase; letter-spacing: 0.04em; }',
        '[data-zarishnoksha-variant="3"][data-p-italic] .inserted-copy { font-style: italic; }',
      ].join("\n")
    : [
        '@scope ([data-zarishnoksha-variant="1"]) {',
        "  :scope .inserted-copy {",
...
export function insertTargetFromEvent(event) {
  ...
  const classes = Array.isArray(anchor.classes) ? anchor.classes.join(" ") : anchor.classes || "";
  const text = typeof anchor.textContent === "string" ? anchor.textContent.trim().slice(0, 80) : "";
  return {
...
function readAttrFromOuterHtml(outerHTML, attr) {
  if (!outerHTML) return null;
  const match = String(outerHTML).match(
    new RegExp("\\s" + attr.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "\\s*=\\s*([\"'])(.*?)\\1"),
  );
...
export function htmlToJsx(html) {
  return selfCloseHtmlVoidTagsForJsx(
    String(html)
      ...
      .replace(/\sstyle=(["'])([\s\S]*?)\1/g, (_match, _quote, value) => {
        ...
        return (
          " style={{ " +
          entries.map(({ prop, value }) => `${formatJsxStyleKey(prop)}: ${JSON.stringify(value)}`).join(", ") +
          " }}"
        );
      }),
  );
...
export const HOIST_ATTR = "data-zarishnoksha-hoist-id";
...
function normalizeVariantSelectorQuotes(css) {
  return String(css).replace(
    /\[data-zarishnoksha-variant=(['"])(\d+)\1\]/g,
    (_match, _quote, id) => `[data-zarishnoksha-variant="${id}"]`,
  );
}
...
export async function applyManualEditBatchToSource(batch, { tmp, sourceEdits, repair = null } = {}) {
  ...
  for (const entry of batch?.entries || []) {
    ...
    const entrySourceEdits = sourceEditQueue ? sourceEditQueue.filter((edit) => edit.entryId === entry.id) : null;
...
function candidateAttemptsForOp(batch, entry, op) {
  const attempts = [];
  const seen = new Set();
  const numericDisplayEdit =
    /^-?\d+(?:\.\d+)?$/.test(String(op?.originalText || "").trim()) &&
  ...
  const opCandidates = (batch?.candidates || []).filter(
    (candidate) => candidate.entryId === entry.id && (!candidate.ref || candidate.ref === op.ref),
  );
  const relatedSiblingRefs = relatedSiblingRefsForOp(entry, op);
  const siblingCandidates = (batch?.candidates || []).filter(
    (candidate) => candidate.entryId === entry.id && candidate.ref && relatedSiblingRefs.has(candidate.ref),
  );
  ...
  for (const candidate of opCandidates) {
    ...
    if (sourceHint?.status === "ok")
      add(sourceHint.relativeFile || sourceHint.file, sourceHint.line, "candidate_source_hint");
...
function replaceTextInSource(body, { originalText, newText, line, contextHints = [], keyRenames = [] }) {
  ...
  const matches = allIndexesOf(body, original).filter(
    (index) => !numericDisplayEdit || numericReplacementAllowedAt(body, index, original),
  );
...
function replaceNumericValueForContextKey(body, { originalText, newText, contextHints = [] }) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    lines[index] =
      lines[index].slice(0, match.index) +
...
function replaceTypedNumericDisplayExpression(body, { originalText, newText, line }) {
  ...
  for (const index of candidateIndexes) {
    ...
    if (stringCall) {
      ...
      lines[index] =
        lineText.slice(0, stringCall.index) + replacement + lineText.slice(stringCall.index + stringCall[0].length);
      return { ok: true, body: lines.join("\n") };
    }
    ...
    if (bareExpression) {
      const replacement = `{${JSON.stringify(displayTrimmed)}}`;
      lines[index] =
        lineText.slice(0, bareExpression.index) +
        replacement +
        lineText.slice(bareExpression.index + bareExpression[0].length);
      return { ok: true, body: lines.join("\n") };
    }
  }
...
function replaceNearLine(body, originalText, newText, line, keyRenames = []) {
  ...
  for (const i of indexes) {
    ...
    if (idx === -1) continue;
    if (isNumericDisplayEdit(originalText, newText) && !numericReplacementAllowedOnLine(lines[i], idx, originalText))
      continue;
...
function numericReplacementAllowedAt(body, index, originalText) {
  ...
  return numericReplacementAllowedOnLine(source.slice(lineStart, lineEnd), index - lineStart, originalText);
}
...
function candidatesForEntry(batch, entryId) {
  ...
  for (const candidate of batch?.candidates || []) {
    ...
    if (candidate.sourceHint?.relativeFile) {
      out.push({
        file: candidate.sourceHint.relativeFile,
        line: candidate.sourceHint.line,
        kind: "candidate_source_hint",
      });
    }
...
function hasBaseVariantRule(scopedCss, variantId, styleMode) {
  ...
  if (styleMode === "astro-global-prefixed") {
    return new RegExp(String.raw`\[data-zarishnoksha-variant=${q}\](?:\s|>|\.|#|\[${HOIST_ATTR}=)`).test(scopedCss);
  }
  return new RegExp(String.raw`@scope\s*\(\s*\[data-zarishnoksha-variant=${q}\]\s*\)`).test(scopedCss);
}

function renderBaseVariantRule(variantId, styleMode) {
  if (styleMode === "astro-global-prefixed") {
    return [`[data-zarishnoksha-variant="${variantId}"] > * {`, "  --zarishnoksha-variant-ready: 1;", "}"].join("\n");
  }
  return [
    `@scope ([data-zarishnoksha-variant="${variantId}"]) {`,
    "  :scope > * { --zarishnoksha-variant-ready: 1; }",
    "}",
  ].join("\n");
}
...
// `style="..."`, and tag the element with `data-zarishnoksha-hoist-id="N"`.
...
function renderHoistedInlineStyleRule({ variantId, hoistId, declarations, styleMode }) {
  ...
  if (styleMode === "astro-global-prefixed") {
    return [`[data-zarishnoksha-variant="${variantId}"] ${target} {`, ...lines.map((line) => line.slice(2)), "}"].join(
      "\n",
    );
  }
  return [`@scope ([data-zarishnoksha-variant="${variantId}"]) {`, `  :scope ${target} {`, ...lines, "  }", "}"].join(
    "\n",
  );
}
 ...
 *   - data-zarishnoksha-params stays a single-quoted JSON string (JSX-legal)
 */
function renderVariantsBlock({ sessionId, indent, output, commentSyntax, file, styleMode }) {
  ...
  const styleLines = isJsx
    ? [
        indent + '  <style data-zarishnoksha-css="' + sessionId + '">{`',
        ...output.scopedCss.split("\n").map((l) => indent + "    " + l),
        indent + "  `}</style>",
      ]
    : [
        indent + "  <style" + (isAstroGlobalCss ? " is:inline" : "") + ' data-zarishnoksha-css="' + sessionId + '">',
        ...output.scopedCss.split("\n").map((l) => indent + "    " + l),
        indent + "  </style>",
      ];
  ...
  const variantBlocks = output.variants.map((v, i) => {
    const idx = i + 1;
    const paramsAttr =
      v.params && v.params.length
        ? " data-zarishnoksha-params='" + attrEscape(JSON.stringify(v.params), { svelte: isSvelte }) + "'"
        : "";
    let styleAttr = "";
    ...
    return [
      ...
      indent + '  <div data-zarishnoksha-variant="' + idx + '"' + styleAttr + paramsAttr + ">",
      indent + "    " + inner,
      indent + "  </div>",
    ].join("\n");
  });
...
async function spliceVariantsIntoWrapper({ tmp, wrapInfo, sessionId, output }) {
  ...
  const markerIdx = lines.findIndex((l) => l.includes("Variants: insert below this line"));
  if (markerIdx === -1) {
  ...
  const wrapperIndent = indent.replace(/ {2}$/, "");
  ...
  const next = [...lines.slice(0, markerIdx + 1), block, ...lines.slice(markerIdx + 1)];
...
async function writeSvelteComponentVariants({ tmp, wrapInfo, event, output }) {
  ...
  for (let i = 0; i < output.variants.length; i++) {
    ...
    if (isInsert && /\bdata-zarishnoksha-[\w-]*\s*=/.test(markup)) {
      throw new Error(`Svelte insert variant ${variantId} contains preview-only data-zarishnoksha attributes`);
    }
...
function svelteCssForVariant(scopedCss, variantId, tag) {
  ...
  const rewritten = chunks
    .join("\n")
    .replace(new RegExp(String.raw`\\[data-zarishnoksha-variant=["']${variantId}["']\\]\\s*>\\s*`, "g"), "")
    .replace(new RegExp(String.raw`\\[data-zarishnoksha-variant=["']${variantId}["']\\][^{]*>\\s*`, "g"), "")
    .replace(/:scope(?:\[[^\]]+\])?\s*>\s*/g, "")
    .replace(/:scope(?:\[[^\]]+\])?/g, tag)
    .split("\n")
...
function extractVariantCssChunks(css, variantId) {
  ...
  for (const line of lines) {
    if (
      line.includes(`[data-zarishnoksha-variant="${variantId}"]`) ||
      line.includes(`[data-zarishnoksha-variant='${variantId}']`)
    ) {
      collecting = true;
      depth = 0;
 ...
 * @param {string} opts.scriptsDir Path to the zarishnoksha scripts dir.
...
export async function runAgentLoop({
  ...
  while (!signal.aborted) {
    ...
    if (event.type === "generate") {
      ...
      log(
        `generate id=${event.id} mode=${isInsert ? "insert" : "replace"}${isInsert ? "" : ` action=${event.action}`} count=${event.count}`,
      );
      try {
        let wrapInfo;
        if (isInsert) {
    ...
    if (event.type === "manual_edit_apply") {
      ...
      const opCount =
        (event.batch?.entries || []).reduce((sum, entry) => sum + (entry.ops?.length || 0), 0) || entryCount;
      ...
      try {
        ...
        if (failedCount > 0) {
          log(
            `Applied ${appliedCount}/${entryCount} edit(s); ${failedCount} stayed staged because ${result.failed?.[0]?.reason || "one or more entries failed"}.`,
          );
        } else if (event.chunk) {
          const finalChunk = event.chunk.index === event.chunk.total;
          log(
            `Applied ${appliedCount}/${entryCount} entry(s) for chunk ${event.chunk.index}/${event.chunk.total}; ${finalChunk ? "waiting for server verification." : "polling for the next Apply chunk."}`,
          );
        } else {
        ...
        const failedEntries = (event.batch?.entries || [])
          .map((entry) => ({
            entryId: entry.id,
...
export function buildSteerContext({ tmp, event, wrapTarget, sourceFile }) {
  ...
  const targetFileAbs = sourceFile ? path.join(tmp, sourceFile) : findSteerTargetFile(tmp, target);
  ...
  const tagLine = source
    .split("\n")
    .find((line) => new RegExp(`<${tag}\\b`, "i").test(line) && line.includes(classToken));
  return {
    tmp,
    target,
    targetFile,
...
export function addSteerMarkerToSource(body, target = { classes: "hero-title", tag: "h1" }) {
  ...
  const openTagRe = new RegExp(`(<${escapedTag}\\b(?=[^>]*\\b(?:className|class)\\s*=\\s*${classValue})[^>]*)(>)`, "i");
...
async function runInsert({ tmp, scriptsDir, id, count, position, classes, tag, elementId, text }) {
  const args = [path.join(scriptsDir, "live-insert.mjs"), "--id", id, "--count", String(count), "--position", position];
  if (elementId) args.push("--element-id", elementId);
  if (classes) args.push("--classes", classes);
  if (tag) args.push("--tag", tag);
  if (text) args.push("--text", text);
 ...
 *
 *   1. Locate the carbonize block (bracketed by `zarishnoksha-carbonize-start`
 *      and `zarishnoksha-carbonize-end`).
 ...
 *      `<div data-zarishnoksha-variant="N" style="display: contents"|...>` wrapper
...
async function runCarbonizeCleanup({ tmp, file, sessionId /* , variant */ }) {
  ...
  const startRe = new RegExp(
    "[ \\t]*(?:<!--|\\{/\\*)\\s*zarishnoksha-carbonize-start\\s+" + sessionId + "\\s*(?:-->|\\*/\\})\\n",
  );
  const endRe = new RegExp(
    "[ \\t]*(?:<!--|\\{/\\*)\\s*zarishnoksha-carbonize-end\\s+" + sessionId + "\\s*(?:-->|\\*/\\})\\n?",
  );
  ...
  // 2. Unwrap the temporary `<div data-zarishnoksha-variant="N" ...>` placed
  // around the accepted content. For JSX targets, live-accept also adds an
  // outer `<div data-zarishnoksha-carbonize>` so the carbonize block and accepted
  // node occupy one child slot; strip that shell after the accepted node is
  // clean.
  body = unwrapDivAttributeWrapper(body, "data-zarishnoksha-variant", { expandSingleLineContainer: true });
  body = unwrapDivAttributeWrapper(body, "data-zarishnoksha-carbonize");

  // 3. Strip any `data-zarishnoksha-hoist-id` attributes the normalize step
  ...
  body = body.replace(/\s+data-zarishnoksha-hoist-id="[^"]*"/g, "");
