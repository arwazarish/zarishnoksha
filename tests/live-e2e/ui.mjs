 ...
 *
 * Selector strategy: live-browser.js uses deterministic ids (`zarishnoksha-live-*`)
...
const BAR_ID = "#zarishnoksha-live-bar";
const GLOBAL_BAR_ID = "#zarishnoksha-live-global-bar";
const PICKER_ID = "#zarishnoksha-live-picker";
const EDIT_BADGE_ID = "#zarishnoksha-live-edit-badge";
const PENDING_DOCK_ID = "#zarishnoksha-live-pending-dock";
...
const STEER_CHAT_ID = "#zarishnoksha-live-page-chat";
const STEER_INPUT_ID = "#zarishnoksha-live-page-chat-input";
const PICK_TOGGLE = "#zarishnoksha-live-pick-toggle";
...
const INSERT_TOGGLE = "#zarishnoksha-live-insert-toggle";
const DETECT_TOGGLE = "#zarishnoksha-live-detect-toggle";
const DETECT_BADGE = "#zarishnoksha-live-detect-badge";
const DESIGN_TOGGLE = "#zarishnoksha-live-design-toggle";
const DESIGN_HOST = "#zarishnoksha-live-design-host";
const EXIT_BUTTON = "#zarishnoksha-live-exit";
const INSERT_INPUT_ID = "#zarishnoksha-live-insert-input";
const INSERT_CREATE_ID = "#zarishnoksha-live-insert-create";
const ANNOTATION_ID = "#zarishnoksha-live-annot";
const ANNOTATION_PINS_ID = "#zarishnoksha-live-annot-pins";
const ANNOTATION_CLEAR_ID = "#zarishnoksha-live-annot-clear";

/**
 * Wait for the live handshake to complete:
 *   - window.__zarishnoksha_LIVE_INIT__ set
 *   - global bar mounted
...
export async function waitForHandshake(page, { timeout = 20_000 } = {}) {
  await page.waitForFunction(() => window.__zarishnoksha_LIVE_INIT__ === true, { timeout });
  await installLiveQueryHelpers(page);
  await page.waitForFunction((sel) => Boolean(window.__zarishnokshaLiveQuery?.(sel)), GLOBAL_BAR_ID, { timeout });
...
export async function assertBottomBarIdle(page, { timeout = 5_000 } = {}) {
  ...
  await page.waitForFunction(
    ({ ids }) => ids.every((sel) => Boolean(window.__zarishnokshaLiveQuery(sel))),
    {
      ids: [
        GLOBAL_BAR_ID,
        PICK_TOGGLE,
        ...
        STEER_CHAT_ID,
        STEER_INPUT_ID,
        "#zarishnoksha-live-page-chat-voice",
        EXIT_BUTTON,
      ],
    },
    { timeout },
  ...
  const snapshot = await page.evaluate(
    ({ pickSel, insertSel, detectSel, designSel }) => {
      const q = window.__zarishnokshaLiveQuery;
      return {
...
export async function runLiveChromeBottomBarSmoke(
  page,
  { expectDetectMinCount = 1, designTitle = "", designRawText = "" } = {},
) {
...
function installLiveQueryHelpersInPage() {
  window.__zarishnokshaLiveQuery = (selector) => {
    const root = window.__zarishnoksha_LIVE_CHROME_CORE__?.root?.() || window.__zarishnoksha_LIVE_UI_ROOT__ || null;
    return root?.querySelector?.(selector) || document.querySelector(selector);
  };
  window.__zarishnokshaLiveQueryAll = (selector) => {
    const root = window.__zarishnoksha_LIVE_CHROME_CORE__?.root?.() || window.__zarishnoksha_LIVE_UI_ROOT__ || null;
...
export async function installLiveQueryHelpers(page, { timeout = 5_000 } = {}) {
  ...
  await withTimeout(page.evaluate(installLiveQueryHelpersInPage), timeout, "install live query helpers");
}
...
async function clickLiveControl(page, selector) {
  ...
  const clicked = await page.evaluate((sel) => {
    const el = window.__zarishnokshaLiveQuery(sel);
...
async function readControlActive(page, selector) {
  await installLiveQueryHelpers(page);
  return page.evaluate((sel) => window.__zarishnokshaLiveQuery(sel)?.dataset.active === "true", selector);
}
...
async function ensureLiveControlActive(page, selector, active) {
  ...
  await page.waitForFunction(
    ({ sel, expected }) => window.__zarishnokshaLiveQuery(sel)?.dataset.active === (expected ? "true" : "false"),
...
export async function runPickInsertToggleSmoke(page) {
  ...
  await page.waitForFunction(
    ({ pickSel, insertSel }) =>
      window.__zarishnokshaLiveQuery(pickSel)?.dataset.active === "true" &&
      window.__zarishnokshaLiveQuery(insertSel)?.dataset.active === "false",
  ...
  await page.waitForFunction(
    ({ pickSel, insertSel }) =>
      window.__zarishnokshaLiveQuery(pickSel)?.dataset.active === "false" &&
      window.__zarishnokshaLiveQuery(insertSel)?.dataset.active === "true",
    { pickSel: PICK_TOGGLE, insertSel: INSERT_TOGGLE },
    { timeout: 5_000 },
  );
...
export async function runDetectSmoke(page, { expectMinCount = 1 } = {}) {
  ...
  await page.waitForFunction(
    ({ badgeSel, expectMin }) => {
      const badge = window.__zarishnokshaLiveQuery(badgeSel);
      const count = parseInt(badge?.textContent || "0", 10);
      const overlays = document.querySelectorAll(".zarishnoksha-overlay").length;
  ...
  await page.waitForFunction(
    () =>
      [...document.querySelectorAll(".zarishnoksha-overlay")].every(
  ...
  await page.waitForFunction(
    ({ badgeSel }) => {
      const badge = window.__zarishnokshaLiveQuery(badgeSel);
      return (
        document.querySelectorAll(".zarishnoksha-overlay").length === 0 &&
...
export async function runDesignPanelSmoke(page, { title = "", rawText = "" } = {}) {
  ...
  await page.waitForFunction(
    ({ hostSel, titleText }) => {
      const host = window.__zarishnokshaLiveQuery(hostSel);
  ...
  await page.evaluate((hostSel) => {
    const root = window.__zarishnokshaLiveQuery(hostSel)?.shadowRoot;
    const raw = [...(root?.querySelectorAll(".tab") || [])].find((btn) => /Raw/i.test(btn.textContent || ""));
    raw?.click();
  }, DESIGN_HOST);
  await page.waitForFunction(
    ({ hostSel, expected }) => {
      const text = window.__zarishnokshaLiveQuery(hostSel)?.shadowRoot?.textContent || "";
  ...
  await page.waitForFunction(
    ({ hostSel, toggleSel }) => {
      const panel = window.__zarishnokshaLiveQuery(hostSel)?.shadowRoot?.querySelector(".panel");
      const toggle = window.__zarishnokshaLiveQuery(toggleSel);
...
export async function clickExitLiveMode(page) {
  ...
  await page.waitForFunction(
    ({ barSel }) => {
      const bar = window.__zarishnokshaLiveQuery?.(barSel);
      return window.__zarishnoksha_LIVE_INIT__ === false && (!bar || !bar.isConnected);
    },
...
export async function drawAnnotationPinAndStroke(page, { comment = "Make this area easier to scan" } = {}) {
  await installLiveQueryHelpers(page);
  ...
  await page.waitForFunction(
    (pinsSel) => Boolean(window.__zarishnokshaLiveQuery(pinsSel)?.querySelector("input")),
    ANNOTATION_PINS_ID,
    { timeout: 5_000 },
  );
  await page.evaluate(
    ({ pinsSel, value }) => {
      const input = window.__zarishnokshaLiveQuery(pinsSel)?.querySelector("input");
  ...
  await page.waitForFunction(
    ({ pinsSel, expected }) => {
      const pins = window.__zarishnokshaLiveQuery(pinsSel);
  ...
  await page.waitForFunction(
    ({ annotSel, clearSel }) => {
      const annot = window.__zarishnokshaLiveQuery(annotSel);
      const clear = window.__zarishnokshaLiveQuery(clearSel);
...
async function waitForAnnotationRect(page) {
  ...
  return page.evaluate((sel) => {
    const rect = window.__zarishnokshaLiveQuery(sel).getBoundingClientRect();
    return {
      left: rect.left,
      top: rect.top,
      width: rect.width,
...
export async function pickElement(page, selector, opts = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    const visible = await page.waitForSelector(BAR_ID, { state: "visible", timeout: 5_000 }).then(
      () => true,
      () => false,
    );
    if (visible) break;
...
async function hideAnnotationOverlay(page) {
  await page
    .evaluate(() => {
      const annot = window.__zarishnokshaLiveQuery("#zarishnoksha-live-annot");
...
async function ensurePickerActive(page) {
  ...
  const clicked = await page.evaluate((sel) => {
    const btn = window.__zarishnokshaLiveQuery(sel);
    if (!btn) return false;
    btn.click();
    return true;
  }, PICK_TOGGLE_ID);
  ...
  await page.waitForFunction((sel) => window.__zarishnokshaLiveQuery(sel)?.dataset.active === "true", PICK_TOGGLE_ID, {
    timeout: 5_000,
  });
}
...
async function resetPickMode(page) {
  ...
  await page
    ...
    .catch(() => {});
  await page
    .waitForFunction((sel) => window.__zarishnokshaLiveQuery(sel)?.dataset.active === "true", PICK_TOGGLE_ID, {
      timeout: 5_000,
    })
    .catch(() => {});
}
      ...
      .waitForFunction(
        ({ barSel, goLabel }) => {
          ...
          return ![...bar.querySelectorAll("button")].some(
            (button) => (button.getAttribute("aria-label") || "") === goLabel,
          );
        },
        { barSel: BAR_ID, goLabel: GO_BUTTON_ARIA_LABEL },
        { timeout: 3_000 },
    ...
    if (process.env.zarishnoksha_E2E_DEBUG) {
      const snapshot = await page
        .evaluate((barSel) => {
          ...
          const root =
            window.__zarishnoksha_LIVE_CHROME_CORE__?.root?.() || window.__zarishnoksha_LIVE_UI_ROOT__ || null;
          const bar = query(barSel);
          const toast = query("#zarishnoksha-live-toast");
          const wrapper = document.querySelector("[data-zarishnoksha-variants]");
          return {
            liveInit: window.__zarishnoksha_LIVE_INIT__,
            adapter: window.__zarishnoksha_LIVE_ADAPTER__,
            ...
            wrapper: wrapper
              ? {
                  ...
                  html: wrapper.outerHTML.slice(0, 600),
                }
              : null,
            ...
            scripts: document.querySelectorAll("script[data-zarishnoksha-live-script]").length,
...
function findAndClickBarButton({ barSel, textMatch }) {
  const bar = window.__zarishnokshaLiveQuery(barSel);
  if (!bar) return false;
  const btn = [...bar.querySelectorAll("button")].find((candidate) => {
  ...
  try {
    ...
    return await withTimeout(
      page.evaluate((barSel) => {
        const wrapper = window.__zarishnokshaLiveQuery("[data-zarishnoksha-variants]");
        if (wrapper) {
          const variants = [
            ...wrapper.querySelectorAll('[data-zarishnoksha-variant]:not([data-zarishnoksha-variant="original"])'),
          ];
          const visible = variants.find((variant) => variant.style.display !== "none");
          const idx = visible ? parseInt(visible.dataset.zarishnokshaVariant || "0", 10) : 0;
          if (idx > 0) return idx;
        }
        const bar = window.__zarishnokshaLiveQuery(barSel);
        if (!bar) return null;
...
export async function clickEditCopy(page) {
  ...
  await page.waitForFunction(
    () => window.__zarishnokshaLiveQuery('[data-zarishnoksha-editable="true"]')?.isContentEditable === true,
    { timeout: 5_000 },
  );
}
...
export async function clickSaveEdit(page) {
  ...
  await page.waitForFunction(() => !window.__zarishnokshaLiveQuery('[data-zarishnoksha-editable="true"]'), {
    timeout: 5_000,
  });
}

async function clickEditBadgeButton(page, label) {
  const proxyRect = await page
    .evaluate((text) => {
      const proxies = [...document.querySelectorAll('[data-zarishnoksha-edit-badge-proxy="true"]')];
...
export async function assertApplyDockVisible(page, expectedCount, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    ({ dockSel, expected }) => {
      const dock = window.__zarishnokshaLiveQuery(dockSel);
      if (!dock || dock.style.display === "none") return false;
      const pill = [...dock.querySelectorAll("button")].find((btn) => /Apply copy edit/.test(btn.textContent || ""));
...
export async function preparePageForBarInteraction(page) {
  await page.evaluate(() => {
    for (const el of window.__zarishnokshaLiveQueryAll("astro-dev-toolbar")) {
...
export async function waitForSteerInputFocused(page, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    (inputSel) => {
      const input = window.__zarishnokshaLiveQuery(inputSel);
      const active =
        window.__zarishnoksha_LIVE_CHROME_CORE__?.activeElementDeep?.() ||
...
export async function waitForSteerInputValue(page, value, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    ({ inputSel, value: expected }) => window.__zarishnokshaLiveQuery(inputSel)?.value === expected,
...
export async function waitForSteerLocked(page, { timeout = 5_000 } = {}) {
  await page.waitForFunction(
    (sel) => window.__zarishnokshaLiveQuery(sel)?.dataset.processing === "true",
    STEER_CHAT_ID,
    { timeout },
  );
}
...
export async function waitForSteerUnlocked(page, { timeout = 15_000 } = {}) {
  await page.waitForFunction(
    (sel) => {
      const chat = window.__zarishnokshaLiveQuery(sel);
      const input = window.__zarishnokshaLiveQuery("#zarishnoksha-live-page-chat-input");
...
async function ensureToggleActive(page, selector, shouldBeActive) {
  ...
  await page.locator(selector).click({ timeout: 5_000 });
  await page.waitForFunction(
    ({ sel, active }) => window.__zarishnokshaLiveQuery(sel)?.dataset.active === (active ? "true" : "false"),
 ...
 */
export async function runInsertFlow(
  page,
  { anchorSelector, position = "after", prompt = "Add a testimonial strip" } = {},
) {
  ...
  await page.waitForFunction(
    () => {
      const line = window.__zarishnokshaLiveQuery("#zarishnoksha-live-insert-line");
  ...
  await page.waitForFunction(
    ({ sel, value }) => window.__zarishnokshaLiveQuery(sel)?.value === value,
