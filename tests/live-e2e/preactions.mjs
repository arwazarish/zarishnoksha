...
export async function runPreActions(page, actions) {
  ...
  const wasActive = await page
    .evaluate((sel) => window.__zarishnokshaLiveQuery?.(sel)?.dataset.active === "true", PICK_TOGGLE)
    .catch(() => false);
  ...
  return page
    .evaluate(() => {
      ...
      return {
        ...
        hasShadowRoot: Boolean(document.getElementById("zarishnoksha-live-root")?.shadowRoot),
        ...
        wrapper: wrapper
          ? {
              ...
              html: wrapper.outerHTML.slice(0, 800),
            }
          : null,
