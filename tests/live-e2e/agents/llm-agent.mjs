 ...
 * zarishnoksha_E2E_LLM_PROVIDER=deepseek. Override the model via { model } when
 * constructing, or via zarishnoksha_E2E_LLM_MODEL at the call site.
 *
...
import { applyManualEditBatchToSource, applySteerEdits, loadManualEditEventBatch } from "../agent.mjs";
...
export const VARIANT_SYSTEM_INSTRUCTIONS = [
  "You are an automated subagent inside zarishnoksha's live-mode test harness.",
  ...
  '- Insert mode variants must contain visible inserted content. Do not return empty roots, placeholder-only roots, inline style= attributes, or test hooks like <div data-zarishnoksha-e2e-variant="1"></div>.',
  ...
  '- Replace mode: for non-bare elements where the existing children must stay in place, add a harmless root attribute such as data-zarishnoksha-e2e-variant="1" or another non-copy styling hook so the markup is materially changed without changing visible text.',
  ...
  '- Do NOT emit the wrapping <div data-zarishnoksha-variant="N">. The orchestrator wraps your content.',
  "- Do NOT emit the outer <style data-zarishnoksha-css> tag. Only its contents go in scopedCss.",
...
export const MANUAL_EDIT_SYSTEM_INSTRUCTIONS = [
  "<role>",
  "You are an automated source-edit planner for one zarishnoksha live manual_edit_apply batch or chunk.",
  "</role>",
  "",
  "<workflow>",
  ...
  "- Never copy live runtime scaffolding into sourceEdits: no contenteditable, data-zarishnoksha-* attributes, variant wrappers, live markers, <style>, <script>, comments, or generated browser attributes.",
  "</source_edit_rules>",
  "",
  "OUTPUT CONTRACT — return ONLY a JSON object with this exact shape. No prose, no code fences, no commentary:",
  "",
...
const STEER_SYSTEM_INSTRUCTIONS = [
  ...
  "- Never edit temporary preview or scratch paths such as node_modules/.zarishnoksha-live; Steer edits must land in the real app source file.",
 ...
 * @property {'anthropic' | 'deepseek'=} provider Override zarishnoksha_E2E_LLM_PROVIDER.
...
export function resolveLlmAgentConfig(opts = {}, env = process.env) {
  ...
  if (provider === "anthropic") {
    return {
      provider,
      model: opts.model || env.zarishnoksha_E2E_LLM_MODEL || DEFAULT_ANTHROPIC_MODEL,
  ...
  if (provider === "deepseek") {
    return {
      provider,
      model: opts.model || env.zarishnoksha_E2E_LLM_MODEL || DEFAULT_DEEPSEEK_MODEL,
  ...
  throw new Error(`Unsupported zarishnoksha_E2E_LLM_PROVIDER: ${provider}`);
}

function resolveProvider(opts, env) {
  const explicit = opts.provider || env.zarishnoksha_E2E_LLM_PROVIDER;
      ...
      for (let attempt = 0; attempt < MANUAL_EDIT_RESPONSE_MAX_ATTEMPTS; attempt += 1) {
        ...
        if (isInsert) {
          userMessage = [
            ...
            "Do not use data-zarishnoksha-* attributes or empty test-hook-only roots.",
            "Do not use inline style= attributes; put all visual rules in scopedCss.",
            "Return corrected JSON only.",
          ].join("\n");
        } else {
          const expectedText = normalizeVisibleText(elementVisibleText(event.element));
          userMessage = [
            baseUserMessage,
            "",
            "VALIDATION ERROR",
    ...
    async applyManualEdits(event, context = {}) {
      ...
      const buildBaseUserMessage = () =>
        [
          ...
          event.evidencePath
            ? `The poll event was compact; full source evidence was loaded from ${event.evidencePath}.`
            : "",
          "",
          "<manual_edit_event>",
          JSON.stringify(
      ...
      for (let attempt = 0; attempt < MANUAL_EDIT_RESPONSE_MAX_ATTEMPTS; attempt += 1) {
        ...
        try {
          ...
          if (attempt + 1 < MANUAL_EDIT_RESPONSE_MAX_ATTEMPTS) {
            ...
            userMessage = manualEditRetryMessage(
              baseUserMessage,
              splitValidationPredicates(err.message),
              previousRejectedResponse,
            );
            continue;
          }
          throw err;
        ...
        const coverageError =
          validateManualEditPlanningCoverage(parsed, batch) || validateManualEditCoverage(parsed, batch);
        if (coverageError) {
          if (attempt + 1 < MANUAL_EDIT_RESPONSE_MAX_ATTEMPTS) {
            ...
            userMessage = manualEditRetryMessage(
              baseUserMessage,
              splitValidationPredicates(coverageError),
              previousRejectedResponse,
            );
            continue;
          }
        ...
        if (appliedEntryIds.length > 0) {
          ...
          if (applied.failed.length > 0) {
            ...
            if (attempt + 1 < MANUAL_EDIT_RESPONSE_MAX_ATTEMPTS && applied.appliedEntryIds.length === 0) {
              ...
              userMessage = manualEditRetryMessage(
                baseUserMessage,
                [`sourceEdits failed to apply: ${reason}`],
                previousRejectedResponse,
              );
              continue;
            }
...
export function parseVariantResponse(text) {
  ...
  if (typeof parsed.scopedCss !== "string") {
    throw new Error(
      `LLM agent: missing or non-string scopedCss in response. Parsed (first 500 chars):\n${previewParsed()}`,
    );
  }
  if (/<\/?style\b/i.test(parsed.scopedCss)) {
    throw new Error(
      `LLM agent: scopedCss must contain CSS rules only, not a <style> tag. Parsed (first 500 chars):\n${previewParsed()}`,
    );
  }
  if (parsed.scopedCss.includes("`")) {
    throw new Error(
      `LLM agent: scopedCss must not contain backticks because JSX targets wrap it in a template literal. Parsed (first 500 chars):\n${previewParsed()}`,
    );
  }
  if (parsed.scopedCss.includes("${")) {
    throw new Error(
      `LLM agent: scopedCss must not contain template interpolation because JSX targets wrap it in a template literal. Parsed (first 500 chars):\n${previewParsed()}`,
    );
  }
  ...
  for (const [i, v] of parsed.variants.entries()) {
    if (typeof v.innerHtml !== "string" || !v.innerHtml.trim()) {
      throw new Error(
        `LLM agent: variants[${i}].innerHtml missing or empty. Parsed (first 500 chars):\n${previewParsed()}`,
      );
    }
    const htmlError = validateVariantInnerHtml(v.innerHtml);
    if (htmlError) {
      throw new Error(
        `LLM agent: variants[${i}].innerHtml ${htmlError}. Parsed (first 500 chars):\n${previewParsed()}`,
      );
    }
    if (/<\/?style\b/i.test(v.innerHtml)) {
      throw new Error(
        `LLM agent: variants[${i}].innerHtml must not include a <style> tag; put preview CSS in scopedCss. Parsed (first 500 chars):\n${previewParsed()}`,
      );
    }
    if (v.params !== undefined && !Array.isArray(v.params)) {
      throw new Error(
        `LLM agent: variants[${i}].params must be an array if present. Parsed (first 500 chars):\n${previewParsed()}`,
      );
    }
  }
  return parsed;
...
function validateVariantInnerHtml(html) {
  ...
  if (/\bdata-zarishnoksha-variants?\s*=/.test(html)) return "must not include zarishnoksha wrapper attributes";
...
export function validateInsertVariantOutput(parsed, event = {}) {
  for (const [i, variant] of parsed.variants.entries()) {
    ...
    if (/\bdata-zarishnoksha-[\w-]*\s*=/.test(html)) {
      return `insert variant ${i} contains preview-only data-zarishnoksha attributes`;
    }
  ...
  for (const entry of batch?.entries || []) {
    ...
    for (const op of entry.ops || []) {
      ...
      if (matchingEdits.length === 0) {
        ...
        entryErrors.push(
          `manual edit entry ${entry.id} is marked applied but no sourceEdit newText contains staged copy ${JSON.stringify(op.newText)}`,
        );
        continue;
      }
...
function opHasResolvableCandidateEvidence(entry, op, batch) {
  const candidates = (batch?.candidates || []).filter(
    (candidate) => candidate.entryId === entry.id && (!candidate.ref || !op.ref || candidate.ref === op.ref),
  );
  ...
  return candidates.some((candidate) => {
    ...
    return (
      ...
      nearby.length > 0 &&
      Array.isArray(candidate.textMatches) &&
      candidate.textMatches.length > 0
    );
  });
}
...
function sourceEditTargetsObjectKeyMatch(edit, batch, entryId, ref, oldText) {
  ...
  return objectKeyMatchesForOp(batch, entryId, ref, oldText).some(
    (match) => match.file === editFile && match.line === editLine,
  );
}
...
function validateTypedDisplayEdit(entry, op, matchingEdits) {
  ...
  const modelDataEdit = matchingEdits.find(
    (edit) => isPlainNumericModelDataStringEdit(edit, original, next) && !sourceEditLooksLikeCoupledLookup(entry, edit),
  );
  if (modelDataEdit) {
  ...
  if (
    expressionEdits.some(
      (edit) => hasQuotedDisplayExpression(edit.newText, next) || isSafeStaticMarkupDisplayReplacement(edit, next),
    )
  )
    return null;
...
function validatePairedLookupCountEdit(entry, sourceEdits) {
  ...
  if (
    !isPlainIntegerText(nextCount) &&
    countEdits.length > 0 &&
    countEdits.every((edit) => !sourceEditLooksLikeCoupledLookup(entry, edit))
  )
    return null;
...
function validateCoupledSourceKeyEdit(entry, batch, sourceEdits) {
  for (const op of entry?.ops || []) {
    ...
    if (!oldText || !newText || oldText === newText || isIntegerLikeText(oldText) || isIntegerLikeText(newText))
      continue;
...
function sourceEditUpdatesObjectKey(edit, match, oldText, newText) {
  ...
  const writesKeyText =
    hasObjectKeyLiteral(replacement, newText) ||
...
export function parseManualEditResponse(text) {
  ...
  if (!["done", "partial", "error"].includes(parsed.status)) {
    throw new Error(
      `LLM agent: manual edit status must be done, partial, or error. Parsed (first 500 chars):\n${previewParsed()}`,
    );
  }
  for (const key of ["coverage", "appliedEntryIds", "failed", "files", "notes", "sourceEdits"]) {
    if (parsed[key] !== undefined && !Array.isArray(parsed[key])) {
      throw new Error(
        `LLM agent: manual edit ${key} must be an array if present. Parsed (first 500 chars):\n${previewParsed()}`,
      );
    }
  }
  ...
  const coverage = (parsed.coverage || []).map((item, i) => {
    if (!item || typeof item !== "object") {
      throw new Error(
        `LLM agent: manual edit coverage[${i}] must be an object. Parsed (first 500 chars):\n${previewParsed()}`,
      );
    }
    if (typeof item.entryId !== "string" || !item.entryId) {
      throw new Error(
        `LLM agent: manual edit coverage[${i}].entryId missing or empty. Parsed (first 500 chars):\n${previewParsed()}`,
      );
    }
    return {
      entryId: item.entryId,
      coveredOps: Array.isArray(item.coveredOps)
        ? item.coveredOps.map(assertStringValue(`coverage[${i}].coveredOps`, previewParsed))
        : [],
      sourceTargets: Array.isArray(item.sourceTargets)
        ? item.sourceTargets.map(assertStringValue(`coverage[${i}].sourceTargets`, previewParsed))
        : [],
      coupledKeyEdits: Array.isArray(item.coupledKeyEdits)
        ? item.coupledKeyEdits.map(assertStringValue(`coverage[${i}].coupledKeyEdits`, previewParsed))
        : [],
  ...
  const failed = (parsed.failed || []).map((item, i) => {
    if (!item || typeof item !== "object") {
      throw new Error(
        `LLM agent: manual edit failed[${i}] must be an object. Parsed (first 500 chars):\n${previewParsed()}`,
      );
    }
    if (typeof item.entryId !== "string" || !item.entryId) {
      throw new Error(
        `LLM agent: manual edit failed[${i}].entryId missing or empty. Parsed (first 500 chars):\n${previewParsed()}`,
      );
    }
    if (typeof item.reason !== "string" || !item.reason) {
      throw new Error(
        `LLM agent: manual edit failed[${i}].reason missing or empty. Parsed (first 500 chars):\n${previewParsed()}`,
      );
    }
    return {
      entryId: item.entryId,
  ...
  const sourceEdits = (parsed.sourceEdits || []).map((item, i) => {
    if (!item || typeof item !== "object") {
      throw new Error(
        `LLM agent: manual edit sourceEdits[${i}] must be an object. Parsed (first 500 chars):\n${previewParsed()}`,
      );
    }
    for (const key of ["entryId", "file", "originalText", "newText"]) {
      if (typeof item[key] !== "string" || !item[key]) {
        throw new Error(
          `LLM agent: manual edit sourceEdits[${i}].${key} missing or empty. Parsed (first 500 chars):\n${previewParsed()}`,
        );
      }
    }
    return {
    ...
    JSON.stringify(
      {
        ...
        requiredCorrection:
          "Return complete corrected JSON for the whole current batch. Preserve all prior valid corrections and satisfy every predicate at once.",
      },
      null,
      2,
    ),
...
function assertStringValue(key, previewParsed) {
  return (value, i) => {
    if (typeof value !== "string" || !value) {
      throw new Error(
        `LLM agent: manual edit ${key}[${i}] must be a non-empty string. Parsed (first 500 chars):\n${previewParsed()}`,
      );
    }
    return value;
  };
