...
export { FIXTURES_DIR, REPO_ROOT, SCRIPTS_DIR };
...
export function stageFixture(name, fixture) {
  ...
  const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-e2e-"));
...
export function runInstall(
  tmp,
  command,
  { timeoutMs = readTimeoutEnv("zarishnoksha_E2E_INSTALL_TIMEOUT_MS", 180_000) } = {},
) {
...
export function stopLiveServer(tmp) {
  try {
    execFileSync(process.execPath, [join(SCRIPTS_DIR, "live-server.mjs"), "stop", "--keep-inject"], {
      cwd: tmp,
      stdio: "ignore",
    });
  } catch {
...
export function startDevServer(tmp, runtime) {
  ...
  const ready = new Promise((resolve, reject) => {
    const readyTimeoutMs = readTimeoutEnv("zarishnoksha_E2E_DEV_READY_TIMEOUT_MS", runtime.readyTimeoutMs ?? 120_000);
    const timeout = setTimeout(() => {
      reject(new Error(`dev server ready timeout (${readyTimeoutMs}ms). Tail:\n${bufLog.join("")}`));
    }, readyTimeoutMs);
...
export async function bootFixtureSession({ name, fixture, browser, agent, wrapTarget, log = () => {} }) {
  ...
  const teardown = async () => {
    ...
    try {
      if (agentDone) await agentDone.catch(() => {});
    } catch {}
    try {
      if (dev?.child) await stopDevServer(dev.child);
    } catch {}
    try {
