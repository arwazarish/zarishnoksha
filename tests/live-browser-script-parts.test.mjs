...
describe("live browser script parts", () => {
  it("resolves the canonical browser script order", () => {
    ...
    assert.deepEqual(
      ...
      ["session-state", "dom-helpers", "browser-ui"],
    );
  ...
  it("reads each part with an injected reader", () => {
    ...
    assert.deepEqual(
      loaded.map((part) => part.source),
      ["source:live-browser-session.js", "source:live-browser-dom.js", "source:live-browser.js"],
    );
  });
  ...
  it("assembles prelude, session helper, and browser UI in order", () => {
    ...
    const tokenIndex = script.indexOf("window.__zarishnoksha_TOKEN__");
    const portIndex = script.indexOf("window.__zarishnoksha_PORT__");
    const vocabIndex = script.indexOf("window.__zarishnoksha_VOCAB__");
    ...
    assert.match(script, /zarishnoksha live script part: session-state \(live-browser-session\.js\)/);
    assert.match(script, /zarishnoksha live script part: dom-helpers \(live-browser-dom\.js\)/);
    assert.match(script, /zarishnoksha live script part: browser-ui \(live-browser\.js\)/);
  });
});
