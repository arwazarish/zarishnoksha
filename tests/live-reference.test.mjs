...
describe("live reference authoring contract", () => {
  it("keeps setup guidance focused on inferred target paths", () => {
    ...
    assert.match(
      skillSrc,
      /infer the concrete path and run `node \{\{scripts_path\}\}\/context\.mjs --target <path>` instead/,
    );
    assert.doesNotMatch(skillSrc, /Use this same scripts directory for all zarishnoksha helper commands/);
  ...
  it("keeps monorepo live guidance short and target-driven", () => {
    ...
    assert.match(
      liveMd,
      /infer the concrete path and run `node \{\{scripts_path\}\}\/live\.mjs --target <path>` instead/,
    );
  ...
  it("keeps the live prompt focused on the foreground poll loop", () => {
    const liveMd = readFileSync(join(ROOT, "skill/reference/live.md"), "utf-8");
    const manualAgentMd = readFileSync(join(ROOT, "skill/agents/zarishnoksha-manual-edit-applier.md"), "utf-8");
    ...
    assert.doesNotMatch(liveMd, /zarishnoksha_LIVE_COPY_AGENT|mock/);
    ...
    assert.match(liveMd, /delegate source edits to `zarishnoksha_manual_edit_applier`/);
    ...
    assert.match(manualAgentMd, /codex-name: zarishnoksha_manual_edit_applier/);
    ...
    assert.match(
      manualAgentMd,
      /Never leave source changes behind for entries that are failed, omitted, or absent from `appliedEntryIds`/,
    );
  ...
  it("passes cssAuthoring into the LLM E2E agent instead of hard-coding scoped CSS", () => {
    ...
    assert.doesNotMatch(
      llmAgent,
      /with @scope \(\[data-zarishnoksha-variant=/,
