...
describe("live-session-store", () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-session-store-"));
  });
  ...
  it("rebuilds an active snapshot from an append-only journal after process restart", () => {
    ...
    assert.equal(
      active.length,
      1,
      "event=live_session_store.active_restart actor=agent operation=list_active_sessions risk=server_restart_loses_live_state expected=one active session actual=" +
        active.length +
        " suggestion=inspect journal replay and completed phase filtering",
    );
  ...
  it("does not duplicate parse diagnostics when valid entries follow a corrupted journal line", () => {
    ...
    appendFileSync(
      join(dir, "corrupt-then-valid.jsonl"),
      JSON.stringify({
        seq: 1,
    ...
    assert.equal(
      ...
      1,
      "event=live_session_store.duplicate_parse_diagnostic actor=store operation=journal_replay risk=duplicate_status_noise expected=1 actual=" +
        parseDiagnostics.length,
    );
  });
  ...
  it("preserves zero-valued checkpoint revisions and empty explicit fields", () => {
    ...
    assert.equal(
      ...
      0,
      "event=live_session_store.zero_checkpoint_revision actor=browser operation=checkpoint_replay risk=zero_revision_dropped expected=0 actual=" +
        snapshot.checkpointRevision,
    );
  ...
  it("ignores stale checkpoints and keeps the newest browser state", () => {
    ...
    assert.equal(
      ...
      true,
      "event=live_session_store.stale_checkpoint actor=browser operation=checkpoint_replay risk=old_browser_state_overwrites_newer_choice expected=stale diagnostic actual=" +
  ...
  it("keeps carbonize-required accepted sessions active until explicit completion", () => {
    ...
    assert.equal(
      snapshot.phase,
      "carbonize_required",
      "event=live_session_store.carbonize_required actor=agent operation=accept_ack risk=carbonize_session_hidden_from_recovery expected=carbonize_required actual=" +
        snapshot.phase,
    );
    assert.equal(snapshot.sourceFile, "src/App.jsx");
    assert.equal(snapshot.pendingEvent, null);
    assert.equal(
      store.listActiveSessions().some((s) => s.id === "carbonize-session"),
      true,
    );
  ...
  it("clears pending events when an agent error is acknowledged", () => {
    ...
    assert.equal(
      ...
      null,
      "event=live_session_store.agent_error_ack actor=agent operation=restart_replay risk=acknowledged_error_event_redelivered expected=null actual=" +
        JSON.stringify(store.listActiveSessions()[0].pendingEvent),
    );
  });
  ...
  it("recovers legacy journals from .zarishnoksha-live/sessions", () => {
    const legacyDir = getLegacyLiveSessionsDir(tmp);
    mkdirSync(legacyDir, { recursive: true });
    appendFileSync(
      join(legacyDir, "legacy-session.jsonl"),
      JSON.stringify({
        seq: 1,
        id: "legacy-session",
        type: "generate",
    ...
    assert.equal(snapshot.expectedVariants, 2);
    assert.equal(
      store.listActiveSessions().some((s) => s.id === "legacy-session"),
      true,
    );
