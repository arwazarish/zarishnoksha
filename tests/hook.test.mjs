/**
 * Unit tests for the zarishnoksha design hook.
...
function mkTmp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "zarishnoksha-hook-"));
}
...
describe("readConfig()", () => {
  ...
  it("parses hook runtime and legacy hook detector filters", () => {
    fs.mkdirSync(path.join(cwd, ".zarishnoksha"), { recursive: true });
    fs.writeFileSync(
      getConfigPath(cwd),
      JSON.stringify({
        hook: {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        detector: {
          ignoreRules: ["side-tab"],
          ignoreFiles: ["src/legacy/**"],
          ignoreValues: [{ rule: "overused-font", value: "inter", reason: "team default" }],
        },
      }),
    );
    fs.writeFileSync(
...
describe("ensureHookGitExcludes()", () => {
  ...
  it("adds hook runtime files to local git info exclude, not tracked .gitignore", () => {
    ...
    assert.match(exclude, /\.zarishnoksha\/hook\.cache\.json/);
    assert.match(exclude, /\.zarishnoksha\/hook\.pending\.json/);
    assert.match(exclude, /\.zarishnoksha\/config\.local\.json/);

    const second = ensureHookGitExcludes(cwd);
    assert.equal(second.changed, false);
    const rewritten = fs.readFileSync(path.join(cwd, ".git", "info", "exclude"), "utf-8");
    assert.equal((rewritten.match(/zarishnoksha-hook-ignore-start/g) || []).length, 1);
  });
});
...
describe("filterFindings()", () => {
  it("drops by ignoreRules and ignores legacy minSeverity config", () => {
    ...
    assert.deepEqual(
      filtered.map((f) => f.antipattern),
      ["gradient-text", "overused-font"],
    );
  });
  ...
  it("does not treat source comments as hook suppression", () => {
    const content = [
      ...
      "<!-- zarishnoksha: ignore side-tab -->",
      '<div style="border-left: 4px solid #7c3aed; border-radius: 16px;">Card</div>',
    ].join("\n");
    const filtered = filterFindings([finding("overused-font", 2), finding("side-tab", 4)], content, ".html", {
      ignoreRules: [],
      minSeverity: "warning",
      limits: DEFAULT_CONFIG.limits,
    });
  ...
  it("drops only matching rule/value pairs from ignoreValues", () => {
    ...
    assert.deepEqual(
      filtered.map((f) => `${f.antipattern}:${f.line}`),
      ["overused-font:2", "bounce-easing:4", "side-tab:3"],
    );
  });
  ...
  it("scopes ignoreValues to file globs when files are provided", () => {
    const findings = [
      ...
      {
        ...finding("design-system-color", 2, { file: "/tmp/project/site/styles/feature.css" }),
        ignoreValue: "#8b5cf6",
      },
      {
        ...finding("design-system-color", 3, { file: "/tmp/project/site/styles/home-kinpaku.css" }),
        ignoreValue: "oklch(60% 0.25 350 / 0.22)",
      },
    ];
    const filtered = filterFindings(findings, "", ".css", {
      ignoreRules: [],
    ...
    assert.deepEqual(
      ...
      ["/tmp/project/site/styles/feature.css:2"],
    );
  });
  ...
  it("matches equivalent design-system color ignore values", () => {
    const findings = [
      {
        ...finding("design-system-color", 1, { file: "/tmp/project/site/styles/rgb.css" }),
        ignoreValue: "rgb(139, 92, 246)",
      },
      { ...finding("design-system-color", 2, { file: "/tmp/project/site/styles/hex.css" }), ignoreValue: "#8b5cf6" },
      {
        ...finding("design-system-color", 3, { file: "/tmp/project/site/styles/alpha.css" }),
        ignoreValue: "rgba(139, 92, 246, 0.5)",
      },
      { ...finding("design-system-color", 4, { file: "/tmp/project/site/styles/other.css" }), ignoreValue: "#8b5cf7" },
      {
        ...finding("design-system-radius", 5, { file: "/tmp/project/site/styles/radius.css" }),
        ignoreValue: "rgb(139, 92, 246)",
      },
    ];
    const filtered = filterFindings(findings, "", ".css", {
    ...
    assert.deepEqual(
      filtered.map((f) => `${f.antipattern}:${f.line}`),
      ["design-system-color:3", "design-system-color:4", "design-system-radius:5"],
    );
  });
  ...
  it("allows wildcard ignoreValues only when scoped to files", () => {
    ...
    assert.deepEqual(
      filtered.map((f) => `${f.antipattern}:${f.line}`),
      ["design-system-color:2", "design-system-font:3"],
    );
  });
  ...
  it("extracts overused-font values from primary, CSS, and Google font snippets", () => {
    ...
    assert.equal(
      extractFindingIgnoreValue(
        finding("overused-font", 1, { snippet: "https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400" }),
      ),
      "plus jakarta sans",
    );
...
describe("hook-admin.mjs", () => {
  ...
  it("ignore-value writes shared config by default without creating local config", () => {
    ...
    assert.deepEqual(
      shared.ignoreValues.map(({ rule, value, reason }) => ({ rule, value, reason })),
  ...
  it("ignore-value --local writes private config and status reports local ignores", () => {
    ...
    assert.match(status, /local file:\s+\.zarishnoksha\/config\.local\.json/);
    assert.match(status, /ignoreValues:\s+overused-font=inter/);
  });

  it("a /zarishnoksha hooks edit preserves sibling hook fields (consent, quiet)", () => {
    fs.mkdirSync(path.join(cwd, ".zarishnoksha"), { recursive: true });
  ...
  it("hooks on accepts declined consent and installs missing provider manifests", () => {
    ...
    for (const provider of [".claude", ".agents", ".cursor", ".github"]) {
      fs.mkdirSync(path.join(cwd, provider, "skills", "zarishnoksha", "scripts"), { recursive: true });
    }
    ...
    assert.equal(claude.split("skills/zarishnoksha/scripts/hook.mjs").length - 1, 1);

    const codex = fs.readFileSync(path.join(cwd, ".codex", "hooks.json"), "utf-8");
    assert.match(codex, /\.agents\/skills\/zarishnoksha\/scripts\/hook\.mjs/);
    const cursor = fs.readFileSync(path.join(cwd, ".cursor", "hooks.json"), "utf-8");
    assert.match(cursor, /\.cursor\/skills\/zarishnoksha\/scripts\/hook-before-edit\.mjs/);
    const github = JSON.parse(fs.readFileSync(path.join(cwd, ".github", "hooks", "zarishnoksha.json"), "utf-8"));
    assert.equal(github.hooks.postToolUse[0].matcher, "edit|create|apply_patch");
    assert.match(github.hooks.postToolUse[0].bash, /\.github\/skills\/zarishnoksha\/scripts\/hook\.mjs/);
  });

  it("ignore-rule overused-font requires explicit broad suppression", () => {
    assert.throws(() => runAdmin(["ignore-rule", "overused-font"]), /ignore-value overused-font <font>|--all-values/);
    assert.equal(fs.existsSync(getConfigPath(cwd)), false);
  });
  ...
  it("ignore-rule overused-font --all-values writes a whole-rule suppression", () => {
    const out = runAdmin([
      ...
      "--all-values",
      "--reason",
      "User asked to ignore overused fonts generally",
    ]);
  ...
  it("ignore-file writes shared config that suppresses a later hook run", async () => {
    ...
    fs.writeFileSync(
      file,
      '<div style="border-left: 4px solid #7c3aed; border-radius: 16px; padding: 16px;">Card</div>',
    );
...
describe("renderTemplate()", () => {
  it("starts with the versioned envelope and caps to maxFindings", () => {
    ...
    assert.match(text, /\.\.\. and 7 more \(see \/zarishnoksha audit\)\./);
  ...
  it("emits a directive footer (imperative + judgment clause + confirmed ignore guidance)", () => {
    ...
    // the rationale.
    const text = renderTemplate([finding("side-tab", 1, { name: "X" })], "/x/Card.tsx", DEFAULT_CONFIG, { cwd: "/x" });
    ...
    assert.match(text, /inline `zarishnoksha-disable <rule>` comment only when the waiver must travel with a file/);
    assert.match(text, /ignore-value \.\.\. --shared/);
    assert.match(text, /ignore-rule overused-font --all-values/);
    assert.match(text, /\/zarishnoksha hooks ignore-file Card\.tsx/);
    assert.match(text, /ignore-rule <id>/);
    assert.match(text, /\/zarishnoksha audit/);
  });
  ...
  it("shows the exact value-specific command for overused-font findings", () => {
    ...
    assert.match(text, /\/zarishnoksha hooks ignore-value overused-font Roboto --shared/);
    assert.match(text, /ignore-rule overused-font --all-values/);
  });
  ...
  it("shows the exact value-specific command for bounce-easing findings", () => {
    ...
    assert.match(text, /\/zarishnoksha hooks ignore-value bounce-easing bounce-ball --shared/);
  });

  it("drops the L<line> prefix when line is 0", () => {
    const text = renderTemplate([finding("side-tab", 0, { name: "X" })], "/x/a.tsx", DEFAULT_CONFIG, { cwd: "/x" });
  ...
  it("does not suggest ignore-value for rules that cannot be value-filtered", () => {
    ...
    assert.doesNotMatch(text, /\/zarishnoksha hooks ignore-value side-tab Inter/);
  });
...
describe("writeAuditLog()", () => {
  ...
  it("appends NDJSON when zarishnoksha_HOOK_LOG is set", () => {
    const log = path.join(cwd, "audit.ndjson");
    writeAuditLog({ zarishnoksha_HOOK_LOG: log }, { event: "PostToolUse", emitted: true });
    writeAuditLog({ zarishnoksha_HOOK_LOG: log }, { event: "PostToolUse", emitted: false });
  ...
  it("is a no-op when zarishnoksha_HOOK_LOG is unset", () => {
  ...
  it("prefers the env var over config hook.auditLog", () => {
    ...
    writeAuditLog({ zarishnoksha_HOOK_LOG: envLog }, { event: "PostToolUse" }, cwd);
  ...
  it("resolves config auditLog from entry.cwd (the event project root), not the fallback cwd", () => {
    ...
    fs.mkdirSync(path.join(projectDir, ".zarishnoksha"), { recursive: true });
    fs.writeFileSync(
      path.join(projectDir, ".zarishnoksha", "config.json"),
...
`,
    ...
    return {
      loadDesignSystemForCwd: (projectCwd) =>
        fs.existsSync(path.join(projectCwd, "DESIGN.md"))
          ...
          : null,
      detectText: (_content, _filePath, options = {}) => (options.designSystem ? [designFinding()] : []),
      detectHtml: (_filePath, options = {}) => (options.designSystem ? [designFinding()] : []),
    };
  }
  ...
  it("handles a GitHub Copilot apply_patch event end-to-end (interactive/cloud path)", async () => {
    ...
    const file = writeFixture("src/Card.tsx", "noop");
    const det = fakeDetector([finding("side-tab", 1, { name: "Side-tab" })]);
    const patch = ["*** Begin Patch", `*** Update File: ${file}`, "+noop", "*** End Patch"].join("\n");
  ...
  it("emits a clean ack when the file has zero findings", async () => {
    ...
    assert.match(r.stdout, /keep following the project design system and the zarishnoksha skill guidance/);
    assert.equal(r.audit.emitted, true);
  ...
  it("zarishnoksha_HOOK_QUIET=1 suppresses clean and pending acks, keeps findings emission", async () => {
  ...
  it("re-entrancy guard short-circuits when zarishnoksha_HOOK_DEPTH is set", async () => {
    const file = writeFixture("src/Card.tsx", "noop");
    const det = fakeDetector([finding("side-tab", 1)]);
    const r = await runHook({
      stdinJson: JSON.stringify(eventFor(file)),
      env: { zarishnoksha_HOOK_DEPTH: "1" },
      cwd,
      detector: det,
    });
    assert.equal(r.stdout, "");
  ...
  it("zarishnoksha_HOOK_DISABLED kill switch", async () => {
    const file = writeFixture("src/Card.tsx", "noop");
    const det = fakeDetector([finding("side-tab", 1)]);
    for (const v of ["1", "true", "yes", "on", "TRUE"]) {
      const r = await runHook({
        stdinJson: JSON.stringify(eventFor(file)),
        env: { zarishnoksha_HOOK_DISABLED: v },
        cwd,
        detector: det,
      });
  ...
  it("adds a non-blocking note when DESIGN.md is newer than the sidecar", async () => {
    ...
    assert.match(r.stdout, /No deterministic design-quality issues found/);
    assert.match(r.stdout, /DESIGN\.md is newer than \.zarishnoksha\/design\.json/);
    assert.match(r.stdout, /\/zarishnoksha document/);
  });
  ...
  it("emits one-shot suppression notice on the 7th edit and silences after", async () => {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      last = await runHook({
        ...
        env: {},
        cwd,
        detector: { detectText: () => f, detectHtml: () => f },
      });
    }
  ...
  it("emits suppressionNotice text on the threshold-crossing edit", async () => {
    ...
    assert.match(r.stdout, /Run \/zarishnoksha audit to revisit/);
  });
  ...
  it("honors an inline zarishnoksha-disable comment so the hook scans the file clean", async () => {
    // The hook runs the same engine as `npx zarishnoksha detect`, so an in-file
    ...
    const flaggedRun = await runHook({
      stdinJson: JSON.stringify(eventFor(flagged)),
      env: {},
      cwd,
      detector: { detectHtml, detectText },
    });
    ...
    const waived = writeFixture(
      "src/Waived.tsx",
      'const css = "font-family: Inter"; // zarishnoksha-disable-line overused-font',
    );
    const waivedRun = await runHook({
      stdinJson: JSON.stringify(eventFor(waived)),
      env: {},
      cwd,
      detector: { detectHtml, detectText },
    });
...
describe("suppressionNotice()", () => {
  it("starts with envelope and mentions /zarishnoksha audit", () => {
...
describe("renderCleanAck() / renderPendingAck()", () => {
  it("renderCleanAck stays short and ends with the steer line", () => {
    ...
    assert.match(
      text,
      /^\[zarishnoksha@1\] Design hook scanned src\/App\.jsx\. No deterministic design-quality issues found\./,
    );
    assert.match(text, /keep following the project design system and the zarishnoksha skill guidance/);
  ...
  it("renderPendingAck quotes up to 3 known findings and counts the rest", () => {
    ...
    assert.match(text, /^\[zarishnoksha@1\] Design hook scanned src\/SlopCard\.jsx\./);
...
describe("resolveTargetFiles()", () => {
  it("uses file_path when present and falls back to apply_patch command", () => {
    ...
    assert.deepEqual(
      resolveTargetFiles(
        { tool_name: "apply_patch", tool_input: { command: "*** Update File: src/x.css\n" } },
        "/proj",
      ),
      ["/proj/src/x.css"],
    );
...
describe("resolveHarness() / normalizeHookEvent()", () => {
  it("routes explicit env and Cursor conversation_id to cursor harness", () => {
    assert.equal(resolveHarness({ zarishnoksha_HOOK_HARNESS: "cursor" }), "cursor");
  ...
  it("routes a GitHub Copilot postToolUse event (toolName/toolArgs) to the github harness", () => {
    ...
    assert.equal(resolveHarness({ zarishnoksha_HOOK_HARNESS: "github" }), "github");
...
describe("expandScanTargets()", () => {
  ...
  it("does not follow imports from traversal-looking primary targets", () => {
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), "zarishnoksha-hook-outside-"));
    try {
      fs.writeFileSync(
        path.join(outside, "App.jsx"),
...
describe("runHook() — co-located stylesheet scan", () => {
  ...
  it("flags slop in styles.css when only App.jsx was edited", async () => {
    ...
    write("src/styles.css", "body { font-family: 'Inter', sans-serif; }");
    const det = {
      detectText: (content, filePath) => (filePath.endsWith(".css") ? [finding("overused-font", 8)] : []),
      detectHtml: () => [],
    };
    const r = await runHook({
      stdinJson: JSON.stringify({
  ...
  it("flags slop in co-located .sass when only App.jsx was edited", async () => {
    ...
    const det = {
      detectText: (content, filePath) => (filePath.endsWith(".sass") ? [finding("side-tab", 2)] : []),
      detectHtml: () => [],
    };
    const r = await runHook({
      stdinJson: JSON.stringify({
  ...
  it("emits fresh findings for every file scanned in the same hook run", async () => {
    const app = write(
      "src/App.jsx",
      'export default function App() { return <main className="border-l-4 border-blue-500" />; }',
    );
  ...
  it("does not bump edit count for passively co-scanned stylesheets", async () => {
    ...
    const styles = write("src/styles.css", "body { font-family: 'Inter', sans-serif; }");
    const det = {
      detectText: (content, filePath) => (filePath.endsWith("styles.css") ? [finding("overused-font", 1)] : []),
      detectHtml: () => [],
    };
...
describe("Cursor hook scripts", () => {
  ...
  it("preToolUse denies proposed writes with detector findings before they land", () => {
    ...
    const out = execFileSync(process.execPath, [path.join("skill", "scripts", "hook-before-edit.mjs")], {
      ...
      env: { ...process.env, zarishnoksha_HOOK_LOG: logPath },
      encoding: "utf-8",
    });
  ...
  it("preToolUse allows clean proposed writes", () => {
    const out = execFileSync(process.execPath, [path.join("skill", "scripts", "hook-before-edit.mjs")], {
      ...
      env: { ...process.env, zarishnoksha_HOOK_LOG: "" },
      encoding: "utf-8",
    });
  ...
  it("preToolUse downgrades repeated identical denials to allow-with-warning after the edit threshold", () => {
    ...
    const input = JSON.stringify({
      ...
      tool_input: {
        file_path: filePath,
        content:
          '<style>.card { border-left: 4px solid #7c3aed; border-radius: 16px; padding: 16px; }</style><div class="card">Hello</div>',
      },
    });
  ...
  it("preToolUse honors truthy zarishnoksha_HOOK_DISABLED values before stdin parsing", () => {
    const logPath = path.join(cwd, "hook.ndjson");
    ...
    const out = execFileSync(process.execPath, [path.join("skill", "scripts", "hook-before-edit.mjs")], {
      ...
      env: {
        ...process.env,
        zarishnoksha_HOOK_DISABLED: "true",
        zarishnoksha_HOOK_LOG: logPath,
      },
      encoding: "utf-8",
    });
...
describe("runHook() — emission enrichment", () => {
  ...
  it("returns emission.kind fresh with findings on new hits", async () => {
    ...
    const r = await runHook({
      ...
      env: { zarishnoksha_HOOK_HARNESS: "claude" },
      cwd,
