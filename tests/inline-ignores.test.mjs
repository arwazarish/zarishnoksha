...
import { applyInlineIgnores, isInlineIgnored, parseInlineIgnores } from "../cli/engine/shared/inline-ignores.mjs";
...
describe("parseInlineIgnores", () => {
  test("whole-file directive collects rules", () => {
    const d = parseInlineIgnores("/* zarishnoksha-disable overused-font, bounce-easing */");
  ...
  test("bare directive and explicit * both mean every rule", () => {
    expect([...parseInlineIgnores("// zarishnoksha-disable").file]).toEqual(["*"]);
    expect([...parseInlineIgnores("// zarishnoksha-disable *").file]).toEqual(["*"]);
  });
  ...
  test("disable-line targets its own line, disable-next-line targets the line below", () => {
    const content = [
      "a", // line 1
      "b /* zarishnoksha-disable-line overused-font */", // line 2
      "// zarishnoksha-disable-next-line side-tab", // line 3 -> targets line 4
      "d", // line 4
    ].join("\n");
  ...
  test("strips eslint -- and biome : reasons from the rule list", () => {
    expect([...parseInlineIgnores("// zarishnoksha-disable overused-font -- brand font, exported doc").file]).toEqual([
      "overused-font",
    ]);
    expect([...parseInlineIgnores("# zarishnoksha-disable bounce-easing: intentional bounce").file]).toEqual([
      "bounce-easing",
    ]);
  });

  test("strips trailing comment closers across syntaxes", () => {
    expect([...parseInlineIgnores("<!-- zarishnoksha-disable overused-font -->").file]).toEqual(["overused-font"]);
    expect([...parseInlineIgnores("{/* zarishnoksha-disable overused-font */}").file]).toEqual(["overused-font"]);
    expect([...parseInlineIgnores("{# zarishnoksha-disable overused-font #}").file]).toEqual(["overused-font"]);
  });
  ...
  test("directive keyword is case-insensitive (fast-path matches the regex)", () => {
    expect([...parseInlineIgnores("// zarishnoksha-Disable overused-font").file]).toEqual(["overused-font"]);
    expect([...parseInlineIgnores("/* zarishnoksha-DISABLE-LINE side-tab */").line.get(1)]).toEqual(["side-tab"]);
  });
...
describe("applyInlineIgnores / isInlineIgnored", () => {
  ...
  test("whole-file directive drops every matching finding regardless of line", () => {
    const out = applyInlineIgnores(findings, "/* zarishnoksha-disable overused-font */");
    expect(out.map(rules)).toEqual(["side-tab"]);
  });

  test("* drops everything", () => {
    expect(applyInlineIgnores(findings, "// zarishnoksha-disable *")).toEqual([]);
  });

  test("line-scoped directive only affects the matching line and rule", () => {
    const content = ["", "", "", "", "x /* zarishnoksha-disable-line overused-font */"].join("\n");
  ...
  test("isInlineIgnored never matches a line-scoped directive for a line-less finding", () => {
    const d = parseInlineIgnores("x /* zarishnoksha-disable-line overused-font */");
...
describe("detectText honors inline directives", () => {
  ...
  test("disable-line suppresses a same-line finding", () => {
    ...
    const waived = detectText(
      ".a { font-family: Inter; } /* zarishnoksha-disable-line overused-font */",
      "a.css",
      opts,
    );
  ...
  test("disable-next-line suppresses the finding on the following line", () => {
    const content = "/* zarishnoksha-disable-next-line overused-font */\n.a { font-family: Inter; }";
    const waived = detectText(content, "a.css", opts);
    expect(waived.some((f) => f.antipattern === "overused-font")).toBe(false);
  });

  test("whole-file directive suppresses regardless of where the finding is", () => {
    const content = "/* zarishnoksha-disable overused-font */\n.a {}\n.b { font-family: Inter; }";
    const waived = detectText(content, "a.css", opts);
    expect(waived.some((f) => f.antipattern === "overused-font")).toBe(false);
  });

  test("inlineIgnores:false bypasses the directive", () => {
    const content = ".a { font-family: Inter; } /* zarishnoksha-disable-line overused-font */";
  ...
  test("line keys align with detector line numbers on CRLF endings", () => {
    ...
    const content =
      ".a { font-family: Inter; }\r\n.b { font-family: Roboto; } /* zarishnoksha-disable-line overused-font */";
  ...
  test("a directive for one rule leaves other findings intact", () => {
    const content = ".a { font-family: Inter; } /* zarishnoksha-disable-line side-tab */";
    const out = detectText(content, "a.css", opts);
...
<h1>Heading</h1><h2>Sub</h2></body></html>`;
  ...
  test("whole-file directive in an HTML comment suppresses it", async () => {
    const file = await writeTmp(page("<!-- zarishnoksha-disable overused-font -- exported brand doc -->"));
    const waived = await detectHtml(file, { providers: [] });
    expect(waived.some((f) => f.antipattern === "overused-font")).toBe(false);
  });

  test("inlineIgnores:false bypasses it", async () => {
    const file = await writeTmp(page("<!-- zarishnoksha-disable overused-font -->"));
...
describe("detect CLI end-to-end", () => {
  ...
  test("inline directive is honored by default, --no-inline-ignores and --no-config bypass it", async () => {
    const file = await writeTmp(
      "<!DOCTYPE html><html><head><!-- zarishnoksha-disable overused-font -->\n" +
        "<style>body { font-family: Inter, sans-serif; }</style></head>\n" +
...
async function writeTmp(content, ext = ".html") {
  if (!tmpDir) tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "zarishnoksha-inline-"));
