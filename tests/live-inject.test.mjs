...
function runInject(cwd, configPath, args) {
  try {
    const out = execFileSync("node", [INJECT, ...args], {
      cwd,
      encoding: "utf-8",
      env: { ...process.env, zarishnoksha_LIVE_CONFIG: configPath },
...
function runInjectDefault(cwd, args) {
  try {
    const out = execFileSync("node", [INJECT, ...args], {
      cwd,
      encoding: "utf-8",
      env: { ...process.env, zarishnoksha_LIVE_CONFIG: "" },
      stdio: ["ignore", "pipe", "pipe"],
    });
    return JSON.parse(out.trim());
  } catch (err) {
...
describe("live-inject — insert/remove round-trip preserves file bytes", () => {
  let tmp;
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-inject-test-"));
  });
  afterEach(() => {
    rmSync(tmp, { recursive: true, force: true });
  });

  it("reports .zarishnoksha/live/config.json as the default missing config path", () => {
    const result = runInjectDefault(tmp, ["--check"]);

    assert.equal(result.ok, false);
    assert.equal(result.error, "config_missing");
    assert.equal(result.path, join(realpathSync(tmp), ".zarishnoksha", "live", "config.json"));
  });

  it("uses .zarishnoksha/live/config.json without an environment override", () => {
    const original = `<html>
  <body>
    <p>Content</p>
  </body>
...
`;
    ...
    const configDir = join(tmp, ".zarishnoksha", "live");
...
`;
  ...
  it("round-trips through CSP-meta patch and revert (insert mutates the meta tag, remove restores it)", () => {
    ...
    // and stashes the original directives in `data-zarishnoksha-csp-original`.
...
`;
    ...
    assert.match(
      afterInject,
      /<script is:inline src="http:\/\/localhost:8400\/live\.js"><\/script>/,
      "astro inject should carry is:inline",
    );
...
<html>
  <body>
    <h1>{title}</h1>
    <!-- zarishnoksha-live-start -->
    <script src="http://localhost:8400/live.js"></script>
    <!-- zarishnoksha-live-end --></body>
</html>
`;
    const file = join(tmp, "Layout.astro");
    writeFileSync(file, original);
    ...
    assert.equal(
      (afterInject.match(/zarishnoksha-live-start/g) || []).length,
      1,
      "reinjection should leave one live block",
    );
    assert.match(
      afterInject,
      /<script is:inline src="http:\/\/localhost:8400\/live\.js"><\/script>/,
      "astro reinject should restore is:inline",
    );
    assert.doesNotMatch(
      afterInject,
      /<script src="http:\/\/localhost:8400\/live\.js"><\/script>/,
      "bare astro live script must not survive",
    );
  });
...
`;
  ...
  it("preserves the character after an insertAfter anchor with no trailing newline (#227)", () => {
    ...
    assert.ok(
      afterInject.includes("<!-- zarishnoksha-live-end -->\nX</head>"),
