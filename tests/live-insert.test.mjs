...
import { buildInsertWrapperLines, computeInsertLine, isInsertPosition } from "../skill/scripts/live-insert.mjs";
...
describe("buildInsertWrapperLines", () => {
  it("produces an insert scaffold without an original variant", () => {
    ...
    assert.match(joined, /zarishnoksha-variants-start abc12345/);
    assert.match(joined, /data-zarishnoksha-variants="abc12345"/);
    assert.match(joined, /data-zarishnoksha-mode="insert"/);
    assert.match(joined, /Variants: insert below this line/);
    assert.doesNotMatch(joined, /data-zarishnoksha-variant="original"/);
  });
  ...
  it("keeps marker comments inside the wrapper for JSX", () => {
    ...
    const joined = lines.join("\n");
    assert.match(joined, /^\s*<div data-zarishnoksha-variants="jsx12345"/m);
    assert.match(joined, /{\/\* zarishnoksha-variants-start jsx12345 \*\/}/);
    assert.doesNotMatch(joined, /data-zarishnoksha-variant="original"/);
  });
});
...
describe("live-insert CLI integration", () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-insert-test-"));
  });
...
<body>
  ...
  <section class="features">
    ...
    assert.match(after, /data-zarishnoksha-mode="insert"/);
    assert.match(after, /zarishnoksha-variants-start ins12345/);
    assert.ok(after.indexOf('class="hero"') < after.indexOf("zarishnoksha-variants-start ins12345"));
    assert.ok(after.indexOf("zarishnoksha-variants-start ins12345") < after.indexOf('class="features"'));
    assert.doesNotMatch(after, /data-zarishnoksha-variant="original"/);
  });
  ...
  <section class="cta">CTA</section>
    ...
    assert.ok(after.indexOf("zarishnoksha-variants-start ins99999") < after.indexOf('class="cta"'));
    assert.ok(after.indexOf('class="hero"') < after.indexOf("zarishnoksha-variants-start ins99999"));
  });
  ...
  return (
    ...
    assert.match(after, /data-zarishnoksha-mode="insert"/);
    assert.match(after, /{\/\* zarishnoksha-variants-start jsxins01 \*\/}/);
  });
