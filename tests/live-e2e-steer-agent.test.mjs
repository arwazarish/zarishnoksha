...
import {
  addSteerMarkerToSource,
  createFakeAgent,
  findSteerTargetFile,
  runAgentLoop,
  STEER_MARKER_ATTR,
} from "./live-e2e/agent.mjs";
import { FIXTURES_DIR, SCRIPTS_DIR, stageFixture, startLiveServer, stopLiveServer } from "./live-e2e/session.mjs";
...
describe("live-e2e steer agent handler", () => {
  ...
  it("agent loop handles steer POST and writes the marker", async () => {
    ...
    assert.match(updated, new RegExp(STEER_MARKER_ATTR + '="e2e"'), "fake agent should mark hero after steer event");
  });
});
