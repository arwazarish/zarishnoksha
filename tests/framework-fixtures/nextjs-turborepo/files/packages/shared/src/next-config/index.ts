...
import { buildConnectSrc, getSupabaseOrigin, HSTS_VALUE, PERMISSIONS_POLICY_VALUE } from "../security/origins";
...
const KNOWN_SUPABASE_CUSTOM_DOMAINS = ["db.example.com", "db-staging.example.com"] as const;

const KNOWN_SUPABASE_CUSTOM_ORIGINS = KNOWN_SUPABASE_CUSTOM_DOMAINS.map((h) => `https://${h}`);
...
function buildCSPConfig(options: SharedNextConfigOptions = {}): CSPConfig {
  ...
  const styleSrc = ["'self'", "'unsafe-inline'", ...(isPreview ? ["https://fonts.googleapis.com"] : [])].join(" ");
  ...
  const imgSrc = [
    ...
    ...(supabaseOrigin &&
    !KNOWN_SUPABASE_CUSTOM_ORIGINS.includes(supabaseOrigin as (typeof KNOWN_SUPABASE_CUSTOM_ORIGINS)[number])
      ? [supabaseOrigin]
      : []),
    ...(options.enableMapbox ? ["https://api.mapbox.com", "https://*.tiles.mapbox.com"] : []),
  ...
  const connectExtras = [
    ...(options.enableMapbox
      ...
      : []),
    ...(isPreview ? ["https://vercel.live", "wss://*.pusher.com", "https://*.pusher.com"] : []),
...
export function buildSecurityHeaders(options: SharedNextConfigOptions = {}): NextConfig["headers"] {
  return () => {
...
export function buildApiProxyRewrites(): NextConfig["rewrites"] {
  return () => {
    ...
    if (process.env.NEXT_PUBLIC_API_PROXY === "1" && process.env.NEXT_PUBLIC_API_URL) {
...
export function buildWebpackConfig(options: SharedNextConfigOptions = {}): NextConfig["webpack"] {
 ...
 */
export function createBaseNextConfig(options: SharedNextConfigOptions = {}): NextConfig {
  const transpilePackages = Array.from(
    new Set([...DEFAULT_WORKSPACE_TRANSPILE_PACKAGES, ...(options.transpilePackages || [])]),
  );
