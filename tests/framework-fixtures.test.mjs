...
function stageFixture(name) {
  ...
  const tmp = mkdtempSync(join(tmpdir(), "zarishnoksha-fixture-"));
  ...
  writeFileSync(join(tmp, ".zarishnoksha", "live", "config.json"), JSON.stringify(fixture.config));
    ...
    it("is-generated classifies files correctly", () => {
      ...
      try {
        for (const rel of fixture.sourceFiles || []) {
          assert.equal(isGeneratedFile(rel, { cwd: tmp }), false, `${rel} should classify as source`);
        }
        for (const rel of fixture.generatedFiles || []) {
          assert.equal(isGeneratedFile(rel, { cwd: tmp }), true, `${rel} should classify as generated`);
        }
      } finally {
        rmSync(tmp, { recursive: true, force: true });
      }
    ...
    it("live-inject --port adds the script tag to every config file", () => {
      ...
      try {
        ...
        const ignored = execFileSync(
          ...
          [
            "check-ignore",
            ".zarishnoksha/live/server.json",
            ".zarishnoksha/live/sessions/example.jsonl",
            ".zarishnoksha/live/previews/example/v1.html",
            ".zarishnoksha/live/deferred-svelte-component-accepts.json",
            "src/lib/zarishnoksha/zarishnokshaLiveRoot.svelte",
            "src/lib/zarishnoksha/__runtime.js",
            "src/lib/zarishnoksha/a4ac4e74/v3.svelte",
          ],
          { cwd: tmp, encoding: "utf-8" },
        );
        assert.match(ignored, /\.zarishnoksha\/live\/server\.json/);
        assert.match(ignored, /\.zarishnoksha\/live\/sessions\/example\.jsonl/);
        assert.match(ignored, /\.zarishnoksha\/live\/previews\/example\/v1\.html/);
        assert.match(ignored, /\.zarishnoksha\/live\/deferred-svelte-component-accepts\.json/);
        assert.match(ignored, /src\/lib\/zarishnoksha\/zarishnokshaLiveRoot\.svelte/);
        assert.match(ignored, /src\/lib\/zarishnoksha\/__runtime\.js/);
        assert.match(ignored, /src\/lib\/zarishnoksha\/a4ac4e74\/v3\.svelte/);
        if (result.adapter === "sveltekit") {
          ...
          const root = readFileSync(join(tmp, "src/lib/zarishnoksha/zarishnokshaLiveRoot.svelte"), "utf-8");
          assert.match(layout, /zarishnoksha-live-svelte-start/, "SvelteKit layout got the adapter marker");
          assert.match(layout, /zarishnokshaLiveRoot/, "SvelteKit layout renders the adapter host");
          assert.doesNotMatch(appHtml, /zarishnoksha-live-start/, "SvelteKit app.html must remain untouched");
        ...
        for (const r of result.results) {
          ...
          assert.match(body, /zarishnoksha-live-start/);
    ...
    it("live-inject --remove strips the script tag cleanly", () => {
      ...
      try {
        ...
        if (result.adapter === "sveltekit") {
          const layout = readFileSync(join(tmp, "src/routes/+layout.svelte"), "utf-8");
          const appHtml = readFileSync(join(tmp, "src/app.html"), "utf-8");
          assert.doesNotMatch(layout, /zarishnokshaLiveRoot/);
          assert.doesNotMatch(layout, /zarishnoksha-live-svelte-start/);
          assert.doesNotMatch(appHtml, /zarishnoksha-live-start/);
          assert.equal(existsSync(join(tmp, "src/lib/zarishnoksha/zarishnokshaLiveRoot.svelte")), false);
          return;
        }
        for (const r of result.results) {
          const body = readFileSync(join(tmp, r.file), "utf-8");
          assert.doesNotMatch(body, /zarishnoksha-live-start/);
        ...
        for (const [i, wc] of (fixture.wrapCases || []).entries()) {
          ...
          if (wc.expectsError) {
            assert.equal(
              parsed.error,
              wc.expectsError,
              `wrap case "${wc.name}": expected error ${wc.expectsError}, got ${JSON.stringify(parsed)}`,
            );
          } else {
            assert.equal(
              parsed.file,
