...
describe("live-browser.js regression guards", () => {
  it("resolveCanvasBackground does not fall back to `getComputedStyle(...).backgroundColor || ...`", () => {
    ...
    // canvas color).
    const buggy = /getComputedStyle\(document\.(?:body|documentElement)\)\.backgroundColor\s*\|\|/;
    assert.ok(
      !buggy.test(SOURCE),
  ...
  it("shader bitmap decode failure keeps a visible fallback overlay", () => {
    ...
    assert.doesNotMatch(SOURCE, /new Image\(/, "shader fallback should not use an image element fallback");
  });
  ...
  it("uses a Svelte-gated painted-ancestor crop proxy for shader capture", () => {
    ...
    assert.match(
      SOURCE,
      /function shouldUseAncestorCropShaderProxy\(el\) \{[\s\S]{0,260}?window\.__zarishnoksha_LIVE_ADAPTER__[\s\S]{0,280}?currentPreviewMode === 'svelte-component' \|\| svelteComponentSession[\s\S]{0,260}?dataset\?\.zarishnokshaPreview === 'svelte-component';/,
  ...
  it("pins edit badge button metrics instead of inheriting host button chrome", () => {
    ...
    assert.match(calloutStyle, /fontSize: '10px'/, "edit badge controls should not scale from host rem settings");
    assert.match(
      calloutStyle,
      /lineHeight: '16px'/,
  ...
  it("global bar includes expandable page chat affordance", () => {
    ...
    assert.match(SOURCE, /webkitSpeechRecognition|SpeechRecognition/, "steer voice must use the Web Speech API");
    assert.doesNotMatch(
      SOURCE,
    ...
    assert.match(SOURCE, /function syncAgentPollingUi\(/, "global bar brand must reflect agent poll connectivity");
    assert.match(SOURCE, /case 'agent_polling':/, "browser must listen for agent_polling SSE updates");
    assert.match(
      SOURCE,
  ...
  it("lets page editables keep Enter and arrow keydown events", () => {
    ...
    assert.match(
      ...
      "zarishnoksha inline edit rows must keep their existing Escape-cancel path",
    );
  });
  ...
  it("pick mode preference persists in localStorage", () => {
    assert.match(
      SOURCE,
      /const INTERACTION_PREFS_KEY = 'zarishnoksha-live-interaction';[\s\S]{0,3000}?function saveInteractionPrefs\(\)/,
  ...
  it("detect mode shows an empty result toast once per requested scan", () => {
    ...
    assert.match(
      SOURCE,
      /window\.postMessage\(\{ source: 'zarishnoksha-command', action: 'remove' \}, '\*'\);[\s\S]{0,80}?activeDetectScanId = null;[\s\S]{0,80}?pendingDetectScanId = null;/,
  ...
  it("insert mode UI and generate payload guards", () => {
    ...
    assert.match(
      ...
      "Create button must reflect prompt/annotation gate",
    );
    assert.match(
      SOURCE,
    ...
    assert.match(SOURCE, /data-zarishnoksha-insert-placeholder/, "placeholder element must be marked");
    assert.match(
      SOURCE,
      /showHighlight\(el\)[\s\S]{0,120}?data-zarishnoksha-insert-placeholder/,
    ...
    assert.match(SOURCE, /data-zarishnoksha-placeholder-resize/, "placeholder edge handles on annotation overlay");
    ...
    assert.match(
      ...
      "placeholder width styles applied by kind",
    );
    assert.match(
      SOURCE,
  ...
  it("configure row groups selection and input on the left, trailing controls before submit", () => {
    ...
    assert.match(SOURCE, /const CONFIGURE_PILL_PAD_Y = '3px'/, "modifier pills share 3px vertical padding");
    assert.match(
      SOURCE,
  ...
  it("configure bar keeps selection pill, inline controls, outline, and instant tooltips", () => {
    ...
    assert.match(SOURCE, /gridArea: '1 \/ 1'/, "selection pill swaps tag and clear faces without resizing the bar");
    assert.match(
      SOURCE,
    ...
    assert.match(
      ...
      "action dropdown is an inline bar control, not a pill",
    );
  ...
  it("variant count pill cycles through x1", () => {
    ...
    assert.doesNotMatch(SOURCE, /selectedCount >= 4 \? 2/, "variant count must not skip x1 by wrapping 4 back to 2");
  });
