 ...
 *   3. zarishnoksha_CONTEXT_DIR env var as a power-user escape hatch (only
...
beforeEach(() => {
  scratch = fs.mkdtempSync(path.join(os.tmpdir(), "zarishnoksha-loadctx-"));
  savedEnv = process.env.zarishnoksha_CONTEXT_DIR;
  delete process.env.zarishnoksha_CONTEXT_DIR;
});

afterEach(() => {
  if (savedEnv === undefined) delete process.env.zarishnoksha_CONTEXT_DIR;
  else process.env.zarishnoksha_CONTEXT_DIR = savedEnv;
...
describe("resolveContextDir", () => {
  ...
  it("uses zarishnoksha_CONTEXT_DIR as a fallback when defaults are empty (relative path)", () => {
    write("design/PRODUCT.md");
    process.env.zarishnoksha_CONTEXT_DIR = "design";
    assert.equal(resolveContextDir(scratch), path.join(scratch, "design"));
  });

  it("uses zarishnoksha_CONTEXT_DIR as a fallback when defaults are empty (absolute path)", () => {
    const elsewhere = fs.mkdtempSync(path.join(os.tmpdir(), "zarishnoksha-elsewhere-"));
    try {
      process.env.zarishnoksha_CONTEXT_DIR = elsewhere;
  ...
  it("default paths win over zarishnoksha_CONTEXT_DIR (lazy escape hatch)", () => {
  ...
  it("ignores empty zarishnoksha_CONTEXT_DIR", () => {
    write("PRODUCT.md");
    process.env.zarishnoksha_CONTEXT_DIR = "   ";
    assert.equal(resolveContextDir(scratch), scratch);
  });
...
describe("loadContext (monorepo project context)", () => {
  ...
  it("does not discover dependency or generated directories as workspace candidates", () => {
    ...
    const res = spawnSync(process.execPath, [SCRIPT_PATH], {
      cwd: scratch,
      encoding: "utf8",
      env: { ...process.env, zarishnoksha_NO_UPDATE_CHECK: "1" },
    });
    assert.equal(res.status, 0);
    const selection = parseTargetSelection(res.stdout);

    assert.deepEqual(
      selection.targetCandidates.map((candidate) => candidate.path),
      ["packages/ui"],
    );
  });
  ...
  it("uses the last --target value when duplicate target flags are provided", () => {
    ...
    const res = spawnSync(
      process.execPath,
      [SCRIPT_PATH, "--target", "apps/marketing/src/App.jsx", "--target", "apps/dashboard/src/App.jsx"],
      {
        cwd: scratch,
        encoding: "utf8",
        env: { ...process.env, zarishnoksha_NO_UPDATE_CHECK: "1" },
...
describe("loadContext (zarishnoksha_CONTEXT_DIR escape hatch)", () => {
  ...
  it("reports a missing override directory as no-context, not as a crash", () => {
    process.env.zarishnoksha_CONTEXT_DIR = "no/such/dir";
    const ctx = loadContext(scratch);
...
describe("context.mjs update check", () => {
  ...
  function setup(cacheObj, { disable = false, host } = {}) {
    ...
    const env = {
      ...process.env,
      zarishnoksha_UPDATE_CACHE: cachePath(),
      zarishnoksha_NO_UPDATE_CHECK: disable ? "1" : "",
      ...(host ? { zarishnoksha_UPDATE_HOST: host } : {}),
    };
  ...
  it("appends UPDATE_AVAILABLE when the cached latest version is newer", () => {
    ...
    assert.equal(res.status, 0);
    assert.match(res.stdout, /UPDATE_AVAILABLE: A newer zarishnoksha skill is available/);
    assert.match(res.stdout, /installed v1\.0\.0, latest v2\.0\.0/);
    assert.match(res.stdout, /npx zarishnoksha update/);
  ...
  it("respects zarishnoksha_NO_UPDATE_CHECK", () => {
