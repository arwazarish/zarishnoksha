 ...
 * exposes the accepted element under stale data-zarishnoksha runtime wrappers.
 *
 * Run with:
...
import { clickAccept, clickGo, clickNext, getVisibleVariant, pickElement, waitForHandshake } from "./live-e2e/ui.mjs";
...
describe("live-e2e accept cleanup regression", () => {
  it("cleans accepted LLM DOM wrappers after carbonize cleanup", async (t) => {
    ...
    try {
      ...
      assert.doesNotMatch(finalSource, /data-zarishnoksha-variants="/, "source variants wrapper removed");
      assert.doesNotMatch(finalSource, /zarishnoksha-variants-start/, "source variants markers removed");
      assert.doesNotMatch(finalSource, /zarishnoksha-carbonize-start/, "source carbonize markers removed");
      assert.doesNotMatch(finalSource, /data-zarishnoksha-variant="/, "source variant wrapper removed");
...
async function waitForAcceptedDomClean(page, selector, { timeoutMs, sourceFile, finalSource }) {
  try {
    await page.waitForFunction(
      (sel) => {
        ...
        return (
          ...
          matches.every(
            (el) =>
              !el.closest("[data-zarishnoksha-variants],[data-zarishnoksha-variant],[data-zarishnoksha-carbonize]"),
          )
        );
      },
      selector,
    ...
    throw new Error("accepted DOM stayed stale after clean source; diagnostic=" + JSON.stringify(diagnostic, null, 2), {
      cause: err,
    });
  }
}
...
async function collectAcceptedDomDiagnostic(page, selector) {
  return page.evaluate((sel) => {
    ...
    const stale = matches.filter((el) =>
      el.closest("[data-zarishnoksha-variants],[data-zarishnoksha-variant],[data-zarishnoksha-carbonize]"),
    );
    const local = {};
    for (const key of [
      "zarishnoksha-live-session",
      "zarishnoksha-live-session-handled",
      "zarishnoksha-live-session-scroll",
    ]) {
    ...
    return {
      ...
      remainingVariantsWrapperCount: document.querySelectorAll("[data-zarishnoksha-variants]").length,
      remainingVariantWrapperCount: document.querySelectorAll("[data-zarishnoksha-variant]").length,
      liveBarText: document.querySelector("#zarishnoksha-live-bar")?.textContent || "",
      localStorage: local,
      heroHtml: matches.map((el) => el.outerHTML.slice(0, 500)),
      staleAncestorHtml: stale.map((el) => {
        const ancestor = el.closest("[data-zarishnoksha-variants],[data-zarishnoksha-variant]");
...
function isSourceClean(source) {
  return (
    !source.includes("data-zarishnoksha-variants=") &&
    !source.includes("zarishnoksha-variants-start") &&
    !source.includes("zarishnoksha-carbonize-start") &&
    !source.includes("data-zarishnoksha-variant=")
  );
}
