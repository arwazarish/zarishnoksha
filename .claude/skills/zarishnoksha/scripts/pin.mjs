 ...
 *
 * `pin audit` creates a lightweight /audit skill that redirects to /zarishnoksha audit.
...
const PIN_MARKER = "<!-- zarishnoksha-pinned-skill -->";
...
/**
 * Find harness skill directories that have an zarishnoksha skill installed.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, "skills");
    // Only pin in harness dirs that already have zarishnoksha installed
    const zarishnokshaDir = join(skillsDir, "zarishnoksha");
    if (existsSync(zarishnokshaDir) || existsSync(join(skillsDir, "i-zarishnoksha"))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Shortcut for /zarishnoksha ${command}.`;
...
This is a pinned shortcut for \`{{command_prefix}}zarishnoksha ${command}\`.

Invoke {{command_prefix}}zarishnoksha ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log("No harness directories with zarishnoksha installed found.");
    return false;
  }
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use /zarishnoksha ${command} to access it.`);
  } else {
