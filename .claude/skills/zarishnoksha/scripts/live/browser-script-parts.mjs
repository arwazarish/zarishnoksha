...
export function assembleLiveBrowserScript({ token, port, vocabulary, parts }) {
  const prelude =
    `window.__zarishnoksha_TOKEN__ = '${token}';\n` +
    `window.__zarishnoksha_PORT__ = ${port};\n` +
    ...
    `window.__zarishnoksha_VOCAB__ = ${JSON.stringify(vocabulary)};\n`;

  const body = parts
    .map((part) => {
      const file = part.file || path.basename(part.path || "");
      return `// --- zarishnoksha live script part: ${part.name} (${file}) ---\n${part.source}`;
    })
    .join("\n");
