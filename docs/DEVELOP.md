...
Documentation for contributors to zarishnoksha.
...
```
zarishnoksha/
  skill/                          # Source of truth (edit these!)
    SKILL.src.md                  # Skill definition (transformed per provider)
    reference/*.md                # Command and domain references
    scripts/                      # Skill runtime (detect, live mode, hooks)
    agents/                       # Sub-agent definitions
  cli/                            # CLI tool + anti-pattern detection engine
    bin/cli.js                    # CLI entrypoint (npm bin)
    engine/                       # Detection engine (source of truth for rules)
  site/                           # Astro marketing/documentation site
    pages/, components/, styles/  # Astro site source
    content/                      # Skill editorials, tutorials, reference docs
