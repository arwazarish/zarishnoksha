...
There is **one** user-invocable skill, `zarishnoksha`, with **23 commands** underneath it. Users type `/zarishnoksha polish`, `/zarishnoksha audit`, etc. The skill is defined in `skill/`:
...
- `scripts/pin.mjs` — creates/removes lightweight redirect shims so users can have `/audit` as a standalone shortcut that delegates to `/zarishnoksha audit`.
...
PRODUCT.md at the project root carries a `## Register` section with a bare value (`brand` or `product`). `/zarishnoksha init` asks about register first because it shapes every downstream answer.
...
The build system compiles the ZarishNoksha skill from `skill/` to provider-specific formats in `dist/`. The default build is source-first and does not sync tracked root harness folders; the release build performs the tracked distribution sync:
...
- `{{available_commands}}` — auto-populated list of commands (from `zarishnoksha_SUB_COMMANDS` in `scripts/lib/utils.js`)
...
Unit tests (build orchestration, detector logic) run via `bun test`. Fixture tests (HTML detection against the static engine) run via `node --test` because bun is too slow with the HTML-parsing / DOM fixture path. The `test` script handles this split automatically.
...
zarishnoksha_E2E_ONLY=vite8-react-modal bun run test:live-e2e # scope to one fixture
zarishnoksha_E2E_DEBUG=1 bun run test:live-e2e                # dump page DOM + dev-server tail on failure
```
...
**LLM agent (opt-in)**: set `zarishnoksha_E2E_AGENT=llm` to swap the fake agent for `tests/live-e2e/agents/llm-agent.mjs`, which calls Claude (default Haiku 4.5) via `@anthropic-ai/sdk`. Requires `ANTHROPIC_API_KEY` in env; the test runner skips with a clear message when it's unset. Override the model with `zarishnoksha_E2E_LLM_MODEL=claude-sonnet-4-6` if Haiku produces unreliable JSON. Caching is on — live.md is the cacheable prefix, and after the first call subsequent fixtures pay only the cache-read rate. Pass rate on a typical sweep is 18/19; the modal fixture's intrinsic state-loss flake is amplified by LLM latency and may need a re-run. **This path hits the API and costs money** — keep it out of CI unless you really want it there.
...
zarishnoksha_SKILL_BEHAVIOR_MODELS=gemini-3.1-flash-lite bun run test:skill-behavior   # scope to one provider
zarishnoksha_SKILL_BEHAVIOR_VERBOSE=1 bun run test:skill-behavior          # dump per-scenario trace JSON to stderr (use when iterating)
```
...
**Auth** lives in repo-root `.env` (copied from `~/code/zarishnoksha-evals/.env`, gitignored). Providers skip cleanly when their key is unset; they don't fail.
...
6. `/zarishnoksha polish` → loads `reference/polish.md`
7. `/zarishnoksha audit` → loads `reference/audit.md`
8. existing SvelteKit project → agent reads at least one project code file
9. `context.mjs` emits `UPDATE_AVAILABLE` (seeded newer version) → agent surfaces it but does **not** auto-run `npx zarishnoksha skills update`
...
The CLI lives in this repo under `cli/`: `cli/bin/` (entry + sub-commands), `cli/engine/` (the detect-antipatterns rule engine + browser variant), `cli/lib/` (helpers shared by CLI and Cloudflare Pages Functions). Published to npm as `zarishnoksha`.

```bash
npx zarishnoksha detect [file-or-dir-or-url...]   # detect anti-patterns
npx zarishnoksha detect --fast --json src/         # regex-only, JSON output
npx zarishnoksha live                              # start browser overlay server
npx zarishnoksha install                       # install skills
npx zarishnoksha --help                            # show help
```
...
**IMPORTANT**: Always use `node` (not `bun`) to run the detect CLI. Bun's HTML/DOM parsing is extremely slow and will cause scans with HTML files to hang for minutes.
...
- After bumping, run `bun run build:release` so the committed `./plugin` subtree (`plugin/.claude-plugin/plugin.json` + `plugin/skills/zarishnoksha/SKILL.md`) is regenerated to the new version. The build validator (`validatePluginVersions` in `scripts/build.js`) fails if `marketplace.json`, the `./plugin` manifest, or the bundled `SKILL.md` frontmatter disagree with `plugin.json` — this guards the marketplace install path against version drift (issue #274).
...
All commands live under `/zarishnoksha`. To add a new one:
...
4. Add the command name to `zarishnoksha_SUB_COMMANDS` in `scripts/lib/utils.js`
...
5. **Two adapters**: `checkElementXxxDOM(el)` for the browser (`getComputedStyle` + `getBoundingClientRect`) and `checkElementXxx(el, tag, window)` for the Node static-engine path (`parseFloat(style.width)` instead of layout). Wire **both** into **both** element loops — the browser loop (~line 1837) and the Node loop in `cli/engine/engines/static-html/detect-html.mjs`. Forgetting one is the most common mistake; symptom is "test passes, live page silent" or vice versa.
...
### Conventions and static-engine gotchas
...
- **The Node path doesn't lay out**: `getBoundingClientRect()` returns 0×0. Read `parseFloat(style.width)` and `parseFloat(style.height)` from explicit CSS instead.
- **`background:` shorthand isn't decomposed in the Node path**: use the existing `resolveBackground()` and `resolveGradientStops()` helpers (~line 631 / 670).
- **Computed colors aren't normalized in the Node path**: `parseGradientColors()` handles both hex and rgb forms.
...
The eval framework lives in a separate private repo at `~/code/zarishnoksha-evals/`. It measures whether the `/zarishnoksha` skill improves or harms AI-generated frontend design by running the same brief through a model with and without the skill loaded.
...
```bash
cd ~/code/zarishnoksha-evals
bun run serve            # dashboard on http://localhost:8723
```

The eval runners read this repo's skill from `../zarishnoksha/skill/` and staged provider skills from `../zarishnoksha/build/_data/dist/*`. Run `bun run build` in this repo before an eval sweep if you want the Claude/Gemini staged skills to reflect your latest edits.
